...
/* ---- Gate admin DURCI (hash du PIN présent) : le nom auto-asserté ne suffit plus ---- */
const CODE = '200807';
...
ok(r.status === 403 && j.reason === 'need_admin_code', 'DURCI : token nom-admin SANS code → 403 need_admin_code (trou fermé)');
...
ok(j.ok === true && j.count >= 3, 'grant via header x-kdmc-admin → accès fiches OK');
