...
| 2026-05-22 | « /conseil » 5 agents (Contradicteur/Fondamentaliste/Expansionniste/Outsider/Exécutant) + LLM Council (Karpathy) | Skill multi-agents délibération | Gratuit | Déjà couvert : skill `llm-council.md` + `crew-experts.ts` + règle « APEX MULTI-IA PARALLÈLE » + commandes `contrarian` / `firstprinciples` / `analyst` / `critic` | ⚖️ Déjà couvert — rien à installer |
| 2026-05-22 | taste-skill / impeccable (re-montrés) | Skills design CC | Gratuit | Déjà évalués le 2026-05-21 (cf. lignes ci-dessus) | ⚖️ Doublon d'évaluation — TikTok recycle les mêmes outils |
| 2026-05-22 | coreyhaines31/marketingskills (41 skills) | Skills marketing CC | Gratuit (MIT, ~30k★) | Repo réputé (Corey Haines). Apex avait déjà `apex-marketing-psy` + tool `generate_marketing_copy` (partiel) ; les 41 couvrent CRO, copywriting, SEO, emails, ads, pricing, launch, analytics, etc. | ✅ Intégré — pack namespacé `.claude/skills/marketing/` (n'encombre pas le namespace plat). Cœur utile pour Kevin : copywriting, content-strategy, emails, social, launch, pricing, cro. Le reste = niche growth SaaS |
| 2026-05-22 | JuliusBrussee/caveman | Skill CC — sortie « caveman » très concise pour économiser ~75 % des tokens de sortie | Gratuit | Économie de tokens sur la sortie de Claude Code | ❌ Écarté — gimmick : rend les explications télégraphiques et moches. Kevin n'est pas codeur et a besoin de français clair → ce serait une régression UX. Gain réel marginal ; côté tokens on a déjà `apex-context-mode` + `cache-audit` |
| 2026-05-22 | ds2api (ClackHwang) | Proxy non-officiel — fausse API « gratuite » en scrapant le chat web DeepSeek (dossier `pow` = contourne l'anti-bot proof-of-work) | « Gratuit » mais viole les CGU DeepSeek | Apex a déjà DeepSeek via l'API OFFICIELLE (légale, très bon marché) | ❌ Écarté — repo ARCHIVÉ (mort/non maintenu), viole les CGU DeepSeek (risque ban du compte), risque sécurité (cookies de session), serveur Go non hébergeable dans une PWA. Non intégré (cf. règle OWASP A03 + précédent claw-code) |
