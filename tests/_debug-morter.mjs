// Tracer comment MORTER L récupère "16/3"'" au lieu de "16/3*"
...
async function main(){
  ...
  const out = await page.evaluate(async ({ pages, rawText }) => {
    ...
    const MORTER = window.A.employees.find(e => e.name === 'MORTER L');
    if(!MORTER) return { error: 'MORTER L not found' };

    // Trace every assignment to MORTER L's overrides row
    const traces = [];
    ...
    const final = ov[MORTER.id] || {};
    return { traces: traces.slice(0, 80), final: Object.assign({}, final), id: MORTER.id };
  }, { pages: passA.pages, rawText });

  if(out.error){ console.error('ERR:', out.error); return; }
  console.log('=== MORTER L final state ===');
