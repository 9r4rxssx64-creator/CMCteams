...
// État app simulé côté "cloud" (Firebase) — 1 équipe BJ + 1 roulettes en MAI 2026.
// v1.44 : un mois HORS des boards générés (juil.→oct. 2026 sont couverts par boards-gen.js) — pour un mois
// couvert, l'appartenance aux équipes vient du PDF (miroir de l'app, leçon #264), pas de Firebase.
const EMPS = [
  { id: 'C1', name: 'CLOUD CHEF', family: 'bj', teamHistory: { '2026-4': '1' }, familyHistory: { '2026-4': 'bj' } },
  { id: 'C2', name: 'CLOUD DEUX', family: 'bj', teamHistory: { '2026-4': '1' }, familyHistory: { '2026-4': 'bj' } },
  { id: 'R1', name: 'CLOUD ROUL', family: 'roulettes', teamHistory: { '2026-4': 'r1' }, familyHistory: { '2026-4': 'roulettes' } }
];
const OV = { '2026-4': {
...
const META = { '2026-4': { C1: { 5: { bg: 'CONV' } } } };
const MIRROR = { '1': 'r1' };
...
try {
  ...
  await ctx.route(/cmcteams-c16ab-default-rtdb\.europe-west1\.firebasedatabase\.app\/cmcteams\/.*\.json.*/, route => {
    ...
    else if (/\/cmc_team_mirror_2026-4\.json/.test(url)) body = JSON.stringify(MIRROR);
  ...
  await page.waitForFunction(() => window._depLiveMonths && window._depLiveMonths['2026-4'], { timeout: 15000 }).catch(() => {});

  const r = await page.evaluate(() => {
    const ids = Object.keys(BOARDS);
    const bj = ids.find(id => id === '2026-05-1');
    const roul = ids.find(id => id === '2026-05-r1');
    ...
    return {
      ...
      mirrorLink: (typeof DEP_MIRROR === 'object') ? (DEP_MIRROR['2026-05-1'] || null) : null
    };
  });
  ...
  ok(r.live.indexOf('2026-4') >= 0, 'mois mai 2026 construit depuis Firebase (live)');
  ...
  ok(r.mirrorLink === '2026-05-r1', 'lien miroir reconstruit depuis Firebase (1 → r1)');
  ...
  await page.evaluate(() => window._depSyncCell('C1', '2026-4', 2, 'CP'));
  ...
  ok(!!(lastPut && lastPut['2026-4']), 'le PUT contient bien le mois 2026-4 (objet ENTIER, pas partiel)');
  ok(!!(lastPut && lastPut['2026-4'] && lastPut['2026-4'].C1 && lastPut['2026-4'].C1['2'] === 'CP'), 'cellule C1 jour 2 = CP dans le cmc_ov écrit');
  ok(!!(lastPut && lastPut['2026-4'] && lastPut['2026-4'].C2 && lastPut['2026-4'].C2['1'] === '20/5'), 'les autres employés/cellules sont PRÉSERVÉS (C2 intact)');
  ok(!!(lastPut && lastPut['2026-4'] && lastPut['2026-4'].C1 && lastPut['2026-4'].C1['1'] === '20/5c'), 'les autres jours du même employé préservés (C1 j1 intact)');
  await ctx.close();
