...
| Version | Changements |
|---------|-------------|
| **v9.771** | **🔬 Audit fidélité cell-par-cell sur capture JUIN_2026_V1 réelle — 4 bugs P0 + bug RH/R + bug CLM (Kevin « tu me dis que l'import fonctionne à 100/100 ? »)**. Audit `tests/audit-fidelity-doimport-juin.mjs` qui appelle le vrai `doImport()` sur la capture réelle puis compare cellule-par-cellule prod vs standalone géométrique. **Désaccords passés de 70+ → 5** (-93 %), dont **5 restants sont des bugs côté STD géo, pas du parser PROD** : (1) `_parseEncadresStatuts` (l. 39637) faisait `A.overrides[key][emp.id]={}` (efface tout) puis écrivait CP → écrasait la rotation de CAISSON JC + LANTERI E (CP 30j au lieu de leur vraie rotation). Fix : skip si emp a ≥10 cellules grille parsées. (2) `_applyCodeArr` (l. 40326) ré-effaçait la grille jour-par-jour puis transformait `16/3*` (CCDP) en `16/3"'` (Convention) pour MORTER L. Fix : check `_existCnt<5` avant reset. (3) Casse `c` chef (l. 41258) : `var code=_codeRaw.toUpperCase()` cassait `20/5"c` en `20/5"C` (MATTONE F, BASILE G, FOREST M, MAGAGNIN J, PASTOR P). Fix : préserve `c` minuscule final avant uppercase. (4) Code `CLM` (l. 41367) skippé car la condition `/\d/.test(code)` exclut alpha purs → MILLO W j26-27 décalés. Fix : apprend aussi codes alpha 2-5 lettres. (5) Bug « RH seul » (Kevin signalé : « il ne prenait en compte que les RH pas le R ») : `_getRhDays` (l. 38745, 37021, 26327) ignorait `R`. Fix : RH primaire (préserve cycle SBM standard) + R fallback si emp sans RH. **TENTATIVE _pdfSection (annulée)** : sub-agent a essayé de détecter section PDF par géométrie (titres « Roulettes » / « Chefs black Jack » / « Employés CMC ») pour résoudre l'affichage sections d'absence (GAZAGNE F chef BJ détaché en formation → affiché « Roul. » au lieu de « Chefs BJ »). Résultat : régression sur ORENGO N (employé CMC → bj géom) ET pas de résolution pour GAZAGNE F (l'encadré AF Formation est physiquement DANS la zone Roulettes du PDF y=910 entre titres « Roulettes » y=1167 et « Chefs black Jack » y=878). **Conclusion** : la géométrie pure ne suffit pas — la cause racine est **ligne 41238 v9.703** du parser grille qui écrase `emp.family` vers `roulettes` pour les chefs BJ qui apparaissent dans la zone Roulettes du PDF. À fixer dans une prochaine itération en protégeant `emp.family` quand l'emp est dans un encadré (= pas dans la grille jour-par-jour). |
