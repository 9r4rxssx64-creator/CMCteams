...
| v9.67 | **Chargement rapide + sécurité + stabilité**. Splash screen instantané, Firebase différé après premier render, diaporama optimisé (cache _bgSig). **Fix email OTP** : toast honnête si EmailJS non configuré, bloc config admin, écran vérification adapté. **Auto-save fiches employés** : onblur sans re-render + batch-save 9 champs + boutons Sauvegarder/Fermer. **Admin édite tous les champs fiche** : adresse, naissance, USM, poste éditables. **CODE_HOURS complet** tous shifts + CDP. **font-size:10px→11px** WCAG. **Seuil repos SBM** convention art.17.5. **Sécurité** : TTL session vérifié toutes les 2min, fbApplyData validation type + try/catch, delEmp confirmation + nettoyage orphelines + audit. |
