...
| Version | Changements |
|---------|-------------|
| **v9.784** | **🔐 Auth anonyme Firebase — étape 1 durcissement P0 (crew ultra-review).** `cmcFbAnonAuth()` (Identity Toolkit REST `accounts:signUp` returnSecureToken) + `_fbAuthQS()` ajoute `?auth=<idToken>` sur `fbWrite`/`fbLoadAll`/`fbPushAll`/`fbStartListening`. Refresh auto ~58min + ré-ouverture SSE. **Fallback gracieux TOTAL** : sans clé Web (`FB_WEB_APIKEY`/`cmc_fb_apikey`) ou si échec → token null → aucun `?auth=` → comportement identique à aujourd'hui (#54 respecté). `firebase-rules-hardened.json` prêt : reste 2 actions Kevin (activer Anonymous console + clé Web) avant publication. Tests 85/85 PASS. |
| **v9.783** | **🎨 Crew ultra-review — quick-wins UX + perf + sécu.** Touch targets tous ≥44px (`.nt` 36→44, mobile 34→44, `.chat-act-btn` 34→44, `.empty-cta` min-height 44). Contraste `--cmc-text-dim/faint` éclairci (WCAG AA+ ~6-7:1). 4 messages d'erreur user-friendly (Firebase/Apex/EmailJS) + détail technique conservé en `console.error` (cause exacte). `doLogout` nettoie intervals globaux restants (anti-fuite session 8h). No-op `_postMigrationFamilies` supprimé. Tests 85/85 PASS, 0 régression. |
