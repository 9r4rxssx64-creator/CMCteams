...
126. **PAGE ÉCRITE À LA MAIN DANS UN DOSSIER DE BUILD = SUPPRIMÉE AU BUILD AUTO SUIVANT (messages.html, v9.855 → wipe build v13.4.337, réparé 2026-07-04)** — La page « Messages employés dans Apex » (`apex-ai-v13/messages.html`, créée par v9.855 #2006) a été **effacée en prod** par l'auto-deploy suivant : `auto-deploy-apex-v13-build.yml` fait `find apex-ai-v13 -mindepth 1 … -exec rm -rf` puis `cp -r dist/*` → TOUT fichier hors build est balayé à CHAQUE build (leçon #54/#57 vue depuis l'autre bout : le dossier de build appartient AU BUILD, à personne d'autre). Le smoke `test:apex-messages` (câblé `test:ci`) a fait exactement son travail — d'abord classé à tort « artefact absent du checkout » alors que c'était une VRAIE régression prod. Preuve par l'historique : `git log --all -- <chemin>` + `git cat-file -e <commit>:<chemin>` → PRÉSENT dans 82f8c7d (création), ABSENT dans 27cb698 (build, 333 deletions). **FIX** : source de vérité déplacée dans `apex-ai/v13/assets/messages.html` — ⚠️ le `publicDir` Vite de CE projet est **`assets`** (PAS `public/` par défaut — lire `vite.config` avant de choisir l'emplacement) → copiée à la racine de `dist/` à chaque build → survit aux wipes ; + restauration immédiate de `apex-ai-v13/messages.html` (prod réparée sans attendre un build). SMOKE PASSED re-mesuré. **OBLIGATIONS** : (1) ne JAMAIS poser un fichier à la main dans un dossier cible d'un workflow de build/deploy — le mettre dans la SOURCE que le build copie (publicDir Vite) ; (2) un test qui échoue sur « fichier absent » → vérifier l'HISTORIQUE avant de classer « environnement » : un fichier qui a existé puis disparaît dans un commit de build = régression réelle ; (3) toute page statique Apex v13 passe par `assets/`. ❌→✅
