...
63. **Restructure services/ en domaines (chantier 1) sans mettre à jour les tests → 69 fichiers de tests cassés** (Apex v13.4.244, 2026-05-21 — CRITIQUE TESTS) — le commit `c8f913ef` « chantier 1 — restructure services/ en 9 domaines » a déplacé ~172 fichiers `services/<x>.ts` → `services/<domaine>/<x>.ts` MAIS n'a touché aucun fichier de test. Conséquence : 69 fichiers de tests (649 tests) cassés silencieusement — leurs `vi.mock('../../services/<x>.js')` pointaient vers des chemins plats disparus → le mock ne s'appliquait JAMAIS → le vrai module se chargeait → `mockedFn.mockReturnValue is not a function`. Idem pour les tests qui lisent du source via `readFileSync('services/<x>.ts')` → ENOENT. **FIX v13.4.244** : 68 fichiers de tests réalignés sur les chemins domaines (mocks `vi.mock`, imports dynamiques, `readFileSync`). **OBLIGATION** : tout déplacement/renommage de fichier source DOIT, dans le MÊME commit, `grep` les tests qui le référencent (`vi.mock`, `import`, `readFileSync`) et mettre à jour leurs chemins — sinon les mocks deviennent des no-op invisibles (le pire : test « vert » qui ne teste plus rien, ou « rouge » non lié à une régression réelle). Un `npm test` complet est obligatoire après toute restructure de dossiers. ❌→✅

62. **Wake-word restart réutilisait l'instance SpeechRecognition morte au lieu d'en créer une fraîche** (Apex v13.4.244, 2026-05-21 — FONCTIONNEL iOS) — `voice-print.ts` et `wake-word.ts` : le handler `rec.onend` faisait `setTimeout(() => rec.start(), 500)` en ré-appelant `.start()` sur la MÊME instance `rec` déjà passée par `onend`. Sur iOS Safari, une instance SpeechRecognition est morte après `onend` → re-`start()` échoue silencieusement (ou `InvalidStateError`) → le wake-word « Dis Apex » meurt après le 1er cycle. Les tests `voice-wake-word-v13.4.51-fresh-instance.test.ts` exigeaient le pattern « fresh instance » mais le code ne l'avait jamais implémenté (Declaration ≠ Deployment, cf. Erreur #28). **FIX v13.4.244** : extraction de `_buildWakeRecognition()`/`_buildRecognition()` (+ `_wireWakeRecognition`) ; `onend` (et la reprise post-suspend) créent désormais une instance NEUVE entièrement re-câblée. **OBLIGATION** : tout restart de SpeechRecognition après `onend` DOIT instancier un objet neuf, jamais réutiliser l'ancien (cf. Erreur #43). ❌→✅
