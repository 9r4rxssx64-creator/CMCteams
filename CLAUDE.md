...
56. **Promesses de fix non tenues 5 versions consécutives — pattern Erreur #28 reproduit** (Apex v13.4.93→98, Kevin 2026-05-15 — CRITIQUE CONFIANCE) — sur 5 commits successifs (v13.4.93 "FIX critiques UX iPhone Kevin", v13.4.94 "Anti-zoom iOS Safari renforce", v13.4.95 "Anti-zoom RADICAL", v13.4.97 "Paste Extractor wire chat", v13.4.98 "Anti-zoom INLINE"), j'ai annoncé à chaque fois avoir CORRIGÉ le zoom Safari iOS. Réalité : Kevin a dû redemander 5 fois ("Zoom tjs UX", "Encore zoom"). Cause : le fix anti-zoom JS dépendait du bundle qui pouvait arriver après le premier touch lors d'une transition SW. La vraie solution (script inline `<head>` AVANT bootstrap.js) aurait dû être la PREMIÈRE tentative. **OBLIGATIONS** : (a) test mental obligatoire **AVANT** push d'un fix UX iPhone : "le code de mon fix est-il chargé AVANT que l'utilisateur puisse interagir avec la page ?" Si non → script inline obligatoire. (b) Si Kevin redemande le même fix → la cause racine n'a pas été comprise, STOP et faire audit factuel cible (pas re-tenter avec un autre patch). (c) Documenter dans CLAUDE.md TOUT pattern qui se reproduit 2 fois consécutivement. ❌→✅
