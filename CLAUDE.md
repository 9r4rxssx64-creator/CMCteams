...
90. **GitHub MCP FONCTIONNE dans cet environnement Claude Code web — merge par API, fin de la friction proxy (Claude Code, 2026-06-06, shops/chez-lolo)** — contrairement aux leçons #78/#79/#80/#86 (proxy git `127.0.0.1` non-propageant, auto-merge bot bloqué, MCP « absent »), les outils `mcp__github__*` se sont chargés et sont pleinement opérationnels : `get_me`, `get_file_contents` (ref=branche/main), `list_commits`, `create_pull_request`, `merge_pull_request` (squash), `actions_list`. J'ai livré 4 PR Chez Lolo (#849/#851/#853) **créées + mergées par API**, chacune **vérifiée sur le vrai main** via `get_file_contents`/`list_commits` (SHA témoin). **OBLIGATIONS** : (a) quand le GitHub MCP est chargé (le vérifier via ToolSearch en début de session), **l'utiliser pour merger** (create_pull_request → merge_pull_request) au lieu de dépendre du proxy `git push` (qui peut ne pas propager) ou de l'auto-merge bot (qui peut être bloqué par le réglage « Allow GitHub Actions to create and approve PRs », cf. #86) ; (b) **toujours vérifier la propagation réelle** après merge via `get_file_contents` au `ref=main` (un fichier témoin léger comme `sw.js`), JAMAIS via `git show origin/main` du proxy (leçon #79 : le proxy ment) ; (c) le MCP est instable/peut flapper (a disparu puis réapparu en cours de session) — si absent, retomber sur « je prépare le diff, Kevin merge sur github.com » ; (d) pour un worker auto-déployé, confirmer le run via `actions_list(list_workflow_runs, <workflow>.yml)`. ✅

91. **ÉGRESS RÉSEAU SANDBOXÉ — `curl`/fetch sortant bloqué (« Host not in allowlist »), NE PAS inventer de données qu'on ne peut pas vérifier en ligne (Claude Code, 2026-06-06, Printify blueprints)** — pour mapper les `blueprint_id` Printify des garments manquants (polo/tank/shirt/zip/jogger/short/bandana) de Chez Lolo, j'ai voulu interroger le catalogue Printify via le worker déployé : `curl https://ld-printify-order.9r4rxssx64.workers.dev/` → **« Host not in allowlist »** (sandbox CCR bloque tout égress vers `*.workers.dev` et domaines tiers, cf. règle « SANDBOX BLOCKAGES »). Conséquence : impossible de récupérer les vrais IDs d'ici. **DÉCISION CONFORME aux règles** (« reproduction à l'identique », « jamais estimer/inventer », « pas de Security Theater ») : NE PAS baker d'IDs devinés (un mauvais blueprint = commande Printify fausse). J'ai laissé le fallback **sûr** (garment inconnu → tee + `warning` remonté + statut **on-hold** = Kevin valide avant tout débit) et documenté l'action dans KEVIN_ACTIONS_TODO (#B). **OBLIGATIONS** : (a) tout enrichissement de données externes (IDs catalogue, prix, variantes) doit venir d'une **source vérifiable** ; si l'égress est bloqué, livrer le **mécanisme de découverte** (ex : endpoint worker `/blueprints?q=` à appeler depuis un contexte réseau) + demander la donnée à Kevin, plutôt que deviner ; (b) un fallback honnête + warning + garde-fou (on-hold) vaut mieux qu'une donnée inventée qui « a l'air complète ». ✅
