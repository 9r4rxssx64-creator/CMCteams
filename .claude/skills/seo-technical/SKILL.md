...
python .claude/skills/seo/scripts/agent_ux_check.py https://example.com --json
```
...
without scoring, use `python .claude/skills/seo/scripts/render_page.py <url> --a11y-tree --json`.
...
If Google API credentials are configured, use `python .claude/skills/seo/scripts/pagespeed_check.py <url> --json` for real PSI + CrUX field data (replaces lab-only CWV estimates), `python .claude/skills/seo/scripts/crux_history.py <url> --json` for 25-week CWV trends, and `python .claude/skills/seo/scripts/gsc_inspect.py <url> --json` for real indexation status per URL.
