...
instead of WebSearch for SERP data. Run `python .claude/skills/seo/scripts/dataforseo_costs.py check serp_organic_live_advanced --count N`
...
- All URLs fetched via `python .claude/skills/seo/scripts/fetch_page.py` (SSRF protection via `validate_url()`)
