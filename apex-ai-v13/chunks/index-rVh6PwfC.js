import{e as n}from"./escape-html-BlQj2yEF.js";import{a as h}from"./monitoring-Bl85iYvy.js";import{toast as b}from"./toast-CRdbcLoc.js";import"./multi-source-analyze-DfuDcC7m.js";import"./apex-kb-CAfWvMOn.js";import"./credential-patterns-CLzI061R.js";import"./haptic-CQFg2PXZ.js";const c={action:"",actor:"",search:""};function w(t){const a=new Map,s=new Map;let i=1/0,d=0;for(const e of t)a.set(e.action,(a.get(e.action)??0)+1),s.set(e.actor,(s.get(e.actor)??0)+1),e.ts<i&&(i=e.ts),e.ts>d&&(d=e.ts);const u=[...a.entries()].map(([e,r])=>({action:e,count:r})).sort((e,r)=>r.count-e.count).slice(0,5),g=[...s.entries()].map(([e,r])=>({actor:e,count:r})).sort((e,r)=>r.count-e.count).slice(0,5);return{total:t.length,topActions:u,topActors:g,oldest_ts:i===1/0?null:i,newest_ts:d===0?null:d}}function $(t,a){return t.filter(s=>!(a.action&&!s.action.toLowerCase().includes(a.action.toLowerCase())||a.actor&&!s.actor.toLowerCase().includes(a.actor.toLowerCase())||a.search&&![s.target??"",JSON.stringify(s.details??{})].join(" ").toLowerCase().includes(a.search.toLowerCase())))}function v(t){return new Date(t).toLocaleString("fr-FR",{dateStyle:"short",timeStyle:"medium"})}function S(t){const a=t.target?n(t.target):'<em style="opacity:0.5">—</em>',s=t.details?Object.keys(t.details).length:0,i=s>0?`<button class="ax-btn-glass" data-audit-details='${n(JSON.stringify(t.details??{}))}' style="font-size:10px;padding:2px 6px">${s} fields</button>`:"";return`
    <tr class="ax-gs-184">
  ...
  `;t.innerHTML=d;const u=t.querySelector("#audit-filter-action"),g=t.querySelector("#audit-filter-actor"),e=t.querySelector("#audit-filter-search"),r=t.querySelector("#audit-filter-reset"),y=200;let x=null;const f=(o,l)=>{o&&o.addEventListener("input",()=>{x&&clearTimeout(x),x=setTimeout(()=>{c[l]=o.value,m(t)},y)})};f(u,"action"),f(g,"actor"),f(e,"search"),r?.addEventListener("click",()=>{c.action="",c.actor="",c.search="",m(t)}),t.querySelector("#audit-export-json")?.addEventListener("click",()=>{const o=new Blob([JSON.stringify(a,null,2)],{type:"application/json"}),l=URL.createObjectURL(o),p=document.createElement("a");p.href=l,p.download=`apex-audit-log-${new Date().toISOString().slice(0,10)}.json`,p.click(),URL.revokeObjectURL(l),b.success(`📥 Export OK : ${a.length} entries → JSON téléchargé`,{duration:4e3})}),t.querySelectorAll("[data-audit-details]").forEach(o=>{o.addEventListener("click",()=>{const l=o.getAttribute("data-audit-details")??"{}";try{const p=JSON.parse(l);alert(`Details :
`+JSON.stringify(p,null,2))}catch{alert("Details : "+l)}})})}export{m as render};
