...
>📊 Usage</a>`:"";return`
        <div class="ax-service-health-row ax-modernized-card"
          ...
          ${f}
          ${x}
        </div>`}).join("")}async function I(t){const r={};if(t.length===0)return r;try{const{linksRegistry:o}=await g(async()=>{const{linksRegistry:i}=await import("./multi-source-analyze-twQruZB2.js").then(e=>e.c);return{linksRegistry:i}},__vite__mapDeps([3,1,0,2]),import.meta.url);for(const i of t)r[i]={recharge:o.getRechargeLink(i),usage:o.getUsageLink(i),apiKeys:o.getApiKeysLink(i)}}catch{}return r}async function W(t){d?.cleanup(),d=u("dashboard");const r=m.get("user");if(!y("admin.dashboard",t,r?.id))return;const o=r?.name?`Bonjour ${r.name}`:"Bonjour",[i,e,c]=await Promise.all([k(),$(),z()]),l=await I(c.map(s=>s.service)),n=_();t.innerHTML=h.withNonce(`
    <style>
