import{a as $}from"./escape-html-DGIYNPKb.js";import{c as M}from"./listener-cleanup-Y2rGGxxX.js";import{l as k}from"./monitoring-DMtdadhB.js";import{i as z}from"../core/main-DWKp7aw7.js";import{g as S}from"./apex-tools-dispatch-core-DQuM_oqn.js";import{c as l}from"./voice-M72bdHzo.js";import{haptic as C}from"./haptic-CQFg2PXZ.js";import{toast as u}from"./toast-CRdbcLoc.js";import"./apex-kb-z_yh4cKT.js";import"./credential-patterns-CLzI061R.js";import"./multi-source-analyze-twQruZB2.js";import"./apex-tools-dispatch-skills-DpZTMLLg.js";import"./apex-tools-dispatch-data-BwOi3ySV.js";import"./apex-tools-dispatch-finance-DoRAfEZC.js";import"./apex-tools-dispatch-misc-_39PAfAX.js";import"./apex-tools-misc-Co2hbyeh.js";import"./apex-tools-registry-core-3xEquv0D.js";import"./apex-tools-registry-skills-DRX4PPQ_.js";const A="kdmc_admin";let v=null;function Z(){v?.cleanup(),v=null}function y(e){if(!e||e===0)return"—";const t=new Date(e),r=i=>i.toString().padStart(2,"0");return`${r(t.getDate())}/${r(t.getMonth()+1)}/${t.getFullYear()} ${r(t.getHours())}:${r(t.getMinutes())}`}function b(e){return Math.round(e*100)+"%"}function _(){const e=z.get("user");return!e||!e.id?null:{id:e.id,name:e.name??"",isAdmin:e.id===A}}function x(e,t,r,i){t&&e.bind(t,r,i)}function D(e){const t=Math.max(0,Math.min(1,e))*100,r=t>=85?"#22c55e":t>=50?"#ffa500":"#ff6666";return`
  ...
  `}function q(e){const t=l.getPrintFor(e.id),r=l.needsCalibration(e.id),i=l.isExclusiveMode(),o=l.getPhaseDetails(e.id),p=l.isExclusiveAnticipated();if(!t)return`
    ...
    `;const s=t.confidence_score??0,n=t.samples_count,a=r.needs?`<div style="background:rgba(255,165,0,0.1);border:1px solid rgba(255,165,0,0.3);border-radius:8px;padding:10px 14px;margin:10px 0;color:#ffa500;font-size:13px">
       ...
       </div>`:"",d=Math.round(o.progress*100),c=o.phase==="open"?"#9ca3af":o.phase==="learning"?"#facc15":o.phase==="refining"?"#fb923c":"#22c55e",g=o.samples_to_next>0?`<span style="color:#888;font-size:12px;margin-left:6px">(${o.samples_to_next} samples avant phase suivante)</span>`:'<span style="color:#22c55e;font-size:12px;margin-left:6px">(phase finale atteinte ✓)</span>',f=`
    <div style="background:rgba(15,15,28,0.5);border:1px solid ${c}33;border-radius:10px;padding:14px;margin:14px 0">
      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;margin-bottom:8px">
        <div style="font-size:14px;color:#fff;font-weight:600">Apprentissage voix : ${n}/20 samples (${d}%)</div>
        <div style="background:${c}22;border:1px solid ${c}55;color:${c};font-size:12px;font-weight:700;padding:4px 10px;border-radius:14px">${$(o.label)}</div>
      </div>
      ...
      <div style="font-size:12px;color:#aaa;margin-top:8px">
        Seuil actuel : <b>${Math.round(o.threshold*100)}%</b> de similarité requise${g}
      </div>
      ${p&&o.phase==="exclusive"&&n<20?'<div style="font-size:12px;color:#22c55e;margin-top:6px">⚡ Mode exclusif anticipé activé — phase exclusive forcée dès 10 samples</div>':""}
    </div>
  `;return`
    <div style="background:rgba(20,20,35,0.5);border:1px solid rgba(201,162,39,0.2);border-radius:12px;padding:18px;margin-bottom:14px">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:14px;margin-bottom:16px">
        <div>
          ...
          <div style="color:#fff;font-size:22px;font-weight:700">${n} <span style="font-size:13px;color:#888">/ 20</span></div>
        </div>
        <div>
  ...
  `}function P(e){if(!e.isAdmin)return"";const t=l.listPrints(),r=l.getStats(),i=l.getUnknownAttempts(),o=i.slice(-10).reverse(),p=t.map(n=>{const a=n.confidence_score??0;return`
        <tr style="border-top:1px solid rgba(255,255,255,0.05)">
          <td style="padding:8px 6px;color:#fff;font-weight:600">${$(n.uid)}${n.uid===A?" 👑":""}</td>
          ...
          <td style="padding:8px 6px"><button data-uid="${$(n.uid)}" class="ax-vbio-admin-del" style="padding:4px 10px;background:rgba(255,102,102,0.15);color:#ff6666;border:1px solid rgba(255,102,102,0.3);border-radius:5px;font-size:11px;cursor:pointer">🗑</button></td>
        </tr>
      `}).join(""),s=o.map(n=>`
      <tr style="border-top:1px solid rgba(255,255,255,0.05)">
        <td style="padding:6px;color:#aaa;font-size:11px">${y(n.ts)}</td>
        <td style="padding:6px;color:#ff6666">${b(n.score)}</td>
        <td style="padding:6px;color:#aaa">${n.pitch} Hz</td>
        <td style="padding:6px;color:#aaa">${n.energy.toFixed(2)}</td>
      </tr>
  ...
  `}async function T(e){if(typeof navigator>"u"||!navigator.mediaDevices?.getUserMedia)return u.warn("Micro non disponible sur ce navigateur"),null;try{const t=await navigator.mediaDevices.getUserMedia({audio:!0}),r=new MediaRecorder(t),i=[];return r.addEventListener("dataavailable",o=>{o.data.size>0&&i.push(o.data)}),await new Promise(o=>{r.addEventListener("stop",()=>{t.getTracks().forEach(s=>s.stop()),new Blob(i,{type:"audio/webm"}).arrayBuffer().then(async s=>{try{const n=window.AudioContext??window.webkitAudioContext;if(!n){o(null);return}const d=await new n().decodeAudioData(s);o(d)}catch{o(null)}})}),r.start(),setTimeout(()=>r.stop(),e)})}catch(t){const r=t instanceof Error?t.message:"erreur";return u.warn(`Micro refusé : ${r}`),null}}async function w(e,t,r){const i=document.createElement("div");i.innerHTML=B(),document.body.appendChild(i);const o=()=>{i.remove()},p=[],s=i.querySelector("#ax-vbio-record"),n=i.querySelector("#ax-vbio-cancel"),a=i.querySelector("#ax-vbio-enroll-status");x(r,n,"click",()=>{o(),u.warn("Enrôlement annulé")});let d=0;const c=async()=>{if(!s)return;s.disabled=!0,s.textContent="🎤 Enregistrement…",a&&(a.textContent=`Sample ${d+1}/3 en cours… parle maintenant !`);const g=await T(3e3);if(!g){s.disabled=!1,s.textContent=`🔴 Réessayer (${d+1}/3)`,a&&(a.textContent="⚠ Échec enregistrement");return}if(p.push(g),d++,d>=3){a&&(a.textContent="✓ 3 samples capturés. Calcul empreinte…");const f=await l.enroll(e,p);f.ok?(l.markCalibrated(e),u.success(`✅ Voix enrôlée — ${f.samples_count} samples, ${b(f.confidence_score??0)} fiabilité`),C.tap(),o(),await m(t)):(a&&(a.textContent=`❌ Échec : ${f.reason??"inconnu"}`),s.disabled=!1,s.textContent="🔴 Réessayer")}else s.disabled=!1,s.textContent=`🔴 Enregistrer (${d+1}/3)`,a&&(a.textContent=`✓ Sample ${d}/3 OK — clique pour le suivant`)};x(r,s,"click",()=>{c()})}async function m(e){if(!e){k.warn("voice-bio","render called without rootEl");return}v?.cleanup(),v=M("voice-bio");const t=v,r=z.get("user")?.id??"anon";if(!S("voice.biometric",e,r))return;const i=_();if(!i){e.innerHTML=`
  ...
  `;e.innerHTML=o;const p=e.querySelector("#ax-vbio-enroll-start");x(t,p,"click",()=>{w(i.id,e,t)});const s=e.querySelector("#ax-vbio-recalibrate");x(t,s,"click",()=>{w(i.id,e,t)});const n=e.querySelector("#ax-vbio-delete");x(t,n,"click",()=>{if(!confirm("Supprimer ton empreinte vocale ? Cette action est irréversible (RGPD Art. 17)."))return;l.deletePrint(i.id)?(u.success("🗑 Empreinte supprimée"),m(e)):u.warn("Échec suppression")});const a=e.querySelector("#ax-vbio-exclusive");x(t,a,"change",()=>{a&&(l.setExclusiveMode(a.checked),u.success(`Mode exclusif : ${a.checked?"ON":"OFF"}`))});const d=e.querySelector("#ax-vbio-exclusive-anticipated");x(t,d,"change",()=>{d&&(l.setExclusiveAnticipated(d.checked),u.success(`Mode exclusif anticipé : ${d.checked?"ON":"OFF"}`),m(e))});const c=e.querySelector("#ax-vbio-recalibrate-banner");if(x(t,c,"click",()=>{w(i.id,e,t)}),i.isAdmin){const g=e.querySelector("#ax-vbio-clear-unknown");x(t,g,"click",()=>{l.clearUnknownAttempts(),u.success("Tentatives ignorées effacées"),m(e)}),e.querySelectorAll(".ax-vbio-admin-del").forEach(f=>{x(t,f,"click",()=>{const h=f.dataset.uid;h&&confirm(`Supprimer empreinte de ${h} ?`)&&(l.deletePrint(h),u.success(`Empreinte ${h} supprimée`),m(e))})})}k.info("voice-bio",`rendered for ${i.id}${i.isAdmin?" (admin)":""}`)}export{Z as dispose,m as render};
