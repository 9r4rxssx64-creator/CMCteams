import{e as o}from"./monitoring-CpAzCFTb.js";import{c as b}from"./listener-cleanup-Y2rGGxxX.js";import{r as l}from"../core/main-Bc624Mxw.js";import{haptic as x}from"./haptic-CQFg2PXZ.js";import{toast as g}from"./toast-BCPNzfMv.js";import"./multi-source-analyze-CABQFfu3.js";import"./credential-patterns-DUMYZEMu.js";import"./apex-kb-DvSa-GZo.js";import"./memory-CZfT7Z1y.js";import"./apex-tools-dispatch-misc-qBiErVMY.js";let n=null,i=null;function A(){n?.cleanup(),n=null,i&&(clearInterval(i),i=null)}function p(){try{const t=localStorage.getItem("apex_v13_signup_pending_id");return t?JSON.parse(localStorage.getItem("apex_v13_signup_requests")??"[]").find(a=>a.id===t)??null:null}catch{return null}}function I(t){n?.cleanup(),n=b("waiting-approval"),s(t),i&&clearInterval(i),i=setInterval(()=>{const e=p();if(e)if(e.status==="approved"){g.success("🎉 Compte approuvé ! Tu peux te connecter.");try{localStorage.removeItem("apex_v13_signup_pending_id")}catch{}setTimeout(()=>l.navigate("landing"),1500)}else(e.status==="rejected"||e.status==="expired")&&s(t)},3e4)}function s(t){const e=p(),a=(()=>{try{return localStorage.getItem("apex_v13_signup_invite_link")??""}catch{return""}})();if(!e){t.innerHTML=`
    ...
    `;const u=t.querySelector("#wa-back");u&&n&&n.bind(u,"click",()=>l.navigate("landing"));return}const r=`${e.prenom} ${e.nom}`,d=e.otp,m=e.expiresAt-Date.now(),v=Math.max(0,Math.ceil(m/(1440*60*1e3))),c=e.status==="approved"?{color:"#10b981",icon:"✅",label:"Approuvé"}:e.status==="rejected"?{color:"#ef4444",icon:"❌",label:"Refusé"}:e.status==="expired"?{color:"#888",icon:"⏰",label:"Expiré"}:{color:"#c9a227",icon:"⏳",label:"En attente Kevin"};t.innerHTML=`
    <div class="ax-gs-402">
      <div class="ax-gs-441">
        <div style="text-align:center;margin-bottom:20px">
          <div style="font-size:42px;margin-bottom:8px">${c.icon}</div>
          <h1 style="margin:0 0 4px;color:${c.color};font-size:24px">${c.label}</h1>
          <p class="ax-gs-442">${o(r)}</p>
        </div>
        ...
        <div style="display:grid;gap:8px;margin-top:16px">
          ...
          <button id="wa-cancel" class="ax-btn ax-btn-ghost ax-btn-block ax-gs-270">${e.status==="rejected"||e.status==="expired"?"📝 Nouvelle demande":"Annuler ma demande"}</button>
          <button id="wa-back" class="ax-btn ax-btn-ghost ax-btn-block ax-gs-270">← Connexion</button>
        </div>
      </div>
    </div>
  `,f(t)}function f(t){if(!n)return;const e=t.querySelector("#wa-check");e&&n.bind(e,"click",()=>{x.tap(),s(t),p()?.status==="awaiting_kevin"&&g.info("Toujours en attente Kevin")});const a=t.querySelector("#wa-cancel");a&&n.bind(a,"click",()=>{x.medium();try{localStorage.removeItem("apex_v13_signup_pending_id"),localStorage.removeItem("apex_v13_signup_invite_link")}catch{}l.navigate("signup")});const r=t.querySelector("#wa-back");r&&n.bind(r,"click",()=>l.navigate("landing"))}export{A as dispose,I as render};
