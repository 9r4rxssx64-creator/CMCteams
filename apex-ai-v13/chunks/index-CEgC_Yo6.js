const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./apex-kb-OcXW5GOw.js","./monitoring-BD1M85nM.js","./multi-source-analyze-DIgwEfjb.js","./credential-patterns-DUMYZEMu.js"])))=>i.map(i=>d[i]);
import{b as S,l as _,e as s,_ as y}from"./monitoring-BD1M85nM.js";import{c as T}from"./listener-cleanup-Y2rGGxxX.js";import{credentialsAudit as k}from"./credentials-audit-B6vF2cgO.js";import{haptic as f}from"./haptic-CQFg2PXZ.js";import{toast as p}from"./toast-BCPNzfMv.js";import"./multi-source-analyze-DIgwEfjb.js";import"./credential-patterns-DUMYZEMu.js";import"./apex-kb-OcXW5GOw.js";let u=null,m=null,v="all";function j(){u?.cleanup(),u=null}const h={ok:{color:"#22cc77",icon:"🟢",label:"OK"},missing:{color:"#888",icon:"⚪",label:"Non config"},corrupted:{color:"#ff6b6b",icon:"🔴",label:"Corrompu"},expired:{color:"#ffaa00",icon:"🟠",label:"Expiré"},unknown:{color:"#aaa",icon:"❓",label:"Inconnu"},decrypt_failed:{color:"#ff6b6b",icon:"🔒",label:"Illisible"}},$={all:"Tous",ai:"🤖 IA",banking:"🏦 Banque",payment:"💳 Paiement",social:"📱 Social",email:"📧 Email",crypto:"₿ Crypto",hosting:"☁️ Hosting",productivity:"📋 Productivité",forbidden:"⚠️ Forbidden"};async function A(e){if(!(S.get("isAdmin")===!0)){e.innerHTML=`
      <div class="ax-gs-21">
        <h2 class="ax-gs-266">Accès admin uniquement</h2>
        <p>Cette section est réservée à Kevin.</p>
      </div>
    `;return}u?.cleanup(),u=T("credentials-registry"),e.innerHTML=`
    <div class="ax-gs-57">
      <p>🔍 Audit credentials en cours...</p>
    </div>
  `;try{m=await k.runFullAudit()}catch(o){_.error("credentials-registry","audit failed",{err:o}),e.innerHTML=`<div style="padding:40px;color:#ff6b6b">Erreur audit : ${s(String(o))}</div>`;return}w(e,m)}function w(e,i){const o=i.security_score,t=o>=80?"#22cc77":o>=60?"#ffaa00":"#ff6b6b",c=[...v==="all"?i.entries:i.entries.filter(r=>r.category===v)].sort((r,l)=>r.configured!==l.configured?r.configured?-1:1:r.service_name.localeCompare(l.service_name)),n=new Map;n.set("all",i.entries.length);for(const r of i.entries)n.set(r.category,(n.get(r.category)??0)+1);const a=[...n.entries()].filter(([r])=>r==="all"||$[r]&&(n.get(r)??0)>0).map(([r,l])=>{const b=v===r,C=$[r]??r;return`<button data-cat="${s(r)}" class="ax-cat-chip" style="
  ...
  `,document.body.appendChild(t);const d=t.querySelector("#ax-recover-input"),c=t.querySelector("#ax-recover-cancel"),n=t.querySelector("#ax-recover-confirm");setTimeout(()=>d?.focus(),50);const a=()=>{t.remove()};c?.addEventListener("click",a),t.addEventListener("click",g=>{g.target===t&&a()}),n?.addEventListener("click",()=>{(async()=>{const g=d?.value.trim()??"";if(!g){p.warn("Recolle ta clé pour récupérer");return}try{const{vault:x}=await y(async()=>{const{vault:l}=await import("./apex-kb-OcXW5GOw.js").then(b=>b.b);return{vault:l}},__vite__mapDeps([0,1,2,3]),import.meta.url),r=await x.recover(i,g);r.ok?(f.success(),p.success(`✅ ${o} récupérée + re-chiffrée`),a(),A(e)):(f.error(),p.error(`❌ ${r.reason??"recover failed"}`))}catch(x){console.error("credentials action:",x),p.error("Action impossible pour le moment — réessaie")}})()})}function L(e){if(!u)return;const i=e.querySelector("#ax-cred-refresh");i&&u.bind(i,"click",()=>{f.tap(),A(e)});const o=e.querySelector("#ax-cred-test-channels");o&&u.bind(o,"click",()=>{f.tap(),(async()=>{p.info("📡 Test alertes en cours...");const{kevinAlerts:t}=await y(async()=>{const{kevinAlerts:n}=await import("./kevin-alerts-D4BOnvIE.js");return{kevinAlerts:n}},__vite__mapDeps([1,2,3,0]),import.meta.url),d=await t.testAllChannels(),c=Object.entries(d).filter(([,n])=>n).map(([n])=>n);c.length===0?p.warn("Aucun channel d'alerte configuré"):p.success(`✅ Channels OK : ${c.join(", ")}`)})()}),e.querySelectorAll("[data-cat]").forEach(t=>{u.bind(t,"click",()=>{v=t.dataset.cat??"all",f.selection(),m&&w(e,m)})}),e.querySelectorAll("[data-recover]").forEach(t=>{u.bind(t,"click",()=>{const d=t.dataset.recover??"",c=t.dataset.service??d;d&&(f.tap(),R(e,d,c))})}),e.querySelectorAll("[data-test]").forEach(t=>{u.bind(t,"click",d=>{d.stopPropagation();const c=t.dataset.test;c&&(f.tap(),(async()=>{const n=t.textContent;t.textContent="⏳ Test...",t.disabled=!0;try{const a=await k.testCredential(c);a.valid===!0?p.success(`✅ ${c} : valide (${a.latency_ms}ms)`):a.valid===!1?p.warn(`❌ ${c} : invalide`):p.info(`❓ ${c} : ${a.error??"test impossible"}`)}catch(a){console.error("credentials test:",a),p.error("Test de la clé impossible — réessaie")}finally{t.textContent=n??"🧪 Tester",t.disabled=!1}})())})}),e.querySelectorAll("[data-cred-detail]").forEach(t=>{u.bind(t,"click",d=>{d.target.closest("button, a")||(async()=>{const n=t.dataset.credDetail;if(!n||!m)return;const a=m.entries.find(l=>l.storage_key===n);if(!a)return;const{drillDown:g}=await y(async()=>{const{drillDown:l}=await import("./drilldown-1SlvGToi.js");return{drillDown:l}},[],import.meta.url),x="ax-drilldown-mount-credentials";let r=document.getElementById(x);r||(r=document.createElement("div"),r.id=x,document.body.appendChild(r)),g.open({id:`cred-${n}`,title:`🔑 ${a.service_name}`,content:()=>{const l=h[a.status]??h.unknown;return`
              <div class="ax-gs-27">
            ...
            `},data:{storageKey:n}},r)})()})})}export{j as dispose,A as render};
