...
Cordialement.`},Y={A1:"Débutant - phrases simples, vocabulaire basique",A2:"Élémentaire - conversations simples",B1:"Intermédiaire - voyages, sujets familiers",B2:"Intermédiaire avancé - autonome, idées complexes",C1:"Avancé - usage souple, sujets variés",C2:"Maîtrise - quasi-natif, nuances subtiles",natif:"Langue maternelle"},Z={tech:["JavaScript","TypeScript","Python","React","Node.js","Docker","Kubernetes","AWS","PostgreSQL","Git","CI/CD","GraphQL","Microservices"],data:["Python","SQL","Pandas","NumPy","Scikit-learn","TensorFlow","PyTorch","Tableau","Power BI","Hadoop","Spark","Statistics","Machine Learning"],marketing:["SEO","SEM","Google Analytics","Google Ads","Facebook Ads","Hubspot","Mailchimp","Content marketing","Copywriting","A/B testing","CRM","Marketing automation"],finance:["Excel avancé","SAP","Sage","Bloomberg","CFA","Modélisation financière","IFRS","Consolidation","Analyse risque","M&A","Trading"],sales:["Salesforce","Pipedrive","Cold calling","Négociation","Account management","Pipeline management","CRM","B2B","B2C","SaaS sales"],rh:["Recrutement","Paie","Droit social","Gestion conflits","Formation","GPEC","Workday","SIRH","Onboarding","Politique RH"],design:["Figma","Sketch","Adobe XD","Photoshop","Illustrator","InDesign","Prototyping","Wireframing","UI/UX","Design system","Motion design"],produit:["Agile","Scrum","Kanban","JIRA","User stories","Roadmap","OKR","Discovery","A/B testing","Analytics","Product Management"],comm:["Communication interne","Communication externe","RP","Réseaux sociaux","Community management","Storytelling","Brand","Crisis management"],juridique:["Droit civil","Droit commercial","Droit social","Droit fiscal","Contrats","Compliance","RGPD","M&A","Contentieux","Property"]};function l(n){return n.replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e]??e)}function h(n){return`${k}${n}`}function y(n,e,t="fr"){return{template:n,lang:t,identite:{prenom:e?.prenom??"",nom:e?.nom??"",email:"",telephone:"",adresse:"",titre:"",photo:"",linkedin:"",github:"",site:""},resume:"",experiences:[],formations:[],certifications:[],projets:[],references:[],competences:[],langues:[{lang:"Français",niveau:"natif"}],loisirs:""}}function w(){return{id:`exp_${Date.now()}_${Math.random().toString(36).slice(2,8)}`,poste:"",entreprise:"",ville:"",date_debut:"",date_fin:"",description:"",achievements:[]}}function T(){return{id:`form_${Date.now()}_${Math.random().toString(36).slice(2,8)}`,diplome:"",ecole:"",ville:"",annee:"",mention:""}}function R(){return{id:`cert_${Date.now()}_${Math.random().toString(36).slice(2,8)}`,nom:"",organisme:"",date:"",url:""}}function F(){return{id:`proj_${Date.now()}_${Math.random().toString(36).slice(2,8)}`,nom:"",description:"",url:"",technologies:[]}}function _(){return{id:`ref_${Date.now()}_${Math.random().toString(36).slice(2,8)}`,nom:"",poste:"",entreprise:"",email:"",telephone:""}}function S(n){return/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(n)}function ee(n){return n?/linkedin\.com\/in\/([a-zA-Z0-9-_%]+)/i.exec(n)?.[1]??"":""}function te(n){return n?/github\.com\/([a-zA-Z0-9-_]+)/i.exec(n)?.[1]??"":""}function D(n){let e=0;return n.identite.prenom&&(e+=8),n.identite.nom&&(e+=8),n.identite.email&&S(n.identite.email)&&(e+=8),n.identite.telephone&&(e+=5),n.identite.titre&&(e+=8),n.resume&&(e+=8),n.experiences.length>0&&(e+=15),n.experiences.length>=3&&(e+=5),n.formations.length>0&&(e+=12),n.competences.length>=3&&(e+=8),n.competences.length>=8&&(e+=5),n.langues.length>=2&&(e+=5),n.identite.linkedin&&(e+=5),Math.min(100,e)}function I(n){const e=[],t=[];let i=100;(!n.identite.email||!S(n.identite.email))&&(e.push("Email manquant ou invalide"),i-=15),n.identite.telephone||(e.push("Téléphone manquant"),i-=10),n.experiences.length===0&&(e.push("Aucune expérience renseignée"),i-=20),n.formations.length===0&&(e.push("Aucune formation renseignée"),i-=15),n.competences.length<3&&(e.push("Moins de 3 compétences listées"),i-=10,t.push("Ajoute au moins 5-8 compétences clés du métier visé"));const r=g.find(c=>c.id===n.template);r&&r.atsScore<80&&(t.push(`Template "${r.label}" peu ATS-friendly (${r.atsScore}/100). Pour grandes entreprises → choisir Classique ou Tech.`),i-=(100-r.atsScore)/5);const a=n.experiences.filter(c=>/\d+\s*(%|€|k€|M€|client|projet|équipe|personne)/i.test(c.description)||c.achievements.some(o=>/\d+/.test(o))).length;return n.experiences.length>0&&a===0&&(t.push('Quantifie tes réussites (ex: "+30% CA", "équipe de 5", "200K€ budget")'),i-=5),n.identite.photo&&n.lang==="en"&&t.push("Photo non conventionnelle en CV anglo-saxon (US/UK)."),{score:Math.max(0,Math.min(100,Math.round(i))),issues:e,suggestions:t}}function ie(n,e){if(!e||e.trim().length===0)return{score:0,keywordsFound:[],keywordsMissing:[]};const t=new Set(["pour","avec","dans","vous","nous","mais","plus","tres","votre","notre","cette","with","this","that","have","from","will","your","were","they","their"]),i=e.toLowerCase().replace(/[^a-zàâçéèêëîïôûùüÿñæœ0-9\s]/g," ").split(/\s+/).filter(s=>s.length>=4&&!t.has(s)),r=new Map;for(const s of i)r.set(s,(r.get(s)??0)+1);const a=Array.from(r.entries()).sort((s,q)=>q[1]-s[1]).slice(0,30).map(([s])=>s),c=JSON.stringify(n).toLowerCase(),o=[],m=[];for(const s of a)c.includes(s)?o.push(s):m.push(s);return{score:a.length>0?Math.round(o.length/a.length*100):0,keywordsFound:o,keywordsMissing:m}}function ne(n,e,t){const i=n.identite.prenom||"[Prénom]",r=n.identite.nom||"[Nom]",a=n.identite.email||"[email]",c=n.identite.telephone||"[téléphone]",o=n.identite.titre||"[Titre actuel]",m=n.experiences[0],x=m?`${m.poste} chez ${m.entreprise}`:"[dernière expérience]",s=n.competences.slice(0,3).join(", ")||"[compétences]";return`${i} ${r}
${a} · ${c}
...
Actuellement ${o}, fort de mon expérience en tant que ${x}, je vous adresse ma candidature au poste de ${e||"[Poste]"} au sein de ${t||"[entreprise]"}.
...
Bien cordialement,
${i} ${r}`}class z{load(e){if(!e)return null;try{const t=localStorage.getItem(h(e));if(!t)return null;const i=JSON.parse(t);return!i||typeof i!="object"?null:i}catch(t){return p.warn("studio-cv","load failed",{err:t}),null}}save(e,t){if(!e)return!1;try{return localStorage.setItem(h(e),JSON.stringify(t)),!0}catch(i){return p.warn("studio-cv","save failed (quota?)",{err:i}),!1}}setTemplate(e,t){const i=this.load(e)??y(t);return i.template=t,this.save(e,i),i}setLang(e,t){const i=this.load(e);return i?(i.lang=t,this.save(e,i)):!1}addExperience(e){const t=this.load(e);return t?(t.experiences.length>=v||(t.experiences.push(w()),this.save(e,t)),t):null}addFormation(e){const t=this.load(e);return t?(t.formations.length>=b||(t.formations.push(T()),this.save(e,t)),t):null}addCertification(e){const t=this.load(e);return t?(t.certifications.length>=M||(t.certifications.push(R()),this.save(e,t)),t):null}addProject(e){const t=this.load(e);return t?(t.projets.length>=P||(t.projets.push(F()),this.save(e,t)),t):null}addReference(e){const t=this.load(e);return t?(t.references.length>=E||(t.references.push(_()),this.save(e,t)),t):null}removeExperience(e,t){const i=this.load(e);return i?(i.experiences=i.experiences.filter(r=>r.id!==t),this.save(e,i)):!1}removeFormation(e,t){const i=this.load(e);return i?(i.formations=i.formations.filter(r=>r.id!==t),this.save(e,i)):!1}setIdentite(e,t){const i=this.load(e);return i?(i.identite={...i.identite,...t},this.save(e,i)):!1}setResume(e,t){const i=this.load(e);return i?(i.resume=t.slice(0,600),this.save(e,i)):!1}setCompetences(e,t){const i=this.load(e);return i?(i.competences=t.slice(0,$),this.save(e,i)):!1}clear(e){return e?(localStorage.removeItem(h(e)),!0):!1}}const d=new z;function f(n){u?.cleanup(),u=A("studios-cv");const e=j.get("user"),t=e?.id??"anon";let i=d.load(t);if(!i){const o={};e?.firstName&&(o.prenom=e.firstName),e?.lastName&&(o.nom=e.lastName),i=y("classique",o),d.save(t,i)}const r=D(i),a=I(i),c=g.map(o=>`
  ...
  `).join("");n.innerHTML=`
    <div class="ax-page" style="padding:16px;max-width:760px;margin:0 auto">
      ...
      <div style="background:rgba(201,162,39,0.05);border:1px solid rgba(201,162,39,0.3);border-radius:12px;padding:14px;margin-bottom:16px">
        <h2 style="margin:0 0 10px 0;font-size:15px;color:#c9a227">Template (${g.length} disponibles)</h2>
      ...
      <div style="background:rgba(201,162,39,0.05);border:1px solid rgba(201,162,39,0.3);border-radius:12px;padding:14px;margin-bottom:16px">
        ...
        <input type="text" id="ax-cv-prenom" placeholder="Prénom" maxlength="100" value="${l(i.identite.prenom)}" style="width:100%;padding:10px;margin-bottom:8px;background:#0a0a14;border:1px solid #333;color:#fff;border-radius:6px;min-height:44px">
        <input type="text" id="ax-cv-nom" placeholder="Nom" maxlength="100" value="${l(i.identite.nom)}" style="width:100%;padding:10px;margin-bottom:8px;background:#0a0a14;border:1px solid #333;color:#fff;border-radius:6px;min-height:44px">
        <input type="text" id="ax-cv-titre" placeholder="Titre (ex: Développeur Full-Stack)" maxlength="200" value="${l(i.identite.titre)}" style="width:100%;padding:10px;margin-bottom:8px;background:#0a0a14;border:1px solid #333;color:#fff;border-radius:6px;min-height:44px">
        <input type="email" id="ax-cv-email" placeholder="Email" maxlength="200" value="${l(i.identite.email)}" style="width:100%;padding:10px;margin-bottom:8px;background:#0a0a14;border:1px solid #333;color:#fff;border-radius:6px;min-height:44px">
        <input type="tel" id="ax-cv-tel" placeholder="Téléphone" maxlength="50" value="${l(i.identite.telephone)}" style="width:100%;padding:10px;margin-bottom:8px;background:#0a0a14;border:1px solid #333;color:#fff;border-radius:6px;min-height:44px">
        <input type="url" id="ax-cv-linkedin" placeholder="LinkedIn URL" maxlength="300" value="${l(i.identite.linkedin)}" style="width:100%;padding:10px;margin-bottom:8px;background:#0a0a14;border:1px solid #333;color:#fff;border-radius:6px;min-height:44px">
      </div>

      <div style="background:rgba(201,162,39,0.05);border:1px solid rgba(201,162,39,0.3);border-radius:12px;padding:14px;margin-bottom:16px">
        <h2 style="margin:0 0 10px 0;font-size:15px;color:#c9a227">Expériences (${i.experiences.length}/${v})</h2>
        <button class="ax-btn ax-btn-primary" id="ax-cv-add-exp" style="min-height:44px">➕ Ajouter une expérience</button>
      </div>

      <div style="background:rgba(201,162,39,0.05);border:1px solid rgba(201,162,39,0.3);border-radius:12px;padding:14px;margin-bottom:16px">
        <h2 style="margin:0 0 10px 0;font-size:15px;color:#c9a227">Formations (${i.formations.length}/${b})</h2>
  ...
  `,O(n,t)}function O(n,e){n.querySelectorAll(".ax-cv-template").forEach(i=>{u.bind(i,"click",()=>{const r=i.dataset.template;r&&(d.setTemplate(e,r),f(n))})});const t=(i,r)=>{const a=n.querySelector(`#${i}`);a&&u.bind(a,"change",()=>{d.setIdentite(e,{[r]:a.value})})};t("ax-cv-prenom","prenom"),t("ax-cv-nom","nom"),t("ax-cv-titre","titre"),t("ax-cv-email","email"),t("ax-cv-tel","telephone"),t("ax-cv-linkedin","linkedin"),n.querySelector("#ax-cv-add-exp")?.addEventListener("click",()=>{d.addExperience(e),f(n)}),n.querySelector("#ax-cv-add-form")?.addEventListener("click",()=>{d.addFormation(e),f(n)}),n.querySelector("#ax-cv-clear")?.addEventListener("click",()=>{d.clear(e),f(n)}),n.querySelectorAll("[data-export]").forEach(i=>{u.bind(i,"click",()=>{const r=i.dataset.export;p.info("studio-cv","export requested",{format:r}),(async()=>{try{const{toast:a}=await C(async()=>{const{toast:c}=await import("./toast-Dgg9rcIP.js");return{toast:c}},__vite__mapDeps([0,1]),import.meta.url).catch(()=>({toast:null}));a?.info(`Export ${r.toUpperCase()} en cours…`)}catch(a){p.warn("studio-cv","export failed",{err:a})}})()})}),n.querySelector("#ax-cv-cover-letter")?.addEventListener("click",()=>{p.info("studio-cv","cover letter requested")}),n.querySelector("#ax-cv-match-offer")?.addEventListener("click",()=>{p.info("studio-cv","match offer requested")}),n.querySelector("#ax-cv-interview")?.addEventListener("click",()=>{p.info("studio-cv","interview simulator requested")})}export{Q as ATS_REQUIRED_KEYWORDS_FR,G as ATS_REQUIRED_SECTIONS,Z as COMPETENCES_PAR_DOMAINE,W as COVER_LETTER_TEMPLATES,K as INTERVIEW_QUESTIONS_FR,M as MAX_CERTIFICATIONS,$ as MAX_COMPETENCES,v as MAX_EXPERIENCES,b as MAX_FORMATIONS,P as MAX_PROJECTS,E as MAX_REFERENCES,Y as NIVEAUX_CECRL,k as STORAGE_PREFIX,g as TEMPLATES,I as calcATSScore,D as calcCompleteness,R as createCertification,w as createExperience,T as createFormation,F as createProject,_ as createReference,d as cvStudioStore,H as dispose,l as escapeHtml,te as extractGitHubUsername,ee as extractLinkedInSlug,ne as generateCoverLetterTemplate,h as getStorageKey,y as initCV,S as isValidEmail,ie as matchOffer,f as render};
//# sourceMappingURL=index-i22wQ2Qs.js.map
