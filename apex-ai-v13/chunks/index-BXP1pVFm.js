import{e as p,a as u}from"./escape-html-BlQj2yEF.js";import{l as g}from"./monitoring-3uBGKGRH.js";import{s as h}from"../core/main-BHBzMEgW.js";import"./apex-kb-BUdkXo7X.js";import"./voice-CKlB4PWs.js";import"./credential-patterns-CLzI061R.js";import"./multi-source-analyze-DfTeMbTC.js";function n(o){return o>=1e3?`${(o/1e3).toFixed(1)}k`:String(o)}function s(o){let i=0;for(let r=0;r<localStorage.length;r++){const t=localStorage.key(r);t&&t.startsWith(o)&&i++}return i}function a(o,i){try{const r=localStorage.getItem(o);if(!r)return 0;const t=JSON.parse(r);return i==="array"&&Array.isArray(t)?t.length:i==="object"&&t&&typeof t=="object"?Object.keys(t).length:Array.isArray(t)?t.length:1}catch{return 0}}function v(){const o=s("ax_")-s("ax_credentials_deleted")-s("ax_persistent_memory")-s("ax_lessons")-s("ax_links_"),i=(()=>{try{const c=localStorage.getItem("apex_v13_multi_keys");if(!c)return 0;const y=JSON.parse(c);return Object.keys(y).length}catch{return 0}})(),r=a("ax_links_registry","object"),t=s("apex_v13_skill_"),d=a("apex_v13_mcp_servers","array"),e=a("apex_v13_persistent_memory","array")+a("apex_v13_facts","array"),l=a("apex_v13_lessons","array")+a("ax_lessons_learned_struct","array"),x=a("apex_v13_code_snippets_index","array"),b=a("apex_v13_conversation_active","array"),m=(()=>{try{const c=localStorage.getItem("apex_v13_economy_mode");return c?JSON.parse(c).active===!0:!1}catch{return!1}})(),f=a("apex_v13_audit","array");return[{emoji:"🔑",title:"Coffre clés API",count:n(Math.max(o,i)),hint:"Chiffrées AES-GCM-256 + triple persistence",route:"vault",color:"#c9a227"},{emoji:"🔗",title:"Liens auto",count:n(r),hint:"Dashboards, billing, docs détectés",route:"admin",color:"#3b82f6"},{emoji:"🎯",title:"Skills 2026",count:n(t),hint:"20 skills auto-syncés (docx/pptx/xlsx/pdf/video/MCP)",route:"skills-2026",color:"#8b5cf6"},{emoji:"🔌",title:"MCP Servers",count:n(d),hint:"BOFiP, Almanac, Legal Hunter — search 18M+ docs",route:"mcp-servers",color:"#06b6d4"},{emoji:"🧠",title:"Mémoire facts",count:n(e),hint:"Cross-session — Apex n'oublie JAMAIS",route:"admin",color:"#ec4899"},{emoji:"📚",title:"Leçons apprises",count:n(l),hint:"Cross-app Apex ↔ CMCteams",route:"admin",color:"#f59e0b"},{emoji:"💻",title:"Code snippets",count:n(x),hint:"Tape /snippets dans chat",route:"chat",color:"#10b981"},{emoji:"💬",title:"Conversation",count:n(b),hint:"Messages persistés (Firebase backup)",route:"chat",color:"#6366f1"},{emoji:"🧪",title:"Runtime Tests",count:"17",hint:"Lancer TOUS les tests réels (≈30s)",route:"runtime-tests",color:"#3b82f6"},{emoji:m?"🔋":"⚡",title:"Mode économie",count:m?"ON":"OFF",hint:m?"Haiku + tokens÷2":"Modèles premium activés",route:"settings",color:m?"#10b981":"#94a3b8"},{emoji:"📊",title:"Audit log",count:n(f),hint:"Trail immutable actions Apex",route:"admin",color:"#64748b"},{emoji:"🚨",title:"Alertes auto",count:"3",hint:"P0 INP + CSP + Vault backup (Apex IA)",route:"admin",color:"#ef4444"}]}function M(o){const i=h.get("user"),r=h.get("isAdmin"),t=i?.name??"invité",d=v();o.innerHTML=`
    <div style="max-width:900px;margin:0 auto;padding:16px 12px;color:#f1f5f9">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:10px">
        ${d.map(e=>`
          <button
            type="button"
            data-route="${u(e.route)}"
            style="
              background:#0f172a;
              border:1px solid ${u(e.color)}33;
              border-radius:12px;
              padding:14px;
              text-align:left;
              cursor:pointer;
            ...
            "
            onmouseover="this.style.borderColor='${u(e.color)}'"
            onmouseout="this.style.borderColor='${u(e.color)}33'"
          >
            <div style="font-size:24px;line-height:1">${p(e.emoji)}</div>
            <div style="font-size:24px;font-weight:700;color:${u(e.color)};margin-top:6px;line-height:1">${p(String(e.count))}</div>
            <div style="font-size:13px;font-weight:600;margin-top:6px;color:#e2e8f0">${p(e.title)}</div>
            <div style="font-size:11px;color:#94a3b8;margin-top:4px;line-height:1.3">${p(e.hint)}</div>
          </button>
        `).join("")}
      </div>
  ...
  `,o.querySelectorAll("[data-route]").forEach(e=>{e.addEventListener("click",()=>{const l=e.getAttribute("data-route");if(l)try{location.hash=`#${l}`}catch(x){g.warn("dashboard-personnel","navigation failed",{err:x,route:l})}})}),g.info("dashboard-personnel",`Rendered (${d.length} cards, user=${t}, admin=${r})`)}export{M as render};
