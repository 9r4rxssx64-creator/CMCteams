const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./sentinels-B2NX4WWS.js","../core/main-BFCqP8AI.js","../assets/css/main-rhfGvOFL.css","./observability-DqcMComk.js","./sentinels-registry-OcrEKp3q.js","./toast-64I4l5xU.js","./haptic-BUEqXK0N.js"])))=>i.map(i=>d[i]);
import{_ as i,l as f}from"../core/main-BFCqP8AI.js";async function c(a){const{sentinels:s}=await i(async()=>{const{sentinels:t}=await import("./sentinels-B2NX4WWS.js");return{sentinels:t}},__vite__mapDeps([0,1,2,3]),import.meta.url),{sentinelsRegistry:u,bootstrapSentinelsRegistry:m}=await i(async()=>{const{sentinelsRegistry:t,bootstrapSentinelsRegistry:e}=await import("./sentinels-registry-OcrEKp3q.js");return{sentinelsRegistry:t,bootstrapSentinelsRegistry:e}},__vite__mapDeps([4,1,2,0,3]),import.meta.url);m();const n=s.list(),y=u.getStatus(),r=u.getMetrics(),x=n.filter(t=>t.lastResult?.ok).length,g=n.filter(t=>t.lastResult&&!t.lastResult.ok).length,b=n.filter(t=>!t.lastResult).length;a.innerHTML=`
    <div class="ax-page" style="padding:16px;max-width:900px;margin:0 auto">
      ...
      <table style="width:100%;border-collapse:collapse;background:rgba(20,20,35,0.5);border-radius:12px;overflow:hidden">
        ...
        <tbody>
          ${n.map(t=>{const e=t.intervalMs>=36e5?`${Math.round(t.intervalMs/36e5)}h`:t.intervalMs>=6e4?`${Math.round(t.intervalMs/6e4)}min`:`${Math.round(t.intervalMs/1e3)}s`,d=t.lastResult?t.lastResult.ok?"✅":"⚠️":"⏳",o=t.lastResult?.msg??"Pas encore exécuté",l=t.lastResult?Math.round((Date.now()-t.lastResult.ts)/6e4):null;return`
  ...
  `,a.querySelector("#ax-sent-run-all")?.addEventListener("click",()=>{(async()=>{const{toast:t}=await i(async()=>{const{toast:e}=await import("./toast-64I4l5xU.js");return{toast:e}},__vite__mapDeps([5,6]),import.meta.url);t.info(`Exécution de ${n.length} sentinelles...`),await Promise.all(n.map(e=>s.runOne(e.id))),t.success("✅ Tous les sentinelles re-exécutés"),await c(a)})()}),a.querySelector("#ax-sent-refresh")?.addEventListener("click",()=>{c(a)}),a.querySelectorAll(".ax-sent-run").forEach(t=>{t.addEventListener("click",()=>{(async()=>{const e=t.dataset.sentId;if(!e)return;const{toast:d}=await i(async()=>{const{toast:l}=await import("./toast-64I4l5xU.js");return{toast:l}},__vite__mapDeps([5,6]),import.meta.url),o=await s.runOne(e);d[o?.ok?"success":"warn"](`${e}: ${o?.msg??"KO"}`),await c(a)})()})}),f.info("feature-sentinels",`rendered ${n.length} sentinels (${x}OK / ${g}WARN)`)}function p(a){return a.replace(/[&<>"']/g,s=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[s]??s)}export{c as render};
//# sourceMappingURL=index-E7wQNFZr.js.map
