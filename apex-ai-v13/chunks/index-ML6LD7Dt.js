...
Joint avis imposition + justificatifs.`,ref_legales:"Art L247 LPF"}}};function s(e){return e.replace(/[&<>"']/g,n=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[n]??n)}function g(e){const n=String(e||"").toLowerCase().trim(),r=t.codes[n];return r||`https://www.legifrance.gouv.fr/search/all?searchField=ALL&query=${encodeURIComponent(e)}`}function J(e,n){const r=String(e||"cassation").toLowerCase();return`${t.jurisprudence[r]??t.jurisprudence.cassation??""}?search=${encodeURIComponent(n)}`}function T(){return Object.keys(t.codes).map(e=>({key:e,label:e.replace(/_/g," "),url:t.codes[e]??""}))}function E(e,n){if(n<8/12)return{indemnite:0,detail:"Pas d'indemnité légale (ancienneté < 8 mois)",formule:"Art L1234-9 Code du travail"};let r=0;const o=Math.min(n,10);return r+=1/4*e*o,n>10&&(r+=1/3*e*(n-10)),{indemnite:Math.round(r),detail:`1/4 mois × ${o} ans${n>10?` + 1/3 mois × ${n-10} ans`:""}`,formule:"Art L1234-9 + R1234-2 Code du travail"}}function D(e,n,r=.0697){return Math.round(e*r*n/365)}function P(e){const n=Math.min(30,e*2.5);return{jours_ouvrables:Math.round(n*10)/10,semaines_equivalent:Math.round(n/6*10)/10}}function q(e,n){const r={civil:5,contravention:1,delit:6,crime:20},o=new Date(e);o.setFullYear(o.getFullYear()+(r[n]||5));const l=o.toISOString().slice(0,10),d=Math.max(0,Math.floor((o.getTime()-Date.now())/864e5));return{date_fait:e,date_prescription:l,jours_restants:d}}function G(e){return e<6?{duree_mois:0,description:"Pas de préavis légal (< 6 mois)"}:e<24?{duree_mois:1,description:"1 mois (entre 6 mois et 2 ans)"}:{duree_mois:2,description:"2 mois (≥ 2 ans)"}}function k(e){const n=e.toLowerCase().trim();return n?Object.keys(t.templates).filter(r=>{const o=t.templates[r];return o?r.includes(n)||o.titre.toLowerCase().includes(n)||(o.ref_legales??"").toLowerCase().includes(n):!1}).map(r=>({key:r,titre:t.templates[r]?.titre??"",destinataire:t.templates[r]?.destinataire??""})):Object.keys(t.templates).map(r=>({key:r,titre:t.templates[r]?.titre??"",destinataire:t.templates[r]?.destinataire??""}))}function R(e){const n=e.toLowerCase();return Object.keys(t.templates).filter(r=>(t.templates[r]?.destinataire??"").toLowerCase().includes(n)).map(r=>({key:r,titre:t.templates[r]?.titre??""}))}function I(){return Object.keys(t.templates).length}function $(e){const n=Math.min(30,e*2.5);return{jours_ouvrables:Math.round(n*10)/10,jours_ouvres:Math.round(n*5/6*10)/10}}function O(e){return{civile_droit_commun:{duree:"5 ans",reference:"Art 2224 Code civil"},creance_consommation:{duree:"2 ans",reference:"Art L218-2 Code conso"},salaire:{duree:"3 ans",reference:"Art L3245-1 Code travail"},licenciement_irregulier:{duree:"12 mois",reference:"Art L1471-1 Code travail"},penal_contravention:{duree:"1 an",reference:"Art 9 Code procédure pénale"},penal_delit:{duree:"6 ans",reference:"Art 8 Code procédure pénale"},penal_crime:{duree:"20 ans",reference:"Art 7 Code procédure pénale"},penal_crime_grave_mineurs:{duree:"30 ans / imprescriptible",reference:"Art 7 al 3"},penal_terrorisme_crime_humanite:{duree:"Imprescriptible",reference:"Art 7"},fiscal_isf_ifi:{duree:"6 ans",reference:"Art L181 LPF"},fiscal_ir:{duree:"3 ans",reference:"Art L169 LPF"},reparation_dommage_corporel:{duree:"10 ans",reference:"Art 2226 Code civil"},inscription_hypothecaire:{duree:"50 ans max",reference:"Art 2434 Code civil"}}[e]??null}function X(e){return t.templates[e]??null}function S(){return Object.keys(t.templates).map(e=>({key:e,titre:t.templates[e]?.titre??e}))}function F(e){const n=p.get("user")?.id??"anon";if(!f("pro.legal",e,n))return;const r=Object.keys(t.codes).map(i=>{const a=t.codes[i]??"";return`<a href="${s(a)}" target="_blank" rel="noopener" style="display:block;padding:9px 12px;color:#5aa8ff;text-decoration:none;border-bottom:1px solid rgba(255,255,255,0.06);transition:background 0.15s" onmouseover="this.style.background='rgba(90,168,255,0.1)'" onmouseout="this.style.background=''">📜 Code ${s(i.replace(/_/g," "))}</a>`}).join(""),o=Object.keys(t.jurisprudence).map(i=>{const a=t.jurisprudence[i]??"";return`<a href="${s(a)}" target="_blank" rel="noopener" style="display:block;padding:9px 12px;color:#5aa8ff;text-decoration:none;border-bottom:1px solid rgba(255,255,255,0.06)">🔍 ${s(i.replace(/_/g," "))}</a>`}).join(""),l=Object.keys(t.monaco).map(i=>{const a=t.monaco[i]??"";return`<a href="${s(a)}" target="_blank" rel="noopener" style="display:block;padding:9px 12px;color:#5aa8ff;text-decoration:none;border-bottom:1px solid rgba(255,255,255,0.06)">${s(i.replace(/_/g," "))}</a>`}).join(""),d=Object.keys(t.organismes).map(i=>{const a=t.organismes[i]??"";return`<a href="${s(a)}" target="_blank" rel="noopener" style="display:block;padding:9px 12px;color:#5aa8ff;text-decoration:none;border-bottom:1px solid rgba(255,255,255,0.06)">${s(i.replace(/_/g," "))}</a>`}).join(""),u=Object.keys(t.templates).map(i=>{const a=t.templates[i];return a?`<div style="padding:8px 0;border-bottom:1px solid rgba(255,255,255,0.06);font-size:13px"><strong>${s(a.titre)}</strong>${a.ref_legales?`<br><small style="color:#888">${s(a.ref_legales)}</small>`:""}</div>`:""}).join("");e.innerHTML=`
  ...
  `,e.querySelector("#legalSearchBtn")?.addEventListener("click",()=>{const i=e.querySelector("#legalQ")?.value??"",a=e.querySelector("#legalResult");if(!a||!i)return;const c=g(i);a.innerHTML=`🔗 <a href="${s(c)}" target="_blank" rel="noopener" style="color:#5aa8ff">${s(c)}</a>`}),m.info("legal-pro","rendered")}export{t as AX_LEGAL_FR,P as calcConcesPaies,$ as calcCongesPayes,E as calcIndemniteLicenciement,D as calcInteretsMoratoires,G as calcPreavisCdi,q as calcPrescription,I as countTemplates,O as getDelaiPrescription,X as getTemplate,J as jurisprudenceSearch,g as legalLookup,T as listCodes,S as listTemplates,R as listTemplatesByDestinataire,F as render,k as searchTemplates};
