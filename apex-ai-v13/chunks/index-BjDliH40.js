...
Joint avis imposition + justificatifs.`,ref_legales:"Art L247 LPF"}}};function g(e){const n=String(e||"").toLowerCase().trim(),i=t.codes[n];return i||`https://www.legifrance.gouv.fr/search/all?searchField=ALL&query=${encodeURIComponent(e)}`}function y(e,n){const i=String(e||"cassation").toLowerCase();return`${t.jurisprudence[i]??t.jurisprudence.cassation??""}?search=${encodeURIComponent(n)}`}function T(){return Object.keys(t.codes).map(e=>({key:e,label:e.replace(/_/g," "),url:t.codes[e]??""}))}function E(e,n){if(n<8/12)return{indemnite:0,detail:"Pas d'indemnité légale (ancienneté < 8 mois)",formule:"Art L1234-9 Code du travail"};let i=0;const s=Math.min(n,10);return i+=1/4*e*s,n>10&&(i+=1/3*e*(n-10)),{indemnite:Math.round(i),detail:`1/4 mois × ${s} ans${n>10?` + 1/3 mois × ${n-10} ans`:""}`,formule:"Art L1234-9 + R1234-2 Code du travail"}}function D(e,n,i=.0697){return Math.round(e*i*n/365)}function P(e){const n=Math.min(30,e*2.5);return{jours_ouvrables:Math.round(n*10)/10,semaines_equivalent:Math.round(n/6*10)/10}}function q(e,n){const i={civil:5,contravention:1,delit:6,crime:20},s=new Date(e);s.setFullYear(s.getFullYear()+(i[n]||5));const l=s.toISOString().slice(0,10),d=Math.max(0,Math.floor((s.getTime()-Date.now())/864e5));return{date_fait:e,date_prescription:l,jours_restants:d}}function G(e){return e<6?{duree_mois:0,description:"Pas de préavis légal (< 6 mois)"}:e<24?{duree_mois:1,description:"1 mois (entre 6 mois et 2 ans)"}:{duree_mois:2,description:"2 mois (≥ 2 ans)"}}function R(e){const n=e.toLowerCase().trim();return n?Object.keys(t.templates).filter(i=>{const s=t.templates[i];return s?i.includes(n)||s.titre.toLowerCase().includes(n)||(s.ref_legales??"").toLowerCase().includes(n):!1}).map(i=>({key:i,titre:t.templates[i]?.titre??"",destinataire:t.templates[i]?.destinataire??""})):Object.keys(t.templates).map(i=>({key:i,titre:t.templates[i]?.titre??"",destinataire:t.templates[i]?.destinataire??""}))}function I(e){const n=e.toLowerCase();return Object.keys(t.templates).filter(i=>(t.templates[i]?.destinataire??"").toLowerCase().includes(n)).map(i=>({key:i,titre:t.templates[i]?.titre??""}))}function $(){return Object.keys(t.templates).length}function k(e){const n=Math.min(30,e*2.5);return{jours_ouvrables:Math.round(n*10)/10,jours_ouvres:Math.round(n*5/6*10)/10}}function O(e){return{civile_droit_commun:{duree:"5 ans",reference:"Art 2224 Code civil"},creance_consommation:{duree:"2 ans",reference:"Art L218-2 Code conso"},salaire:{duree:"3 ans",reference:"Art L3245-1 Code travail"},licenciement_irregulier:{duree:"12 mois",reference:"Art L1471-1 Code travail"},penal_contravention:{duree:"1 an",reference:"Art 9 Code procédure pénale"},penal_delit:{duree:"6 ans",reference:"Art 8 Code procédure pénale"},penal_crime:{duree:"20 ans",reference:"Art 7 Code procédure pénale"},penal_crime_grave_mineurs:{duree:"30 ans / imprescriptible",reference:"Art 7 al 3"},penal_terrorisme_crime_humanite:{duree:"Imprescriptible",reference:"Art 7"},fiscal_isf_ifi:{duree:"6 ans",reference:"Art L181 LPF"},fiscal_ir:{duree:"3 ans",reference:"Art L169 LPF"},reparation_dommage_corporel:{duree:"10 ans",reference:"Art 2226 Code civil"},inscription_hypothecaire:{duree:"50 ans max",reference:"Art 2434 Code civil"}}[e]??null}function X(e){return t.templates[e]??null}function S(){return Object.keys(t.templates).map(e=>({key:e,titre:t.templates[e]?.titre??e}))}function F(e){const n=m.get("user")?.id??"anon";if(!f("pro.legal",e,n))return;const i=Object.keys(t.codes).map(r=>{const a=t.codes[r]??"";return`<a href="${o(a)}" target="_blank" rel="noopener" style="display:block;padding:9px 12px;color:#5aa8ff;text-decoration:none;border-bottom:1px solid rgba(255,255,255,0.06);transition:background 0.15s" onmouseover="this.style.background='rgba(90,168,255,0.1)'" onmouseout="this.style.background=''">📜 Code ${o(r.replace(/_/g," "))}</a>`}).join(""),s=Object.keys(t.jurisprudence).map(r=>{const a=t.jurisprudence[r]??"";return`<a href="${o(a)}" target="_blank" rel="noopener" class="ax-gs-429">🔍 ${o(r.replace(/_/g," "))}</a>`}).join(""),l=Object.keys(t.monaco).map(r=>{const a=t.monaco[r]??"";return`<a href="${o(a)}" target="_blank" rel="noopener" class="ax-gs-429">${o(r.replace(/_/g," "))}</a>`}).join(""),d=Object.keys(t.organismes).map(r=>{const a=t.organismes[r]??"";return`<a href="${o(a)}" target="_blank" rel="noopener" class="ax-gs-429">${o(r.replace(/_/g," "))}</a>`}).join(""),u=Object.keys(t.templates).map(r=>{const a=t.templates[r];return a?`<div style="padding:8px 0;border-bottom:1px solid rgba(255,255,255,0.06);font-size:13px"><strong>${o(a.titre)}</strong>${a.ref_legales?`<br><small class="ax-gs-243">${o(a.ref_legales)}</small>`:""}</div>`:""}).join("");e.innerHTML=`
    <div class="ax-gs-34">
  ...
  `,e.querySelector("#legalSearchBtn")?.addEventListener("click",()=>{const r=e.querySelector("#legalQ")?.value??"",a=e.querySelector("#legalResult");if(!a||!r)return;const c=g(r);a.innerHTML=`🔗 <a href="${o(c)}" target="_blank" rel="noopener" class="ax-gs-244">${o(c)}</a>`}),p.info("legal-pro","rendered")}export{t as AX_LEGAL_FR,P as calcConcesPaies,k as calcCongesPayes,E as calcIndemniteLicenciement,D as calcInteretsMoratoires,G as calcPreavisCdi,q as calcPrescription,$ as countTemplates,O as getDelaiPrescription,X as getTemplate,y as jurisprudenceSearch,g as legalLookup,T as listCodes,S as listTemplates,I as listTemplatesByDestinataire,F as render,R as searchTemplates};
