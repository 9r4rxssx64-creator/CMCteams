import{e as a}from"./escape-html-BlQj2yEF.js";import{c as u}from"./listener-cleanup-Y2rGGxxX.js";import{l as d}from"./monitoring-3uBGKGRH.js";import{s as b}from"../core/main-CDwVTNJR.js";import{autoRestoreCredentials as l}from"./auto-restore-credentials-CE27UXlp.js";import{haptic as g}from"./haptic-CQFg2PXZ.js";import{toast as s}from"./toast-CRdbcLoc.js";import"./apex-kb-BaxCKkKm.js";import"./voice-CKlB4PWs.js";import"./credential-patterns-CLzI061R.js";import"./multi-source-analyze-vwBLLHvK.js";let i=null,n=null;function C(){i?.cleanup(),i=null,n=null}const f={localStorage:{icon:"💾",label:"Local",color:"#22cc77"},idb_shadow:{icon:"🗄️",label:"IDB shadow",color:"#22cc77"},firebase_backup:{icon:"☁️",label:"Firebase backup",color:"#4d9eff"},alias:{icon:"🔗",label:"Alias localStorage",color:"#c9a227"},pattern_match:{icon:"🔍",label:"Pattern match",color:"#ff9d3f"}};function m(e){const t=e.recoverable_from?f[e.recoverable_from]:{icon:"❓",label:"—",color:"#aaa"};return`
  ...
  `;try{const t=await l.auditMissing();n=t;const r=await l.getStats(),o=t.recoverable.length>0?t.recoverable.map(m).join(""):'<p style="color:var(--ax-text-dim);text-align:center;padding:20px">✅ Aucune clé restorable (toutes les clés présentes ou réellement absentes)</p>',p=t.truly_absent.length>0?t.truly_absent.map(v).join(""):'<p style="color:var(--ax-text-dim);text-align:center;padding:20px">🎉 Aucune clé absente — tu as tout configuré</p>',x=r.total_patterns>0?Math.round(r.present_count/r.total_patterns*100):0;e.innerHTML=`
    ...
    `}}function y(e){if(!i)return;const t=e.querySelector("#ax-cs-refresh"),r=e.querySelector("#ax-cs-restore-all");t&&i.bind(t,"click",()=>{c(e)}),r&&i.bind(r,"click",()=>{(async()=>{if(!n||n.recoverable.length===0){s.show("Aucune clé restorable","warn");return}r.disabled=!0,r.textContent="⏳ Restauration en cours…";try{const o=await l.restoreAutomatically();g.success(),o.restored>0?s.show(`✅ ${o.restored} clé(s) restaurée(s) (échec: ${o.failed})`,"success"):s.show(`Aucune clé n'a pu être restaurée (échec: ${o.failed})`,"warn"),await c(e)}catch(o){d.error("credentials-status","restoreAll failed",{err:o}),s.show(`Erreur restore : ${String(o).slice(0,100)}`,"error"),r.disabled=!1}})()})}async function B(e){if(!(b.get("isAdmin")===!0)){e.innerHTML=`
      <div style="padding:40px;text-align:center;color:#999">
        <h2 style="color:#c9a227">Accès admin uniquement</h2>
        <p>Cette section est réservée à Kevin.</p>
      </div>
    `;return}i?.cleanup(),i=u("credentials-status"),await c(e)}export{C as dispose,B as render};
