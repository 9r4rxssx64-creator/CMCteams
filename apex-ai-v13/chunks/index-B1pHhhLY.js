const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./monitoring-3uBGKGRH.js","./multi-source-analyze-DFRy0PZC.js","./apex-kb-DRP-rQ5c.js","./credential-patterns-CLzI061R.js","../assets/css/main-BUIq5pfg.css"])))=>i.map(i=>d[i]);
import{b as v,_ as $}from"./apex-kb-DRP-rQ5c.js";import{c as H}from"./listener-cleanup-Y2rGGxxX.js";import{l as h}from"./monitoring-3uBGKGRH.js";import{s as I,a as M}from"../core/main-BxMVCELi.js";import{apexExecute as y}from"./apex-execute-e1WfN3jI.js";import{auth as P}from"./auth-JGQ_KFuU.js";import{commerce as _}from"./commerce-Ch_Irr9p.js";import{c as D}from"./csp-style-helper-BisGRi53.js";import{i as T,r as z}from"./voice-B7u_Xmm6.js";import{kdmcProjectsRegistry as S}from"./kdmc-projects-registry-DdTqW2gy.js";import{w as j}from"./whatsapp-DWn0pMu9.js";import{haptic as d}from"./haptic-CQFg2PXZ.js";import{toast as p}from"./toast-ClsF1KRZ.js";import"./credential-patterns-CLzI061R.js";import"./multi-source-analyze-DFRy0PZC.js";import"./claude-bridge-DnS0dTTl.js";let f="commerce";const R={commerce:"admin.commerce",users:"admin.users",pending:"admin.users",health:null,projects:null,executions:"admin.executions",knowledge:"admin.kb",bilan:"admin.bilan",consumption:"admin.consumption","audit-log":"admin.audit-log"};function i(t){return t.replace(/[&<>"']/g,s=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[s]??s)}function E(){const t=_.isEnabled();return`
  ...
  `}function B(){const t=P.listUsers(),s=new Set(["admin","family","client_pro","client_free"]),c=t.map(r=>{const l=s.has(r.tier)?r.tier:"client_free";return`
  ...
  `}function V(){const t=S.list(),s=S.count(),c=S.countActive(),r=["active","wip","archived"],l=t.map(n=>{const e=r.includes(n.status)?n.status:"archived",a=n.tech_stack.slice(0,4).map(o=>i(o)).join(", ");return`
      ...
      `}).join("");return`
    <div class="ax-admin-section">
      <h2>📦 Projets KDMC (${s} — ${c} actifs/wip)</h2>
      <p class="ax-muted">
  ...
  `}function U(){return[["commerce","💳 Commerce"],["users","👥 Comptes"],["pending","📨 Attente"],["health","🩺 Santé"],["projects","📦 Projets"],["executions","⚙️ Exec"],["knowledge","📚 KB"],["bilan","📊 Bilan"],["consumption","💰 Conso"],["audit-log","🔒 Audit"]].filter(([c])=>{const r=R[c];return!r||T(r)}).map(([c,r])=>{const l=f===c;return`
      ...
      `}).join("")}function O(){const t=y.listPendingExecutions({limit:30}),s=y.getStats(),c=y.listAllowedTasks(),r=y.listForbiddenTasks(),l={pending:"pending",dispatched:"wip",running:"wip",completed:"ok",failed:"error",cancelled:"archived",timeout:"error"},n=t.map(e=>{const a=l[e.status]??"archived",o=new Date(e.ts_created).toLocaleString(),u=e.duration_ms?`${Math.round(e.duration_ms/1e3)}s`:"—",b=e.workflow_run_url?`<a href="${i(e.workflow_run_url)}" target="_blank" rel="noopener">🔗 Workflow</a>`:'<span class="ax-muted">—</span>',x=e.status==="pending"||e.status==="dispatched";return`
        <li class="ax-execution-row" data-exec-id="${i(e.id)}">
          ...
          <p class="ax-muted">📅 ${i(o)} · ⏱ ${i(u)} · 🚀 ${i(e.src)} · 👤 ${i(e.initiated_by)}</p>
    ...
    <div class="ax-admin-section">
      ...
      <div class="ax-info-card">
        ...
        <ul>
          ...
          <li><strong>En cours</strong> : ${s.pending} pending / ${s.running} running</li>
          <li><strong>Terminées</strong> : ${s.completed} ✅ · ${s.failed} ❌ · ${s.cancelled} 🚫</li>
          <li><strong>Success rate</strong> : ${s.success_rate}%</li>
          <li><strong>Avg duration</strong> : ${Math.round(s.avg_duration_ms/1e3)}s</li>
        </ul>
      </div>
      <div class="ax-info-card">
  ...
  `}function G(){const t=R[f];if(t&&!T(t))return`<div class="ax-admin-section">${z(t)}</div>`;switch(f){case"commerce":return E();case"users":return B();case"pending":return F();case"health":return N();case"projects":return V();case"executions":return O();case"knowledge":return K();case"bilan":return'<div id="ax-admin-mount-bilan" class="ax-admin-section"><p class="ax-muted">Chargement du bilan financier…</p></div>';case"consumption":return'<div id="ax-admin-mount-consumption" class="ax-admin-section"><p class="ax-muted">Chargement consommation IA…</p></div>';case"audit-log":return'<div id="ax-admin-mount-audit-log" class="ax-admin-section"><p class="ax-muted">Chargement audit log immuable…</p></div>'}}async function q(t){const s=t.querySelector("#ax-admin-audits-summary");if(s)try{const{reportsHistory:n}=await $(async()=>{const{reportsHistory:x}=await import("./apex-reports-history-Cv_X_jWe.js");return{reportsHistory:x}},__vite__mapDeps([0]),import.meta.url),e=n.getStats(),a=e.lastLayoutTs?new Date(e.lastLayoutTs).toLocaleString("fr-FR",{day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit"}):"jamais",o=e.lastFunctionalTs?new Date(e.lastFunctionalTs).toLocaleString("fr-FR",{day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit"}):"jamais",u=e.recentBugs>0?"#ff5b5b":"#22cc77",b=e.recentEscalations>0?"#ffaa66":"#22cc77";s.innerHTML=`
        <div class="ax-bounce-tap" data-nav-route="apex-audits-live" style="background:linear-gradient(135deg,rgba(106,138,255,0.10),rgba(180,90,200,0.06));border:1px solid rgba(106,138,255,0.3);border-radius:12px;padding:14px;cursor:pointer">
          ...
          <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:8px;font-size:12px;color:rgba(255,255,255,0.85)">
            ...
            <div>🧪 Fonctionnel : <b>${e.functionalCount}</b> tests<br><span style="font-size:11px;color:rgba(255,255,255,0.5)">${o}</span></div>
            <div>🐛 Bugs 24h : <b style="color:${u}">${e.recentBugs}</b></div>
            <div>📤 Escaladés 24h : <b style="color:${b}">${e.recentEscalations}</b></div>
          </div>
        </div>
      `}catch(n){h.warn("admin","audits summary render failed",{err:n})}const c=t.querySelector("#ax-admin-mount-bilan");if(c)try{(await $(()=>import("./financial-bilan-BEqW-sAR.js"),__vite__mapDeps([1,2,0,3,4]),import.meta.url)).render(c)}catch(n){h.warn("admin","financial-bilan render failed",{err:n}),c.innerHTML='<p class="ax-muted">Bilan indisponible (module ko)</p>'}const r=t.querySelector("#ax-admin-mount-consumption");if(r)try{(await $(()=>import("./consumption-dashboard-CLFqeB5u.js"),__vite__mapDeps([2,0,3,1,4]),import.meta.url)).render(r)}catch(n){h.warn("admin","consumption-dashboard render failed",{err:n}),r.innerHTML='<p class="ax-muted">Consommation indisponible (module ko)</p>'}const l=t.querySelector("#ax-admin-mount-audit-log");if(l){if(!T("admin.audit-log")){l.innerHTML=z("admin.audit-log");return}try{const{auditLog:n}=await $(async()=>{const{auditLog:o}=await import("./apex-kb-DRP-rQ5c.js").then(u=>u.d);return{auditLog:o}},__vite__mapDeps([2,0,3]),import.meta.url);n.init();const e=n.getEntries().slice(-100).reverse(),a=e.map(o=>`<li><code>${i(new Date(o.ts).toLocaleString())}</code> · <strong>${i(o.action)}</strong> · ${i(o.actor||"system")}</li>`).join("");l.innerHTML=`
      ...
      `}catch(n){h.warn("admin","audit-log render failed",{err:n}),l.innerHTML='<p class="ax-muted">Audit log indisponible (module ko)</p>'}}}function K(){const t=v.listRepos(),s=v.getStats(),c=t.map(r=>`
    ...
    <div class="ax-admin-section">
      ...
      <div class="ax-info-card">
        <strong>État :</strong> ${s.repos} repos · ${s.cache_entries} entrées cache · ${s.index_entries} fichiers indexés
        <br>
        <strong>Token GitHub :</strong> ${s.has_token?"✅ configuré":"⚪ Configure ax_github_token dans le Coffre pour 5000 req/h"}
      </div>
  ...
  `}function W(t){t.querySelectorAll("[data-nav-route]").forEach(e=>{m.bind(e,"click",()=>{d.tap();const a=e.dataset.navRoute??"chat";window.location.hash="#"+a})}),t.querySelectorAll('[data-action="select-all"]').forEach(e=>{m.bind(e,"click",()=>{e.select()})}),t.querySelectorAll("[data-tab]").forEach(e=>{m.bind(e,"click",()=>{d.selection(),f=e.dataset.tab,g(t)})});const s=t.querySelector("#commerce-toggle");s&&m.bind(s,"change",()=>{const e=s.checked;d.medium();const a=t.querySelector(".ax-toggle-label");if(a){a.textContent="",a.append("Commercialisation ");const o=document.createElement("strong");o.textContent=e?"ACTIVÉE":"désactivée",a.append(o)}_.setEnabled(e),p.success(`Commercialisation ${e?"activée":"désactivée"}`),g(t)});const c=t.querySelector("#create-user-form");c&&m.bind(c,"submit",e=>{e.preventDefault(),Y(t)}),t.querySelectorAll("[data-user-plan]").forEach(e=>{m.bind(e,"change",()=>{const a=e.dataset.userPlan??"";a&&(_.setUserPlan(a,e.value),h.info("admin",`Plan ${e.value} → ${a}`))})}),t.querySelectorAll("[data-project-save]").forEach(e=>{m.bind(e,"click",()=>{d.tap();const a=e.dataset.projectSave??"";if(!a)return;const o=t.querySelector(`[data-project-version="${CSS.escape(a)}"]`),u=t.querySelector(`[data-project-status="${CSS.escape(a)}"]`),b=(o?.value??"").trim(),x=u?.value??"active";if(!b){p.warn("Version requise");return}S.update(a,{version:b,status:x})?(d.success(),p.success(`${a} → ${b} (${x})`),h.info("admin",`Project ${a} updated`,{version:b,status:x}),g(t)):(d.error(),p.error("Update échoué"))})}),t.querySelectorAll("[data-confirm-otp]").forEach(e=>{m.bind(e,"click",()=>{d.tap();const a=e.dataset.confirmOtp??"";if(!a)return;const o=j.confirm(a);o.ok?(d.success(),p.success("Compte activé"),h.info("admin",`Confirmed user ${o.uid}`),g(t)):(d.error(),p.error("Code OTP invalide ou expiré"))})}),t.querySelectorAll("[data-exec-cancel]").forEach(e=>{m.bind(e,"click",()=>{d.tap();const a=e.dataset.execCancel??"";if(!a)return;y.cancelExecution(a)?(d.success(),p.success("Exécution annulée"),g(t)):(d.warning(),p.warn("Annulation impossible"))})}),t.querySelectorAll("[data-exec-poll]").forEach(e=>{m.bind(e,"click",()=>{d.tap();const a=e.dataset.execPoll??"";a&&y.pollResult(a).then(o=>{o?(p.success(`Statut : ${o.status}`),g(t)):p.warn("Exécution introuvable")})})});const r=t.querySelector("#add-repo-form");r&&m.bind(r,"submit",e=>{e.preventDefault(),d.tap();const o=t.querySelector("#kb-add-repo")?.value.trim()??"";if(!o){p.warn("Indique un repo");return}const u=v.addRepo(o);u.ok?(d.success(),p.success(`Repo ajouté : ${o}`),g(t)):(d.error(),p.error(u.reason??"Erreur ajout repo"))}),t.querySelectorAll("[data-remove-repo]").forEach(e=>{m.bind(e,"click",()=>{d.tap();const a=e.dataset.removeRepo??"";a&&(v.removeRepo(a),p.success(`Repo retiré : ${a}`),g(t))})});const l=t.querySelector("#kb-search-form");l&&m.bind(l,"submit",e=>{e.preventDefault(),d.tap();const a=t.querySelector("#kb-search-query"),o=t.querySelector("#kb-search-results"),u=a?.value.trim()??"";!u||!o||(o.innerHTML='<p class="ax-muted">Recherche en cours...</p>',v.searchCode(u).then(b=>{if(b.length===0){o.innerHTML='<p class="ax-muted">Aucun résultat (configure ax_github_token pour augmenter la limite).</p>';return}o.textContent="";const x=document.createElement("ul");x.className="ax-kb-results-list",b.slice(0,20).forEach(w=>{const A=document.createElement("li");A.className="ax-kb-result";const k=document.createElement("a");k.href=w.htmlUrl,k.target="_blank",k.rel="noopener";const L=document.createElement("code");L.textContent=w.path,k.append(L);const C=document.createElement("span");C.className="ax-muted",C.textContent=`${w.repo} · score ${w.score.toFixed(2)}`,A.append(k," ",C),x.append(A)}),o.append(x)}))});const n=t.querySelector("#kb-clear-cache");n&&m.bind(n,"click",()=>{d.tap();const e=v.clearCache();p.success(`Cache vidé : ${e.cleared} entrées`),g(t)})}async function Y(t){const s=t.querySelector("#cu-name")?.value.trim()??"",c=t.querySelector("#cu-tier")?.value??"family",r=t.querySelector("#cu-email")?.value.trim()??"",l=t.querySelector("#cu-whatsapp")?.value.trim()??"",n=t.querySelector("#cu-pin")?.value??"";if(!s||s.length<2){d.warning(),p.warn("Nom complet requis (min 2 caractères)");return}const e=await P.createUser({name:s,tier:c,...r&&{email:r},...l&&{whatsappPhone:l},...n&&{initialPin:n}}),a=t.querySelector("#create-user-result");if(!a)return;if(!e.ok||!e.uid){d.error();const u=e.reason??"Erreur création";a.innerHTML=`<div class="ax-error">${i(u)}</div>`,p.error(u);return}d.success(),p.success(`Compte ${s} créé`);let o="";if(l){const u=await j.requestConfirmation({uid:e.uid,name:s,whatsappPhone:l});u.ok&&u.inviteLink&&(o=`
    ...
    <div class="ax-success">
      Compte créé : <strong>${i(s)}</strong> (${c})
  ...
  `,g(t)}let m=null;function pe(){m?.cleanup(),m=null}function g(t){if(m?.cleanup(),m=H("admin"),!I.get("isAdmin")){t.innerHTML=`
      <div class="ax-empty">
        <h2>Accès réservé</h2>
  ...
  `),W(t),f==="bilan"||f==="consumption"){const c=f==="bilan"?"ax-admin-mount-bilan":"ax-admin-mount-consumption",r=t.querySelector(`#${c}`);if(r){const l=r.querySelector("p.ax-muted"),n=l?M(l,"admin-table"):()=>{};q(t).finally(()=>{n()})}else q(t)}}export{pe as dispose,g as render};
