...
>📊 Usage</a>`:"";return`
        <div class="ax-service-health-row ax-modernized-card"
          ...
          ${g}
          ${x}
        </div>`}).join("")}async function S(e){const r={};if(e.length===0)return r;try{const{linksRegistry:o}=await d(async()=>{const{linksRegistry:a}=await import("./links-registry-CzuQc9be.js");return{linksRegistry:a}},__vite__mapDeps([7,1,3,4,5]),import.meta.url);for(const a of e)r[a]={recharge:o.getRechargeLink(a),usage:o.getUsageLink(a),apiKeys:o.getApiKeysLink(a)}}catch{}return r}async function O(e){const r=u.get("user"),o=r?.name?`Bonjour ${r.name}`:"Bonjour",[a,t,c]=await Promise.all([h(),y(),v()]),l=await S(c.map(s=>s.service)),n=w();e.innerHTML=`
    <style>
...
//# sourceMappingURL=index-DUTJvtF4.js.map
