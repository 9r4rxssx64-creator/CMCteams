import{e as n}from"./escape-html-BlQj2yEF.js";import{c as x}from"./listener-cleanup-Y2rGGxxX.js";import{s as o}from"./signup-1t4kI6FO.js";import{haptic as p}from"./haptic-CQFg2PXZ.js";import{toast as s}from"./toast-CRdbcLoc.js";import"./multi-source-analyze-Bg1HHfSC.js";import"./apex-kb-D1VtWFD9.js";import"./monitoring-D2lWYrYo.js";import"./credential-patterns-CLzI061R.js";import"./auth-gate-BlV1tKTL.js";import"./auth-B3RTUO4E.js";import"./kevin-alerts-Bw8KkWyP.js";import"./whatsapp-CVPbuW0O.js";let r=null;function P(){r?.cleanup(),r=null}function f(e){return e.length<=4?e:e.slice(0,4)+"***"+e.slice(-2)}function C(e){r?.cleanup(),r=x("signup-approval"),l(e)}function l(e){const i=o.listPending(),t=o.listProcessed().slice(0,20);e.innerHTML=`
  ...
  `,g(e)}function c(e,i){const t=`${e.prenom} ${e.nom}`,a=new Date(e.createdAt).toLocaleString("fr-FR"),d=e.status==="approved"?{color:"#10b981",label:"✅ Approuvé"}:e.status==="rejected"?{color:"#ef4444",label:"❌ Refusé"}:e.status==="expired"?{color:"#888",label:"⏰ Expiré"}:{color:"#c9a227",label:"⏳ En attente"};return`
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap">
        <div class="ax-gs-11">
          ...
          <div style="font-size:12px;color:var(--ax-text-dim,#aaa);display:grid;gap:2px">
            ...
            ${e.rejectReason?`<div class="ax-gs-110">Raison: ${n(e.rejectReason)}</div>`:""}
          </div>
        </div>
        ${i?`
  ...
  `}function g(e){r&&(e.querySelectorAll(".signup-approve-client").forEach(i=>{r.bind(i,"click",()=>{const t=i.dataset.id;t&&(p.tap(),u(e,t,"client"))})}),e.querySelectorAll(".signup-approve-family").forEach(i=>{r.bind(i,"click",()=>{const t=i.dataset.id;t&&(p.tap(),u(e,t,"family"))})}),e.querySelectorAll(".signup-reject").forEach(i=>{r.bind(i,"click",()=>{const t=i.dataset.id;t&&(p.medium(),m(e,t))})}))}async function u(e,i,t){const a=await o.approveSignup({requestId:i,type:t,adminUid:"kdmc_admin"});a.ok?(s.success(`✅ Approuvé ${t==="family"?"(famille)":"(client)"} → uid ${a.uid?.slice(0,16)??"?"}`),l(e)):s.error(a.reason??"Erreur approval")}async function m(e,i){const t=prompt("Raison du refus (sera visible client) :");if(!t)return;const a=await o.rejectSignup({requestId:i,adminUid:"kdmc_admin",reason:t});a.ok?(s.success("Demande rejetée"),l(e)):s.error(a.reason??"Erreur reject")}export{P as dispose,C as render};
