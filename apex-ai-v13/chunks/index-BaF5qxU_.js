const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./monitoring-3uBGKGRH.js","./multi-source-analyze-CDRNXpkc.js","./apex-kb-BL-mRJI7.js","./credential-patterns-CLzI061R.js","../assets/css/main-CHFUwhnN.css"])))=>i.map(i=>d[i]);
import{b as v,_ as $}from"./apex-kb-BL-mRJI7.js";import{e as i}from"./escape-html-BlQj2yEF.js";import{c as I}from"./listener-cleanup-Y2rGGxxX.js";import{l as h}from"./monitoring-3uBGKGRH.js";import{s as H,a as M}from"../core/main-1zxfPgC1.js";import{apexExecute as y}from"./apex-execute-C7FWXc3B.js";import{auth as z}from"./auth-BAQXvZsZ.js";import{commerce as _}from"./commerce-BCyslXSZ.js";import{c as D}from"./csp-style-helper-BisGRi53.js";import{i as T,r as P}from"./voice-BQ1FH04d.js";import{kdmcProjectsRegistry as S}from"./kdmc-projects-registry-DdTqW2gy.js";import{w as j}from"./whatsapp-BfmSusbQ.js";import{haptic as d}from"./haptic-CQFg2PXZ.js";import{toast as p}from"./toast-CRdbcLoc.js";import"./credential-patterns-CLzI061R.js";import"./multi-source-analyze-CDRNXpkc.js";import"./claude-bridge-BTCGJdYu.js";let f="commerce";const R={commerce:"admin.commerce",users:"admin.users",pending:"admin.users",health:null,projects:null,executions:"admin.executions",knowledge:"admin.kb",bilan:"admin.bilan",consumption:"admin.consumption","audit-log":"admin.audit-log"};function E(){const t=_.isEnabled();return`
  ...
  `}function V(){const t=z.listUsers(),r=new Set(["admin","family","client_pro","client_free"]),c=t.map(o=>{const l=r.has(o.tier)?o.tier:"client_free";return`
      <li class="ax-user-row">
        <span class="ax-user-name">${i(o.name)}</span>
        ...
        <select data-user-plan="${i(o.id)}" class="ax-select-sm">
  ...
  `}function N(){const t=S.list(),r=S.count(),c=S.countActive(),o=["active","wip","archived"],l=t.map(n=>{const e=o.includes(n.status)?n.status:"archived",a=n.tech_stack.slice(0,4).map(s=>i(s)).join(", ");return`
      ...
      `}).join("");return`
    <div class="ax-admin-section">
      <h2>📦 Projets KDMC (${r} — ${c} actifs/wip)</h2>
      <p class="ax-muted">
  ...
  `}function U(){return[["commerce","💳 Commerce"],["users","👥 Comptes"],["pending","📨 Attente"],["health","🩺 Santé"],["projects","📦 Projets"],["executions","⚙️ Exec"],["knowledge","📚 KB"],["bilan","📊 Bilan"],["consumption","💰 Conso"],["audit-log","🔒 Audit"]].filter(([c])=>{const o=R[c];return!o||T(o)}).map(([c,o])=>{const l=f===c;return`
    ...
    <div class="ax-admin-section">
      ...
      <div class="ax-info-card">
        ...
        <ul>
          ...
          <li><strong>En cours</strong> : ${r.pending} pending / ${r.running} running</li>
          <li><strong>Terminées</strong> : ${r.completed} ✅ · ${r.failed} ❌ · ${r.cancelled} 🚫</li>
          <li><strong>Success rate</strong> : ${r.success_rate}%</li>
          <li><strong>Avg duration</strong> : ${Math.round(r.avg_duration_ms/1e3)}s</li>
        </ul>
      </div>
      <div class="ax-info-card">
        ...
        <p class="ax-muted">${o.map(e=>`<code>${i(e)}</code>`).join(" · ")}</p>
      </div>
      <h3>📋 Historique récent</h3>
      <ul class="ax-execution-list">${n||`<li class="ax-muted">Aucune exécution pour l'instant.</li>`}</ul>
    </div>
  `}function G(){const t=R[f];if(t&&!T(t))return`<div class="ax-admin-section">${P(t)}</div>`;switch(f){case"commerce":return E();case"users":return V();case"pending":return B();case"health":return F();case"projects":return N();case"executions":return O();case"knowledge":return K();case"bilan":return'<div id="ax-admin-mount-bilan" class="ax-admin-section"><p class="ax-muted">Chargement du bilan financier…</p></div>';case"consumption":return'<div id="ax-admin-mount-consumption" class="ax-admin-section"><p class="ax-muted">Chargement consommation IA…</p></div>';case"audit-log":return'<div id="ax-admin-mount-audit-log" class="ax-admin-section"><p class="ax-muted">Chargement audit log immuable…</p></div>'}}async function q(t){const r=t.querySelector("#ax-admin-audits-summary");if(r)try{const{reportsHistory:n}=await $(async()=>{const{reportsHistory:x}=await import("./apex-reports-history-Cv_X_jWe.js");return{reportsHistory:x}},__vite__mapDeps([0]),import.meta.url),e=n.getStats(),a=e.lastLayoutTs?new Date(e.lastLayoutTs).toLocaleString("fr-FR",{day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit"}):"jamais",s=e.lastFunctionalTs?new Date(e.lastFunctionalTs).toLocaleString("fr-FR",{day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit"}):"jamais",u=e.recentBugs>0?"#ff5b5b":"#22cc77",b=e.recentEscalations>0?"#ffaa66":"#22cc77";r.innerHTML=`
      ...
      `}catch(n){h.warn("admin","audits summary render failed",{err:n})}const c=t.querySelector("#ax-admin-mount-bilan");if(c)try{(await $(()=>import("./financial-bilan-CoQ74pcf.js"),__vite__mapDeps([1,2,0,3,4]),import.meta.url)).render(c)}catch(n){h.warn("admin","financial-bilan render failed",{err:n}),c.innerHTML='<p class="ax-muted">Bilan indisponible (module ko)</p>'}const o=t.querySelector("#ax-admin-mount-consumption");if(o)try{(await $(()=>import("./consumption-dashboard-DlUQPddL.js"),__vite__mapDeps([2,0,3,1,4]),import.meta.url)).render(o)}catch(n){h.warn("admin","consumption-dashboard render failed",{err:n}),o.innerHTML='<p class="ax-muted">Consommation indisponible (module ko)</p>'}const l=t.querySelector("#ax-admin-mount-audit-log");if(l){if(!T("admin.audit-log")){l.innerHTML=P("admin.audit-log");return}try{const{auditLog:n}=await $(async()=>{const{auditLog:s}=await import("./apex-kb-BL-mRJI7.js").then(u=>u.d);return{auditLog:s}},__vite__mapDeps([2,0,3]),import.meta.url);n.init();const e=n.getEntries().slice(-100).reverse(),a=e.map(s=>`<li><code>${i(new Date(s.ts).toLocaleString())}</code> · <strong>${i(s.action)}</strong> · ${i(s.actor||"system")}</li>`).join("");l.innerHTML=`
      ...
      `}catch(n){h.warn("admin","audit-log render failed",{err:n}),l.innerHTML='<p class="ax-muted">Audit log indisponible (module ko)</p>'}}}function K(){const t=v.listRepos(),r=v.getStats(),c=t.map(o=>`
      <li class="ax-repo-row">
        <code>${i(o)}</code>
        ${t.length>1?`<button class="ax-btn ax-btn-sm" data-remove-repo="${i(o)}">Retirer</button>`:""}
      </li>
    `).join("");return`
    <div class="ax-admin-section">
      ...
      <div class="ax-info-card">
        <strong>État :</strong> ${r.repos} repos · ${r.cache_entries} entrées cache · ${r.index_entries} fichiers indexés
        <br>
        <strong>Token GitHub :</strong> ${r.has_token?"✅ configuré":"⚪ Configure ax_github_token dans le Coffre pour 5000 req/h"}
      </div>
  ...
  `}function W(t){t.querySelectorAll("[data-nav-route]").forEach(e=>{m.bind(e,"click",()=>{d.tap();const a=e.dataset.navRoute??"chat";window.location.hash="#"+a})}),t.querySelectorAll('[data-action="select-all"]').forEach(e=>{m.bind(e,"click",()=>{e.select()})}),t.querySelectorAll("[data-tab]").forEach(e=>{m.bind(e,"click",()=>{d.selection(),f=e.dataset.tab,g(t)})});const r=t.querySelector("#commerce-toggle");r&&m.bind(r,"change",()=>{const e=r.checked;d.medium();const a=t.querySelector(".ax-toggle-label");if(a){a.textContent="",a.append("Commercialisation ");const s=document.createElement("strong");s.textContent=e?"ACTIVÉE":"désactivée",a.append(s)}_.setEnabled(e),p.success(`Commercialisation ${e?"activée":"désactivée"}`),g(t)});const c=t.querySelector("#create-user-form");c&&m.bind(c,"submit",e=>{e.preventDefault(),Y(t)}),t.querySelectorAll("[data-user-plan]").forEach(e=>{m.bind(e,"change",()=>{const a=e.dataset.userPlan??"";a&&(_.setUserPlan(a,e.value),h.info("admin",`Plan ${e.value} → ${a}`))})}),t.querySelectorAll("[data-project-save]").forEach(e=>{m.bind(e,"click",()=>{d.tap();const a=e.dataset.projectSave??"";if(!a)return;const s=t.querySelector(`[data-project-version="${CSS.escape(a)}"]`),u=t.querySelector(`[data-project-status="${CSS.escape(a)}"]`),b=(s?.value??"").trim(),x=u?.value??"active";if(!b){p.warn("Version requise");return}S.update(a,{version:b,status:x})?(d.success(),p.success(`${a} → ${b} (${x})`),h.info("admin",`Project ${a} updated`,{version:b,status:x}),g(t)):(d.error(),p.error("Update échoué"))})}),t.querySelectorAll("[data-confirm-otp]").forEach(e=>{m.bind(e,"click",()=>{d.tap();const a=e.dataset.confirmOtp??"";if(!a)return;const s=j.confirm(a);s.ok?(d.success(),p.success("Compte activé"),h.info("admin",`Confirmed user ${s.uid}`),g(t)):(d.error(),p.error("Code OTP invalide ou expiré"))})}),t.querySelectorAll("[data-exec-cancel]").forEach(e=>{m.bind(e,"click",()=>{d.tap();const a=e.dataset.execCancel??"";if(!a)return;y.cancelExecution(a)?(d.success(),p.success("Exécution annulée"),g(t)):(d.warning(),p.warn("Annulation impossible"))})}),t.querySelectorAll("[data-exec-poll]").forEach(e=>{m.bind(e,"click",()=>{d.tap();const a=e.dataset.execPoll??"";a&&y.pollResult(a).then(s=>{s?(p.success(`Statut : ${s.status}`),g(t)):p.warn("Exécution introuvable")})})});const o=t.querySelector("#add-repo-form");o&&m.bind(o,"submit",e=>{e.preventDefault(),d.tap();const s=t.querySelector("#kb-add-repo")?.value.trim()??"";if(!s){p.warn("Indique un repo");return}const u=v.addRepo(s);u.ok?(d.success(),p.success(`Repo ajouté : ${s}`),g(t)):(d.error(),p.error(u.reason??"Erreur ajout repo"))}),t.querySelectorAll("[data-remove-repo]").forEach(e=>{m.bind(e,"click",()=>{d.tap();const a=e.dataset.removeRepo??"";a&&(v.removeRepo(a),p.success(`Repo retiré : ${a}`),g(t))})});const l=t.querySelector("#kb-search-form");l&&m.bind(l,"submit",e=>{e.preventDefault(),d.tap();const a=t.querySelector("#kb-search-query"),s=t.querySelector("#kb-search-results"),u=a?.value.trim()??"";!u||!s||(s.innerHTML='<p class="ax-muted">Recherche en cours...</p>',v.searchCode(u).then(b=>{if(b.length===0){s.innerHTML='<p class="ax-muted">Aucun résultat (configure ax_github_token pour augmenter la limite).</p>';return}s.textContent="";const x=document.createElement("ul");x.className="ax-kb-results-list",b.slice(0,20).forEach(w=>{const A=document.createElement("li");A.className="ax-kb-result";const k=document.createElement("a");k.href=w.htmlUrl,k.target="_blank",k.rel="noopener";const L=document.createElement("code");L.textContent=w.path,k.append(L);const C=document.createElement("span");C.className="ax-muted",C.textContent=`${w.repo} · score ${w.score.toFixed(2)}`,A.append(k," ",C),x.append(A)}),s.append(x)}))});const n=t.querySelector("#kb-clear-cache");n&&m.bind(n,"click",()=>{d.tap();const e=v.clearCache();p.success(`Cache vidé : ${e.cleared} entrées`),g(t)})}async function Y(t){const r=t.querySelector("#cu-name")?.value.trim()??"",c=t.querySelector("#cu-tier")?.value??"family",o=t.querySelector("#cu-email")?.value.trim()??"",l=t.querySelector("#cu-whatsapp")?.value.trim()??"",n=t.querySelector("#cu-pin")?.value??"";if(!r||r.length<2){d.warning(),p.warn("Nom complet requis (min 2 caractères)");return}const e=await z.createUser({name:r,tier:c,...o&&{email:o},...l&&{whatsappPhone:l},...n&&{initialPin:n}}),a=t.querySelector("#create-user-result");if(!a)return;if(!e.ok||!e.uid){d.error();const u=e.reason??"Erreur création";a.innerHTML=`<div class="ax-error">${i(u)}</div>`,p.error(u);return}d.success(),p.success(`Compte ${r} créé`);let s="";if(l){const u=await j.requestConfirmation({uid:e.uid,name:r,whatsappPhone:l});u.ok&&u.inviteLink&&(s=`
    ...
    <div class="ax-success">
      Compte créé : <strong>${i(r)}</strong> (${c})
  ...
  `,g(t)}let m=null;function me(){m?.cleanup(),m=null}function g(t){if(m?.cleanup(),m=I("admin"),!H.get("isAdmin")){t.innerHTML=`
      <div class="ax-empty">
        <h2>Accès réservé</h2>
  ...
  `),W(t),f==="bilan"||f==="consumption"){const c=f==="bilan"?"ax-admin-mount-bilan":"ax-admin-mount-consumption",o=t.querySelector(`#${c}`);if(o){const l=o.querySelector("p.ax-muted"),n=l?M(l,"admin-table"):()=>{};q(t).finally(()=>{n()})}else q(t)}}export{me as dispose,g as render};
