const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./kevin-alerts-CDvGabxb.js"])))=>i.map(i=>d[i]);
import{_ as v}from"./apex-kb-BFHUv3jX.js";import{e as i}from"./html-safe-CCp1QaJu.js";import{c as S}from"./listener-cleanup-Y2rGGxxX.js";import{l as _}from"./monitoring-BAiQJoxJ.js";import{s as z}from"../core/main-7dJ72G_o.js";import{credentialsAudit as w}from"./credentials-audit-C6pni94g.js";import{h as f}from"./haptic-BUEqXK0N.js";import{toast as p}from"./toast-Dgg9rcIP.js";import"./apex-tools-registry-DqA3gZXO.js";import"./credential-patterns-Dy6Wjk7e.js";import"./multi-source-analyze-DAvdcAQw.js";let x=null,m=null,y="all";function K(){x?.cleanup(),x=null}const h={ok:{color:"#22cc77",icon:"🟢",label:"OK"},missing:{color:"#888",icon:"⚪",label:"Non config"},corrupted:{color:"#ff6b6b",icon:"🔴",label:"Corrompu"},expired:{color:"#ffaa00",icon:"🟠",label:"Expiré"},unknown:{color:"#aaa",icon:"❓",label:"Inconnu"},decrypt_failed:{color:"#ff6b6b",icon:"🔒",label:"Illisible"}},$={all:"Tous",ai:"🤖 IA",banking:"🏦 Banque",payment:"💳 Paiement",social:"📱 Social",email:"📧 Email",crypto:"₿ Crypto",hosting:"☁️ Hosting",productivity:"📋 Productivité",forbidden:"⚠️ Forbidden"};async function k(e){if(!(z.get("isAdmin")===!0)){e.innerHTML=`
  ...
  `;try{m=await w.runFullAudit()}catch(s){_.error("credentials-registry","audit failed",{err:s}),e.innerHTML=`<div style="padding:40px;color:#ff6b6b">Erreur audit : ${i(String(s))}</div>`;return}A(e,m)}function A(e,n){const s=n.security_score,t=s>=80?"#22cc77":s>=60?"#ffaa00":"#ff6b6b",l=[...y==="all"?n.entries:n.entries.filter(a=>a.category===y)].sort((a,d)=>a.configured!==d.configured?a.configured?-1:1:a.service_name.localeCompare(d.service_name)),o=new Map;o.set("all",n.entries.length);for(const a of n.entries)o.set(a.category,(o.get(a.category)??0)+1);const r=[...o.entries()].filter(([a])=>a==="all"||$[a]&&(o.get(a)??0)>0).map(([a,d])=>{const b=y===a,C=$[a]??a;return`<button data-cat="${i(a)}" class="ax-cat-chip" style="
  ...
  `}function R(e,n,s){const t=document.createElement("div");t.setAttribute("role","dialog"),t.setAttribute("aria-label",`Récupérer ${s}`),t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);display:flex;align-items:center;justify-content:center;z-index:9999;padding:20px",t.innerHTML=`
    <div style="background:#1a1a2e;border:1px solid rgba(201,162,39,0.4);border-radius:14px;padding:24px;max-width:480px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,0.6)">
      ...
      <input type="password" id="ax-recover-input" autocomplete="off" placeholder="Recolle ta clé ${i(s)}…"
  ...
  `,document.body.appendChild(t);const c=t.querySelector("#ax-recover-input"),l=t.querySelector("#ax-recover-cancel"),o=t.querySelector("#ax-recover-confirm");setTimeout(()=>c?.focus(),50);const r=()=>{t.remove()};l?.addEventListener("click",r),t.addEventListener("click",u=>{u.target===t&&r()}),o?.addEventListener("click",()=>{(async()=>{const u=c?.value.trim()??"";if(!u){p.warn("Recolle ta clé pour récupérer");return}try{const{vault:g}=await v(async()=>{const{vault:d}=await import("./apex-kb-BFHUv3jX.js").then(b=>b.b);return{vault:d}},[],import.meta.url),a=await g.recover(n,u);a.ok?(f.success(),p.success(`✅ ${s} récupérée + re-chiffrée`),r(),k(e)):(f.error(),p.error(`❌ ${a.reason??"recover failed"}`))}catch(g){p.error(`Erreur : ${String(g).slice(0,100)}`)}})()})}function L(e){if(!x)return;const n=e.querySelector("#ax-cred-refresh");n&&x.bind(n,"click",()=>{f.tap(),k(e)});const s=e.querySelector("#ax-cred-test-channels");s&&x.bind(s,"click",()=>{f.tap(),(async()=>{p.info("📡 Test alertes en cours...");const{kevinAlerts:t}=await v(async()=>{const{kevinAlerts:o}=await import("./kevin-alerts-CDvGabxb.js");return{kevinAlerts:o}},__vite__mapDeps([0]),import.meta.url),c=await t.testAllChannels(),l=Object.entries(c).filter(([,o])=>o).map(([o])=>o);l.length===0?p.warn("Aucun channel d'alerte configuré"):p.success(`✅ Channels OK : ${l.join(", ")}`)})()}),e.querySelectorAll("[data-cat]").forEach(t=>{x.bind(t,"click",()=>{y=t.dataset.cat??"all",f.selection(),m&&A(e,m)})}),e.querySelectorAll("[data-recover]").forEach(t=>{x.bind(t,"click",()=>{const c=t.dataset.recover??"",l=t.dataset.service??c;c&&(f.tap(),R(e,c,l))})}),e.querySelectorAll("[data-test]").forEach(t=>{x.bind(t,"click",c=>{c.stopPropagation();const l=t.dataset.test;l&&(f.tap(),(async()=>{const o=t.textContent;t.textContent="⏳ Test...",t.disabled=!0;try{const r=await w.testCredential(l);r.valid===!0?p.success(`✅ ${l} : valide (${r.latency_ms}ms)`):r.valid===!1?p.warn(`❌ ${l} : invalide`):p.info(`❓ ${l} : ${r.error??"test impossible"}`)}catch(r){p.error(`Test failed : ${String(r).slice(0,100)}`)}finally{t.textContent=o??"🧪 Tester",t.disabled=!1}})())})}),e.querySelectorAll("[data-cred-detail]").forEach(t=>{x.bind(t,"click",c=>{c.target.closest("button, a")||(async()=>{const o=t.dataset.credDetail;if(!o||!m)return;const r=m.entries.find(d=>d.storage_key===o);if(!r)return;const{drillDown:u}=await v(async()=>{const{drillDown:d}=await import("./drilldown-1SlvGToi.js");return{drillDown:d}},[],import.meta.url),g="ax-drilldown-mount-credentials";let a=document.getElementById(g);a||(a=document.createElement("div"),a.id=g,document.body.appendChild(a)),u.open({id:`cred-${o}`,title:`🔑 ${r.service_name}`,content:()=>{const d=h[r.status]??h.unknown;return`
            ...
            `},data:{storageKey:o}},a)})()})})}export{K as dispose,k as render};
