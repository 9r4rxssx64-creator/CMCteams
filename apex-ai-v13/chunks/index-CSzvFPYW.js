    ...
    <div class="ax-page" style="padding:16px;max-width:780px;margin:0 auto">
      ...
      <div style="background:rgba(201,162,39,0.05);border:1px solid rgba(201,162,39,0.3);border-radius:12px;padding:14px;margin-bottom:16px">
        ...
        <ul style="margin:0;padding-left:18px;color:var(--ax-text-dim);font-size:13px">
          ${v.map(a=>`<li><strong style="color:#c9a227">${l(a.label)} :</strong> ≥ ${a.min_value_cm} cm — ${l(a.description)}</li>`).join("")}
        </ul>
      </div>

      <p style="margin-top:24px;text-align:center"><a href="#studios" style="color:#c9a227">← Retour studios</a></p>
    </div>
  `,B(e)}function B(e){const i=e.querySelector("#ax-archi-surface-btn");i&&c&&c.bind(i,"click",()=>{x.tap();const r=e.querySelector("#ax-archi-surface"),s=e.querySelector("#ax-archi-surface-out");if(!r||!s)return;const p=parseFloat(r.value),n=z(p);s.textContent=n>0?`Surface habitable estimée : ${n} m²`:"Saisis une surface valide."});const t=e.querySelector("#ax-archi-beton-btn");t&&c&&c.bind(t,"click",()=>{x.tap();const r=parseFloat(e.querySelector("#ax-archi-vol")?.value??""),s=e.querySelector("#ax-archi-dosage")?.value??"",p=e.querySelector("#ax-archi-beton-out");if(!p)return;const n=M(r,s);if(!n){p.textContent="Volume invalide.";return}p.textContent=`Volume : ${n.volume_m3} m³
Ciment : ${n.ciment_kg} kg (${n.sacs_ciment_35kg} sacs de 35 kg)
Sable : ${n.sable_kg} kg
Gravier : ${n.gravier_kg} kg
Eau : ${n.eau_l} L`});const o=e.querySelector("#ax-archi-paint-btn");o&&c&&c.bind(o,"click",()=>{x.tap();const r=parseFloat(e.querySelector("#ax-archi-l")?.value??""),s=parseFloat(e.querySelector("#ax-archi-w")?.value??""),p=parseFloat(e.querySelector("#ax-archi-h")?.value??""),n=e.querySelector("#ax-archi-paint-out");if(!n)return;const u=$(r,s,p,2);if(!u){n.textContent="Dimensions invalides.";return}n.textContent=`Surface peinte (2 couches) : ${u.surface_a_peindre_m2} m²
Total : ${u.litres_total} litres
≈ ${u.pots_5l} pots de 5L + ${u.pots_2_5l} pots de 2.5L`});const a=e.querySelector("#ax-archi-zone"),m=e.querySelector("#ax-archi-zone-out"),d=()=>{if(!a||!m)return;const r=b.find(s=>s.zone===a.value);r&&(m.textContent=`${r.zone} — ${r.description}. Bbio max ${r.bbio_max} pts · Cep max ${r.cep_max} kWh/m²/an.`)};a&&c&&(c.bind(a,"change",d),d()),h.info("studios-architecture","rendered")}export{g as BETON_DOSAGES,S as BLONDEL_MAX,k as BLONDEL_MIN,A as HSP_MIN_FR,T as HSP_MIN_MONACO,v as PMR_NORMS,b as RE2020_ZONES,M as calcBeton,$ as calcPeinture,j as calcRevetement,z as calcSurfaceHabitable,G as checkBlondel,D as dispose,l as escapeHtml,I as render};
