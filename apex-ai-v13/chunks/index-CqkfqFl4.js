import{r as f}from"../core/main-_B861htv.js";import{auth as g}from"./auth-BV0_zylR.js";import{h as c,t as d}from"./toast-D1-s5Kao.js";function m(e){return e.replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]??t)}function v(){const e=location.hash.match(/invite=([A-Za-z0-9+/=]+)/);if(!e||!e[1])return null;try{const r=atob(e[1]??"").split(":")[0];if(r)return{uid:r}}catch{}return null}function q(e){const t=v();e.innerHTML=`
  ...
  `,e.querySelector("#login-form")?.addEventListener("submit",n=>{n.preventDefault(),c.tap(),x(e)}),typeof window<"u"&&window.matchMedia("(pointer: fine)").matches&&e.querySelector("#login-name")?.focus()}async function x(e){const t=e.querySelector("#login-name"),r=e.querySelector("#login-pin"),n=e.querySelector("#login-submit"),i=e.querySelector("#login-error"),a=n?.querySelector(".ax-btn-label"),o=n?.querySelector(".ax-spinner"),p=t?.value.trim()??"",u=r?.value??"";if(p.length<2||u.length<4){c.warning(),i&&(i.innerHTML='<div class="ax-error">Nom et PIN obligatoires (min 4 chiffres)</div>');return}n&&(n.disabled=!0),a&&(a.textContent="Vérification..."),o&&(o.style.display="inline-block");try{const s=await g.login(p,u);if(!s.ok){c.error();const l=s.reason??"Connexion impossible";i&&(i.innerHTML=`<div class="ax-error">${m(l)}</div>`),d.error(l),n&&(n.disabled=!1),a&&(a.textContent="Se connecter"),o&&(o.style.display="none");return}c.success(),d.success("Bienvenue !"),setTimeout(()=>{f.navigate("chat")},200)}catch(s){c.error();const l=s instanceof Error?s.message:"Erreur inattendue";i&&(i.innerHTML=`<div class="ax-error">${m(l)}</div>`),d.error(l),n&&(n.disabled=!1),a&&(a.textContent="Se connecter"),o&&(o.style.display="none")}}export{q as render};
//# sourceMappingURL=index-CqkfqFl4.js.map
