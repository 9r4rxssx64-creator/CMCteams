const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./monitoring-CpAzCFTb.js","./multi-source-analyze-CABQFfu3.js","./credential-patterns-DUMYZEMu.js","./apex-kb-DvSa-GZo.js","./auth-DfuMTtv2.js"])))=>i.map(i=>d[i]);
import{b as I,e as x,_ as l,l as z}from"./monitoring-CpAzCFTb.js";import{c as P}from"./listener-cleanup-Y2rGGxxX.js";import{c as O}from"./csp-style-helper-BEHhIhzj.js";import{r as D}from"./recharge-action-DS8cdMjQ.js";import"./multi-source-analyze-CABQFfu3.js";import"./credential-patterns-DUMYZEMu.js";import"./apex-kb-DvSa-GZo.js";let i=null;function K(){i?.cleanup(),i=null}const L="apex_v13_chat_auto_read";function R(){try{return localStorage.getItem(L)==="1"}catch{return!1}}function V(o){try{localStorage.setItem(L,o?"1":"0")}catch{}}async function q(o){try{const A=await l(()=>import("./voice-CpslOLeZ.js"),__vite__mapDeps([0,1,2,3]),import.meta.url),{listVoices:S,getActiveVoice:g,setActiveVoice:b,speak:v,stopAll:m}=A,h=o.querySelector("#ax-settings-auto-read"),T=o.querySelector("#ax-voice-current"),k=o.querySelector("#ax-voice-list"),E=o.querySelectorAll(".ax-voice-cat-btn"),_=o.querySelector("#ax-settings-rag");_&&(l(async()=>{const{apexMemoryRag:n}=await import("./apex-memory-rag-jR-wRFUZ.js");return{apexMemoryRag:n}},__vite__mapDeps([0,1,2,3]),import.meta.url).then(({apexMemoryRag:n})=>{_.checked=n.isEnabled()}),i.bind(_,"change",()=>{(async()=>{const{apexMemoryRag:n}=await l(async()=>{const{apexMemoryRag:a}=await import("./apex-memory-rag-jR-wRFUZ.js");return{apexMemoryRag:a}},__vite__mapDeps([0,1,2,3]),import.meta.url);n.enable(_.checked);const{toast:p}=await l(async()=>{const{toast:a}=await import("./toast-BCPNzfMv.js");return{toast:a}},[],import.meta.url);p.success(_.checked?"🧠 Mémoire long terme activée":"Mémoire long terme désactivée")})()}));const w=o.querySelector("#ax-settings-cmem");if(w&&(l(async()=>{const{compactMemory:n}=await import("./compact-memory-XEERFFCW.js");return{compactMemory:n}},__vite__mapDeps([0,1,2,3]),import.meta.url).then(({compactMemory:n})=>{w.checked=n.isEnabled()}),i.bind(w,"change",()=>{(async()=>{const{compactMemory:n}=await l(async()=>{const{compactMemory:a}=await import("./compact-memory-XEERFFCW.js");return{compactMemory:a}},__vite__mapDeps([0,1,2,3]),import.meta.url);n.enable(w.checked);const{toast:p}=await l(async()=>{const{toast:a}=await import("./toast-BCPNzfMv.js");return{toast:a}},[],import.meta.url);p.success(w.checked?"⚡ Mémoire compacte activée":"Mémoire compacte désactivée")})()})),!k)return;h&&(h.checked=R(),i.bind(h,"change",()=>{V(h.checked),(async()=>{const{toast:n}=await l(async()=>{const{toast:p}=await import("./toast-BCPNzfMv.js");return{toast:p}},[],import.meta.url);n.success(h.checked?"Lecture auto activée":"Lecture auto désactivée")})()}));const $=()=>{if(!T)return;const n=g(),a=S().find(s=>s.id===n);T.textContent=a?`Voix active : ${a.emoji??"🔊"} ${a.name} (${a.category})`:`Voix active : ${n}`};$();const f=n=>{const p=S(),a=n==="all"?p:p.filter(t=>t.category===n),s=g();k.setAttribute("aria-busy","false"),k.innerHTML=a.map(t=>{const e=t.id===s,r=t.emoji??(t.category==="pro"?"🎙️":t.category==="fun"?"🎉":"🎨"),d=t.description?x(t.description):"";return`
            <div class="ax-voice-item${e?" is-active":""}" data-voice-id="${x(t.id)}">
              <span class="ax-gs-17">${r}</span>
              <div class="ax-gs-6">
                <div class="ax-voice-item__name">${x(t.name)}${e?' <span style="color:var(--ax-gold);font-size:11px">★ active</span>':""}</div>
                <div class="ax-voice-item__meta">${x(t.category)}${d?" · "+d:""}</div>
              </div>
    ...
    <div class="ax-page" style="padding:24px 16px max(24px, env(safe-area-inset-bottom)) 16px;max-width:680px;margin:0 auto;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif">
      <header style="margin-bottom:24px;animation:ax-fade-up 360ms cubic-bezier(0.16,1,0.3,1) backwards">
        ...
        <p class="ax-gs-399">Utilisateur : <strong style="color:rgba(255,255,255,0.9)">${x(A?.name??"inconnu")}</strong> ${S?'<span style="color:var(--ax-gold)">👑 Admin</span>':""}</p>
      </header>
  ...
  `),(async()=>{try{const{memoryBridge:a}=await l(async()=>{const{memoryBridge:r}=await import("./memory-bridge-eU9tqg1j.js");return{memoryBridge:r}},__vite__mapDeps([0,1,2,3]),import.meta.url),s=o.querySelector("#ax-memory-bridge-status"),t=o.querySelector("#ax-memory-bridge-sync"),e=()=>{if(!s)return;const r=a.getHealth(),d=a.getStatus(),c=d.filter(u=>u.last_success).length;s.textContent=`${r.backends_configured} backends configurés · ${c}/${d.length} dernier sync OK`};e(),t&&i&&i.bind(t,"click",()=>{(async()=>{t&&(t.disabled=!0);const r=await a.runAutoSync(),d=r.filter(u=>u.ok).length,{toast:c}=await l(async()=>{const{toast:u}=await import("./toast-BCPNzfMv.js");return{toast:u}},[],import.meta.url);r.length===0?c.warn("Aucun backend configuré"):d===r.length?c.success(`Sync OK (${d}/${r.length})`):c.warn(`Sync partielle (${d}/${r.length})`),e(),t&&(t.disabled=!1)})()})}catch(a){z.warn("feature-settings","memory-bridge wire failed",{err:a})}})();const h=o.querySelector("#ax-conso-scan");h&&i&&i.bind(h,"click",()=>{(async()=>{try{const{consumptionAnomalyDetector:a}=await l(async()=>{const{consumptionAnomalyDetector:e}=await import("./monitoring-CpAzCFTb.js").then(r=>r.J);return{consumptionAnomalyDetector:e}},__vite__mapDeps([0,1,2,3]),import.meta.url),s=a.scanAllVerbose(),t=o.querySelector("#ax-conso-results");if(!t)return;t.innerHTML=s.map(e=>{const r=e.severity==="critical"?"var(--ax-sev-critical)":e.severity==="high"?"var(--ax-sev-high)":e.severity==="medium"?"var(--ax-sev-medium)":e.severity==="low"?"var(--ax-sev-low)":"var(--ax-green)",d=e.severity==="critical"?"ax-sev-critical":e.severity==="high"?"ax-sev-high":e.severity==="medium"?"ax-sev-medium":(e.severity==="low","ax-sev-low"),c=e.severity==="critical"?"🚨":e.severity==="high"?"⚠️":e.severity==="medium"?"🟡":e.severity==="low"?"🔵":"✅";return`<div style="background:rgba(255,255,255,0.03);border-left:3px solid ${r};padding:8px 12px;margin-top:6px;border-radius:4px">
            <strong style="color:${r}">${c} ${e.service}</strong> <span class="ax-sev ${d}">${e.severity}</span>
          ...
          </div>`}).join("")}catch(a){z.warn("feature-settings","conso scan failed",{err:a})}})()});const T=o.querySelector("#ax-zoom-inspector-btn");T&&i&&i.bind(T,"click",()=>{(async()=>{const{apexZoomInspector:a}=await l(async()=>{const{apexZoomInspector:s}=await import("./apex-zoom-inspector-CCrMjTZB.js");return{apexZoomInspector:s}},__vite__mapDeps([0,1,2,3]),import.meta.url);a.isVisible()?a.hide():a.show()})()});const k=o.querySelector("#ax-functional-test-btn");k&&i&&i.bind(k,"click",()=>{(async()=>{const a=o.querySelector("#ax-functional-test-results");if(a){a.innerHTML=`<div style="color:var(--ax-blue)">⏳ Test des boutons en cours (jusqu'à 40 boutons, ~30s)...</div>`;try{const{apexFunctionalTester:s}=await l(async()=>{const{apexFunctionalTester:y}=await import("./apex-functional-tester-rECh-tMD.js");return{apexFunctionalTester:y}},__vite__mapDeps([0,1,2,3]),import.meta.url),{reportsHistory:t}=await l(async()=>{const{reportsHistory:y}=await import("./apex-reports-history-7UGnjHYg.js");return{reportsHistory:y}},__vite__mapDeps([0,1,2,3]),import.meta.url),e=await s.testAndAutoFix({maxButtons:30});t.recordFunctional(e.before,e.fixes,e.after,e.improvement);const r=e.before.tested>0?Math.round(e.before.ok/e.before.tested*100):0,d=e.after?` → après fix : ${Math.round(e.after.ok/Math.max(1,e.after.tested)*100)}% OK (${e.improvement>0?"+":""}${Math.round(e.improvement*100)}%)`:"",c=e.before.details.filter(y=>y.status==="no_response"||y.status==="error").slice(0,5).map(y=>`<li class="ax-gs-250">${x(y.label||"(no label)")} → ${x(y.status)}</li>`).join(""),u=e.fixes.applied.map(y=>x(String(y))).join(", ");a.innerHTML=`
            <div style="background:rgba(106,138,255,0.08);border:1px solid rgba(106,138,255,0.3);border-radius:8px;padding:10px;color:#fff;font-size:12px">
              ...
              ${e.fixes.applied.length?`<div class="ax-gs-75">🔧 Auto-fix appliqué : ${u}</div>`:""}
          ...
          `}catch(s){const t=s instanceof Error?s.message:String(s);a.innerHTML=`<div class="ax-gs-76">❌ Erreur test : ${x(t)}</div>`}}})()});const E=o.querySelector("#ax-layout-inspect-btn");E&&i&&i.bind(E,"click",()=>{(async()=>{const a=o.querySelector("#ax-layout-inspect-results");if(a){a.innerHTML='<div style="color:var(--ax-purple)">⏳ Scan layout...</div>';try{const{apexLayoutInspector:s}=await l(async()=>{const{apexLayoutInspector:c}=await import("./apex-layout-inspector-CLuFMkPI.js");return{apexLayoutInspector:c}},__vite__mapDeps([0,1,2,3]),import.meta.url),{reportsHistory:t}=await l(async()=>{const{reportsHistory:c}=await import("./apex-reports-history-7UGnjHYg.js");return{reportsHistory:c}},__vite__mapDeps([0,1,2,3]),import.meta.url),e=s.scanDom();t.recordLayout(e);const r=e.hiddenButtons.slice(0,5).map(c=>`<li class="ax-gs-250">"${c.label}" → ${c.reason}</li>`).join(""),d=e.overflowingElements.slice(0,5).map(c=>`<li style="color:var(--ax-error);font-size:11px">${c.tag} (+${c.overflowBy}px)</li>`).join("");a.innerHTML=`
          ...
          `}catch(s){const t=s instanceof Error?s.message:String(s);a.innerHTML=`<div class="ax-gs-76">❌ Erreur scan : ${x(t)}</div>`}}})()});const _=o.querySelector("#ax-cf-diagnostic-btn");_&&i&&i.bind(_,"click",()=>{(async()=>{const a=o.querySelector("#ax-cf-diagnostic-results");if(a){a.innerHTML='<div class="ax-gs-168">⏳ Test Cloudflare API en cours...</div>';try{const{apexCloudflareVaultDeploy:s}=await l(async()=>{const{apexCloudflareVaultDeploy:d}=await import("./apex-cloudflare-vault-deploy-DVt8y08p.js");return{apexCloudflareVaultDeploy:d}},__vite__mapDeps([0,1,2,3]),import.meta.url),t=await s.runDiagnostic(),e=(d,c,u)=>`
            <div style="display:flex;align-items:center;gap:8px;padding:6px 0;font-size:13px">
              <span style="color:${c?"var(--ax-green)":"var(--ax-error)"};font-weight:700">${c?"✅":"❌"}</span>
              <span style="flex:1;color:rgba(255,255,255,0.85)">${d}</span>
              ${u?`<span class="ax-gs-107">${u}</span>`:""}
            </div>`;let r='<div style="background:rgba(15,15,25,0.8);border:1px solid rgba(247,131,34,0.3);border-radius:10px;padding:12px;margin-top:10px">';r+='<div style="color:var(--ax-warning);font-weight:700;margin-bottom:8px">☁️ Diagnostic Cloudflare</div>',r+=e("Token Cloudflare présent",t.token_present),r+=e("Token valide (HTTP 200)",t.token_valid,t.http_status?`HTTP ${t.http_status}`:""),r+=e("Account ID accessible",!!t.account_id,t.account_name??t.account_id??""),r+=e("Permission KV (Workers KV Storage:Edit)",t.kv_permission,t.namespace_id?`ns ${t.namespace_id.slice(0,8)}…`:""),r+=e("Permission Workers (auto-deploy futur)",t.workers_permission),r+=e("Namespace apex-vault-kevin existe",t.namespace_exists),t.error_reason&&(r+=`<div style="margin-top:10px;padding:8px;background:rgba(255,91,91,0.1);border-left:3px solid var(--ax-error);color:var(--ax-error);font-size:12px;border-radius:4px">${t.error_reason}</div>`),t.fix_url&&(r+=`<div class="ax-gs-114"><a href="${t.fix_url}" target="_blank" rel="noopener" style="color:var(--ax-blue);font-size:12px">🔗 Fix : ${t.fix_url}</a></div>`),r+="</div>",a.innerHTML=r,z.info("cf-diag-manual",`Diagnostic result : token=${t.token_valid} kv=${t.kv_permission} workers=${t.workers_permission}`)}catch(s){const t=s instanceof Error?s.message:String(s);a.textContent="";const e=document.createElement("div");e.style.color="var(--ax-error)",e.textContent=`❌ Erreur : ${t.slice(0,100)}`,a.append(e)}}})()}),o.querySelectorAll("[data-nav-route]").forEach(a=>{i.bind(a,"click",()=>{const s=a.getAttribute("data-nav-route");s&&(location.hash="#"+s)})});const w=o.querySelector("#ax-settings-logout");w&&i&&i.bind(w,"click",()=>{(async()=>{const{auth:a}=await l(async()=>{const{auth:s}=await import("./auth-DfuMTtv2.js");return{auth:s}},__vite__mapDeps([4,0,1,2,3]),import.meta.url);a.logout(),location.hash="#login"})()});const $=o.querySelector("#ax-settings-tools-auto-embed");if($&&i){try{const a=JSON.parse(localStorage.getItem("ax_settings")??"{}");$.checked=a.tools_auto_embed!==!1}catch{$.checked=!0}i.bind($,"change",()=>{try{const a=JSON.parse(localStorage.getItem("ax_settings")??"{}");a.tools_auto_embed=$.checked,localStorage.setItem("ax_settings",JSON.stringify(a))}catch{}})}const f=o.querySelector("#ax-force-update-btn"),n=o.querySelector("#ax-force-update-status");f&&i&&i.bind(f,"click",()=>{(async()=>{const a=s=>{n&&(n.textContent=s)};f.disabled=!0,f.textContent="⏳ Reset en cours…";try{if(a("🔍 Désinstallation Service Workers…"),"serviceWorker"in navigator){const t=await navigator.serviceWorker.getRegistrations();for(const e of t)await e.unregister();a(`✅ ${t.length} SW désinstallés`)}if(a("🔍 Vidage caches PWA…"),"caches"in window){const t=await caches.keys();for(const e of t)await caches.delete(e);a(`✅ ${t.length} caches vidés`)}a("✅ Reset terminé. Rechargement dans 2s…");const{toast:s}=await l(async()=>{const{toast:t}=await import("./toast-BCPNzfMv.js");return{toast:t}},[],import.meta.url);s.info("🔄 Reset OK — reload imminent"),setTimeout(()=>{location.href=location.pathname+"?_forceupd=1&_reset="+Date.now()},2e3)}catch(s){a(`❌ Erreur : ${String(s)}`),f.disabled=!1,f.textContent="🔄 Force reset PWA + reload"}})()}),q(o);const p=o.querySelector("#ax-settings-notif-test");p&&i&&i.bind(p,"click",()=>{(async()=>{try{if("Notification"in window&&Notification.permission==="granted")new Notification("Test Apex",{body:"Si tu vois ça, push notif fonctionne ✅"});else if("Notification"in window)if(await Notification.requestPermission()==="granted")new Notification("Test Apex",{body:"Push activé ✅"});else{const{toast:s}=await l(async()=>{const{toast:t}=await import("./toast-BCPNzfMv.js");return{toast:t}},[],import.meta.url);s.warn("Permission notifications refusée")}else{const{toast:a}=await l(async()=>{const{toast:s}=await import("./toast-BCPNzfMv.js");return{toast:s}},[],import.meta.url);a.warn("Notifications non supportées par ce navigateur")}}catch{const{toast:a}=await l(async()=>{const{toast:s}=await import("./toast-BCPNzfMv.js");return{toast:s}},[],import.meta.url);a.warn("Test notification échoué")}})()}),z.info("feature-settings","rendered")}export{K as dispose,W as render,q as wireVoiceSection};
