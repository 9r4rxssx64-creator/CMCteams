const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./monitoring-3uBGKGRH.js","./apex-kb-CYBdNpxT.js","./credential-patterns-CLzI061R.js","./multi-source-analyze-C5iju7K1.js","./auth-BkgJ9fQa.js","../assets/css/main-BUIq5pfg.css"])))=>i.map(i=>d[i]);
import{_ as u}from"./apex-kb-CYBdNpxT.js";import{e as y}from"./escape-html-B4YFbUXM.js";import{c as P}from"./listener-cleanup-Y2rGGxxX.js";import{l as A}from"./monitoring-3uBGKGRH.js";import{s as L}from"../core/main-BU35gOTN.js";import{c as E}from"./csp-style-helper-BisGRi53.js";import"./credential-patterns-CLzI061R.js";import"./multi-source-analyze-C5iju7K1.js";let i=null;function F(){i?.cleanup(),i=null}const I="apex_v13_chat_auto_read";function O(){try{return localStorage.getItem(I)==="1"}catch{return!1}}function D(n){try{localStorage.setItem(I,n?"1":"0")}catch{}}async function V(n){try{const $=await u(()=>import("./voice-BS8axcZk.js").then(l=>l.c),__vite__mapDeps([0,1,2]),import.meta.url),{listVoices:k,getActiveVoice:x,setActiveVoice:f,speak:m,stopAll:b}=$,h=n.querySelector("#ax-settings-auto-read"),S=n.querySelector("#ax-voice-current"),_=n.querySelector("#ax-voice-list"),T=n.querySelectorAll(".ax-voice-cat-btn");if(!_)return;h&&(h.checked=O(),i.bind(h,"change",()=>{D(h.checked),(async()=>{const{toast:l}=await u(async()=>{const{toast:d}=await import("./toast-CRdbcLoc.js");return{toast:d}},[],import.meta.url);l.success(h.checked?"Lecture auto activée":"Lecture auto désactivée")})()}));const z=()=>{if(!S)return;const l=x(),g=k().find(v=>v.id===l);S.textContent=g?`Voix active : ${g.emoji??"🔊"} ${g.name} (${g.category})`:`Voix active : ${l}`};z();const w=l=>{const d=k(),g=l==="all"?d:d.filter(e=>e.category===l),v=x();_.innerHTML=g.map(e=>{const a=e.id===v,o=e.emoji??(e.category==="pro"?"🎙️":e.category==="fun"?"🎉":"🎨"),t=e.description?y(e.description):"",r=a?"background:rgba(232,184,48,0.15);border-color:rgba(232,184,48,0.45)":"background:rgba(255,255,255,0.03);border-color:rgba(255,255,255,0.06)";return`
            <div class="ax-voice-item" data-voice-id="${y(e.id)}" style="display:flex;align-items:center;gap:8px;padding:10px;margin-bottom:6px;border:1px solid;border-radius:8px;${r}">
              ...
              <div style="flex:1;min-width:0">
                ...
                <div style="color:rgba(255,255,255,0.5);font-size:11px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${y(e.category)}${t?" · "+t:""}</div>
              </div>
    ...
    <div class="ax-page" style="padding:24px 16px max(24px, env(safe-area-inset-bottom)) 16px;max-width:680px;margin:0 auto;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif">
      <header style="margin-bottom:24px;animation:ax-fade-up 360ms cubic-bezier(0.16,1,0.3,1) backwards">
        ...
        <p style="color:rgba(255,255,255,0.55);margin:0;font-size:14px">Utilisateur : <strong style="color:rgba(255,255,255,0.9)">${y($?.name??"inconnu")}</strong> ${k?'<span style="color:#e8b830">👑 Admin</span>':""}</p>
      </header>
  ...
  `),(async()=>{try{const{memoryBridge:e}=await u(async()=>{const{memoryBridge:r}=await import("./memory-bridge-BRznIrD1.js");return{memoryBridge:r}},__vite__mapDeps([1,0,2]),import.meta.url),a=n.querySelector("#ax-memory-bridge-status"),o=n.querySelector("#ax-memory-bridge-sync"),t=()=>{if(!a)return;const r=e.getHealth(),s=e.getStatus(),c=s.filter(p=>p.last_success).length;a.textContent=`${r.backends_configured} backends configurés · ${c}/${s.length} dernier sync OK`};t(),o&&i&&i.bind(o,"click",()=>{(async()=>{o&&(o.disabled=!0);const r=await e.runAutoSync(),s=r.filter(p=>p.ok).length,{toast:c}=await u(async()=>{const{toast:p}=await import("./toast-CRdbcLoc.js");return{toast:p}},[],import.meta.url);r.length===0?c.warn("Aucun backend configuré"):s===r.length?c.success(`Sync OK (${s}/${r.length})`):c.warn(`Sync partielle (${s}/${r.length})`),t(),o&&(o.disabled=!1)})()})}catch(e){A.warn("feature-settings","memory-bridge wire failed",{err:e})}})();const h=n.querySelector("#ax-conso-scan");h&&i&&i.bind(h,"click",()=>{(async()=>{try{const{consumptionAnomalyDetector:e}=await u(async()=>{const{consumptionAnomalyDetector:t}=await import("./consumption-anomaly-detector-Docw9DBS.js");return{consumptionAnomalyDetector:t}},__vite__mapDeps([0,1,2,3]),import.meta.url),a=e.scanAllVerbose(),o=n.querySelector("#ax-conso-results");if(!o)return;o.innerHTML=a.map(t=>{const r=t.severity==="critical"?"#ff4444":t.severity==="high"?"#ff8844":t.severity==="medium"?"#ffaa00":t.severity==="low"?"#88aaff":"#22cc77",s=t.severity==="critical"?"🚨":t.severity==="high"?"⚠️":t.severity==="medium"?"🟡":t.severity==="low"?"🔵":"✅";return`<div style="background:rgba(255,255,255,0.03);border-left:3px solid ${r};padding:8px 12px;margin-top:6px;border-radius:4px">
          ...
          `}catch(a){const o=a instanceof Error?a.message:String(a);e.innerHTML=`<div style="color:#ff5b5b">❌ Erreur scan : ${y(o)}</div>`}}})()});const z=n.querySelector("#ax-cf-diagnostic-btn");z&&i&&i.bind(z,"click",()=>{(async()=>{const e=n.querySelector("#ax-cf-diagnostic-results");if(e){e.innerHTML='<div style="color:#f78322">⏳ Test Cloudflare API en cours...</div>';try{const{apexCloudflareVaultDeploy:a}=await u(async()=>{const{apexCloudflareVaultDeploy:s}=await import("./apex-cloudflare-vault-deploy-DfYFCQjX.js");return{apexCloudflareVaultDeploy:s}},__vite__mapDeps([1,0,2]),import.meta.url),o=await a.runDiagnostic(),t=(s,c,p)=>`
