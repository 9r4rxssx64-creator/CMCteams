import{e as i}from"./escape-html-BlQj2yEF.js";import{l as c}from"./monitoring-3uBGKGRH.js";import{s as l}from"../core/main-Mpi1QagI.js";import{broadlinkBridge as t}from"./broadlink-bridge-D2cZ20ot.js";import{toast as a}from"./toast-CRdbcLoc.js";import"./apex-kb-DYaZzirt.js";import"./credential-patterns-CLzI061R.js";import"./multi-source-analyze-CO8Xdi3o.js";import"./haptic-CQFg2PXZ.js";const x="kdmc_admin";function u(){return l.get("user")?.id===x}async function p(e){if(!e)return;if(!u()){e.innerHTML=`
    ...
    `;return}const o=await t.status(),r=o.configured?await t.listDevices():[];e.innerHTML=g(o,r),f(e)}function g(e,o){return`
  ...
  `}function b(e){if(e.length===0)return`
    ...
    `;const o=e.map(r=>`
        <div style="display:flex;align-items:center;gap:10px;padding:10px;background:rgba(0,0,0,0.25);border-radius:8px;margin-bottom:6px;flex-wrap:wrap">
          <span style="font-size:18px">${m(r.type)}</span>
          <div style="flex:1;min-width:140px">
            <div style="font-weight:600">${i(r.name)}</div>
            <div style="font-size:11px;color:#888">${i(r.type)} · <code>${i(r.mac)}</code></div>
          </div>
  ...
  `}function m(e){switch(e){case"rm":return"📺";case"sp":return"🔌";case"mp1":return"🔋";case"a1":return"🌡";default:return"📦"}}function f(e){e.querySelector("#ax-bl-login-btn")?.addEventListener("click",()=>{const o=e.querySelector("#ax-bl-email")?.value.trim()??"",r=e.querySelector("#ax-bl-password")?.value??"";if(!o||!r){a.warn("Email et mot de passe requis");return}(async()=>{a.info("🔄 Connexion à Broadlink...");const n=await t.login(o,r);n.ok?(a.success("✅ Connecté à Broadlink"),await p(e)):a.error(`❌ ${n.error??"login échoué"}`)})()}),e.querySelector("#ax-bl-reset")?.addEventListener("click",()=>{confirm("Déconnecter le compte Broadlink ? Devices et token seront supprimés.")&&(async()=>(await t.reset(),a.success("Compte Broadlink déconnecté"),await p(e)))()});for(const o of["#ax-bl-refresh","#ax-bl-refresh-empty"])e.querySelector(o)?.addEventListener("click",()=>{(async()=>{a.info("🔄 Refresh devices...");const r=await t.listDevices(!0);a.success(`${r.length} device(s) trouvé(s)`),await p(e)})()});e.querySelectorAll(".ax-bl-test").forEach(o=>{o.addEventListener("click",()=>{const r=o.dataset.deviceId??"";r&&(async()=>{const n=await t.getLearnedCodes(r);if(n.length===0){a.warn("Aucun code IR appris pour ce device. Apprends-en via app Broadlink → ils s'importent ici.");return}const s=n[0];if(!s)return;a.info(`Test envoi : ${s.name}`);const d=await t.sendIR(r,s.ir_hex);d.ok?a.success(`✅ ${s.name} envoyé`):a.error(`❌ ${d.error??"envoi échoué"}`)})()})}),e.querySelector("#ax-bl-proxy-save")?.addEventListener("click",()=>{const o=e.querySelector("#ax-bl-proxy-url")?.value.trim()??"";if(!o){try{localStorage.removeItem("ax_broadlink_proxy_url")}catch{}a.success("Proxy retiré");return}if(!/^https?:\/\//i.test(o)){a.warn("URL doit commencer par https://");return}try{localStorage.setItem("ax_broadlink_proxy_url",o)}catch{}a.success("Proxy sauvegardé")}),c.info("broadlink-setup","view rendered")}function C(){}export{C as dispose,p as render};
