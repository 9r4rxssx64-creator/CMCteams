import{e as s}from"./escape-html-BlQj2yEF.js";import{s as x,l as m}from"./monitoring-D2lWYrYo.js";import{p as f}from"./apex-tools-dispatch-skills-DOw4cI4G.js";import{mcpRegistry as d}from"./mcp-registry-DcrU0llM.js";import{skillsWatch as b}from"./skills-watch-B8SYINrR.js";import{toast as o}from"./toast-CRdbcLoc.js";import"./multi-source-analyze-Bg1HHfSC.js";import"./apex-kb-D1VtWFD9.js";import"./credential-patterns-CLzI061R.js";import"./haptic-CQFg2PXZ.js";function g(t){const l={alive:{color:"#10b981",emoji:"🟢"},dead:{color:"#ef4444",emoji:"🔴"},unknown:{color:"#94a3b8",emoji:"⚪"}},a=l[t]??l.unknown;return`<span style="color:${a.color};font-weight:600">${a.emoji} ${s(t)}</span>`}function c(t){if(!(x.get("isAdmin")===!0)){t.innerHTML='<div class="ax-gs-37">🔒 Réservé admin Kevin</div>';return}d.init();const a=d.list(),p=b.getLastReport("mcp-health-watch"),u=a.map(e=>{const r=e.toolsExposed?.length??0,i=e.lastCheck?new Date(e.lastCheck).toLocaleString("fr-FR"):"jamais";return`
  ...
  `,t.querySelectorAll("[data-mcp-test]").forEach(e=>{e.addEventListener("click",async()=>{const r=e.getAttribute("data-mcp-test")??"";o.info(`Test MCP ${r}...`);try{const i=await f.healthCheck(r);i.alive?o.success(`✅ ${r} alive (${i.latencyMs}ms)`):o.error(`🔴 ${r} dead`),c(t)}catch(i){o.error(`Erreur test ${r}`),m.warn("mcp.test","failed",{err:i})}})}),t.querySelectorAll("[data-mcp-discover]").forEach(e=>{e.addEventListener("click",async()=>{const r=e.getAttribute("data-mcp-discover")??"";o.info(`Discovery ${r}...`);try{await d.discoverTools(r),o.success(`✅ Tools discovered for ${r}`),c(t)}catch(i){o.error(`Erreur discovery ${r}`),m.warn("mcp.discover","failed",{err:i})}})}),t.querySelectorAll("[data-mcp-remove]").forEach(e=>{e.addEventListener("click",async()=>{const r=e.getAttribute("data-mcp-remove")??"";confirm(`Retirer le server MCP "${r}" ?`)&&(await d.unregister(r),o.success(`✅ ${r} retiré`),c(t))})}),t.querySelector("#mcp-new-submit")?.addEventListener("click",async()=>{const e=t.querySelector("#mcp-new-id")?.value?.trim()??"",r=t.querySelector("#mcp-new-name")?.value?.trim()??"",i=t.querySelector("#mcp-new-url")?.value?.trim()??"",n=t.querySelector("#mcp-new-token-key")?.value?.trim()??"";if(!e||!r||!i){o.error("ID, nom et URL obligatoires");return}if(!/^[a-z][a-z0-9-]+$/.test(e)){o.error("ID doit être kebab-case");return}if(!i.startsWith("https://")){o.error("URL doit commencer par https://");return}await d.register({id:e,name:r,url:i,...n?{tokenKey:n}:{}})?(o.success(`✅ ${e} ajouté`),c(t)):o.error(`❌ ${e} existe déjà`)})}function M(){}export{M as dispose,c as render};
