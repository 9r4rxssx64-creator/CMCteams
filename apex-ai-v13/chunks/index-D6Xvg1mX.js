const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./vault-firebase-backup-BplfPBrQ.js","./apex-kb-BqcSa9F9.js","./monitoring-3uBGKGRH.js","./credential-patterns-Dy6Wjk7e.js","./multi-source-analyze-DlejcKB6.js"])))=>i.map(i=>d[i]);
import{v as $,_ as M}from"./apex-kb-BqcSa9F9.js";import{c as H}from"./listener-cleanup-Y2rGGxxX.js";import{l as v}from"./monitoring-3uBGKGRH.js";import{s as L,a as O}from"../core/main-CAMPL6v5.js";import{autoDiscoverLinks as U}from"./auto-discover-links-BuW2N94d.js";import{c as P}from"./csp-style-helper-BisGRi53.js";import{detectCredential as A,CREDENTIAL_PATTERNS as K}from"./credential-patterns-Dy6Wjk7e.js";import{g as N}from"./apex-tools-dispatch-u3dZ7ong.js";import{l as I}from"./multi-source-analyze-DlejcKB6.js";import{multiKeyVault as y}from"./multi-key-vault-CxkmzyiZ.js";import{haptic as c}from"./haptic-CQFg2PXZ.js";import{toast as l}from"./toast-ClsF1KRZ.js";import"./apex-tools-registry-1KMfZuwO.js";import"./voice-BpgizsmO.js";let n=null;function he(){n?.cleanup(),n=null}function d(e){return e.replace(/[&<>"']/g,a=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[a]??a)}const w=[{id:"ai",label:"🤖 IA & LLM",serviceMatchers:["anthropic","openai","groq","google","gemini","openrouter","cohere","mistral","perplexity","deepseek","xai","elevenlabs","replicate","huggingface","fireworks","togetherai","deepl"],patternCategories:["ai"]},{id:"finance",label:"💳 Paiements & Finance",serviceMatchers:["stripe","paypal","revolut","wise","lydia","n26","boursorama","fortuneo","ing","socgen","bnp","credit_agricole","credit_mutuel","banque_postale","lbp","bpce","shopify"],patternCategories:["finance"]},{id:"devops",label:"🛠 DevOps & Code",serviceMatchers:["github","gitlab","cloudflare","vercel","netlify","railway","aws","heroku","sentry","npm"],patternCategories:["devops"]},{id:"comms",label:"📨 Communications",serviceMatchers:["telegram","discord","slack","brevo","resend","twilio","sendgrid","mailchimp","whatsapp"],patternCategories:["comms"]},{id:"social",label:"🌐 Réseaux sociaux",serviceMatchers:["facebook","instagram","tiktok","youtube","twitter","linkedin"],patternCategories:[]},{id:"storage",label:"☁️ Stockage & Cloud",serviceMatchers:["firebase","supabase","airtable","notion","dropbox","pinecone","weaviate"],patternCategories:["storage"]},{id:"ecommerce",label:"🛒 E-commerce",serviceMatchers:["shopify","stripe_connect","paypal_business"],patternCategories:[]},{id:"crypto",label:"₿ Crypto",serviceMatchers:["coinbase","binance","crypto_com","kraken"],patternCategories:[]},{id:"identity",label:"🆔 Identité Kevin",serviceMatchers:["kevin","iban","siret","vat","bic","apple","microsoft"],patternCategories:["identity"]},{id:"other",label:"📦 Autres",serviceMatchers:[],patternCategories:["saas"]}];function R(e,a){const t=e.toLowerCase();let r=null;for(const i of w)if(i.id!=="other")for(const o of i.serviceMatchers)t.includes(o)&&(!r||o.length>r.matchLen)&&(r={catId:i.id,matchLen:o.length});if(r)return r.catId;if(a){for(const i of w)if(i.patternCategories.includes(a))return i.id}return"other"}function V(){return K.filter(e=>e.category!=="forbidden").map(e=>{const a=$.getKeyStatus(e.storageKey),t=(()=>{try{return localStorage.getItem(e.storageKey)??""}catch{return""}})(),r=t&&t.length>8&&!t.startsWith("AXENC1:")?$.maskKey(t):t.startsWith("AXENC1:")?"🔒 chiffré":"";return{pattern:e,status:a,masked:r}})}function ke(e,a){return e.filter(t=>{if(a.category&&t.pattern.category!==a.category||a.configuredOnly&&t.status==="empty")return!1;if(a.query){const r=a.query.toLowerCase();if(!(t.pattern.name.toLowerCase().includes(r)||t.pattern.storageKey.toLowerCase().includes(r)))return!1}return!0})}function C(){const e=[];let a=[];try{a=y.listAll(!0)}catch(t){v.warn("feature-vault","multiKeyVault.listAll failed",{err:t})}for(const t of a){const r=I.get(t.service),i=K.find(p=>p.storageKey.includes(t.service)),o={id:t.id,service:t.service,serviceName:r?.name??J(t.service),category:R(t.service,i?.category),status:t.status,source:"multi-key"};t.alias!==void 0&&(o.alias=t.alias),t.addedAt!==void 0&&(o.addedAt=t.addedAt),t.lastTestedAt!==void 0&&(o.lastTestedAt=t.lastTestedAt);const g=I.getRechargeLink(t.service);g&&(o.rechargeUrl=g),e.push(o)}return e}function B(){const e=C(),a={total:e.length,active:0,failing:0,invalid:0};for(const t of e)t.status==="active"?a.active+=1:t.status==="failing"||t.status==="rate-limited"?a.failing+=1:t.status==="invalid"&&(a.invalid+=1);return a}function Y(e,a=""){const t=C(),r=a.trim().toLowerCase();return t.filter(i=>i.category!==e.id?!1:r?i.service.toLowerCase().includes(r)||i.serviceName.toLowerCase().includes(r)||(i.alias?.toLowerCase().includes(r)??!1):!0)}function J(e){return e.charAt(0).toUpperCase()+e.slice(1)}async function W(e){const a=e.trim();if(!a)return{ok:!1,reason:"Entrée vide"};const t=A(a);if(!t)return{ok:!1,reason:"Aucun pattern reconnu"};if(t.category==="forbidden")return{ok:!1,reason:"🚨 Type interdit (cartes/seed phrases jamais stockées)"};try{const r=await $.encryptAuto(a);return localStorage.setItem(t.storageKey,r),{ok:!0,pattern_name:t.name,storage_key:t.storageKey}}catch(r){return v.warn("vault-feature","autoDetectAndStore failed",{err:r}),{ok:!1,reason:"Erreur chiffrement"}}}function we(e){try{return localStorage.removeItem(e),!0}catch(a){return v.warn("vault-feature","remove failed",{err:a}),!1}}function X(e){const a={exported_at:new Date().toISOString(),version:1,entries:e.filter(t=>t.status!=="empty").map(t=>{const r=(()=>{try{return localStorage.getItem(t.pattern.storageKey)??""}catch{return""}})();return{storage_key:t.pattern.storageKey,name:t.pattern.name,value_encrypted:r}})};return JSON.stringify(a,null,2)}function G(e){const a=Q[e.status]??"#888",t=Z[e.status]??"⚪",r=(e.preview??"").slice(0,4)+"••••••"+(e.preview??"").slice(-4),i=e.preview?r:"••••••",o=e.rechargeUrl??"",g=e.alias?`<span style="color:#888;font-size:11px">— ${d(e.alias)}</span>`:"",p=e.logoUrl?`<img src="${d(e.logoUrl)}" alt="" loading="lazy" decoding="async" style="width:24px;height:24px;border-radius:6px" onerror="this.style.display='none'">`:"",f=[];e.addedAt&&f.push(`Ajouté ${j(e.addedAt)}`),e.lastTestedAt&&f.push(`Testé ${j(e.lastTestedAt)}`);const s=f.length>0?`<div style="display:flex;gap:8px;font-size:11px;color:#888;margin-bottom:10px">${f.map(u=>`<span>${d(u)}</span>`).join("")}</div>`:"";return`
  ...
  `}const Q={active:"#22cc77",failing:"#ffaa00","rate-limited":"#ffaa00",invalid:"#ff5b5b",unknown:"#888"},Z={active:"🟢",failing:"🟡","rate-limited":"🟡",invalid:"🔴",unknown:"⚪"};function j(e){const a=Date.now()-e;if(a<0||!Number.isFinite(a))return"à l'instant";const t=Math.floor(a/6e4);if(t<1)return"à l'instant";if(t<60)return`il y a ${t}min`;const r=Math.floor(t/60);if(r<24)return`il y a ${r}h`;const i=Math.floor(r/24);return i<30?`il y a ${i}j`:`il y a ${Math.floor(i/30)} mois`}let z="";function m(e){if(n?.cleanup(),n=H("vault"),!L.get("isAdmin")){e.innerHTML=`<div style="padding:40px;text-align:center"><h2 style="color:#c9a227">🔒 Coffre admin</h2><p style="color:#a0a4c0">Cette section est réservée à l'admin Kevin.</p></div>`;return}const t=L.get("user")?.id??"anon";if(!N("admin.vault",e,t))return;const r=B();e.innerHTML=P.withNonce(`
    <style>
    ...
    <div class="ax-vault-page" style="padding:env(safe-area-inset-top,16px) 16px calc(env(safe-area-inset-bottom,16px) + 96px);max-width:1140px;margin:0 auto;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif">
      ...
      <div class="ax-vault-sticky-wrap">
        ...
        <div class="ax-vault-search-row" style="padding-bottom:12px;transition:padding 200ms ease">
          <input type="text" id="ax-vault-search" aria-label="Chercher un service dans le coffre" value="${d(z)}" placeholder="🔍 Chercher un service..."
  ...
  `),_(e),ee(e),E(e),v.info("feature-vault",`rendered (${r.total} entries)`)}function E(e){const a=e.querySelector(".ax-vault-page"),t=e.querySelector("#ax-vault-fab");if(!a)return;let r=0,i=0;const o=()=>{i||(i=requestAnimationFrame(()=>{i=0;const g=window.scrollY||document.documentElement.scrollTop||0;g!==r&&(r=g,g>80?a.classList.add("ax-vault-scrolled"):a.classList.remove("ax-vault-scrolled"))}))};n?n.bind(window,"scroll",o,{passive:!0}):window.addEventListener("scroll",o,{passive:!0}),o(),t&&n&&n.bind(t,"click",()=>{c.tap(),e.querySelector("#ax-vault-test-all")?.click()})}function _(e){const a=e.querySelector("#ax-vault-categories");if(!a)return;if(B().total===0&&!a.dataset.axInitialized){a.dataset.axInitialized="1";const r=O(a,"vault-cards");setTimeout(()=>{r(),_(e)},250);return}a.innerHTML=w.map(r=>{const i=Y(r,z);if(i.length===0&&r.id!=="identity")return"";const o=i.length>0;return`
    ...
    `}).join("")}function ee(e){const a=e.querySelector("#ax-vault-search");if(a){let s=null;n.bind(a,"input",()=>{s&&clearTimeout(s),s=setTimeout(()=>{z=a.value.trim(),_(e),F(e)},240)})}const t=e.querySelector("#ax-vault-add-manual");t&&n&&n.bind(t,"click",()=>{c.tap(),D(e)});const r=e.querySelector("#ax-vault-test-all");r&&n&&n.bind(r,"click",()=>{(async()=>{c.tap(),l.info("Test de toutes les clés en cours…");try{const s=await y.healthCheckAll();l.success(`✅ ${s.tested} testées · ${s.recovered} récupérées · ${s.stillDown} HS`),m(e)}catch(s){v.warn("feature-vault","testAll failed",{err:s}),l.error("Erreur pendant le test global")}})()});const i=e.querySelector("#ax-vault-rescue-fb");i&&n&&n.bind(i,"click",()=>{(async()=>{c.tap();const s=e.querySelector("#ax-vault-rescue-result");s&&(s.innerHTML="⏳ Lecture Firebase backup chiffré…");try{const{vaultFirebaseBackup:u}=await M(async()=>{const{vaultFirebaseBackup:b}=await import("./vault-firebase-backup-BplfPBrQ.js");return{vaultFirebaseBackup:b}},__vite__mapDeps([0,1,2,3,4]),import.meta.url),x=await u.restoreAllFromFirebaseBackup();s&&(s.innerHTML=`<div style="padding:8px;background:rgba(34,204,119,.1);color:#22cc77;border-radius:8px">🔓 ${x.restored} clés restaurées · ${x.failed} échouées · ${x.skipped} ignorées</div>`),x.restored>0?(l.success(`🔓 ${x.restored} clés restaurées depuis Firebase backup`),c.success(),setTimeout(()=>m(e),600)):l.info("Aucune clé trouvée dans Firebase backup")}catch(u){v.warn("feature-vault","rescueFb failed",{err:u}),s&&(s.innerHTML=`<div style="padding:8px;background:rgba(255,88,88,.1);color:#ff5858;border-radius:8px">⚠ ${d(String(u).slice(0,120))}</div>`),l.error("Erreur lecture Firebase backup"),c.error()}})()});const o=e.querySelector("#ax-vault-rescue-all");o&&n&&n.bind(o,"click",()=>{(async()=>{c.tap();const s=e.querySelector("#ax-vault-rescue-result");s&&(s.innerHTML="⏳ Scan 4 sources : alias, IDB, Firebase, pattern…");try{const{autoRestoreCredentials:u}=await M(async()=>{const{autoRestoreCredentials:b}=await import("./auto-restore-credentials-xPo8ipdt.js");return{autoRestoreCredentials:b}},__vite__mapDeps([1,2,3,4]),import.meta.url),x=await u.restoreAutomatically();s&&(s.innerHTML=`<div style="padding:8px;background:rgba(34,204,119,.1);color:#22cc77;border-radius:8px">🔓 ${x.restored} restaurées · ${x.failed} échouées</div>`),x.restored>0?(l.success(`🔓 ${x.restored} clés restaurées (4 sources)`),c.success(),setTimeout(()=>m(e),600)):l.info("Aucune clé trouvable dans les 4 sources. Colle une clé manuellement ci-dessous.")}catch(u){v.warn("feature-vault","rescueAll failed",{err:u}),s&&(s.innerHTML=`<div style="padding:8px;background:rgba(255,88,88,.1);color:#ff5858;border-radius:8px">⚠ ${d(String(u).slice(0,120))}</div>`),l.error("Erreur scan multi-sources"),c.error()}})()});const g=e.querySelector("#ax-vault-cleanup-invalid");g&&n&&n.bind(g,"click",()=>{(async()=>{c.tap();const s=e.querySelector("#ax-vault-rescue-result");if(confirm(`Supprimer définitivement toutes les clés illisibles (decrypt fail) ?

Ces clés sont chiffrées avec une passphrase perdue. Tu devras les recoller pour les retrouver.`)){s&&(s.innerHTML="⏳ Suppression des entrées illisibles…");try{const x=C().filter(h=>h.status==="invalid");let b=0;for(const h of x)try{const q=h.id.startsWith("ax_")||h.id.startsWith("apex_v13_")?h.id:`ax_${h.service}_key`;localStorage.removeItem(q);const S=indexedDB.open("apex_v13_vault_shadow",1);S.onsuccess=()=>{try{S.result.transaction("keys","readwrite").objectStore("keys").delete(q),S.result.close()}catch{}},b++}catch{}s&&(s.innerHTML=`<div style="padding:8px;background:rgba(34,204,119,.1);color:#22cc77;border-radius:8px">🗑 ${b} clé(s) illisibles supprimées. Recolle tes clés via "Détecter & stocker" ci-dessous.</div>`),l.success(`🗑 ${b} clés illisibles supprimées`),c.success(),setTimeout(()=>m(e),800)}catch(u){v.warn("feature-vault","cleanupInvalid failed",{err:u}),l.error("Erreur suppression"),c.error()}}})()});const p=e.querySelector("#ax-vault-paste-btn");p&&n&&n.bind(p,"click",()=>{(async()=>{c.tap();const s=e.querySelector("#ax-vault-paste"),u=e.querySelector("#ax-vault-paste-result");if(!s||!u)return;const x=await W(s.value);if(x.ok){c.success(),l.success(`✅ ${x.pattern_name} stocké`),u.innerHTML=`<div style="padding:8px;background:rgba(34,204,119,.1);color:#22cc77;border-radius:8px">✅ ${d(x.pattern_name)} → ${d(x.storage_key)}</div>`,s.value="";const b=A(s.value.trim());if(b){const h=b.storageKey.replace(/^(ax_|apex_v13_)/,"").replace(/_(?:key|token|pat|sk|pk|id|secret)$/,"");try{await y.addKey(h,s.value.trim())}catch{}}m(e)}else c.error(),l.error(x.reason),u.innerHTML=`<div style="padding:8px;background:rgba(255,88,88,.1);color:#ff5858;border-radius:8px">⚠ ${d(x.reason)}</div>`})()});const f=e.querySelector("#ax-vault-export");f&&n&&n.bind(f,"click",()=>{c.tap();const s=X(V()),u=new Blob([s],{type:"application/json"}),x=URL.createObjectURL(u),b=document.createElement("a");b.href=x,b.download=`apex-vault-${new Date().toISOString().slice(0,10)}.json`,document.body.appendChild(b),b.click(),document.body.removeChild(b),URL.revokeObjectURL(x),l.success("Coffre exporté (chiffré)")}),F(e)}function F(e){e.querySelectorAll('[data-action="test"]').forEach(a=>{n.bind(a,"click",t=>{t.stopPropagation();const r=a.dataset.credId??"";te(e,r,a)})}),e.querySelectorAll('[data-action="recharge"]').forEach(a=>{n.bind(a,"click",t=>{t.stopPropagation();const r=a.dataset.rechargeUrl??"",i=a.dataset.service??"";ae(r,i)})}),e.querySelectorAll('[data-action="discover-links"]').forEach(a=>{n.bind(a,"click",t=>{t.stopPropagation();const r=a.dataset.service??"";re(e,r,a)})}),e.querySelectorAll('[data-action="edit"]').forEach(a=>{n.bind(a,"click",t=>{t.stopPropagation();const r=a.dataset.credId??"";oe(e,r)})}),e.querySelectorAll('[data-action="delete"]').forEach(a=>{n.bind(a,"click",t=>{t.stopPropagation();const r=a.dataset.credId??"";ie(e,r)})}),e.querySelectorAll('[data-action="add-to-cat"]').forEach(a=>{n.bind(a,"click",t=>{t.stopPropagation();const r=a.dataset.catId??"";D(e,r)})})}async function te(e,a,t){if(!a)return;c.tap();const r=t.textContent;t.textContent="⏳ Test…",t.setAttribute("disabled","true");try{const i=await y.testKey(a);i.ok?(c.success(),l.success(`✅ Active (${i.latencyMs}ms)`)):(c.error(),l.error(`❌ ${i.reason??"Test échoué"}`)),m(e)}catch(i){v.warn("feature-vault","testKey failed",{err:i}),c.error(),l.error("Erreur pendant le test"),t.textContent=r,t.removeAttribute("disabled")}}function ae(e,a){if(c.tap(),!e){l.warn(`Aucune page recharge connue pour ${a}`);return}try{window.open(e,"_blank","noopener,noreferrer")}catch(t){v.warn("feature-vault","recharge open failed",{err:t}),l.error("Impossible d'ouvrir le lien")}}async function re(e,a,t){if(!a)return;c.tap();const r=t.textContent;t.textContent="⏳ Recherche…",t.setAttribute("disabled","true");try{const i=await U.discover(a,{force:!0}),o=[];i.login&&o.push("login"),i.dashboard&&o.push("dashboard"),i.billing&&o.push("billing"),i.api_keys&&o.push("api_keys"),i.usage&&o.push("usage"),i.docs&&o.push("docs"),i.password_reset&&o.push("reset_pw"),i.account_settings&&o.push("settings"),i.support&&o.push("support"),i.status_page&&o.push("status"),i.alive&&o.length>0?(c.success(),l.success(`🔗 ${o.length} liens trouvés (${i.source}) : ${o.join(", ")}`)):(c.error(),l.warn(`Aucun lien validé pour ${a} — réessaie plus tard`)),m(e)}catch(i){v.warn("feature-vault","discoverLinks failed",{err:i}),c.error(),l.error("Erreur pendant la recherche de liens")}finally{t.textContent=r,t.removeAttribute("disabled")}}function ie(e,a){if(a&&(c.tap(),!!window.confirm("Supprimer cette clé définitivement ? Elle sera retirée du Coffre + ne sera plus restaurée auto.")))try{y.removeKey(a),c.success(),l.success("Clé supprimée définitivement ✓"),m(e)}catch(t){v.warn("feature-vault","delete failed",{err:t}),c.error(),l.error("Suppression échouée")}}function T(e){let a=e.querySelector("#ax-vault-modal-root");return a||(a=document.createElement("div"),a.id="ax-vault-modal-root",e.appendChild(a)),a}function k(e){const a=T(e);a.innerHTML=""}function D(e,a){const t=T(e),r=w.filter(p=>p.id!=="other").map(p=>`<option value="${d(p.id)}" ${a===p.id?"selected":""}>${d(p.label)}</option>`).join("");t.innerHTML=`
  ...
  `,(()=>{const p=t.querySelector("#ax-vault-modal-close");p&&n&&n.bind(p,"click",()=>k(e))})();const i=t.querySelector('[role="dialog"]');i&&n&&n.bind(i,"click",p=>{p.target===i&&k(e)});const o=t.querySelector("#ax-vault-add-detect");o&&n&&n.bind(o,"click",()=>{(async()=>{c.tap();const p=t.querySelector("#ax-vault-add-value");if(!p)return;const f=A(p.value.trim());if(!f){l.warn("Aucun pattern reconnu");return}if(f.category==="forbidden"){l.error("🚨 Type interdit");return}const s=t.querySelector("#ax-vault-add-service"),u=t.querySelector("#ax-vault-add-cat");if(s){const x=f.storageKey.replace(/^(ax_|apex_v13_)/,"").replace(/_(?:key|token|pat|sk|pk|id|secret)$/,"");s.value=x}u&&(u.value=R(s?.value??"",f.category)),l.success(`Détecté: ${f.name}`)})()});const g=t.querySelector("#ax-vault-add-save");g&&n&&n.bind(g,"click",()=>{(async()=>{c.tap();const p=t.querySelector("#ax-vault-add-service")?.value.trim()??"",f=t.querySelector("#ax-vault-add-alias")?.value.trim()??"",s=t.querySelector("#ax-vault-add-value")?.value.trim()??"";if(!p||!s){l.warn("Service et valeur requis");return}try{const u={};f&&(u.alias=f),await y.addKey(p,s,u),l.success(`✅ Clé ${p} chiffrée + sauvegardée`),k(e),m(e)}catch(u){v.warn("feature-vault","add manual failed",{err:u}),l.error("Erreur pendant la sauvegarde")}})()})}function oe(e,a){const t=T(e),r=y.listAll(!0).find(o=>o.id===a);if(!r){l.error("Clé introuvable");return}t.innerHTML=`
  ...
  `,(()=>{const o=t.querySelector("#ax-vault-modal-close");o&&n&&n.bind(o,"click",()=>k(e))})(),(()=>{const o=t.querySelector("#ax-vault-edit-cancel");o&&n&&n.bind(o,"click",()=>k(e))})();const i=t.querySelector("#ax-vault-edit-save");i&&n&&n.bind(i,"click",()=>{(async()=>{c.tap();const o=t.querySelector("#ax-vault-edit-value")?.value.trim()??"",g=t.querySelector("#ax-vault-edit-alias")?.value.trim()??"";if(!o){l.warn("Valeur requise");return}try{y.markInvalid(a,"replaced via edit");const p={};g&&(p.alias=g),await y.addKey(r.service,o,p),l.success("✅ Clé mise à jour"),k(e),m(e)}catch(p){v.warn("feature-vault","edit save failed",{err:p}),l.error("Erreur pendant la modification")}})()})}export{w as CATEGORIES,W as autoDetectAndStore,C as buildCredentialDisplays,R as classifyService,B as computeStats,he as dispose,d as escapeHtml,X as exportVaultJson,ke as filterVaultEntries,j as formatRelativeTime,Y as getCredentialsForCategory,V as listVaultEntries,we as removeCredential,m as render,G as renderCredentialCard};
