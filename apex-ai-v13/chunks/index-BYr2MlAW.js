import{l as b}from"./monitoring-BAiQJoxJ.js";import{c as v}from"./listener-cleanup-Y2rGGxxX.js";import{s as m,r as y}from"../core/main-C9h0PGqO.js";import{h as d}from"./haptic-BUEqXK0N.js";import{toast as c}from"./toast-Dgg9rcIP.js";import"./apex-kb-BrxoW6ip.js";import"./credential-patterns-Dy6Wjk7e.js";import"./multi-source-analyze-BefgB4O2.js";let r=null,i=1;const x=5,a={name:"",permissionsRequested:!1,capabilities:new Set(["chat","voice","studios"]),mode:"both",done:!1},h=[{id:"chat",label:"💬 Chat IA",desc:"Conversation Claude"},{id:"voice",label:"🎙 Voix",desc:"Dictée + lecture"},{id:"studios",label:"🎨 Studios",desc:"Music/Video/Logo"},{id:"browser",label:"🌐 Browser",desc:"Navigation embed"},{id:"pro",label:"💼 Pro modules",desc:"Cuisine/Médical/Légal"},{id:"crypto",label:"🪙 Crypto",desc:"Wallets + tracking"},{id:"domotique",label:"🏠 Domotique",desc:"Maison connectée"},{id:"remote",label:"📺 Remote TV",desc:"Télécommande univ."}];function l(e){return e.replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]??t)}function B(){r?.cleanup(),r=null}function L(e){if(!e||e==="kdmc_admin")return!1;try{return localStorage.getItem(`apex_v13_onboarding_done_${e}`)!=="true"}catch{return!1}}function z(e){try{localStorage.setItem(`apex_v13_onboarding_done_${e}`,"true"),a.done=!0,b.info("onboarding",`Marked done for uid=${e}`)}catch{}}async function N(e){r?.cleanup(),r=v("onboarding");const t=m.get("user");t?.name&&(a.name=t.name),p(e)}function p(e){const t=`
  ...
  `;e.innerHTML=t,A(e,i)}function k(){return`
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px">
      <div style="display:flex;gap:6px">
        ${Array.from({length:x},(e,t)=>`
          <div style="width:32px;height:4px;border-radius:2px;background:${t+1<=i?"#c9a227":"rgba(255,255,255,0.15)"};transition:background 0.3s"></div>
        `).join("")}
      </div>
  ...
  `}function S(e){switch(e){case 1:return`
        ...
        <label style="display:block;margin-bottom:16px">
          ...
          <input type="text" id="ax-onboarding-name" value="${l(a.name)}" placeholder="Ton prénom" style="width:100%;padding:12px;background:rgba(0,0,0,0.3);border:1px solid rgba(201,162,39,0.3);border-radius:10px;color:#fff;font-size:15px">
        </label>
  ...
  `}function A(e,t){if(!r)return;const s=e.querySelector("#ax-onboarding-skip");s&&r.bind(s,"click",()=>{d.tap(),f()});const u=e.querySelector("#ax-onboarding-next");u&&r.bind(u,"click",()=>{d.tap(),q(e)});const g=e.querySelector("#ax-onboarding-prev");if(g&&r.bind(g,"click",()=>{d.tap(),i=Math.max(1,i-1),p(e)}),t===1){const o=e.querySelector("#ax-onboarding-name");o&&r.bind(o,"input",()=>{a.name=o.value.trim()})}else if(t===2){const o=e.querySelector("#ax-onboarding-grant-perms");o&&r.bind(o,"click",()=>{T()})}else t===3?e.querySelectorAll(".ax-onboarding-cap").forEach(o=>{r.bind(o,"click",()=>{const n=o.dataset.cap;n&&(a.capabilities.has(n)?a.capabilities.delete(n):a.capabilities.add(n),d.tap(),p(e))})}):t===4&&e.querySelectorAll(".ax-mode-choice").forEach(o=>{r.bind(o,"click",()=>{const n=o.dataset.mode;n&&(a.mode=n,d.tap(),p(e))})})}function q(e){if(i===1){if(!a.name||a.name.length<2){c.warn("Tape ton prénom (min 2 caractères)");return}try{localStorage.setItem("apex_v13_user_name",a.name)}catch{}}else if(i===3)try{localStorage.setItem("apex_v13_capabilities",JSON.stringify([...a.capabilities]))}catch{}else if(i===4)try{localStorage.setItem("ax_mode_dual",a.mode)}catch{}if(i===x){f();return}i++,p(e)}async function T(){a.permissionsRequested=!0;let e=0;if(typeof Notification<"u")try{await Notification.requestPermission()==="granted"&&e++}catch{}if(typeof navigator<"u"&&navigator.mediaDevices?.getUserMedia)try{(await navigator.mediaDevices.getUserMedia({audio:!0})).getTracks().forEach(s=>s.stop()),e++}catch{}if(typeof navigator<"u"&&navigator.mediaDevices?.getUserMedia)try{(await navigator.mediaDevices.getUserMedia({video:!0})).getTracks().forEach(s=>s.stop()),e++}catch{}e>0?c.success(`✅ ${e} permission${e>1?"s":""} accordée${e>1?"s":""}`):c.info("Tu pourras autoriser plus tard dans Réglages")}function f(){const e=m.get("user");e?.id&&z(e.id),c.success("🚀 Bienvenue dans Apex !"),setTimeout(()=>y.navigate("chat"),300)}export{B as dispose,L as isOnboardingNeeded,z as markOnboardingDone,N as render};
