...
import{_ as s}from"./apex-kb-D-Wwv6M3.js";import{l as f}from"./monitoring-3uBGKGRH.js";import{e as p}from"./html-safe-CCp1QaJu.js";import{s as w}from"../core/main-BmbDabbw.js";import{g as h}from"./apex-tools-dispatch-CmJZShwa.js";import"./credential-patterns-Dy6Wjk7e.js";import"./multi-source-analyze-Bai9IpOh.js";import"./apex-tools-registry-CXnGO-sA.js";import"./voice-mgMrh-FL.js";const y=[{id:"tv",name:"Télévision",emoji:"📺",category:"tv",capability:"ir",actions:[{id:"power",label:"⏻ Marche/Arrêt"},{id:"vol_up",label:"🔊 Volume +"},{id:"vol_down",label:"🔉 Volume -"},{id:"channel_up",label:"⬆️ Chaîne +"},{id:"channel_down",label:"⬇️ Chaîne -"},{id:"mute",label:"🔇 Muet"},{id:"source",label:"🔌 Source"}]},{id:"lights",name:"Lumières",emoji:"💡",category:"light",capability:"bluetooth",actions:[{id:"on",label:"⏻ Allumer"},{id:"off",label:"⏼ Éteindre"},{id:"dim_up",label:"☀ Plus fort"},{id:"dim_down",label:"🌙 Plus faible"},{id:"color",label:"🎨 Couleur"}]},{id:"speaker",name:"Enceintes",emoji:"🔊",category:"audio",capability:"bluetooth",actions:[{id:"play_pause",label:"⏯ Play/Pause"},{id:"next",label:"⏭ Suivant"},{id:"prev",label:"⏮ Précédent"},{id:"vol_up",label:"🔊 Volume +"},{id:"vol_down",label:"🔉 Volume -"},{id:"airplay",label:"📡 AirPlay"}]},{id:"thermo",name:"Thermostat",emoji:"🌡",category:"climate",capability:"wifi",actions:[{id:"heat_up",label:"🔥 Chauffer +"},{id:"heat_down",label:"❄️ Chauffer -"},{id:"mode",label:"🔄 Mode"},{id:"eco",label:"🌱 Éco"}]},{id:"camera",name:"Caméras",emoji:"📹",category:"camera",capability:"wifi",actions:[{id:"view",label:"👁 Voir live"},{id:"snapshot",label:"📸 Snapshot"},{id:"record",label:"⏺ Enregistrer"}]},{id:"shade",name:"Volets",emoji:"🪟",category:"shade",capability:"bluetooth",actions:[{id:"up",label:"⬆️ Monter"},{id:"down",label:"⬇️ Descendre"},{id:"stop",label:"⏹ Stop"}]},{id:"wifi",name:"Wi-Fi",emoji:"📶",category:"network",capability:"nfc",actions:[{id:"share_nfc",label:"📲 Partager via NFC"},{id:"qr",label:"🔲 Générer QR Wi-Fi"}]},{id:"ev",name:"Borne EV",emoji:"🔌",category:"ev",capability:"nfc",actions:[{id:"badge",label:"💳 Badge RFID"},{id:"start",label:"⚡ Démarrer charge"},{id:"stop",label:"⏹ Arrêter"}]}];async function F(i){const u=w.get("user")?.id??"anon";if(!h("module.remote",i,u))return;const{deviceControl:c}=await s(async()=>{const{deviceControl:e}=await import("./device-control-CaZvB_tg.js");return{deviceControl:e}},__vite__mapDeps([0,1,2]),import.meta.url),m=c.detectDevice(),b=c.listAllSupported(),o=b.includes("nfc"),d=b.includes("bluetooth"),g=b.includes("share");i.innerHTML=`
  ...
  `,i.querySelectorAll("[data-remote-device]").forEach(e=>{e.addEventListener("click",()=>{const a=e.dataset.remoteDevice,t=e.dataset.remoteAction;S(a??"",t??"")})}),i.querySelector("#ax-remote-scan-bt")?.addEventListener("click",()=>{(async()=>{const e=await c.requestBluetoothDevice([{services:["battery_service"]}]),{toast:a}=await s(async()=>{const{toast:t}=await import("./toast-Dgg9rcIP.js");return{toast:t}},[],import.meta.url);e.ok&&e.data?a.success(`🔵 Trouvé : ${e.data.name??e.data.id}`):a.warn(e.reason??"Bluetooth non disponible")})()}),i.querySelector("#ax-remote-scan-nfc")?.addEventListener("click",()=>{(async()=>{const{toast:e}=await s(async()=>{const{toast:t}=await import("./toast-Dgg9rcIP.js");return{toast:t}},[],import.meta.url),a=await c.requestNFCRead(t=>{const r=t.map(n=>{const l=n.data;return typeof l=="string"?l:JSON.stringify(l)}).join(" · ").slice(0,100);e.success(`📲 Tag lu : ${r}`)});a.ok?e.success("📲 Approche un tag NFC pour lire"):e.warn(a.reason??"NFC non disponible")})()}),i.querySelector("#ax-remote-write-nfc")?.addEventListener("click",()=>{(async()=>{const e=await c.requestNFCWrite([{recordType:"text",data:"Apex Remote — "+new Date().toISOString()}]),{toast:a}=await s(async()=>{const{toast:t}=await import("./toast-Dgg9rcIP.js");return{toast:t}},[],import.meta.url);e.ok?a.success("✍️ Approche le tag pour écrire"):a.warn(e.reason??"NFC write non disponible")})()}),i.querySelector("#ax-remote-vibrate")?.addEventListener("click",()=>{(async()=>{const e=await c.vibrate([100,30,100,30,200]),{toast:a}=await s(async()=>{const{toast:t}=await import("./toast-Dgg9rcIP.js");return{toast:t}},[],import.meta.url);e.ok?a.success("📳 Vibration envoyée"):a.warn(e.reason??"Vibration non disponible (iOS Safari)")})()}),i.querySelector("#ax-remote-photos")?.addEventListener("click",()=>{(async()=>{const e=await c.getPhotosFromGallery(),{toast:a}=await s(async()=>{const{toast:t}=await import("./toast-Dgg9rcIP.js");return{toast:t}},[],import.meta.url);e.ok&&e.data?a.success(`📸 ${e.data.length} photos sélectionnées (analyse EXIF en cours)`):a.warn(e.reason??"Sélection annulée")})()}),i.querySelector("#ax-remote-share")?.addEventListener("click",()=>{(async()=>{const e=await c.shareContent({title:"Apex AI v13",text:"Mon assistant intelligent personnel",url:location.origin+location.pathname}),{toast:a}=await s(async()=>{const{toast:t}=await import("./toast-Dgg9rcIP.js");return{toast:t}},[],import.meta.url);e.ok?a.success("📤 Partagé"):a.warn(e.reason??"Partage annulé")})()}),i.querySelector("#ax-remote-scan-lan")?.addEventListener("click",()=>{(async()=>{const{toast:e}=await s(async()=>{const{toast:n}=await import("./toast-Dgg9rcIP.js");return{toast:n}},[],import.meta.url),{networkScan:a}=await s(async()=>{const{networkScan:n}=await import("./network-scan-BxBx4fLL.js");return{networkScan:n}},__vite__mapDeps([0,1,2]),import.meta.url);e.info("🔍 Scan LAN en cours (peut prendre 30-60s)...");const t=await a.scan(),r=i.querySelector("#ax-remote-lan-results");if(r){if(!t.ok){r.innerHTML=`<p style="color:#ffaa00;font-size:13px">⚠️ ${p(t.reason??"Scan échoué")}</p>`;return}r.innerHTML=`
      ...
      `).join("")}})()});const v=async()=>{const{cardEmulator:e}=await s(async()=>{const{cardEmulator:r}=await import("./card-emulator-ZFEDvrfZ.js");return{cardEmulator:r}},__vite__mapDeps([1,0,2]),import.meta.url),a=e.getStatus(),t=i.querySelector("#ax-remote-emulator-status");if(t)if(a.connected){t.replaceChildren(),t.append("🟢 Connecté : ");const r=document.createElement("strong");r.textContent=String(a.device??""),t.appendChild(r),t.append(` (${String(a.connection??"")}) · ${Number(a.uptime_sec)||0}s`)}else t.textContent="⚪ Aucun émulateur connecté"};v();const x=(e,a)=>{i.querySelector(e)?.addEventListener("click",()=>{(async()=>{const{toast:t}=await s(async()=>{const{toast:l}=await import("./toast-Dgg9rcIP.js");return{toast:l}},[],import.meta.url),{cardEmulator:r}=await s(async()=>{const{cardEmulator:l}=await import("./card-emulator-ZFEDvrfZ.js");return{cardEmulator:l}},__vite__mapDeps([1,0,2]),import.meta.url),n=await r[a]();n.ok?(t.success("✅ Connecté"),await v()):t.warn(n.reason??"Connexion échouée")})()})};x("#ax-remote-flipper-usb","connectFlipperUSB"),x("#ax-remote-flipper-ble","connectFlipperBLE"),x("#ax-remote-proxmark","connectProxmarkSerial"),x("#ax-remote-chameleon","connectChameleonSerial"),f.info("feature-remote",`rendered ${y.length} device cards`)}async function S(i,u){const{deviceControl:c}=await s(async()=>{const{deviceControl:o}=await import("./device-control-CaZvB_tg.js");return{deviceControl:o}},__vite__mapDeps([0,1,2]),import.meta.url),{toast:m}=await s(async()=>{const{toast:o}=await import("./toast-Dgg9rcIP.js");return{toast:o}},[],import.meta.url),{auditLog:b}=await s(async()=>{const{auditLog:o}=await import("./apex-kb-D-Wwv6M3.js").then(d=>d.c);return{auditLog:o}},__vite__mapDeps([1,0,2]),import.meta.url);switch(b.record("remote.action",{details:{device:i,action:u}}),c.vibrate([30]),i){case"wifi":if(u==="qr"){m.info("🔲 Génération QR Wi-Fi → vue dédiée Sprint 4");return}if(u==="share_nfc"){const o=await c.requestNFCWrite([{recordType:"text",data:"WIFI:S:MyNetwork;T:WPA;P:password;;"}]);m[o.ok?"success":"warn"](o.ok?"✍️ Approche le tag NFC":o.reason??"NFC KO");return}break;case"speaker":if(u==="airplay"){const o=await c.openMusic("");m[o.ok?"success":"warn"](o.ok?"🎵 Apple Music ouvert":"iOS only");return}break;case"camera":if(u==="snapshot"){const o=await c.requestCamera({video:!0});if(o.ok&&o.data){const d=await c.takePhoto(o.data);m[d.ok?"success":"warn"](d.ok?"📸 Photo prise":d.reason??"KO")}return}break}m.info(`📡 ${i} · ${u} envoyé (bridge IR/BT à configurer Sprint 4)`)}export{F as render};
