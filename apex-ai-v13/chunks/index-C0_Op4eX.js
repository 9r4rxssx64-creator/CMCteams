...
import{_ as u,p as o,s as M}from"./monitoring-Cq_vIOf8.js";import{c as D}from"./listener-cleanup-Y2rGGxxX.js";import"./multi-source-analyze-CgbS5D-d.js";import"./credential-patterns-DUMYZEMu.js";import"./apex-kb-DY2-6tey.js";let i=null;function P(){i?.cleanup(),i=null}async function g(n){i?.cleanup(),i=D("sentinels");const{sentinels:x}=await u(async()=>{const{sentinels:t}=await import("./sentinels-Bec2c20E.js");return{sentinels:t}},__vite__mapDeps([0,1,2,3]),import.meta.url),{sentinelsRegistry:m,bootstrapSentinelsRegistry:f}=await u(async()=>{const{sentinelsRegistry:t,bootstrapSentinelsRegistry:a}=await import("./sentinels-registry-FAkFldDh.js");return{sentinelsRegistry:t,bootstrapSentinelsRegistry:a}},__vite__mapDeps([0,1,2,3]),import.meta.url);f();const e=x.list(),y=m.getStatus(),p=m.getMetrics(),v=e.filter(t=>t.lastResult?.ok).length,$=e.filter(t=>t.lastResult&&!t.lastResult.ok).length,R=e.filter(t=>!t.lastResult).length;n.innerHTML=`
