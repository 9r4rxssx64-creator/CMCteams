import{e as s}from"./escape-html-BlQj2yEF.js";import{l as c}from"./monitoring-3uBGKGRH.js";import{x as b}from"./apex-tools-dispatch-skills-CE_nvnLN.js";import{toast as o}from"./toast-CRdbcLoc.js";import"./apex-kb-BUdkXo7X.js";import"./voice-CKlB4PWs.js";import"./credential-patterns-CLzI061R.js";import"./haptic-CQFg2PXZ.js";function w(r){r.innerHTML=`
...
Courses,0,400</textarea>
  ...
  `,r.querySelector("#xlsx-generate")?.addEventListener("click",async()=>{const n=r.querySelector("#xlsx-filename")?.value||"tableau.xlsx",p=r.querySelector("#xlsx-sheetname")?.value||"Sheet1",d=r.querySelector("#xlsx-data")?.value??"",x=r.querySelector("#xlsx-freeze")?.checked??!0,a=d.split(`
`).map(e=>e.trim()).filter(Boolean).map(e=>e.split(",").map(t=>{const l=t.trim(),i=Number(l);return!isNaN(i)&&l!==""?i:l}));if(a.length===0){o.error("Aucune donnée à exporter");return}o.info("Génération en cours...");try{const e=await b.generate({filename:n,sheets:[{name:p,data:a,freezeHeader:x}]}),t=r.querySelector("#xlsx-result");if(!t)return;e.success?(t.innerHTML=`
          ...
          </div>`,o.success(`✅ ${e.filename}`)):(t.innerHTML=`<p style="color:#ef4444">❌ ${s(e.error??"Erreur")}</p>`,o.error(`❌ ${e.error??"Erreur"}`))}catch(e){c.warn("studio-xlsx","failed",{err:e}),o.error(`❌ ${e instanceof Error?e.message:"Erreur"}`)}})}export{w as render};
