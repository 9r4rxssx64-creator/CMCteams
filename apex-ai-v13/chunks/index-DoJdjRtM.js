const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./sentinels-CaJydNhM.js","../core/main-D4zwvifF.js","../assets/css/main-rhfGvOFL.css","./observability-BT73ufBP.js","./toast-64I4l5xU.js","./haptic-BUEqXK0N.js"])))=>i.map(i=>d[i]);
import{_ as i,l as g}from"../core/main-D4zwvifF.js";async function d(e){const{sentinels:s}=await i(async()=>{const{sentinels:t}=await import("./sentinels-CaJydNhM.js");return{sentinels:t}},__vite__mapDeps([0,1,2,3]),import.meta.url),n=s.list(),p=n.filter(t=>t.lastResult?.ok).length,x=n.filter(t=>t.lastResult&&!t.lastResult.ok).length,u=n.filter(t=>!t.lastResult).length;e.innerHTML=`
  ...
  `,e.querySelector("#ax-sent-run-all")?.addEventListener("click",()=>{(async()=>{const{toast:t}=await i(async()=>{const{toast:a}=await import("./toast-64I4l5xU.js");return{toast:a}},__vite__mapDeps([4,5]),import.meta.url);t.info(`Exécution de ${n.length} sentinelles...`),await Promise.all(n.map(a=>s.runOne(a.id))),t.success("✅ Tous les sentinelles re-exécutés"),await d(e)})()}),e.querySelector("#ax-sent-refresh")?.addEventListener("click",()=>{d(e)}),e.querySelectorAll(".ax-sent-run").forEach(t=>{t.addEventListener("click",()=>{(async()=>{const a=t.dataset.sentId;if(!a)return;const{toast:r}=await i(async()=>{const{toast:o}=await import("./toast-64I4l5xU.js");return{toast:o}},__vite__mapDeps([4,5]),import.meta.url),l=await s.runOne(a);r[l?.ok?"success":"warn"](`${a}: ${l?.msg??"KO"}`),await d(e)})()})}),g.info("feature-sentinels",`rendered ${n.length} sentinels (${p}OK / ${x}WARN)`)}function c(e){return e.replace(/[&<>"']/g,s=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[s]??s)}export{d as render};
//# sourceMappingURL=index-DoJdjRtM.js.map
