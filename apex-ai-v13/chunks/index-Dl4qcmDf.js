import{l as b}from"./monitoring-B17vNBOa.js";import{c as h}from"./listener-cleanup-Y2rGGxxX.js";import{s as v}from"../core/main-Xwj-tso0.js";import{autoBackup as s}from"./auto-backup-DdqgcfI4.js";import{h as c}from"./haptic-BUEqXK0N.js";import{toast as n}from"./toast-Dgg9rcIP.js";import"./apex-kb-DzVgIzgN.js";import"./apex-tools-registry-DloDnFZi.js";import"./credential-patterns-BybElwOv.js";import"./apex-tools-dispatch-BkpbMyVI.js";let i=null;function U(){i?.cleanup(),i=null}function d(t){return t.replace(/[&<>"']/g,r=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[r]??r)}function f(t){return t<1024?t+" B":t<1024*1024?(t/1024).toFixed(1)+" KB":(t/(1024*1024)).toFixed(2)+" MB"}function m(t){const r=new Date(t),e=o=>o.toString().padStart(2,"0");return`${e(r.getDate())}/${e(r.getMonth()+1)}/${r.getFullYear()} ${e(r.getHours())}:${e(r.getMinutes())}`}const k={manual:"💾 Manuel",daily:"📅 Quotidien",weekly:"🌐 Hebdo","pre-rollback":"↩ Pre-rollback"};function w(t){const r=k[t]??t;return`<span style="padding:2px 6px;border-radius:4px;background:rgba(201,162,39,0.15);color:#c9a227;font-size:11px;font-weight:600">${d(r)}</span>`}function J(){}function _(t){const r=t.last_backup_age_h<0?"Jamais":t.last_backup_age_h===0?"< 1h":`${t.last_backup_age_h}h`,e=t.last_backup_age_h<0||t.last_backup_age_h>48?"#ff6666":t.last_backup_age_h>26?"#ffa500":"#22c55e",o=t.integrity_ok?"#22c55e":"#ff6666",a=t.integrity_ok?"✓ OK":"⚠ Cassée",p=s.isQuotaCritical()?'<span style="margin-left:10px;padding:3px 8px;background:rgba(255,165,0,0.15);color:#ffa500;border-radius:4px;font-size:11px">⚠ Quota localStorage > 4 MB</span>':"";return`
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:10px;padding:14px;background:rgba(20,20,35,0.5);border:1px solid rgba(201,162,39,0.2);border-radius:12px;margin-bottom:14px">
      ...
      <div>
        ...
        <div style="color:${o};font-size:18px;font-weight:600">${a}</div>
      </div>
    </div>
  ...
  `}function $(t){const r=d(t.id),e=d(t.type),o=m(t.ts),a=f(t.size_bytes),p=t.hash.slice(0,12);return`
    <tr data-backup-id="${r}" style="border-bottom:1px solid rgba(201,162,39,0.1)">
      <td style="padding:10px;color:#fff;font-size:13px">${d(o)}</td>
      <td style="padding:10px">${w(e)}</td>
      <td style="padding:10px;color:#999;font-size:12px">${d(a)}</td>
  ...
  `}function B(t){return t.length===0?`
    ...
    <div id="ax-backup-modal" role="dialog" aria-modal="true"
      ...
      <div style="max-width:600px;width:100%;background:#0a0a14;border:1px solid rgba(201,162,39,0.4);border-radius:14px;overflow:hidden">
        ...
        <div style="max-height:60vh;overflow-y:auto;padding:14px">
          <pre style="margin:0;color:#a0a4c0;font-size:12px;white-space:pre-wrap;font-family:monospace">${d(o)}</pre>
  ...
  `}function g(t){if(i?.cleanup(),i=h("admin-backup"),!v.get("isAdmin")){t.innerHTML=`
    ...
    `;return}s.init();const e=s.getStats(),o=s.list();t.innerHTML=`
  ...
  `,A(t)}function A(t){t.querySelectorAll('[data-action="snapshot-now"]').forEach(r=>{i.bind(r,"click",()=>{(async()=>{c.tap();try{const e=await s.snapshot("manual");n.success(`Backup ${e.id} créé (${(e.size_bytes/1024).toFixed(1)} KB)`),g(t)}catch(e){b.warn("admin-backup","snapshot failed",{err:e}),n.error("Erreur backup : "+(e instanceof Error?e.message:"fail"))}})()})}),t.querySelectorAll('[data-action="export-config"]').forEach(r=>{i.bind(r,"click",()=>{(async()=>{c.tap();try{const e=await s.export();navigator.clipboard?(await navigator.clipboard.writeText(e),n.success("Coffre copié dans le presse-papier")):(b.info("admin-backup","export config (no clipboard)",{length:e.length}),n.info("Coffre exporté (voir console)"))}catch(e){n.error("Erreur export : "+(e instanceof Error?e.message:"fail"))}})()})}),t.querySelectorAll('[data-action="import-config"]').forEach(r=>{i.bind(r,"click",()=>{c.tap(),j(t)})}),t.querySelectorAll('[data-action="cleanup-now"]').forEach(r=>{i.bind(r,"click",()=>{(async()=>{c.tap();try{const e=await s.cleanup();e.deleted>0?n.info(`${e.deleted} anciens backups supprimés`):n.info("Rien à nettoyer"),g(t)}catch(e){n.error("Erreur cleanup : "+(e instanceof Error?e.message:"fail"))}})()})}),t.querySelectorAll("[data-view]").forEach(r=>{i.bind(r,"click",()=>{const e=r.dataset.view;if(!e)return;c.tap();const o=s.get(e);if(!o){n.error("Backup introuvable");return}q(t,o)})}),t.querySelectorAll("[data-restore]").forEach(r=>{i.bind(r,"click",()=>{const e=r.dataset.restore;!e||!confirm(`Restaurer le backup ${e} ? Cette action écrase l'état actuel.

Un backup pre-rollback automatique sera créé avant.`)||(async()=>{c.warning();try{const a=await s.restore(e);a.ok?n.success(`Restauration OK — ${a.restored.length} clés restaurées`):n.warn(`Restauration partielle — ${a.restored.length} OK, ${a.errors?.length??0} erreurs`),g(t)}catch(a){n.error("Erreur restore : "+(a instanceof Error?a.message:"fail"))}})()})}),t.querySelectorAll("[data-delete]").forEach(r=>{i.bind(r,"click",()=>{const e=r.dataset.delete;if(!e||!confirm(`Supprimer définitivement le backup ${e} ?`))return;c.warning(),s.delete(e)?(n.info("Backup supprimé"),g(t)):n.error("Erreur suppression")})})}function q(t,r){l();const e=document.createElement("div");e.innerHTML=C(r);const o=e.firstElementChild;o&&(document.body.appendChild(o),i.bind(o,"click",a=>{const p=a.target;if(p===o){l();return}p.closest('[data-action="modal-close"]')&&l()}))}function j(t){l();const r=document.createElement("div");r.innerHTML=S();const e=r.firstElementChild;e&&(document.body.appendChild(e),i.bind(e,"click",o=>{const a=o.target;if(a===e){l();return}if(a.closest('[data-action="modal-close"]')){l();return}if(a.closest('[data-action="import-confirm"]')){const x=e.querySelector("#ax-backup-import-data")?.value.trim()??"";if(!x){n.warn("Coffre vide");return}(async()=>{c.warning();try{const u=await s.import(x);u.ok?n.success(`Import OK — ${u.restored.length} clés restaurées`):n.warn(`Import partiel — ${u.errors?.[0]??"fail"}`),l(),g(t)}catch(u){n.error("Erreur import : "+(u instanceof Error?u.message:"fail"))}})()}}))}function l(){const t=document.getElementById("ax-backup-modal");t&&t.parentNode&&t.parentNode.removeChild(t)}export{J as _resetState,U as dispose,g as render};
//# sourceMappingURL=index-Dl4qcmDf.js.map
