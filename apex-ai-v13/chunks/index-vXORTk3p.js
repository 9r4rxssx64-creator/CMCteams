const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./monitoring-D2lWYrYo.js","./multi-source-analyze-Bg1HHfSC.js","./apex-kb-D1VtWFD9.js","./credential-patterns-CLzI061R.js","./auth-B3RTUO4E.js"])))=>i.map(i=>d[i]);
import{_ as p}from"./apex-kb-D1VtWFD9.js";import{e as u}from"./escape-html-BlQj2yEF.js";import{c as E}from"./listener-cleanup-Y2rGGxxX.js";import{s as I,l as T}from"./monitoring-D2lWYrYo.js";import{c as O}from"./csp-style-helper-BisGRi53.js";import{r as D}from"./recharge-action-C73aOkPx.js";import"./credential-patterns-CLzI061R.js";import"./multi-source-analyze-Bg1HHfSC.js";let i=null;function K(){i?.cleanup(),i=null}const P="apex_v13_chat_auto_read";function V(){try{return localStorage.getItem(P)==="1"}catch{return!1}}function q(s){try{localStorage.setItem(P,s?"1":"0")}catch{}}async function R(s){try{const k=await p(()=>import("./voice-BF9gEhMU.js"),__vite__mapDeps([0,1,2,3]),import.meta.url),{listVoices:S,getActiveVoice:x,setActiveVoice:m,speak:v,stopAll:g}=k,_=s.querySelector("#ax-settings-auto-read"),A=s.querySelector("#ax-voice-current"),w=s.querySelector("#ax-voice-list"),L=s.querySelectorAll(".ax-voice-cat-btn");if(!w)return;_&&(_.checked=V(),i.bind(_,"change",()=>{q(_.checked),(async()=>{const{toast:l}=await p(async()=>{const{toast:d}=await import("./toast-CRdbcLoc.js");return{toast:d}},[],import.meta.url);l.success(_.checked?"Lecture auto activée":"Lecture auto désactivée")})()}));const z=()=>{if(!A)return;const l=x(),b=S().find(h=>h.id===l);A.textContent=b?`Voix active : ${b.emoji??"🔊"} ${b.name} (${b.category})`:`Voix active : ${l}`};z();const $=l=>{const d=S(),b=l==="all"?d:d.filter(t=>t.category===l),h=x();w.setAttribute("aria-busy","false"),w.innerHTML=b.map(t=>{const r=t.id===h,a=t.emoji??(t.category==="pro"?"🎙️":t.category==="fun"?"🎉":"🎨"),e=t.description?u(t.description):"";return`
            <div class="ax-voice-item${r?" is-active":""}" data-voice-id="${u(t.id)}">
              <span class="ax-gs-17">${a}</span>
              <div class="ax-gs-6">
                <div class="ax-voice-item__name">${u(t.name)}${r?' <span style="color:var(--ax-gold);font-size:11px">★ active</span>':""}</div>
          ...
          `}).join("")};$("all"),L.forEach(l=>{i.bind(l,"click",()=>{const d=l.getAttribute("data-cat");d&&$(d)})}),i.bind(w,"click",l=>{const d=l.target,b=d.closest("[data-test-voice]"),h=d.closest("[data-set-voice]");if(b){const t=b.getAttribute("data-test-voice");if(!t)return;(async()=>{g();const r=await v("Bonjour Kevin, je suis ta voix.",t);if(!r.ok){const{toast:a}=await p(async()=>{const{toast:e}=await import("./toast-CRdbcLoc.js");return{toast:e}},[],import.meta.url);a.warn(`Test échoué : ${r.reason??"erreur"}`)}})();return}if(h){const t=h.getAttribute("data-set-voice");if(!t)return;(async()=>{await m(t),z();const r=s.querySelector(".ax-voice-cat-btn[data-cat]:focus")?.getAttribute("data-cat")??"all";$(r);const{toast:a}=await p(async()=>{const{toast:c}=await import("./toast-CRdbcLoc.js");return{toast:c}},[],import.meta.url),o=S().find(c=>c.id===t);a.success(o?`Voix par défaut : ${o.name}`:"Voix mise à jour")})()}})}catch(k){T.warn("feature-settings","wireVoiceSection failed",{err:k})}}function U(s){i?.cleanup(),i=E("settings");const k=I.get("user"),S=I.get("isAdmin")??!1,x="background:linear-gradient(135deg,rgba(20,20,35,0.7),rgba(14,14,28,0.5));backdrop-filter:blur(16px) saturate(140%);-webkit-backdrop-filter:blur(16px) saturate(140%);border:1px solid rgba(255,255,255,0.08);border-radius:16px;padding:20px;margin-top:14px;max-width:100%;box-sizing:border-box;transition:all 240ms cubic-bezier(0.16,1,0.3,1)",m="margin:0 0 12px;font-size:15px;font-weight:700;color:#fff;letter-spacing:-0.01em;display:flex;align-items:center;gap:10px",v="display:inline-flex;align-items:center;justify-content:center;width:32px;height:32px;background:linear-gradient(135deg,rgba(232,184,48,0.2),rgba(201,162,39,0.08));border:1px solid rgba(232,184,48,0.25);border-radius:10px;font-size:16px",g="width:100%;max-width:100%;box-sizing:border-box;min-height:44px;padding:12px 16px;font-size:14px;font-weight:600;border-radius:10px;cursor:pointer;-webkit-tap-highlight-color:transparent;transition:all 180ms cubic-bezier(0.16,1,0.3,1);white-space:normal;word-break:break-word;overflow-wrap:anywhere;text-align:left";s.innerHTML=O.withNonce(`
    <style>
  ...
  `),(async()=>{try{const{memoryBridge:t}=await p(async()=>{const{memoryBridge:o}=await import("./memory-bridge-DjZCGhDX.js");return{memoryBridge:o}},__vite__mapDeps([2,0,1,3]),import.meta.url),r=s.querySelector("#ax-memory-bridge-status"),a=s.querySelector("#ax-memory-bridge-sync"),e=()=>{if(!r)return;const o=t.getHealth(),c=t.getStatus(),n=c.filter(y=>y.last_success).length;r.textContent=`${o.backends_configured} backends configurés · ${n}/${c.length} dernier sync OK`};e(),a&&i&&i.bind(a,"click",()=>{(async()=>{a&&(a.disabled=!0);const o=await t.runAutoSync(),c=o.filter(y=>y.ok).length,{toast:n}=await p(async()=>{const{toast:y}=await import("./toast-CRdbcLoc.js");return{toast:y}},[],import.meta.url);o.length===0?n.warn("Aucun backend configuré"):c===o.length?n.success(`Sync OK (${c}/${o.length})`):n.warn(`Sync partielle (${c}/${o.length})`),e(),a&&(a.disabled=!1)})()})}catch(t){T.warn("feature-settings","memory-bridge wire failed",{err:t})}})();const _=s.querySelector("#ax-conso-scan");_&&i&&i.bind(_,"click",()=>{(async()=>{try{const{consumptionAnomalyDetector:t}=await p(async()=>{const{consumptionAnomalyDetector:e}=await import("./monitoring-D2lWYrYo.js").then(o=>o.G);return{consumptionAnomalyDetector:e}},__vite__mapDeps([0,1,2,3]),import.meta.url),r=t.scanAllVerbose(),a=s.querySelector("#ax-conso-results");if(!a)return;a.innerHTML=r.map(e=>{const o=e.severity==="critical"?"var(--ax-sev-critical)":e.severity==="high"?"var(--ax-sev-high)":e.severity==="medium"?"var(--ax-sev-medium)":e.severity==="low"?"var(--ax-sev-low)":"var(--ax-green)",c=e.severity==="critical"?"ax-sev-critical":e.severity==="high"?"ax-sev-high":e.severity==="medium"?"ax-sev-medium":(e.severity==="low","ax-sev-low"),n=e.severity==="critical"?"🚨":e.severity==="high"?"⚠️":e.severity==="medium"?"🟡":e.severity==="low"?"🔵":"✅";return`<div style="background:rgba(255,255,255,0.03);border-left:3px solid ${o};padding:8px 12px;margin-top:6px;border-radius:4px">
            <strong style="color:${o}">${n} ${e.service}</strong> <span class="ax-sev ${c}">${e.severity}</span>
          ...
          </div>`}).join("")}catch(t){T.warn("feature-settings","conso scan failed",{err:t})}})()});const A=s.querySelector("#ax-zoom-inspector-btn");A&&i&&i.bind(A,"click",()=>{(async()=>{const{apexZoomInspector:t}=await p(async()=>{const{apexZoomInspector:r}=await import("./apex-zoom-inspector-DwPoomZc.js");return{apexZoomInspector:r}},__vite__mapDeps([2,0,1,3]),import.meta.url);t.isVisible()?t.hide():t.show()})()});const w=s.querySelector("#ax-functional-test-btn");w&&i&&i.bind(w,"click",()=>{(async()=>{const t=s.querySelector("#ax-functional-test-results");if(t){t.innerHTML=`<div style="color:var(--ax-blue)">⏳ Test des boutons en cours (jusqu'à 40 boutons, ~30s)...</div>`;try{const{apexFunctionalTester:r}=await p(async()=>{const{apexFunctionalTester:f}=await import("./apex-functional-tester-CPxNz8CX.js");return{apexFunctionalTester:f}},__vite__mapDeps([2,0,1,3]),import.meta.url),{reportsHistory:a}=await p(async()=>{const{reportsHistory:f}=await import("./apex-reports-history-CfS6zTb6.js");return{reportsHistory:f}},__vite__mapDeps([0,1,2,3]),import.meta.url),e=await r.testAndAutoFix({maxButtons:30});a.recordFunctional(e.before,e.fixes,e.after,e.improvement);const o=e.before.tested>0?Math.round(e.before.ok/e.before.tested*100):0,c=e.after?` → après fix : ${Math.round(e.after.ok/Math.max(1,e.after.tested)*100)}% OK (${e.improvement>0?"+":""}${Math.round(e.improvement*100)}%)`:"",n=e.before.details.filter(f=>f.status==="no_response"||f.status==="error").slice(0,5).map(f=>`<li style="color:var(--ax-sev-high);font-size:11px">${u(f.label||"(no label)")} → ${u(f.status)}</li>`).join(""),y=e.fixes.applied.map(f=>u(String(f))).join(", ");t.innerHTML=`
            <div style="background:rgba(106,138,255,0.08);border:1px solid rgba(106,138,255,0.3);border-radius:8px;padding:10px;color:#fff;font-size:12px">
              ...
              <div>Testés : <b>${e.before.tested}</b>/${e.before.totalButtons} · OK : <b style="color:var(--ax-green)">${e.before.ok} (${o}%)</b> · No-response : <b style="color:var(--ax-sev-high)">${e.before.noResponse}</b> · Erreurs : <b style="color:var(--ax-error)">${e.before.errors}</b> · Skipped : ${e.before.skipped}${c}</div>
          ...
          `}catch(r){const a=r instanceof Error?r.message:String(r);t.innerHTML=`<div class="ax-gs-76">❌ Erreur test : ${u(a)}</div>`}}})()});const L=s.querySelector("#ax-layout-inspect-btn");L&&i&&i.bind(L,"click",()=>{(async()=>{const t=s.querySelector("#ax-layout-inspect-results");if(t){t.innerHTML='<div style="color:var(--ax-purple)">⏳ Scan layout...</div>';try{const{apexLayoutInspector:r}=await p(async()=>{const{apexLayoutInspector:n}=await import("./apex-layout-inspector-Dg92twMG.js");return{apexLayoutInspector:n}},__vite__mapDeps([2,0,1,3]),import.meta.url),{reportsHistory:a}=await p(async()=>{const{reportsHistory:n}=await import("./apex-reports-history-CfS6zTb6.js");return{reportsHistory:n}},__vite__mapDeps([0,1,2,3]),import.meta.url),e=r.scanDom();a.recordLayout(e);const o=e.hiddenButtons.slice(0,5).map(n=>`<li style="color:var(--ax-sev-high);font-size:11px">"${n.label}" → ${n.reason}</li>`).join(""),c=e.overflowingElements.slice(0,5).map(n=>`<li style="color:var(--ax-error);font-size:11px">${n.tag} (+${n.overflowBy}px)</li>`).join("");t.innerHTML=`
            <div style="background:rgba(180,90,200,0.08);border:1px solid rgba(180,90,200,0.3);border-radius:8px;padding:10px;color:#fff;font-size:12px">
              ...
              ${o?`<div class="ax-gs-75">Boutons cachés:</div><ul style="margin:2px 0 0 16px;padding:0">${o}</ul>`:""}
              ${c?`<div class="ax-gs-75">Éléments overflow:</div><ul style="margin:2px 0 0 16px;padding:0">${c}</ul>`:""}
              <div class="ax-gs-167">→ Historique complet dans Admin (Apex Audits Live)</div>
            </div>
          `}catch(r){const a=r instanceof Error?r.message:String(r);t.innerHTML=`<div class="ax-gs-76">❌ Erreur scan : ${u(a)}</div>`}}})()});const z=s.querySelector("#ax-cf-diagnostic-btn");z&&i&&i.bind(z,"click",()=>{(async()=>{const t=s.querySelector("#ax-cf-diagnostic-results");if(t){t.innerHTML='<div class="ax-gs-168">⏳ Test Cloudflare API en cours...</div>';try{const{apexCloudflareVaultDeploy:r}=await p(async()=>{const{apexCloudflareVaultDeploy:c}=await import("./apex-cloudflare-vault-deploy-DTGZG8rI.js");return{apexCloudflareVaultDeploy:c}},__vite__mapDeps([2,0,1,3]),import.meta.url),a=await r.runDiagnostic(),e=(c,n,y)=>`
            ...
            </div>`;let o='<div style="background:rgba(15,15,25,0.8);border:1px solid rgba(247,131,34,0.3);border-radius:10px;padding:12px;margin-top:10px">';o+='<div style="color:var(--ax-warning);font-weight:700;margin-bottom:8px">☁️ Diagnostic Cloudflare</div>',o+=e("Token Cloudflare présent",a.token_present),o+=e("Token valide (HTTP 200)",a.token_valid,a.http_status?`HTTP ${a.http_status}`:""),o+=e("Account ID accessible",!!a.account_id,a.account_name??a.account_id??""),o+=e("Permission KV (Workers KV Storage:Edit)",a.kv_permission,a.namespace_id?`ns ${a.namespace_id.slice(0,8)}…`:""),o+=e("Permission Workers (auto-deploy futur)",a.workers_permission),o+=e("Namespace apex-vault-kevin existe",a.namespace_exists),a.error_reason&&(o+=`<div style="margin-top:10px;padding:8px;background:rgba(255,91,91,0.1);border-left:3px solid var(--ax-error);color:var(--ax-error);font-size:12px;border-radius:4px">${a.error_reason}</div>`),a.fix_url&&(o+=`<div class="ax-gs-114"><a href="${a.fix_url}" target="_blank" rel="noopener" style="color:var(--ax-blue);font-size:12px">🔗 Fix : ${a.fix_url}</a></div>`),o+="</div>",t.innerHTML=o,T.info("cf-diag-manual",`Diagnostic result : token=${a.token_valid} kv=${a.kv_permission} workers=${a.workers_permission}`)}catch(r){const a=r instanceof Error?r.message:String(r);t.textContent="";const e=document.createElement("div");e.style.color="var(--ax-error)",e.textContent=`❌ Erreur : ${a.slice(0,100)}`,t.append(e)}}})()}),s.querySelectorAll("[data-nav-route]").forEach(t=>{i.bind(t,"click",()=>{const r=t.getAttribute("data-nav-route");r&&(location.hash="#"+r)})});const $=s.querySelector("#ax-settings-logout");$&&i&&i.bind($,"click",()=>{(async()=>{const{auth:t}=await p(async()=>{const{auth:r}=await import("./auth-B3RTUO4E.js");return{auth:r}},__vite__mapDeps([4,2,0,1,3]),import.meta.url);t.logout(),location.hash="#login"})()});const l=s.querySelector("#ax-settings-tools-auto-embed");if(l&&i){try{const t=JSON.parse(localStorage.getItem("ax_settings")??"{}");l.checked=t.tools_auto_embed!==!1}catch{l.checked=!0}i.bind(l,"change",()=>{try{const t=JSON.parse(localStorage.getItem("ax_settings")??"{}");t.tools_auto_embed=l.checked,localStorage.setItem("ax_settings",JSON.stringify(t))}catch{}})}const d=s.querySelector("#ax-force-update-btn"),b=s.querySelector("#ax-force-update-status");d&&i&&i.bind(d,"click",()=>{(async()=>{const t=r=>{b&&(b.textContent=r)};d.disabled=!0,d.textContent="⏳ Reset en cours…";try{if(t("🔍 Désinstallation Service Workers…"),"serviceWorker"in navigator){const a=await navigator.serviceWorker.getRegistrations();for(const e of a)await e.unregister();t(`✅ ${a.length} SW désinstallés`)}if(t("🔍 Vidage caches PWA…"),"caches"in window){const a=await caches.keys();for(const e of a)await caches.delete(e);t(`✅ ${a.length} caches vidés`)}t("✅ Reset terminé. Rechargement dans 2s…");const{toast:r}=await p(async()=>{const{toast:a}=await import("./toast-CRdbcLoc.js");return{toast:a}},[],import.meta.url);r.info("🔄 Reset OK — reload imminent"),setTimeout(()=>{location.href=location.pathname+"?_forceupd=1&_reset="+Date.now()},2e3)}catch(r){t(`❌ Erreur : ${String(r)}`),d.disabled=!1,d.textContent="🔄 Force reset PWA + reload"}})()}),R(s);const h=s.querySelector("#ax-settings-notif-test");h&&i&&i.bind(h,"click",()=>{(async()=>{try{if("Notification"in window&&Notification.permission==="granted")new Notification("Test Apex",{body:"Si tu vois ça, push notif fonctionne ✅"});else if("Notification"in window)if(await Notification.requestPermission()==="granted")new Notification("Test Apex",{body:"Push activé ✅"});else{const{toast:r}=await p(async()=>{const{toast:a}=await import("./toast-CRdbcLoc.js");return{toast:a}},[],import.meta.url);r.warn("Permission notifications refusée")}else{const{toast:t}=await p(async()=>{const{toast:r}=await import("./toast-CRdbcLoc.js");return{toast:r}},[],import.meta.url);t.warn("Notifications non supportées par ce navigateur")}}catch{const{toast:t}=await p(async()=>{const{toast:r}=await import("./toast-CRdbcLoc.js");return{toast:r}},[],import.meta.url);t.warn("Test notification échoué")}})()}),T.info("feature-settings","rendered")}export{K as dispose,U as render,R as wireVoiceSection};
