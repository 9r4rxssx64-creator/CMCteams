...
import{_ as s,l as y}from"../core/main-CZnL79Kv.js";const g=[{id:"tv",name:"Télévision",emoji:"📺",category:"tv",capability:"ir",actions:[{id:"power",label:"⏻ Marche/Arrêt"},{id:"vol_up",label:"🔊 Volume +"},{id:"vol_down",label:"🔉 Volume -"},{id:"channel_up",label:"⬆️ Chaîne +"},{id:"channel_down",label:"⬇️ Chaîne -"},{id:"mute",label:"🔇 Muet"},{id:"source",label:"🔌 Source"}]},{id:"lights",name:"Lumières",emoji:"💡",category:"light",capability:"bluetooth",actions:[{id:"on",label:"⏻ Allumer"},{id:"off",label:"⏼ Éteindre"},{id:"dim_up",label:"☀ Plus fort"},{id:"dim_down",label:"🌙 Plus faible"},{id:"color",label:"🎨 Couleur"}]},{id:"speaker",name:"Enceintes",emoji:"🔊",category:"audio",capability:"bluetooth",actions:[{id:"play_pause",label:"⏯ Play/Pause"},{id:"next",label:"⏭ Suivant"},{id:"prev",label:"⏮ Précédent"},{id:"vol_up",label:"🔊 Volume +"},{id:"vol_down",label:"🔉 Volume -"},{id:"airplay",label:"📡 AirPlay"}]},{id:"thermo",name:"Thermostat",emoji:"🌡",category:"climate",capability:"wifi",actions:[{id:"heat_up",label:"🔥 Chauffer +"},{id:"heat_down",label:"❄️ Chauffer -"},{id:"mode",label:"🔄 Mode"},{id:"eco",label:"🌱 Éco"}]},{id:"camera",name:"Caméras",emoji:"📹",category:"camera",capability:"wifi",actions:[{id:"view",label:"👁 Voir live"},{id:"snapshot",label:"📸 Snapshot"},{id:"record",label:"⏺ Enregistrer"}]},{id:"shade",name:"Volets",emoji:"🪟",category:"shade",capability:"bluetooth",actions:[{id:"up",label:"⬆️ Monter"},{id:"down",label:"⬇️ Descendre"},{id:"stop",label:"⏹ Stop"}]},{id:"wifi",name:"Wi-Fi",emoji:"📶",category:"network",capability:"nfc",actions:[{id:"share_nfc",label:"📲 Partager via NFC"},{id:"qr",label:"🔲 Générer QR Wi-Fi"}]},{id:"ev",name:"Borne EV",emoji:"🔌",category:"ev",capability:"nfc",actions:[{id:"badge",label:"💳 Badge RFID"},{id:"start",label:"⚡ Démarrer charge"},{id:"stop",label:"⏹ Arrêter"}]}];async function w(n){const{deviceControl:c}=await s(async()=>{const{deviceControl:e}=await import("./device-control-Cr-2suZ7.js");return{deviceControl:e}},__vite__mapDeps([0,1,2,3]),import.meta.url),p=c.detectDevice(),l=c.listAllSupported(),u=l.includes("nfc"),o=l.includes("bluetooth"),m=l.includes("share");n.innerHTML=`
  ...
  `,n.querySelectorAll("[data-remote-device]").forEach(e=>{e.addEventListener("click",()=>{const a=e.dataset.remoteDevice,t=e.dataset.remoteAction;_(a??"",t??"")})}),n.querySelector("#ax-remote-scan-bt")?.addEventListener("click",()=>{(async()=>{const e=await c.requestBluetoothDevice([{services:["battery_service"]}]),{toast:a}=await s(async()=>{const{toast:t}=await import("./toast-Dgg9rcIP.js");return{toast:t}},__vite__mapDeps([4,5]),import.meta.url);e.ok&&e.data?a.success(`🔵 Trouvé : ${e.data.name??e.data.id}`):a.warn(e.reason??"Bluetooth non disponible")})()}),n.querySelector("#ax-remote-scan-nfc")?.addEventListener("click",()=>{(async()=>{const{toast:e}=await s(async()=>{const{toast:t}=await import("./toast-Dgg9rcIP.js");return{toast:t}},__vite__mapDeps([4,5]),import.meta.url),a=await c.requestNFCRead(t=>{const r=t.map(i=>{const d=i.data;return typeof d=="string"?d:JSON.stringify(d)}).join(" · ").slice(0,100);e.success(`📲 Tag lu : ${r}`)});a.ok?e.success("📲 Approche un tag NFC pour lire"):e.warn(a.reason??"NFC non disponible")})()}),n.querySelector("#ax-remote-write-nfc")?.addEventListener("click",()=>{(async()=>{const e=await c.requestNFCWrite([{recordType:"text",data:"Apex Remote — "+new Date().toISOString()}]),{toast:a}=await s(async()=>{const{toast:t}=await import("./toast-Dgg9rcIP.js");return{toast:t}},__vite__mapDeps([4,5]),import.meta.url);e.ok?a.success("✍️ Approche le tag pour écrire"):a.warn(e.reason??"NFC write non disponible")})()}),n.querySelector("#ax-remote-vibrate")?.addEventListener("click",()=>{(async()=>{const e=await c.vibrate([100,30,100,30,200]),{toast:a}=await s(async()=>{const{toast:t}=await import("./toast-Dgg9rcIP.js");return{toast:t}},__vite__mapDeps([4,5]),import.meta.url);e.ok?a.success("📳 Vibration envoyée"):a.warn(e.reason??"Vibration non disponible (iOS Safari)")})()}),n.querySelector("#ax-remote-photos")?.addEventListener("click",()=>{(async()=>{const e=await c.getPhotosFromGallery(),{toast:a}=await s(async()=>{const{toast:t}=await import("./toast-Dgg9rcIP.js");return{toast:t}},__vite__mapDeps([4,5]),import.meta.url);e.ok&&e.data?a.success(`📸 ${e.data.length} photos sélectionnées (analyse EXIF en cours)`):a.warn(e.reason??"Sélection annulée")})()}),n.querySelector("#ax-remote-share")?.addEventListener("click",()=>{(async()=>{const e=await c.shareContent({title:"Apex AI v13",text:"Mon assistant intelligent personnel",url:location.origin+location.pathname}),{toast:a}=await s(async()=>{const{toast:t}=await import("./toast-Dgg9rcIP.js");return{toast:t}},__vite__mapDeps([4,5]),import.meta.url);e.ok?a.success("📤 Partagé"):a.warn(e.reason??"Partage annulé")})()}),n.querySelector("#ax-remote-scan-lan")?.addEventListener("click",()=>{(async()=>{const{toast:e}=await s(async()=>{const{toast:i}=await import("./toast-Dgg9rcIP.js");return{toast:i}},__vite__mapDeps([4,5]),import.meta.url),{networkScan:a}=await s(async()=>{const{networkScan:i}=await import("./network-scan-r1XG3HtB.js");return{networkScan:i}},__vite__mapDeps([6,1,2,3]),import.meta.url);e.info("🔍 Scan LAN en cours (peut prendre 30-60s)...");const t=await a.scan(),r=n.querySelector("#ax-remote-lan-results");if(r){if(!t.ok){r.innerHTML=`<p style="color:#ffaa00;font-size:13px">⚠️ ${t.reason??"Scan échoué"}</p>`;return}r.innerHTML=`
      ...
      `).join("")}})()});const x=async()=>{const{cardEmulator:e}=await s(async()=>{const{cardEmulator:r}=await import("./card-emulator-CojhfQTP.js");return{cardEmulator:r}},__vite__mapDeps([10,1,2,3]),import.meta.url),a=e.getStatus(),t=n.querySelector("#ax-remote-emulator-status");t&&(a.connected?t.innerHTML=`🟢 Connecté : <strong>${a.device}</strong> (${a.connection}) · ${a.uptime_sec}s`:t.textContent="⚪ Aucun émulateur connecté")};x();const b=(e,a)=>{n.querySelector(e)?.addEventListener("click",()=>{(async()=>{const{toast:t}=await s(async()=>{const{toast:d}=await import("./toast-Dgg9rcIP.js");return{toast:d}},__vite__mapDeps([4,5]),import.meta.url),{cardEmulator:r}=await s(async()=>{const{cardEmulator:d}=await import("./card-emulator-CojhfQTP.js");return{cardEmulator:d}},__vite__mapDeps([10,1,2,3]),import.meta.url),i=await r[a]();i.ok?(t.success("✅ Connecté"),await x()):t.warn(i.reason??"Connexion échouée")})()})};b("#ax-remote-flipper-usb","connectFlipperUSB"),b("#ax-remote-flipper-ble","connectFlipperBLE"),b("#ax-remote-proxmark","connectProxmarkSerial"),b("#ax-remote-chameleon","connectChameleonSerial"),y.info("feature-remote",`rendered ${g.length} device cards`)}async function _(n,c){const{deviceControl:p}=await s(async()=>{const{deviceControl:o}=await import("./device-control-Cr-2suZ7.js");return{deviceControl:o}},__vite__mapDeps([0,1,2,3]),import.meta.url),{toast:l}=await s(async()=>{const{toast:o}=await import("./toast-Dgg9rcIP.js");return{toast:o}},__vite__mapDeps([4,5]),import.meta.url),{auditLog:u}=await s(async()=>{const{auditLog:o}=await import("./audit-log-DdIhd9PZ.js");return{auditLog:o}},__vite__mapDeps([3,1,2]),import.meta.url);switch(u.record("remote.action",{details:{device:n,action:c}}),p.vibrate([30]),n){case"wifi":if(c==="qr"){l.info("🔲 Génération QR Wi-Fi → vue dédiée Sprint 4");return}if(c==="share_nfc"){const o=await p.requestNFCWrite([{recordType:"text",data:"WIFI:S:MyNetwork;T:WPA;P:password;;"}]);l[o.ok?"success":"warn"](o.ok?"✍️ Approche le tag NFC":o.reason??"NFC KO");return}break;case"speaker":if(c==="airplay"){const o=await p.openMusic("");l[o.ok?"success":"warn"](o.ok?"🎵 Apple Music ouvert":"iOS only");return}break;case"camera":if(c==="snapshot"){const o=await p.requestCamera({video:!0});if(o.ok&&o.data){const m=await p.takePhoto(o.data);l[m.ok?"success":"warn"](m.ok?"📸 Photo prise":m.reason??"KO")}return}break}l.info(`📡 ${n} · ${c} envoyé (bridge IR/BT à configurer Sprint 4)`)}export{w as render};
//# sourceMappingURL=index-CtyS475L.js.map
