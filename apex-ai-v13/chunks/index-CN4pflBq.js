...
import{_ as u}from"./apex-kb-0vuBuWYy.js";import{c as M}from"./listener-cleanup-Y2rGGxxX.js";import{l as D}from"./monitoring-BAiQJoxJ.js";import"./apex-tools-registry-DFaDwWZQ.js";import"./credential-patterns-Dy6Wjk7e.js";let i=null;function O(){i?.cleanup(),i=null}async function m(a){i?.cleanup(),i=M("sentinels");const{sentinels:d}=await u(async()=>{const{sentinels:t}=await import("./sentinels-mEdWpDUz.js");return{sentinels:t}},[],import.meta.url),{sentinelsRegistry:g,bootstrapSentinelsRegistry:h}=await u(async()=>{const{sentinelsRegistry:t,bootstrapSentinelsRegistry:n}=await import("./sentinels-registry-DCa9OdqB.js");return{sentinelsRegistry:t,bootstrapSentinelsRegistry:n}},[],import.meta.url);h();const s=d.list(),$=g.getStatus(),x=g.getMetrics(),y=s.filter(t=>t.lastResult?.ok).length,f=s.filter(t=>t.lastResult&&!t.lastResult.ok).length,R=s.filter(t=>!t.lastResult).length;a.innerHTML=`
