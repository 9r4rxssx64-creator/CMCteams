...
import{_ as c,e as r,l as b}from"./monitoring-BAwFG35Q.js";import{c as f}from"./listener-cleanup-Y2rGGxxX.js";import"./multi-source-analyze-DRqrcvIZ.js";import"./credential-patterns-DUMYZEMu.js";import"./apex-kb-CGmn9N6T.js";let o=null;function R(){o?.cleanup(),o=null}const h={"ai-provider":"🤖 IA Provider","lib-npm":"📦 Lib npm","api-service":"🌐 API Service","browser-api":"🌍 Browser API","tts-stt":"🎙 TTS/STT",vision:"👁 Vision","image-gen":"🎨 Image Gen","video-gen":"🎬 Video Gen","vector-db":"🗄 Vector DB",auth:"🔐 Auth","mobile-framework":"📱 Mobile",bundler:"⚙ Bundler"};function y(i){const s=i.estimatedGain;if(!s)return"—";const n=[];return s.perf!==void 0&&n.push(`perf +${s.perf}%`),s.cost!==void 0&&n.push(`cost +${s.cost}%`),s.capabilities!==void 0&&n.push(`cap +${s.capabilities}%`),s.bundleSize!==void 0&&n.push(`bundle -${s.bundleSize}%`),n.join(", ")||"—"}function _(i){return{"upgrade-asap":'<span class="ax-gs-14">⚡ ASAP</span>',"upgrade-soon":'<span class="ax-gs-44">⏳ SOON</span>',monitor:'<span class="ax-gs-25">👀 MONITOR</span>',skip:'<span class="ax-gs-64">— SKIP</span>',"breaking-changes":'<span class="ax-gs-13">⚠ BREAKING</span>'}[i]}async function d(i){o?.cleanup(),o=f("innovation");const{innovationWatch:s}=await c(async()=>{const{innovationWatch:a}=await import("./innovation-watch-DGzTRf0J.js");return{innovationWatch:a}},__vite__mapDeps([0,1,2,3]),import.meta.url),n=s.getStats(),p=s.getUpdates().filter(a=>(a.status??"pending")==="pending"),m=n.lastScan===0?"jamais":new Date(n.lastScan).toLocaleString("fr-FR"),l=new Map;for(const a of p){const t=l.get(a.category)??[];t.push(a),l.set(a.category,t)}i.innerHTML=`
                ...
                <tbody>
                  ${t.map(e=>`
                    <tr class="ax-gs-184">
                      ...
                      <td style="padding:8px;font-size:11px">${_(e.recommendation)}</td>
  ...
  `;const x=i.querySelector("#ax-inno-scan");x&&o&&o.bind(x,"click",()=>{(async()=>{const{toast:a}=await c(async()=>{const{toast:t}=await import("./toast-BCPNzfMv.js");return{toast:t}},[],import.meta.url);a.info("Scan en cours…");try{const t=await s.runScan();a.success(`✅ ${t.summary}`),await d(i)}catch(t){console.error("[innovation] scan",t),a.error("Analyse impossible — réessaie dans un instant")}})()});const g=i.querySelector("#ax-inno-refresh");g&&o&&o.bind(g,"click",()=>{d(i)});const v=i.querySelector("#ax-inno-reset");v&&o&&o.bind(v,"click",()=>{(async()=>{const{toast:a}=await c(async()=>{const{toast:t}=await import("./toast-BCPNzfMv.js");return{toast:t}},[],import.meta.url);typeof confirm=="function"&&!confirm("Reset historique innovation watch ?")||(s.reset(),a.success("Historique réinitialisé"),await d(i))})()}),i.querySelectorAll(".ax-inno-apply").forEach(a=>{o.bind(a,"click",()=>{(async()=>{const t=a.dataset.id;if(!t)return;const{toast:e}=await c(async()=>{const{toast:u}=await import("./toast-BCPNzfMv.js");return{toast:u}},[],import.meta.url);s.markUpdate(t,"applied"),e.success("Marqué comme appliqué"),await d(i)})()})}),i.querySelectorAll(".ax-inno-skip").forEach(a=>{o.bind(a,"click",()=>{(async()=>{const t=a.dataset.id;if(!t)return;const{toast:e}=await c(async()=>{const{toast:u}=await import("./toast-BCPNzfMv.js");return{toast:u}},[],import.meta.url);s.markUpdate(t,"skipped"),e.info("Skippé"),await d(i)})()})}),b.info("feature-innovation",`rendered ${p.length} pending updates (${n.totalUpdatesDetected} total)`)}export{R as dispose,d as render};
