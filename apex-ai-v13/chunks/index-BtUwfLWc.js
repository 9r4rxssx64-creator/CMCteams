...
import{_ as v}from"./apex-kb-D1VtWFD9.js";import{e as i}from"./escape-html-BlQj2yEF.js";import{c as S}from"./listener-cleanup-Y2rGGxxX.js";import{s as z,l as _}from"./monitoring-D2lWYrYo.js";import{credentialsAudit as k}from"./credentials-audit-DaRGJZQp.js";import{haptic as f}from"./haptic-CQFg2PXZ.js";import{toast as p}from"./toast-CRdbcLoc.js";import"./credential-patterns-CLzI061R.js";import"./multi-source-analyze-Bg1HHfSC.js";let x=null,m=null,y="all";function H(){x?.cleanup(),x=null}const h={ok:{color:"#22cc77",icon:"🟢",label:"OK"},missing:{color:"#888",icon:"⚪",label:"Non config"},corrupted:{color:"#ff6b6b",icon:"🔴",label:"Corrompu"},expired:{color:"#ffaa00",icon:"🟠",label:"Expiré"},unknown:{color:"#aaa",icon:"❓",label:"Inconnu"},decrypt_failed:{color:"#ff6b6b",icon:"🔒",label:"Illisible"}},$={all:"Tous",ai:"🤖 IA",banking:"🏦 Banque",payment:"💳 Paiement",social:"📱 Social",email:"📧 Email",crypto:"₿ Crypto",hosting:"☁️ Hosting",productivity:"📋 Productivité",forbidden:"⚠️ Forbidden"};async function w(e){if(!(z.get("isAdmin")===!0)){e.innerHTML=`
      <div class="ax-gs-21">
  ...
  `;try{m=await k.runFullAudit()}catch(s){_.error("credentials-registry","audit failed",{err:s}),e.innerHTML=`<div style="padding:40px;color:#ff6b6b">Erreur audit : ${i(String(s))}</div>`;return}A(e,m)}function A(e,o){const s=o.security_score,t=s>=80?"#22cc77":s>=60?"#ffaa00":"#ff6b6b",l=[...y==="all"?o.entries:o.entries.filter(a=>a.category===y)].sort((a,c)=>a.configured!==c.configured?a.configured?-1:1:a.service_name.localeCompare(c.service_name)),n=new Map;n.set("all",o.entries.length);for(const a of o.entries)n.set(a.category,(n.get(a.category)??0)+1);const r=[...n.entries()].filter(([a])=>a==="all"||$[a]&&(n.get(a)??0)>0).map(([a,c])=>{const b=y===a,C=$[a]??a;return`<button data-cat="${i(a)}" class="ax-cat-chip" style="
      ...
      ">${i(C)} (${c})</button>`}).join(""),u=o.recommendations.length===0?'<p style="color:#22cc77;margin:0">✅ Aucune recommandation — config saine</p>':o.recommendations.map(a=>`<li style="margin:8px 0;color:#ffaa00">${i(a)}</li>`).join(""),g=l.length===0?'<p style="color:var(--ax-text-dim);text-align:center;padding:24px">Aucun credential dans cette catégorie</p>':l.map(a=>T(a)).join("");e.innerHTML=`
    <div class="ax-page" style="padding:16px;max-width:960px;margin:0 auto">
      <header style="margin-bottom:24px">
        ...
        <p style="color:var(--ax-text-dim);margin:0;font-size:13px">
          ${o.total_patterns} patterns reconnus · ${o.configured_count} configurés ·
          ${o.encrypted_count} chiffrés AES-GCM-256 · ${o.firebase_backup_count} backup Firebase
        </p>
      </header>
  ...
  `,L(e),_.info("credentials-registry",`rendered : ${o.configured_count}/${o.total_patterns} configured, score=${s}`)}function T(e){const o=h[e.status],s=[e.persisted.local?'<span title="localStorage" style="color:#22cc77">💾</span>':'<span title="pas en local" style="color:#666">·</span>',e.persisted.idb?'<span title="IndexedDB shadow" style="color:#22cc77">🗄️</span>':'<span title="pas en IDB" style="color:#666">·</span>',e.persisted.firebase?'<span title="Firebase backup" style="color:#22cc77">☁️</span>':'<span title="pas en Firebase" style="color:#666">·</span>'].join(" ");return`
    <article data-cred-detail="${i(e.storage_key)}" style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.08);
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:10px">
        ...
        <div class="ax-gs-83">
          <span style="color:${o.color};font-size:13px">${o.icon} ${o.label}</span>
  ...
  `}function R(e,o,s){const t=document.createElement("div");t.setAttribute("role","dialog"),t.setAttribute("aria-label",`Récupérer ${s}`),t.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);display:flex;align-items:center;justify-content:center;z-index:9999;padding:20px",t.innerHTML=`
  ...
  `,document.body.appendChild(t);const d=t.querySelector("#ax-recover-input"),l=t.querySelector("#ax-recover-cancel"),n=t.querySelector("#ax-recover-confirm");setTimeout(()=>d?.focus(),50);const r=()=>{t.remove()};l?.addEventListener("click",r),t.addEventListener("click",u=>{u.target===t&&r()}),n?.addEventListener("click",()=>{(async()=>{const u=d?.value.trim()??"";if(!u){p.warn("Recolle ta clé pour récupérer");return}try{const{vault:g}=await v(async()=>{const{vault:c}=await import("./apex-kb-D1VtWFD9.js").then(b=>b.b);return{vault:c}},__vite__mapDeps([0,1,2,3]),import.meta.url),a=await g.recover(o,u);a.ok?(f.success(),p.success(`✅ ${s} récupérée + re-chiffrée`),r(),w(e)):(f.error(),p.error(`❌ ${a.reason??"recover failed"}`))}catch(g){p.error(`Erreur : ${String(g).slice(0,100)}`)}})()})}function L(e){if(!x)return;const o=e.querySelector("#ax-cred-refresh");o&&x.bind(o,"click",()=>{f.tap(),w(e)});const s=e.querySelector("#ax-cred-test-channels");s&&x.bind(s,"click",()=>{f.tap(),(async()=>{p.info("📡 Test alertes en cours...");const{kevinAlerts:t}=await v(async()=>{const{kevinAlerts:n}=await import("./kevin-alerts-Bw8KkWyP.js");return{kevinAlerts:n}},__vite__mapDeps([0,1,2,3]),import.meta.url),d=await t.testAllChannels(),l=Object.entries(d).filter(([,n])=>n).map(([n])=>n);l.length===0?p.warn("Aucun channel d'alerte configuré"):p.success(`✅ Channels OK : ${l.join(", ")}`)})()}),e.querySelectorAll("[data-cat]").forEach(t=>{x.bind(t,"click",()=>{y=t.dataset.cat??"all",f.selection(),m&&A(e,m)})}),e.querySelectorAll("[data-recover]").forEach(t=>{x.bind(t,"click",()=>{const d=t.dataset.recover??"",l=t.dataset.service??d;d&&(f.tap(),R(e,d,l))})}),e.querySelectorAll("[data-test]").forEach(t=>{x.bind(t,"click",d=>{d.stopPropagation();const l=t.dataset.test;l&&(f.tap(),(async()=>{const n=t.textContent;t.textContent="⏳ Test...",t.disabled=!0;try{const r=await k.testCredential(l);r.valid===!0?p.success(`✅ ${l} : valide (${r.latency_ms}ms)`):r.valid===!1?p.warn(`❌ ${l} : invalide`):p.info(`❓ ${l} : ${r.error??"test impossible"}`)}catch(r){p.error(`Test failed : ${String(r).slice(0,100)}`)}finally{t.textContent=n??"🧪 Tester",t.disabled=!1}})())})}),e.querySelectorAll("[data-cred-detail]").forEach(t=>{x.bind(t,"click",d=>{d.target.closest("button, a")||(async()=>{const n=t.dataset.credDetail;if(!n||!m)return;const r=m.entries.find(c=>c.storage_key===n);if(!r)return;const{drillDown:u}=await v(async()=>{const{drillDown:c}=await import("./drilldown-1SlvGToi.js");return{drillDown:c}},[],import.meta.url),g="ax-drilldown-mount-credentials";let a=document.getElementById(g);a||(a=document.createElement("div"),a.id=g,document.body.appendChild(a)),u.open({id:`cred-${n}`,title:`🔑 ${r.service_name}`,content:()=>{const c=h[r.status]??h.unknown;return`
              <div class="ax-gs-27">
                <table style="width:100%;font-size:13px">
                  ...
                  <tr><td style="padding:4px;color:var(--ax-text-dim)">Statut</td><td style="color:${c.color}">${c.icon} ${c.label}</td></tr>
            ...
            `},data:{storageKey:n}},a)})()})})}export{H as dispose,w as render};
