...
import{_ as u,e as o,l as M}from"./monitoring-0om1TPnh.js";import{c as D}from"./listener-cleanup-Y2rGGxxX.js";import"./multi-source-analyze-DkIR_hUc.js";import"./credential-patterns-DUMYZEMu.js";import"./apex-kb-CxXqiCNR.js";let i=null;function P(){i?.cleanup(),i=null}async function g(n){i?.cleanup(),i=D("sentinels");const{sentinels:p}=await u(async()=>{const{sentinels:t}=await import("./sentinels-DH7omeUU.js");return{sentinels:t}},__vite__mapDeps([0,1,2,3]),import.meta.url),{sentinelsRegistry:m,bootstrapSentinelsRegistry:f}=await u(async()=>{const{sentinelsRegistry:t,bootstrapSentinelsRegistry:a}=await import("./sentinels-registry-CwkaSjc5.js");return{sentinelsRegistry:t,bootstrapSentinelsRegistry:a}},__vite__mapDeps([0,1,2,3]),import.meta.url);f();const e=p.list(),y=m.getStatus(),x=m.getMetrics(),v=e.filter(t=>t.lastResult?.ok).length,$=e.filter(t=>t.lastResult&&!t.lastResult.ok).length,R=e.filter(t=>!t.lastResult).length;n.innerHTML=`
