import{e as r}from"./escape-html-BlQj2yEF.js";import{l as m}from"./monitoring-glbGSPAY.js";import{s as g}from"./apex-tools-dispatch-skills-DGxLIlGa.js";import{toast as l}from"./toast-CRdbcLoc.js";import"./multi-source-analyze-BKtnCZaJ.js";import"./apex-kb-KUpdSyu5.js";import"./credential-patterns-CLzI061R.js";import"./haptic-CQFg2PXZ.js";const x=[{id:"invoice",label:"Facture",emoji:"💰"},{id:"quote",label:"Devis",emoji:"📋"},{id:"contract-signed",label:"Contrat à signer",emoji:"✍️"},{id:"report-standard",label:"Rapport",emoji:"📊"},{id:"certificate",label:"Certificat",emoji:"🏆"},{id:"receipt",label:"Reçu",emoji:"🧾"},{id:"custom",label:"Texte libre",emoji:"📝"}];function q(a){a.innerHTML=`
    <div class="ax-gs-169">
  ...
  `,a.querySelector("#pdf-generate")?.addEventListener("click",async()=>{const t=a.querySelector("#pdf-template")?.value??"invoice",n=a.querySelector("#pdf-number")?.value??"",c=a.querySelector("#pdf-client")?.value??"",o=a.querySelector("#pdf-address")?.value??"",p=a.querySelector("#pdf-items")?.value??"",i=a.querySelector("#pdf-watermark")?.value??"",d=p.split(`
`).map(e=>e.trim()).filter(Boolean).map(e=>{const s=e.split("|").map(u=>u.trim());return{description:s[0]??"",quantity:parseFloat(s[1]??"1")||1,unit_price:parseFloat(s[2]??"0")||0}});l.info("Génération en cours...");try{const e=await g.generate({template:t,data:{number:n,client_name:c,client_address:o,items:d,date:new Date().toLocaleDateString("fr-FR")},...i?{options:{watermark:i}}:{}}),s=a.querySelector("#pdf-result");if(!s)return;e.success?(s.innerHTML=`
          <div class="ax-gs-47">
          ...
          </div>`,l.success(`✅ ${e.filename}`)):(s.innerHTML=`<p class="ax-gs-257">❌ ${r(e.error??"Erreur")}</p>`,l.error(`❌ ${e.error??"Erreur"}`))}catch(e){m.warn("studio-pdf","failed",{err:e}),l.error(`❌ ${e instanceof Error?e.message:"Erreur"}`)}})}export{q as render};
