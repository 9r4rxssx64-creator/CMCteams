import{e as f}from"./escape-html-BlQj2yEF.js";import{c as U}from"./listener-cleanup-Y2rGGxxX.js";import{a as b,l as p,f as Y,s as K}from"./monitoring-IylaM7fC.js";import{m as V,r as X}from"../core/main-DlcG_zKO.js";import{aiRouter as R}from"./ai-router-DSnDr42P.js";import{multiKeyVault as Q}from"./multi-key-vault-tFz1bwKp.js";import{haptic as L}from"./haptic-CQFg2PXZ.js";import{modalSheet as x}from"./modal-sheet-oR7SW-wv.js";import{toast as S}from"./toast-CRdbcLoc.js";import"./multi-source-analyze-EZnZFDrm.js";import"./apex-kb-CSQ6lrzQ.js";import"./credential-patterns-CLzI061R.js";import"./chat-fallback-C5CS4zjP.js";import"./apex-tools-dispatch-core-CrIin2T7.js";import"./apex-tools-dispatch-skills-BIR2EZr6.js";import"./apex-tools-dispatch-data-_yBit20W.js";import"./apex-tools-dispatch-finance-D84Ce07W.js";import"./apex-tools-dispatch-misc-BaXkVqrC.js";import"./apex-tools-misc-rxIglcY4.js";import"./apex-tools-registry-core-BMhHY4vU.js";import"./apex-tools-registry-skills-DRX4PPQ_.js";import"./economy-mode-BN9VLOeq.js";const B=3e4,C="ax_crew_runs_history",Z=50,F={anthropic:"reasoning",openai:"code-quality",gemini:"vision",groq:"speed",openrouter:"general",mistral:"multilingual"};class W{async run(e){const t=Date.now(),s=e.mode??"consensus",r=e.timeoutMs??B;if(e.members.length<2)throw new Error("Crew Experts requires at least 2 members");const n=e.members.map(d=>this.runMember(d,e.task,e.systemPrompt,s,r,e.signal)),a=(await Promise.allSettled(n)).map((d,h)=>{const v=e.members[h];if(!v)throw new Error("member undefined");if(d.status==="fulfilled")return d.value;const A=d.reason instanceof Error?d.reason.message:String(d.reason);return{provider:v.provider,expertise:v.expertise??F[v.provider],text:"",latencyMs:0,ok:!1,error:A.slice(0,200)}}),c=this.synthesize(a,s),u=this.detectConflicts(a),m=a.filter(d=>d.ok),l=m.length>=Math.ceil(a.length/2),y={task:e.task.slice(0,500),mode:s,responses:a,synthesis:c,conflicts:u,consensus:l,totalLatencyMs:Date.now()-t,ts:Date.now()};return this.persistHistory(y),b.record("crew.run",{details:{mode:s,members:e.members.length,successful:m.length,latencyMs:y.totalLatencyMs,consensus:l}}),p.info("crew-experts",`${m.length}/${e.members.length} providers OK · ${y.totalLatencyMs}ms · consensus=${l}`),y}async runMember(e,t,s,r,n,o){const a=Date.now(),c=e.expertise??F[e.provider],u=new AbortController,m=setTimeout(()=>u.abort(),n);o&&o.addEventListener("abort",()=>u.abort(),{once:!0});const l=this.buildMemberPrompt(s,c,r,e.systemPromptOverride);let y="",d;try{if(await R.stream([{role:"user",content:t}],l,h=>{h.text&&(y+=h.text)},h=>{d=h}),clearTimeout(m),d)throw d;return{provider:e.provider,expertise:c,text:y,latencyMs:Date.now()-a,ok:!0}}catch(h){clearTimeout(m);const v=h instanceof Error?h.message:String(h);return{provider:e.provider,expertise:c,text:y,latencyMs:Date.now()-a,ok:!1,error:v.slice(0,200)}}}buildMemberPrompt(e,t,s,r){if(r)return r;let n="";return s==="specialized"?n=`
...
`)}detectConflicts(e){const t=e.filter(u=>u.ok&&u.text);if(t.length<2)return[];const s=[],r=t.map(u=>u.text.length),n=Math.max(...r),o=Math.min(...r);o>0&&n/o>3&&s.push(`Divergence longueur : ${o}c → ${n}c (3×+)`);const a=["oui","safe","sécurisé","recommandé","préférable"],c=["non","dangereux","éviter","déconseillé","risqué"];for(let u=0;u<t.length-1;u++)for(let m=u+1;m<t.length;m++){const l=t[u],y=t[m];if(!l||!y)continue;const d=l.text.toLowerCase(),h=y.text.toLowerCase(),v=a.some(g=>d.includes(g)),A=c.some(g=>d.includes(g)),M=a.some(g=>h.includes(g)),T=c.some(g=>h.includes(g));(v&&T||A&&M)&&s.push(`${this.providerName(l.provider)} vs ${this.providerName(y.provider)} : avis opposés`)}return s}providerName(e){return{anthropic:"Claude",openai:"GPT",gemini:"Gemini",groq:"Groq",openrouter:"OpenRouter",mistral:"Mistral"}[e]}persistHistory(e){try{const t=localStorage.getItem(C)??"[]",s=JSON.parse(t),r=Array.isArray(s)?s:[];r.push(e);const n=r.slice(-Z);localStorage.setItem(C,JSON.stringify(n))}catch(t){p.warn("crew-experts","history persist failed",{err:t})}}history(){try{const e=localStorage.getItem(C)??"[]",t=JSON.parse(e);return Array.isArray(t)?t:[]}catch{return[]}}shouldUseCrew(e){const t=e.toLowerCase();return!!(/\b(audit|expert|complet|consulte|exhaustif|approfondi|concert|tous?\s+les?\s+angles)\b/i.test(t)||e.length>600||/\b(suppress|delete|effac|paiement|payer|valid|critique)\b/i.test(t)&&e.length>100)}defaultMembers(e="specialized"){return e==="specialized"?[{provider:"anthropic",expertise:"security"},{provider:"openai",expertise:"code-quality"},{provider:"gemini",expertise:"perf"},{provider:"groq",expertise:"ux"}]:[{provider:"anthropic"},{provider:"openai"},{provider:"gemini"}]}}const ee=new W,P="apex_v13_code_review_history",te=30,j={"claude-md-compliance":{provider:"anthropic",expertise:"reasoning"},"bug-detection":{provider:"openai",expertise:"code-quality"},"redundancy-check":{provider:"gemini",expertise:"analysis"},"git-history-context":{provider:"groq",expertise:"speed"},"code-patterns":{provider:"openrouter",expertise:"general"}},J=["claude-md-compliance","bug-detection","redundancy-check","git-history-context","code-patterns"];class se{async review(e){const t=Date.now(),s=e.confidenceThreshold??80,r=await this.normalizeDiff(e.diff),n=r.slice(0,500),c=(V.getDocsContext()["CLAUDE.md"]?.content??"").slice(0,4e3),u=J.map(g=>({...j[g],systemPromptOverride:this.buildRolePrompt(g,c)})),m=this.buildTaskPrompt(r);let l;try{l=await ee.run({task:m,systemPrompt:"Code reviewer expert.",members:u,mode:e.specialized===!1?"consensus":"specialized"})}catch(g){const $=g instanceof Error?g.message:String(g);p.warn("code-review-multi-agent","crew run failed",{err:$}),l={task:m.slice(0,500),mode:"specialized",responses:[],synthesis:"Erreur lors du lancement des agents.",conflicts:[],consensus:!1,totalLatencyMs:Date.now()-t,ts:Date.now()}}const y=J.map((g,$)=>{const w=l.responses[$];if(!w)return{role:g,provider:j[g].provider,findings:[],confidence:0,rawText:"",ok:!1,error:"no response"};const N=this.parseAgentFindings(w.text),H=this.estimateConfidence(w.text,N);return{role:g,provider:w.provider,findings:N,confidence:H,rawText:w.text,ok:w.ok,...w.error&&{error:w.error}}}),d=y.filter(g=>g.ok&&g.confidence>=s),h=d.reduce((g,$)=>g+$.findings.length,0),v=d.reduce((g,$)=>g+$.findings.filter(w=>w.severity==="critical").length,0),A=this.computeScore(d),M=this.buildConsensus(d,l.synthesis),T={diffPreview:n,agents:y,consensus:M,finalScore:A,totalFindings:h,criticalFindings:v,reviewedAt:Date.now(),durationMs:Date.now()-t};return this.persistReport(T),b.record("code-review.run",{details:{agents:d.length,findings:h,critical:v,score:A,latencyMs:T.durationMs}}),p.info("code-review-multi-agent",`Review complete: ${d.length}/${y.length} agents · ${h} findings · score=${A}`),T}history(){try{const e=localStorage.getItem(P)??"[]",t=JSON.parse(e);return Array.isArray(t)?t:[]}catch{return[]}}async normalizeDiff(e){if(typeof e=="string")return e;try{const t=e.sha?`${e.url}/${e.sha}.diff`:e.url,s=await fetch(t,{method:"GET"});if(!s.ok)throw new Error(`fetch diff failed: HTTP ${s.status}`);return await s.text()}catch(t){return p.warn("code-review-multi-agent","fetch diff failed",{err:t}),`[Erreur fetch diff: ${e.url}]`}}buildRolePrompt(e,t){const s=`Tu es un reviewer expert. Réponds STRICTEMENT en JSON suivant le format :
{
  "findings": [
...
${t}`),s.join(`
`)}persistReport(e){try{const t=localStorage.getItem(P)??"[]",s=JSON.parse(t),r=Array.isArray(s)?s:[];r.push(e);const n=r.slice(-te);localStorage.setItem(P,JSON.stringify(n))}catch(t){p.warn("code-review-multi-agent","persist failed",{err:t})}}}const re=new se,I="apex_v13_frontend_designs_history",ne=15,ie=[{rx:/font-family:\s*['"]?Inter['"]?/gi,replacement:"font-family: Georgia, 'Times New Roman', serif",reason:"Inter banni (slop)"},{rx:/font-family:\s*['"]?Roboto['"]?/gi,replacement:"font-family: Georgia, serif",reason:"Roboto banni (slop)"},{rx:/color:\s*#007bff/gi,replacement:"color: #c9a227",reason:"Bootstrap blue banni (slop)"},{rx:/background:\s*#28a745/gi,replacement:"background: #c9a227",reason:"Bootstrap green banni (slop)"}];class oe{async generate(e){const t=Date.now(),s=e.framework??"vanilla",r=this.buildSystemPrompt(s,e),n=`Crée un composant pour : ${e.prompt}

Retourne STRICTEMENT en JSON : {"html": "...", "css": "...", "js": "..."}`;let o="",a;try{await R.stream([{role:"user",content:n}],r,l=>{l.text&&(o+=l.text)},l=>{a=l})}catch(l){a=l instanceof Error?l:new Error(String(l))}if(a||!o)return p.warn("frontend-design","IA generation failed, fallback skeleton",{err:a?.message}),this.fallbackSkeleton(e,s,t,o);let c;try{const l=o.match(/\{[\s\S]*"html"[\s\S]*\}/);if(!l)throw new Error("JSON manquant");c=JSON.parse(l[0])}catch(l){return p.warn("frontend-design","parse failed, fallback",{err:l}),this.fallbackSkeleton(e,s,t,o)}const u={html:this.sanitizeHtml(c.html??""),css:this.applyAntiSlop(c.css??""),js:this.sanitizeJs(c.js??"")},m={html:u.html,css:u.css,js:u.js,framework:s,generatedAt:Date.now(),durationMs:Date.now()-t,rawText:o.slice(0,5e3)};return this.persistOutput(e,m),b.record("frontend-design.generate",{details:{framework:s,prompt:e.prompt.slice(0,100),durationMs:m.durationMs}}),p.info("frontend-design",`Generated ${s} component (${m.durationMs}ms)`),m}history(){try{const e=localStorage.getItem(I)??"[]",t=JSON.parse(e);return Array.isArray(t)?t:[]}catch{return[]}}buildPreviewSrcdoc(e){const t=e.framework==="react"?`<script crossorigin src="https://unpkg.com/react@18/umd/react.production.min.js"><\/script>
...
`)}persistSession(e){try{const t=this.listSessions(),s=t.findIndex(n=>n.sessionId===e.sessionId);s>=0?t[s]=e:t.push(e);const r=t.slice(0,fe);localStorage.setItem(G,JSON.stringify(r))}catch(t){p.warn("superpowers","persist failed",{err:t})}}}const _=new ye;let k=null;function Xe(){k?.cleanup(),k=null}const ve=[{id:"security-review",emoji:"🔒",name:"Security Review",description:"Scan runtime exhaustif : secrets en clair, CSP violations, vault drift, audit integrity.",status:"installed",buttonLabel:"Lancer scan"},{id:"code-review-5-agents",emoji:"👥",name:"Code Review 5 Agents",description:"5 IA en parallèle (CLAUDE.md compliance / bugs / redondance / git history / patterns).",status:"configurable",buttonLabel:"Reviewer un diff"},{id:"frontend-design",emoji:"🎨",name:"Frontend Design",description:"Génère un composant UI production-grade depuis prompt avec anti-slop strict.",status:"configurable",buttonLabel:"Générer composant"},{id:"superpowers",emoji:"⚡",name:"Superpowers",description:"7-step methodology : brainstorm → plan → dev → test → review → ship → reflect.",status:"configurable",buttonLabel:"Démarrer session"},{id:"gstack-roles",emoji:"🏛",name:"GStack Roles",description:"7 rôles spécialisés (CEO/Designer/Engineer/QA/Release/Reviewer/Reflector).",status:"configurable",buttonLabel:"Lancer pipeline"}];function Se(i){const e=i.status==="installed"?'<span style="background:rgba(34,204,119,0.15);color:#22cc77;padding:3px 10px;border-radius:10px;font-size:11px;font-weight:600;letter-spacing:0.02em">✅ Actif</span>':'<span style="background:rgba(232,184,48,0.15);color:#e8b830;padding:3px 10px;border-radius:10px;font-size:11px;font-weight:600;letter-spacing:0.02em">⚙️ Config</span>';return`
  ...
  `}async function be(){S.info("🔒 Scan en cours...");try{const i=await me.runFullScan(),e=i.findings.length>0?i.findings.map(t=>`
        <li class="ax-gs-304">
...
${f(e.js)}</pre>
      ...
      `,actions:[{label:"Fermer",variant:"ghost",onClick:()=>x.closeAll()}]})}catch(e){p.warn("yury-plugins","frontend design failed",{err:e}),S.error("Génération échouée")}}}async function Ae(){const i=window.prompt("Nom de la tâche pour la session Superpowers :","Refactor auth flow");if(!i)return;const e=_.start(i);S.info(`⚡ Session ${e} démarrée — avancement step 1/7...`);try{const t=await _.advance(e),s=_.getState(e);x.open({title:`⚡ Superpowers — ${f(i)}`,content:`
        <div class="ax-gs-12">
          <p class="ax-gs-303">
