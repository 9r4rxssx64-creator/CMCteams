const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./apex-kb-D1VtWFD9.js","./monitoring-D2lWYrYo.js","./multi-source-analyze-Bg1HHfSC.js","./credential-patterns-CLzI061R.js","../assets/css/main-Dx4ezkU4.css"])))=>i.map(i=>d[i]);
import{_ as b}from"./apex-kb-D1VtWFD9.js";import{e as n}from"./escape-html-BlQj2yEF.js";import{c as w}from"./listener-cleanup-Y2rGGxxX.js";import{l as y}from"./monitoring-D2lWYrYo.js";import{haptic as x}from"./haptic-CQFg2PXZ.js";import{toast as u}from"./toast-CRdbcLoc.js";import"./credential-patterns-CLzI061R.js";import"./multi-source-analyze-Bg1HHfSC.js";let d=null;function V(){d?.cleanup(),d=null}const $={security:{icon:"🔒",label:"Sécurité"},performance:{icon:"⚡",label:"Performance"},ux:{icon:"🎨",label:"UX"},tests:{icon:"🧪",label:"Tests"},architecture:{icon:"🏗",label:"Architecture"},ai_safety:{icon:"🤖",label:"AI Safety"}},m={p0_critical:{color:"#ff5858",icon:"🚨",label:"P0 Critical"},p1_high:{color:"#ff8c42",icon:"⚠️",label:"P1 High"},p2_medium:{color:"#ffaa00",icon:"🟡",label:"P2 Medium"},p3_low:{color:"#5aa8ff",icon:"ℹ️",label:"P3 Low"},info:{color:"#888",icon:"💡",label:"Info"}};function A(e){return e>=90?"#22cc77":e>=75?"#a0c878":e>=60?"#ffaa00":e>=40?"#ff8c42":"#ff5858"}function k(e){return e>=95?"A+":e>=85?"A":e>=70?"B":e>=55?"C":e>=40?"D":"F"}function S(e,i){const t={p0_critical:5,p1_high:4,p2_medium:3,p3_low:2,info:1},r=t[i];return e.filter(o=>t[o.severity]>=r)}function R(){try{const e=localStorage.getItem("ax_lessons_learned_struct")??"[]",i=JSON.parse(e);return Array.isArray(i)?i.filter(t=>{if(t===null||typeof t!="object")return!1;const r=t;return typeof r.id=="string"&&typeof r.title=="string"&&typeof r.ts=="number"}):[]}catch{return[]}}let s=null,c=!1,g="p2_medium";function L(e){const i=A(e.total_score),t=k(e.total_score);return`
    ...
    <section style="margin-bottom:16px">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:10px">
        ${Object.keys($).map(i=>{const t=e.axes[i],r=$[i],o=t.score>=18?"#22cc77":t.score>=14?"#a0c878":t.score>=10?"#ffaa00":"#ff5858";return`
            <div style="background:rgba(20,20,35,0.6);border:1px solid rgba(255,255,255,0.05);border-radius:10px;padding:12px;text-align:center;border-left:3px solid ${o}">
              <div style="font-size:24px;margin-bottom:4px">${n(r.icon)}</div>
              <div style="font-size:18px;font-weight:900;color:${o}">${t.score}/20</div>
    ...
    </section>`}function D(e){const i=S(e,g);return i.length===0?'<p style="color:#a0a4c0;font-size:12px;padding:20px;text-align:center">Aucun finding pour ce filtre.</p>':i.slice(0,50).map(t=>{const r=m[t.severity],o=t.auto_fix_success?"✅ Auto-fixed":t.escalated_to_claude?"📤 Escaladé Claude":"🚨 À traiter";return`
        ...
        </div>`}).join("")}function E(e){return e.length===0?'<p style="color:#a0a4c0;font-size:12px">Aucune lesson learned pour le moment.</p>':[...e].sort((t,r)=>r.ts-t.ts).slice(0,50).map(t=>{const r=t.severity==="critical"?"#ff5858":t.severity==="warn"?"#ffaa00":"#5aa8ff",o=new Date(t.ts).toLocaleString("fr-FR");return`
        ...
        </div>`}).join("")}async function v(e){if(d?.cleanup(),d=w("self-diag"),!s)try{const{apexSelfAudit:a}=await b(async()=>{const{apexSelfAudit:p}=await import("./apex-self-audit-30LE5p6b.js");return{apexSelfAudit:p}},__vite__mapDeps([0,1,2,3,4]),import.meta.url);s=a.getLastReport()}catch(a){y.warn("feature-self-diag","load last report failed",{err:a})}const i=R();let t=[];try{const{sentinels:a}=await b(async()=>{const{sentinels:p}=await import("./sentinels-DZ-dPvtb.js");return{sentinels:p}},__vite__mapDeps([0,1,2,3]),import.meta.url);t=a.list()}catch{}const r=t.filter(a=>a.lastResult?.ok).length,o=t.filter(a=>a.lastResult&&!a.lastResult.ok).length,l=t.filter(a=>!a.lastResult).length;e.innerHTML=`
      ...
      ${s?`
        ...
        <section style="margin-bottom:24px">
          ...
          <div style="margin-bottom:8px;display:flex;gap:6px;flex-wrap:wrap">
            ${["p0_critical","p1_high","p2_medium","p3_low","info"].map(a=>`
              <button class="ax-diag-sev-btn ${g===a?"ax-tab-active":""}"
                data-diag-sev="${a}"
                style="background:${g===a?`rgba(${a==="p0_critical"?"255,88,88":"201,162,39"},.15)`:"transparent"};color:${g===a?m[a].color:"#a0a4c0"};border:1px solid rgba(201,162,39,.3);padding:5px 10px;border-radius:8px;font-size:11px;cursor:pointer">
                ${m[a].icon} ${m[a].label}
              </button>
            `).join("")}
          </div>
        ...
        <section style="margin-bottom:24px">
          ...
          <ul style="list-style:disc;padding-left:24px;color:#a0a4c0;font-size:12px;line-height:1.6">
            ${s.next_steps.map(a=>`<li>${n(a)}</li>`).join("")}
          </ul>
        </section>
      `:`
      ...
      <section style="margin-bottom:24px">
        ...
        <div style="background:rgba(20,20,35,0.6);border:1px solid rgba(255,255,255,0.05);border-radius:14px;padding:14px">
          <div style="display:flex;gap:14px;align-items:center;flex-wrap:wrap;font-size:13px">
            ...
            <span><strong style="color:#ffaa00">${o}</strong> WARN</span>
  ...
  `,I(e),y.info("feature-self-diag",`rendered (report=${s?"yes":"no"}, lessons=${i.length}, sentinels=${t.length})`)}function I(e){const i=d??w("self-diag");d||(d=i);const t=e.querySelector("#ax-diag-run");t&&i.bind(t,"click",()=>{_(e,!1)});const r=e.querySelector("#ax-diag-brutal");r&&i.bind(r,"click",()=>{_(e,!0)});const o=e.querySelector("#ax-diag-export");o&&i.bind(o,"click",()=>{(async()=>{if(x.tap(),!!s)try{const{apexSelfAudit:l}=await b(async()=>{const{apexSelfAudit:z}=await import("./apex-self-audit-30LE5p6b.js");return{apexSelfAudit:z}},__vite__mapDeps([0,1,2,3,4]),import.meta.url),a=l.formatReportMarkdown(s),p=new Blob([a],{type:"text/markdown"}),h=URL.createObjectURL(p),f=document.createElement("a");f.href=h,f.download=`apex-audit-${new Date().toISOString().slice(0,10)}.md`,document.body.appendChild(f),f.click(),document.body.removeChild(f),URL.revokeObjectURL(h),u.success("Rapport exporté")}catch(l){y.warn("feature-self-diag","export failed",{err:l}),u.error("Export échoué")}})()}),e.querySelectorAll("[data-diag-sev]").forEach(l=>{i.bind(l,"click",()=>{x.selection(),g=l.dataset.diagSev,v(e)})})}async function _(e,i){if(!c){c=!0,x.medium(),u.info(i?"🔥 Audit brutal en cours...":"🔍 Audit en cours..."),v(e);try{const{apexSelfAudit:t}=await b(async()=>{const{apexSelfAudit:o}=await import("./apex-self-audit-30LE5p6b.js");return{apexSelfAudit:o}},__vite__mapDeps([0,1,2,3,4]),import.meta.url),r=await t.runFullAudit(i);s=r,x.success(),u.success(`✅ Audit terminé : ${r.total_score}/100`)}catch(t){y.warn("feature-self-diag","audit failed",{err:t}),x.error(),u.error("Audit échoué")}finally{c=!1,v(e)}}}export{V as dispose,n as escapeHtml,S as filterFindingsBySeverity,R as loadLessons,v as render,A as scoreColor,k as scoreGrade};
