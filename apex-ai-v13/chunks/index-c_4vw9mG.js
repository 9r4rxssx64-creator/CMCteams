const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./persistent-memory-store-BrM_IezU.js"])))=>i.map(i=>d[i]);
import{_ as g}from"./apex-kb-0vuBuWYy.js";import{l as M}from"./monitoring-BAiQJoxJ.js";import{m as b}from"../core/main-BRJyZFXI.js";import{toast as u}from"./toast-Dgg9rcIP.js";import"./apex-tools-registry-DFaDwWZQ.js";import"./credential-patterns-Dy6Wjk7e.js";import"./multi-source-analyze-DzTKkB_U.js";import"./haptic-BUEqXK0N.js";const L="kdmc_admin";function c(s){return s.replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]??t)}function S(){try{const s=localStorage.getItem("apex_v13_user");return s?JSON.parse(s):null}catch{return null}}function h(s){if(!s)return"—";try{return new Date(s).toLocaleString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"})}catch{return"—"}}function x(s){if(!s)return"jamais";const t=Date.now()-s,r=t/36e5;return r<1?`${Math.floor(t/6e4)}min`:r<24?`${Math.floor(r)}h`:`${Math.floor(r/24)}j`}function R(s){const t=S(),r=t?.id===L,n=t?.name??"Anonyme";s.innerHTML=`
  ...
  `,w(s,t,r),j(s,t,r)}async function w(s,t,r){await Promise.all([v(s,t),r?T(s):Promise.resolve(),D(s),_(s),H(s)])}async function v(s,t){const r=s.querySelector("#my-facts-content");if(r){if(!t){r.innerHTML="<em>Pas de user connecté.</em>";return}try{const{persistentMemory:n}=await g(async()=>{const{persistentMemory:e}=await import("./persistent-memory-store-BrM_IezU.js");return{persistentMemory:e}},__vite__mapDeps([0]),import.meta.url),d=(await n.list()).filter(e=>e.scope===t.id).sort((e,i)=>i.importance-e.importance);if(d.length===0){r.innerHTML="<em>Aucun fact mémorisé pour ton compte. Les facts seront extraits automatiquement de tes messages chat.</em>";return}const o=d.slice(0,100).map((e,i)=>`
    ...
    `,r.querySelectorAll("tr[data-fact-idx]").forEach(e=>{e.addEventListener("click",()=>{const i=Number(e.dataset.factIdx??"-1"),l=d[i];l&&(async()=>{const{drillDown:y}=await g(async()=>{const{drillDown:p}=await import("./drilldown-1SlvGToi.js");return{drillDown:p}},[],import.meta.url),f="ax-drilldown-mount-knowledge";let m=document.getElementById(f);m||(m=document.createElement("div"),m.id=f,document.body.appendChild(m)),y.open({id:`fact-${i}`,title:`🧠 Fact ${c(l.category)}`,content:()=>`
            ...
            `,data:{factIdx:i}},m)})()})})}catch(n){r.innerHTML=`<em style="color:#c00;">Erreur chargement : ${c(String(n))}</em>`}}}async function T(s){const t=s.querySelector("#cross-user-content");if(t)try{const{persistentMemory:r}=await g(async()=>{const{persistentMemory:o}=await import("./persistent-memory-store-BrM_IezU.js");return{persistentMemory:o}},__vite__mapDeps([0]),import.meta.url),n=await r.list(),a=new Map;for(const o of n){const e=a.get(o.scope)??[];e.push(o),a.set(o.scope,e)}if(a.size===0){t.innerHTML="<em>Aucun user n'a encore de facts mémorisés.</em>";return}const d=[];for(const[o,e]of a){const i=e.sort((l,y)=>y.importance-l.importance).slice(0,3);d.push(`
      ...
      `)}t.innerHTML=`<p>${a.size} user(s), ${n.length} fact(s) total</p>${d.join("")}`}catch(r){t.innerHTML=`<em style="color:#c00;">Erreur : ${c(String(r))}</em>`}}async function D(s){const t=s.querySelector("#lessons-content");if(t)try{const r=localStorage.getItem("ax_lessons_learned_struct");if(!r){t.innerHTML="<em>Aucune lesson cross-app encore.</em>";return}const n=JSON.parse(r);if(n.length===0){t.innerHTML="<em>Liste vide.</em>";return}const d=[...n].sort((o,e)=>e.ts-o.ts).slice(0,30).map(o=>{const e=o.severity==="critical"?"#ff4444":o.severity==="warn"?"#ffa94d":"#888",i=o.resolved?"✅":"⏳";return`
  ...
  `}async function H(s){const t=s.querySelector("#audit-content");if(t)try{const r=localStorage.getItem("ax_memory_audit_log");if(!r){t.innerHTML="<em>Sentinelle memory-watch n'a pas encore tourné (1×/jour).</em>";return}const n=JSON.parse(r);if(n.length===0){t.innerHTML="<em>Log vide.</em>";return}const a=n[n.length-1];if(!a){t.innerHTML="<em>Log vide.</em>";return}t.innerHTML=`
