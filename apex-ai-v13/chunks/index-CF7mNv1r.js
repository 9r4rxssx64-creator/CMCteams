const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./apex-self-audit-D7zuzDfO.js","../core/main-BSLFHN2z.js","../assets/css/main-rhfGvOFL.css","./audit-log-BmtV8LrZ.js","./soc2-compliance-7wncvrKK.js","./vault-Cjwtoqo0.js","./credential-patterns-Ct__OCbr.js","./sentinels-DOLaSY61.js","./observability-BwTrC78I.js"])))=>i.map(i=>d[i]);
import{_ as u,l as g}from"../core/main-BSLFHN2z.js";import{h as c}from"./haptic-BUEqXK0N.js";import{toast as p}from"./toast-64I4l5xU.js";function n(e){return e.replace(/[&<>"']/g,i=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[i]??i)}const b={security:{icon:"🔒",label:"Sécurité"},performance:{icon:"⚡",label:"Performance"},ux:{icon:"🎨",label:"UX"},tests:{icon:"🧪",label:"Tests"},architecture:{icon:"🏗",label:"Architecture"},ai_safety:{icon:"🤖",label:"AI Safety"}},f={p0_critical:{color:"#ff5858",icon:"🚨",label:"P0 Critical"},p1_high:{color:"#ff8c42",icon:"⚠️",label:"P1 High"},p2_medium:{color:"#ffaa00",icon:"🟡",label:"P2 Medium"},p3_low:{color:"#5aa8ff",icon:"ℹ️",label:"P3 Low"},info:{color:"#888",icon:"💡",label:"Info"}};function h(e){return e>=90?"#22cc77":e>=75?"#a0c878":e>=60?"#ffaa00":e>=40?"#ff8c42":"#ff5858"}function $(e){return e>=95?"A+":e>=85?"A":e>=70?"B":e>=55?"C":e>=40?"D":"F"}function _(e,i){const t={p0_critical:5,p1_high:4,p2_medium:3,p3_low:2,info:1},o=t[i];return e.filter(a=>t[a.severity]>=o)}function w(){try{const e=localStorage.getItem("ax_lessons_learned_struct")??"[]",i=JSON.parse(e);return Array.isArray(i)?i.filter(t=>{const o=t;return typeof o.id=="string"&&typeof o.title=="string"&&typeof o.ts=="number"}):[]}catch{return[]}}let s=null,l=!1,x="p2_medium";function z(e){const i=h(e.total_score),t=$(e.total_score);return`
    ...
    <section style="margin-bottom:16px">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:10px">
        ${Object.keys(b).map(i=>{const t=e.axes[i],o=b[i],a=t.score>=18?"#22cc77":t.score>=14?"#a0c878":t.score>=10?"#ffaa00":"#ff5858";return`
    ...
    </section>`}function k(e){const i=_(e,x);return i.length===0?'<p style="color:#a0a4c0;font-size:12px;padding:20px;text-align:center">Aucun finding pour ce filtre.</p>':i.slice(0,50).map(t=>{const o=f[t.severity],a=t.auto_fix_success?"✅ Auto-fixed":t.escalated_to_claude?"📤 Escaladé Claude":"🚨 À traiter";return`
        ...
        </div>`}).join("")}async function y(e){if(!s)try{const{apexSelfAudit:r}=await u(async()=>{const{apexSelfAudit:m}=await import("./apex-self-audit-D7zuzDfO.js");return{apexSelfAudit:m}},__vite__mapDeps([0,1,2,3,4,5,6]),import.meta.url);s=r.getLastReport()}catch(r){g.warn("feature-self-diag","load last report failed",{err:r})}const i=w();let t=[];try{const{sentinels:r}=await u(async()=>{const{sentinels:m}=await import("./sentinels-DOLaSY61.js");return{sentinels:m}},__vite__mapDeps([7,1,2,8]),import.meta.url);t=r.list()}catch{}const o=t.filter(r=>r.lastResult?.ok).length,a=t.filter(r=>r.lastResult&&!r.lastResult.ok).length,d=t.filter(r=>!r.lastResult).length;e.innerHTML=`
      ...
      ${s?`
        ...
        <section style="margin-bottom:24px">
          ...
          <div style="margin-bottom:8px;display:flex;gap:6px;flex-wrap:wrap">
            ${["p0_critical","p1_high","p2_medium","p3_low","info"].map(r=>`
              <button class="ax-diag-sev-btn ${x===r?"ax-tab-active":""}"
                data-diag-sev="${r}"
                style="background:${x===r?`rgba(${r==="p0_critical"?"255,88,88":"201,162,39"},.15)`:"transparent"};color:${x===r?f[r].color:"#a0a4c0"};border:1px solid rgba(201,162,39,.3);padding:5px 10px;border-radius:8px;font-size:11px;cursor:pointer">
                ${f[r].icon} ${f[r].label}
              </button>
            `).join("")}
          </div>
        ...
        <section style="margin-bottom:24px">
          ...
          <ul style="list-style:disc;padding-left:24px;color:#a0a4c0;font-size:12px;line-height:1.6">
            ${s.next_steps.map(r=>`<li>${n(r)}</li>`).join("")}
          </ul>
        </section>
      `:`
  ...
  `,L(e),g.info("feature-self-diag",`rendered (report=${s?"yes":"no"}, lessons=${i.length}, sentinels=${t.length})`)}function L(e){e.querySelector("#ax-diag-run")?.addEventListener("click",()=>{v(e,!1)}),e.querySelector("#ax-diag-brutal")?.addEventListener("click",()=>{v(e,!0)}),e.querySelector("#ax-diag-export")?.addEventListener("click",()=>{(async()=>{if(c.tap(),!!s)try{const{apexSelfAudit:i}=await u(async()=>{const{apexSelfAudit:r}=await import("./apex-self-audit-D7zuzDfO.js");return{apexSelfAudit:r}},__vite__mapDeps([0,1,2,3,4,5,6]),import.meta.url),t=i.formatReportMarkdown(s),o=new Blob([t],{type:"text/markdown"}),a=URL.createObjectURL(o),d=document.createElement("a");d.href=a,d.download=`apex-audit-${new Date().toISOString().slice(0,10)}.md`,document.body.appendChild(d),d.click(),document.body.removeChild(d),URL.revokeObjectURL(a),p.success("Rapport exporté")}catch(i){g.warn("feature-self-diag","export failed",{err:i}),p.error("Export échoué")}})()}),e.querySelectorAll("[data-diag-sev]").forEach(i=>{i.addEventListener("click",()=>{c.selection(),x=i.dataset.diagSev,y(e)})})}async function v(e,i){if(!l){l=!0,c.medium(),p.info(i?"🔥 Audit brutal en cours...":"🔍 Audit en cours..."),y(e);try{const{apexSelfAudit:t}=await u(async()=>{const{apexSelfAudit:a}=await import("./apex-self-audit-D7zuzDfO.js");return{apexSelfAudit:a}},__vite__mapDeps([0,1,2,3,4,5,6]),import.meta.url),o=await t.runFullAudit(i);s=o,c.success(),p.success(`✅ Audit terminé : ${o.total_score}/100`)}catch(t){g.warn("feature-self-diag","audit failed",{err:t}),c.error(),p.error("Audit échoué")}finally{l=!1,y(e)}}}export{n as escapeHtml,_ as filterFindingsBySeverity,w as loadLessons,y as render,h as scoreColor,$ as scoreGrade};
//# sourceMappingURL=index-CF7mNv1r.js.map
