...
import{_ as l}from"./apex-kb-DEL_sHoX.js";import{a as n}from"./escape-html-DGIYNPKb.js";import{c as R}from"./listener-cleanup-Y2rGGxxX.js";import{l as P}from"./monitoring-DMtdadhB.js";import"./credential-patterns-CLzI061R.js";let o=null;function M(){o?.cleanup(),o=null}const p={anthropic:"🧠",openai:"🤖",groq:"⚡",gemini:"✨",mistral:"🌊",cohere:"🔷",xai:"🔥",perplexity:"🔍",deepseek:"🐋",openrouter:"🌐"};function f(t){return t<0?"—":t<1e3?`${Math.round(t)}ms`:`${(t/1e3).toFixed(1)}s`}function $(t){return t<0?"—":`${Math.round(t)}%`}function z(t){if(t===0)return"jamais";const a=Date.now()-t;return a<6e4?"à l'instant":a<36e5?`il y a ${Math.round(a/6e4)} min`:a<864e5?`il y a ${Math.round(a/36e5)} h`:`il y a ${Math.round(a/864e5)} j`}function w(t){return!t||t.last_ping_ts===0?'<span style="color:#666">⚪</span>':t.last_ping_ok?t.latency_avg_ms>3e3?'<span style="color:#f0c020">🟡</span>':'<span style="color:#22cc77">🟢</span>':'<span style="color:#ff5b5b">🔴</span>'}function v(t){return t>=80?"#22cc77":t>=60?"#c9a227":t>=40?"#f0c020":"#ff5b5b"}async function c(t){o?.cleanup(),o=R("smart-router");const{smartRouter:a}=await l(async()=>{const{smartRouter:e}=await import("./smart-router-BvxeHUox.js");return{smartRouter:e}},__vite__mapDeps([0,1,2]),import.meta.url),S=await a.rankProviders(),m=await a.getRecommendations(),u=a.getOverride(),i=S[0],g=a.getAllProviders(),s=i?await a.getStats(i.provider):null,y=await Promise.all(g.map(async e=>({provider:e,stats:await a.getStats(e),score:await a.scoreProvider(e)})));y.sort((e,r)=>r.score.total-e.score.total),t.innerHTML=`
    <div class="ax-page" style="padding:16px;max-width:1100px;margin:0 auto">
      ...
      ${u?`
        <div style="background:rgba(240,192,32,0.15);border:1px solid #c9a227;padding:10px 14px;border-radius:8px;margin-bottom:16px;display:flex;align-items:center;justify-content:space-between;gap:12px">
          <span>📌 <strong>Override admin actif</strong> : ${n(p[u]??"")} <strong>${n(u)}</strong> forcé pour tous les calls IA.</span>
        ...
        <div style="background:linear-gradient(135deg,rgba(34,204,119,0.15),rgba(20,20,35,0.5));padding:18px;border-radius:12px;border:2px solid ${v(i.score.total)};margin-bottom:18px">
          <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;margin-bottom:8px">
            <div>
              ...
              <h2 style="margin:4px 0;color:#fff;font-size:24px">${p[i.provider]} ${n(i.provider)}</h2>
            </div>
            <div style="text-align:right">
              ...
              <div style="font-size:11px;color:var(--ax-text-dim)">${w(s)} ${s?z(s.last_ping_ts):"pas de ping"}</div>
            </div>
          </div>
          <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:10px;margin-top:12px">
            <div style="background:rgba(0,0,0,0.3);padding:8px;border-radius:6px">
              ...
              <div style="font-size:10px;color:var(--ax-text-dim)">${s?f(s.latency_avg_ms):"—"} avg</div>
            </div>
            <div style="background:rgba(0,0,0,0.3);padding:8px;border-radius:6px">
              <div style="font-size:10px;color:var(--ax-text-dim)">💰 Quota (30%)</div>
              <div style="font-size:13px;color:#fff">${i.score.quota_pts}pts</div>
              <div style="font-size:10px;color:var(--ax-text-dim)">${s?$(s.quota_remaining_pct):"—"}</div>
            </div>
            <div style="background:rgba(0,0,0,0.3);padding:8px;border-radius:6px">
              ...
              <div style="font-size:10px;color:var(--ax-text-dim)">${s?Math.round(s.success_rate*100):0}% success</div>
            </div>
            <div style="background:rgba(0,0,0,0.3);padding:8px;border-radius:6px">
              ...
              <div style="font-size:10px;color:var(--ax-text-dim)">${s?Math.round(s.uptime_24h*100):0}% / 24h</div>
            </div>
          </div>
        </div>
          ...
          <ul style="margin:0;padding-left:20px;color:#fff;font-size:13px">
            ${m.map(e=>`
              <li style="margin-bottom:4px">
                Bascule ${p[e.from]} <strong>${n(e.from)}</strong> → ${p[e.to]} <strong>${n(e.to)}</strong>
                <span style="color:#22cc77">économie ${e.savings_pct}%</span>
                <div style="font-size:11px;color:var(--ax-text-dim)">${n(e.reason)}</div>
              </li>
            `).join("")}
          </ul>
        </div>
          ...
          <tbody>
            ${y.map(({provider:e,stats:r,score:x})=>`
              <tr style="border-bottom:1px solid rgba(255,255,255,0.05)${e===i?.provider?";background:rgba(34,204,119,0.05)":""}">
                ...
                <td style="padding:10px;font-size:13px"><strong>${p[e]} ${n(e)}</strong></td>
  ...
  `;const d=t.querySelector("#ax-sr-retest");d&&o&&o.bind(d,"click",()=>{(async()=>{const{toast:e}=await l(async()=>{const{toast:r}=await import("./toast-CRdbcLoc.js");return{toast:r}},[],import.meta.url);d.disabled=!0,d.textContent="⏳ Test en cours…",e.info("Ping de 10 providers en parallèle…");try{await a.pingAllProviders(),e.success("✅ Tous les providers re-testés"),await c(t)}catch(r){e.error("Erreur ping: "+(r instanceof Error?r.message:String(r))),d.disabled=!1,d.textContent="🔄 Re-tester tous maintenant"}})()});const b=t.querySelector("#ax-sr-refresh");b&&o&&o.bind(b,"click",()=>{c(t)});const h=t.querySelector("#ax-sr-reset");h&&o&&o.bind(h,"click",()=>{(async()=>{const{toast:e}=await l(async()=>{const{toast:r}=await import("./toast-CRdbcLoc.js");return{toast:r}},[],import.meta.url);typeof confirm=="function"&&!confirm("Reset toutes les stats Smart Router ? (samples + scores + override)")||(a.resetAll(),e.success("Stats reset"),await c(t))})()});const _=t.querySelector("#ax-sr-clear-override");_&&o&&o.bind(_,"click",()=>{(async()=>{const{toast:e}=await l(async()=>{const{toast:r}=await import("./toast-CRdbcLoc.js");return{toast:r}},[],import.meta.url);a.setOverride(null),e.success("Override retiré — auto-routing actif"),await c(t)})()}),t.querySelectorAll(".ax-sr-force").forEach(e=>{o.bind(e,"click",()=>{(async()=>{const r=e.dataset.provider;if(!r)return;const{toast:x}=await l(async()=>{const{toast:k}=await import("./toast-CRdbcLoc.js");return{toast:k}},[],import.meta.url);a.setOverride(r),x.success(`📌 ${r} forcé pour tous les calls`),await c(t)})()})}),P.info("feature-smart-router",`rendered ${g.length} providers, best=${i?.provider??"none"}`)}export{M as dispose,c as render};
