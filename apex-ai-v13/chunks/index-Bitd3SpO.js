const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./kdmc-projects-registry-sRmoyobO.js","./multi-source-analyze-DAvdcAQw.js"])))=>i.map(i=>d[i]);
import{_ as g}from"./apex-kb-BFHUv3jX.js";import{l as u}from"./monitoring-BAiQJoxJ.js";import{c as b}from"./listener-cleanup-Y2rGGxxX.js";import{s as m}from"../core/main-CfzzJEKO.js";import{h}from"./haptic-BUEqXK0N.js";import"./apex-tools-registry-DqA3gZXO.js";import"./credential-patterns-Dy6Wjk7e.js";import"./multi-source-analyze-DAvdcAQw.js";let d=null;function N(){d?.cleanup(),d=null}function i(t){return t.replace(/[&<>"']/g,r=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[r]??r)}const y=[{id:"chat",icon:"💬",label:"Chat",description:"Conversation IA",route:"chat",color:"#5aa8ff"},{id:"vault",icon:"🔐",label:"Coffre",description:"Clés API & secrets",route:"vault",color:"#c9a227"},{id:"browser",icon:"🌐",label:"Browser",description:"Naviguer & embed",route:"browser",color:"#22cc77"},{id:"studios",icon:"🎨",label:"Studios",description:"Créatif (musique/vidéo)",route:"studios",color:"#a878ff"},{id:"pro",icon:"🎓",label:"Pro",description:"Modules expert",route:"pro",color:"#ff6b9d"},{id:"self-diag",icon:"🩺",label:"Audit",description:"Auto-diagnostic",route:"self-diag",color:"#38d8c8"},{id:"settings",icon:"⚙️",label:"Réglages",description:"Configurer Apex",route:"settings",color:"#a0a4c0"},{id:"rgpd",icon:"🛡",label:"RGPD",description:"Mes données",route:"rgpd",color:"#e8b830"}];async function v(){let t=0,r=0,o=0,a=0,e=0,c=0;try{const l=localStorage.getItem("apex_v13_messages_24h")??"0",n=parseInt(l,10);Number.isNaN(n)||(t=n)}catch{}try{const l=localStorage.getItem("apex_v13_tokens_24h")??"0",n=parseInt(l,10);Number.isNaN(n)||(r=n)}catch{}try{const{kdmcProjectsRegistry:l}=await g(async()=>{const{kdmcProjectsRegistry:n}=await import("./kdmc-projects-registry-sRmoyobO.js");return{kdmcProjectsRegistry:n}},__vite__mapDeps([0]),import.meta.url);o=l.countActive()}catch{}try{const{sentinels:l}=await g(async()=>{const{sentinels:s}=await import("./sentinels-CXmhhPX0.js");return{sentinels:s}},[],import.meta.url),n=l.list();e=n.length,a=n.filter(s=>s.lastResult?.ok).length}catch{}try{const l=localStorage.getItem("ax_claude_todo")??"[]",n=JSON.parse(l);Array.isArray(n)&&(c=n.filter(s=>s.status==="pending").length)}catch{}return[{id:"messages",icon:"💬",label:"Messages 24h",value:t,color:"#5aa8ff",route:"chat"},{id:"tokens",icon:"🔢",label:"Tokens 24h",value:r.toLocaleString("fr-FR"),color:"#c9a227",route:"self-diag"},{id:"projects",icon:"📦",label:"Projets actifs",value:o,color:"#22cc77",route:"admin"},{id:"sentinels",icon:"🛡",label:"Sentinelles OK",value:`${a}/${e}`,color:a===e?"#22cc77":"#ffaa00",route:"sentinels"},{id:"todos",icon:"📋",label:"Todos en attente",value:c,color:c>0?"#ff5858":"#22cc77",route:"self-diag"}]}async function w(){const t=[];try{const{sentinels:r}=await g(async()=>{const{sentinels:e}=await import("./sentinels-CXmhhPX0.js");return{sentinels:e}},[],import.meta.url),a=r.list().filter(e=>e.lastResult&&!e.lastResult.ok);for(const e of a.slice(0,3))t.push({id:`sentinel_${e.id}`,level:"warn",title:`Sentinelle ${e.name}`,description:e.lastResult?.msg??"KO",ts:e.lastResult?.ts??Date.now(),action_route:"sentinels"})}catch{}try{const r=JSON.parse(localStorage.getItem("apex_v13_credentials_expiring")??"[]");if(Array.isArray(r))for(const o of r.slice(0,3))o.days_left<30&&t.push({id:`cred_${o.service}`,level:o.days_left<7?"error":"warn",title:`Credential ${o.service}`,description:`Expire dans ${o.days_left} jours`,ts:Date.now(),action_route:"vault"})}catch{}return t}async function k(){try{const{multiKeyVault:t}=await g(async()=>{const{multiKeyVault:a}=await import("./multi-key-vault-2r8FkVmV.js");return{multiKeyVault:a}},[],import.meta.url),r=t.getKnownServices(),o=[];for(const a of r){const e=t.getStats(a),c=t.getServiceLight(a);o.push({service:a,light:c,totalKeys:e.total,activeKeys:e.active,failingKeys:e.failing,invalidKeys:e.invalid,lastSuccess:e.lastSuccess})}return o}catch{return[]}}function $(){const t=[];try{const r=localStorage.getItem("ax_claude_todo")??"[]",o=JSON.parse(r);if(Array.isArray(o))for(const a of o.filter(e=>e.status==="pending").slice(0,5))t.push({id:a.id??`todo_${Date.now()}`,source:"apex_todo",title:a.reason??"Todo sans description",severity:a.severity??"medium",ts_created:a.ts??Date.now()})}catch{}return t}function z(t,r=0){return`
    ...
    </button>`}function _(t,r=0){return`
    <button class="ax-shortcut-card ax-modernized-card ax-bounce-tap" data-route="${i(t.route)}"
    ...
    </button>`}function S(t){return t.length===0?`
      <div style="display:flex;align-items:center;gap:12px;padding:16px;background:linear-gradient(135deg,rgba(34,204,119,0.08),rgba(34,204,119,0.03));border:1px solid rgba(34,204,119,0.15);border-radius:12px">
        ...
        </div>`}).join("")}function A(t){return t.length===0?`
      <div style="display:flex;align-items:center;gap:12px;padding:16px;background:linear-gradient(135deg,rgba(34,204,119,0.08),rgba(34,204,119,0.03));border:1px solid rgba(34,204,119,0.15);border-radius:12px">
        <span style="font-size:20px;filter:drop-shadow(0 2px 6px rgba(34,204,119,0.4))">🎉</span>
        ...
        </div>`}).join("")}function R(t,r={}){if(t.length===0)return`
      ...
      </div>`;const o={green:"#22cc77",yellow:"#ffaa00",red:"#ff5b5b",gray:"#666b80"},a={green:"OK",yellow:"Partiel",red:"Panne",gray:"Non testé"};return t.map((e,c)=>{const l=o[e.light],n=a[e.light],s=`${e.activeKeys}/${e.totalKeys} active${e.failingKeys>0?` · ${e.failingKeys} failing`:""}${e.invalidKeys>0?` · ${e.invalidKeys} invalid`:""}`,p=r[e.service]??{recharge:null,usage:null},x=p.recharge&&(e.light==="yellow"||e.light==="red"||e.light==="gray")&&p.recharge?`<a class="ax-recharge-btn" href="${i(p.recharge)}" target="_blank" rel="noopener noreferrer"
...
>📊 Usage</a>`:"";return`
        <div class="ax-service-health-row ax-modernized-card"
          ...
          ${x}
          ${f}
        </div>`}).join("")}async function K(t){const r={};if(t.length===0)return r;try{const{linksRegistry:o}=await g(async()=>{const{linksRegistry:a}=await import("./multi-source-analyze-DAvdcAQw.js").then(e=>e.c);return{linksRegistry:a}},__vite__mapDeps([1]),import.meta.url);for(const a of t)r[a]={recharge:o.getRechargeLink(a),usage:o.getUsageLink(a),apiKeys:o.getApiKeysLink(a)}}catch{}return r}async function V(t){d?.cleanup(),d=b("dashboard");const r=m.get("user"),o=r?.name?`Bonjour ${r.name}`:"Bonjour",[a,e,c]=await Promise.all([v(),w(),k()]),l=await K(c.map(s=>s.service)),n=$();t.innerHTML=`
    <style>
  ...
  `,t.querySelectorAll("[data-route]").forEach(s=>{d.bind(s,"click",()=>{h.tap();const p=s.dataset.route;p&&(window.location.hash="#"+p)})}),u.info("feature-dashboard",`rendered (${a.length} kpis, ${e.length} alerts, ${n.length} todos)`)}export{v as computeKpis,N as dispose,i as escapeHtml,w as loadAlerts,K as loadRechargeLinks,k as loadServiceHealth,$ as loadTodos,V as render,R as renderServiceHealthCard};
