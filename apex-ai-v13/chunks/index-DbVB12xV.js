...
>📊 Usage</a>`:"";return`
        <div class="ax-service-health-row ax-modernized-card"
          ...
          ${x}
          ${f}
        </div>`}).join("")}async function K(t){const r={};if(t.length===0)return r;try{const{linksRegistry:o}=await g(async()=>{const{linksRegistry:a}=await import("./multi-source-analyze-DzTKkB_U.js").then(e=>e.c);return{linksRegistry:a}},__vite__mapDeps([1]),import.meta.url);for(const a of t)r[a]={recharge:o.getRechargeLink(a),usage:o.getUsageLink(a),apiKeys:o.getApiKeysLink(a)}}catch{}return r}async function V(t){d?.cleanup(),d=b("dashboard");const r=m.get("user"),o=r?.name?`Bonjour ${r.name}`:"Bonjour",[a,e,c]=await Promise.all([v(),w(),k()]),l=await K(c.map(s=>s.service)),n=$();t.innerHTML=`
    <style>
