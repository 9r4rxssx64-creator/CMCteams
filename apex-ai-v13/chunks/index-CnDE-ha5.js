const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./toast-Dgg9rcIP.js","./haptic-BUEqXK0N.js","./persistent-memory-store-Da1_S5iA.js","./consumption-anomaly-detector-DwbpVIMy.js","./multi-source-analyze-DAvdcAQw.js","./push-notifications-Con0RoNe.js","./tokens-dashboard-C5ZzZyK6.js","./auth-UNe5Lub3.js","../core/main-BNLrwE3Y.js","../assets/css/main-CYtYG7GU.css"])))=>i.map(i=>d[i]);
import{_ as l}from"./apex-kb-BFHUv3jX.js";import{l as S}from"./monitoring-BAiQJoxJ.js";import{c as R}from"./listener-cleanup-Y2rGGxxX.js";import{s as z}from"../core/main-BNLrwE3Y.js";import"./apex-tools-registry-DqA3gZXO.js";import"./credential-patterns-Dy6Wjk7e.js";import"./multi-source-analyze-DAvdcAQw.js";let n=null;function M(){n?.cleanup(),n=null}function b(r){return r.replace(/[&<>"']/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[m]??m)}const T="apex_v13_chat_auto_read";function P(){try{return localStorage.getItem(T)==="1"}catch{return!1}}function I(r){try{localStorage.setItem(T,r?"1":"0")}catch{}}async function O(r){try{const m=await l(()=>import("./voice-DZEsA05F.js").then(t=>t.b),[],import.meta.url),{listVoices:w,getActiveVoice:c,setActiveVoice:d,speak:p,stopAll:f}=m,y=r.querySelector("#ax-settings-auto-read"),_=r.querySelector("#ax-voice-current"),x=r.querySelector("#ax-voice-list"),h=r.querySelectorAll(".ax-voice-cat-btn");if(!x)return;y&&(y.checked=P(),n.bind(y,"change",()=>{I(y.checked),(async()=>{const{toast:t}=await l(async()=>{const{toast:a}=await import("./toast-Dgg9rcIP.js");return{toast:a}},__vite__mapDeps([0,1]),import.meta.url);t.success(y.checked?"Lecture auto activée":"Lecture auto désactivée")})()}));const k=()=>{if(!_)return;const t=c(),i=w().find(o=>o.id===t);_.textContent=i?`Voix active : ${i.emoji??"🔊"} ${i.name} (${i.category})`:`Voix active : ${t}`};k();const v=t=>{const a=w(),i=t==="all"?a:a.filter(e=>e.category===t),o=c();x.innerHTML=i.map(e=>{const s=e.id===o,u=e.emoji??(e.category==="pro"?"🎙️":e.category==="fun"?"🎉":"🎨"),g=e.description?b(e.description):"",$=s?"background:rgba(232,184,48,0.15);border-color:rgba(232,184,48,0.45)":"background:rgba(255,255,255,0.03);border-color:rgba(255,255,255,0.06)";return`
  ...
  `,(async()=>{try{const{memoryBridge:t}=await l(async()=>{const{memoryBridge:e}=await import("./memory-bridge-CKmiKdQJ.js");return{memoryBridge:e}},__vite__mapDeps([2]),import.meta.url),a=r.querySelector("#ax-memory-bridge-status"),i=r.querySelector("#ax-memory-bridge-sync"),o=()=>{if(!a)return;const e=t.getHealth(),s=t.getStatus(),u=s.filter(g=>g.last_success).length;a.textContent=`${e.backends_configured} backends configurés · ${u}/${s.length} dernier sync OK`};o(),i&&n&&n.bind(i,"click",()=>{(async()=>{i&&(i.disabled=!0);const e=await t.runAutoSync(),s=e.filter(g=>g.ok).length,{toast:u}=await l(async()=>{const{toast:g}=await import("./toast-Dgg9rcIP.js");return{toast:g}},__vite__mapDeps([0,1]),import.meta.url);e.length===0?u.warn("Aucun backend configuré"):s===e.length?u.success(`Sync OK (${s}/${e.length})`):u.warn(`Sync partielle (${s}/${e.length})`),o(),i&&(i.disabled=!1)})()})}catch(t){S.warn("feature-settings","memory-bridge wire failed",{err:t})}})();const y=r.querySelector("#ax-conso-scan");y&&n&&n.bind(y,"click",()=>{(async()=>{try{const{consumptionAnomalyDetector:t}=await l(async()=>{const{consumptionAnomalyDetector:o}=await import("./consumption-anomaly-detector-DwbpVIMy.js");return{consumptionAnomalyDetector:o}},__vite__mapDeps([3,4,5,6]),import.meta.url),a=t.scanAllVerbose(),i=r.querySelector("#ax-conso-results");if(!i)return;i.innerHTML=a.map(o=>{const e=o.severity==="critical"?"#ff4444":o.severity==="high"?"#ff8844":o.severity==="medium"?"#ffaa00":o.severity==="low"?"#88aaff":"#22cc77",s=o.severity==="critical"?"🚨":o.severity==="high"?"⚠️":o.severity==="medium"?"🟡":o.severity==="low"?"🔵":"✅";return`<div style="background:rgba(255,255,255,0.03);border-left:3px solid ${e};padding:8px 12px;margin-top:6px;border-radius:4px">
          ...
          </div>`}).join("")}catch(t){S.warn("feature-settings","conso scan failed",{err:t})}})()}),r.querySelectorAll("[data-nav-route]").forEach(t=>{n.bind(t,"click",()=>{const a=t.getAttribute("data-nav-route");a&&(location.hash="#"+a)})});const _=r.querySelector("#ax-settings-logout");_&&n&&n.bind(_,"click",()=>{(async()=>{const{auth:t}=await l(async()=>{const{auth:a}=await import("./auth-UNe5Lub3.js");return{auth:a}},__vite__mapDeps([7,4,8,9]),import.meta.url);t.logout(),location.hash="#login"})()});const x=r.querySelector("#ax-settings-tools-auto-embed");if(x&&n){try{const t=JSON.parse(localStorage.getItem("ax_settings")??"{}");x.checked=t.tools_auto_embed!==!1}catch{x.checked=!0}n.bind(x,"change",()=>{try{const t=JSON.parse(localStorage.getItem("ax_settings")??"{}");t.tools_auto_embed=x.checked,localStorage.setItem("ax_settings",JSON.stringify(t))}catch{}})}const h=r.querySelector("#ax-force-update-btn"),k=r.querySelector("#ax-force-update-status");h&&n&&n.bind(h,"click",()=>{(async()=>{const t=a=>{k&&(k.textContent=a)};h.disabled=!0,h.textContent="⏳ Reset en cours…";try{if(t("🔍 Désinstallation Service Workers…"),"serviceWorker"in navigator){const i=await navigator.serviceWorker.getRegistrations();for(const o of i)await o.unregister();t(`✅ ${i.length} SW désinstallés`)}if(t("🔍 Vidage caches PWA…"),"caches"in window){const i=await caches.keys();for(const o of i)await caches.delete(o);t(`✅ ${i.length} caches vidés`)}t("✅ Reset terminé. Rechargement dans 2s…");const{toast:a}=await l(async()=>{const{toast:i}=await import("./toast-Dgg9rcIP.js");return{toast:i}},__vite__mapDeps([0,1]),import.meta.url);a.info("🔄 Reset OK — reload imminent"),setTimeout(()=>{location.href=location.pathname+"?_forceupd=1&_reset="+Date.now()},2e3)}catch(a){t(`❌ Erreur : ${String(a)}`),h.disabled=!1,h.textContent="🔄 Force reset PWA + reload"}})()}),O(r);const v=r.querySelector("#ax-settings-notif-test");v&&n&&n.bind(v,"click",()=>{(async()=>{try{if("Notification"in window&&Notification.permission==="granted")new Notification("Test Apex",{body:"Si tu vois ça, push notif fonctionne ✅"});else if("Notification"in window)if(await Notification.requestPermission()==="granted")new Notification("Test Apex",{body:"Push activé ✅"});else{const{toast:a}=await l(async()=>{const{toast:i}=await import("./toast-Dgg9rcIP.js");return{toast:i}},__vite__mapDeps([0,1]),import.meta.url);a.warn("Permission notifications refusée")}else{const{toast:t}=await l(async()=>{const{toast:a}=await import("./toast-Dgg9rcIP.js");return{toast:a}},__vite__mapDeps([0,1]),import.meta.url);t.warn("Notifications non supportées par ce navigateur")}}catch{const{toast:t}=await l(async()=>{const{toast:a}=await import("./toast-Dgg9rcIP.js");return{toast:a}},__vite__mapDeps([0,1]),import.meta.url);t.warn("Test notification échoué")}})()}),S.info("feature-settings","rendered")}export{M as dispose,N as render,O as wireVoiceSection};
