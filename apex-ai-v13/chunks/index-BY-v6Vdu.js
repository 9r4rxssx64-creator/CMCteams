const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./monitoring-3uBGKGRH.js","./apex-kb-D2Qc1YTF.js","./credential-patterns-CLzI061R.js","./multi-source-analyze-DFKV4H7E.js","./auth-D6bzoH26.js","../assets/css/main-DHnkOnRu.css"])))=>i.map(i=>d[i]);
import{_ as p}from"./apex-kb-D2Qc1YTF.js";import{e as u}from"./escape-html-BlQj2yEF.js";import{c as E}from"./listener-cleanup-Y2rGGxxX.js";import{l as T}from"./monitoring-3uBGKGRH.js";import{s as I}from"../core/main-kmY4gk5m.js";import{c as O}from"./csp-style-helper-BisGRi53.js";import{r as D}from"./recharge-action-C73aOkPx.js";import"./credential-patterns-CLzI061R.js";import"./multi-source-analyze-DFKV4H7E.js";let i=null;function U(){i?.cleanup(),i=null}const P="apex_v13_chat_auto_read";function V(){try{return localStorage.getItem(P)==="1"}catch{return!1}}function q(n){try{localStorage.setItem(P,n?"1":"0")}catch{}}async function R(n){try{const k=await p(()=>import("./voice-jGBLrHNL.js").then(l=>l.e),__vite__mapDeps([0,1,2]),import.meta.url),{listVoices:S,getActiveVoice:x,setActiveVoice:m,speak:f,stopAll:g}=k,w=n.querySelector("#ax-settings-auto-read"),z=n.querySelector("#ax-voice-current"),_=n.querySelector("#ax-voice-list"),L=n.querySelectorAll(".ax-voice-cat-btn");if(!_)return;w&&(w.checked=V(),i.bind(w,"change",()=>{q(w.checked),(async()=>{const{toast:l}=await p(async()=>{const{toast:d}=await import("./toast-CRdbcLoc.js");return{toast:d}},[],import.meta.url);l.success(w.checked?"Lecture auto activée":"Lecture auto désactivée")})()}));const A=()=>{if(!z)return;const l=x(),b=S().find(h=>h.id===l);z.textContent=b?`Voix active : ${b.emoji??"🔊"} ${b.name} (${b.category})`:`Voix active : ${l}`};A();const $=l=>{const d=S(),b=l==="all"?d:d.filter(t=>t.category===l),h=x();_.setAttribute("aria-busy","false"),_.innerHTML=b.map(t=>{const r=t.id===h,a=t.emoji??(t.category==="pro"?"🎙️":t.category==="fun"?"🎉":"🎨"),e=t.description?u(t.description):"",o=r?"background:rgba(232,184,48,0.15);border-color:rgba(232,184,48,0.45)":"background:rgba(255,255,255,0.03);border-color:rgba(255,255,255,0.06)";return`
            <div class="ax-voice-item" data-voice-id="${u(t.id)}" style="display:flex;align-items:center;gap:8px;padding:10px;margin-bottom:6px;border:1px solid;border-radius:8px;${o}">
              <span style="font-size:18px">${a}</span>
              <div style="flex:1;min-width:0">
                <div style="color:#fff;font-size:13px;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${u(t.name)}${r?' <span style="color:var(--ax-gold);font-size:11px">★ active</span>':""}</div>
  ...
  `),(async()=>{try{const{memoryBridge:t}=await p(async()=>{const{memoryBridge:o}=await import("./memory-bridge-DAhcUm3T.js");return{memoryBridge:o}},__vite__mapDeps([1,0,2]),import.meta.url),r=n.querySelector("#ax-memory-bridge-status"),a=n.querySelector("#ax-memory-bridge-sync"),e=()=>{if(!r)return;const o=t.getHealth(),c=t.getStatus(),s=c.filter(v=>v.last_success).length;r.textContent=`${o.backends_configured} backends configurés · ${s}/${c.length} dernier sync OK`};e(),a&&i&&i.bind(a,"click",()=>{(async()=>{a&&(a.disabled=!0);const o=await t.runAutoSync(),c=o.filter(v=>v.ok).length,{toast:s}=await p(async()=>{const{toast:v}=await import("./toast-CRdbcLoc.js");return{toast:v}},[],import.meta.url);o.length===0?s.warn("Aucun backend configuré"):c===o.length?s.success(`Sync OK (${c}/${o.length})`):s.warn(`Sync partielle (${c}/${o.length})`),e(),a&&(a.disabled=!1)})()})}catch(t){T.warn("feature-settings","memory-bridge wire failed",{err:t})}})();const w=n.querySelector("#ax-conso-scan");w&&i&&i.bind(w,"click",()=>{(async()=>{try{const{consumptionAnomalyDetector:t}=await p(async()=>{const{consumptionAnomalyDetector:e}=await import("./consumption-anomaly-detector-BURrdc5z.js");return{consumptionAnomalyDetector:e}},__vite__mapDeps([0,1,2,3]),import.meta.url),r=t.scanAllVerbose(),a=n.querySelector("#ax-conso-results");if(!a)return;a.innerHTML=r.map(e=>{const o=e.severity==="critical"?"var(--ax-sev-critical)":e.severity==="high"?"var(--ax-sev-high)":e.severity==="medium"?"var(--ax-sev-medium)":e.severity==="low"?"var(--ax-sev-low)":"var(--ax-green)",c=e.severity==="critical"?"ax-sev-critical":e.severity==="high"?"ax-sev-high":e.severity==="medium"?"ax-sev-medium":(e.severity==="low","ax-sev-low"),s=e.severity==="critical"?"🚨":e.severity==="high"?"⚠️":e.severity==="medium"?"🟡":e.severity==="low"?"🔵":"✅";return`<div style="background:rgba(255,255,255,0.03);border-left:3px solid ${o};padding:8px 12px;margin-top:6px;border-radius:4px">
          ...
          </div>`}).join("")}catch(t){T.warn("feature-settings","conso scan failed",{err:t})}})()});const z=n.querySelector("#ax-zoom-inspector-btn");z&&i&&i.bind(z,"click",()=>{(async()=>{const{apexZoomInspector:t}=await p(async()=>{const{apexZoomInspector:r}=await import("./apex-zoom-inspector-CyIPo8Da.js");return{apexZoomInspector:r}},__vite__mapDeps([1,0,2]),import.meta.url);t.isVisible()?t.hide():t.show()})()});const _=n.querySelector("#ax-functional-test-btn");_&&i&&i.bind(_,"click",()=>{(async()=>{const t=n.querySelector("#ax-functional-test-results");if(t){t.innerHTML=`<div style="color:var(--ax-blue)">⏳ Test des boutons en cours (jusqu'à 40 boutons, ~30s)...</div>`;try{const{apexFunctionalTester:r}=await p(async()=>{const{apexFunctionalTester:y}=await import("./apex-functional-tester-BEnyiyeG.js");return{apexFunctionalTester:y}},__vite__mapDeps([1,0,2]),import.meta.url),{reportsHistory:a}=await p(async()=>{const{reportsHistory:y}=await import("./apex-reports-history-Cv_X_jWe.js");return{reportsHistory:y}},__vite__mapDeps([0]),import.meta.url),e=await r.testAndAutoFix({maxButtons:30});a.recordFunctional(e.before,e.fixes,e.after,e.improvement);const o=e.before.tested>0?Math.round(e.before.ok/e.before.tested*100):0,c=e.after?` → après fix : ${Math.round(e.after.ok/Math.max(1,e.after.tested)*100)}% OK (${e.improvement>0?"+":""}${Math.round(e.improvement*100)}%)`:"",s=e.before.details.filter(y=>y.status==="no_response"||y.status==="error").slice(0,5).map(y=>`<li style="color:var(--ax-sev-high);font-size:11px">${u(y.label||"(no label)")} → ${u(y.status)}</li>`).join(""),v=e.fixes.applied.map(y=>u(String(y))).join(", ");t.innerHTML=`
          ...
          `}catch(r){const a=r instanceof Error?r.message:String(r);t.innerHTML=`<div style="color:var(--ax-error)">❌ Erreur test : ${u(a)}</div>`}}})()});const L=n.querySelector("#ax-layout-inspect-btn");L&&i&&i.bind(L,"click",()=>{(async()=>{const t=n.querySelector("#ax-layout-inspect-results");if(t){t.innerHTML='<div style="color:var(--ax-purple)">⏳ Scan layout...</div>';try{const{apexLayoutInspector:r}=await p(async()=>{const{apexLayoutInspector:s}=await import("./apex-layout-inspector-C5TNhGYE.js");return{apexLayoutInspector:s}},__vite__mapDeps([1,0,2]),import.meta.url),{reportsHistory:a}=await p(async()=>{const{reportsHistory:s}=await import("./apex-reports-history-Cv_X_jWe.js");return{reportsHistory:s}},__vite__mapDeps([0]),import.meta.url),e=r.scanDom();a.recordLayout(e);const o=e.hiddenButtons.slice(0,5).map(s=>`<li style="color:var(--ax-sev-high);font-size:11px">"${s.label}" → ${s.reason}</li>`).join(""),c=e.overflowingElements.slice(0,5).map(s=>`<li style="color:var(--ax-error);font-size:11px">${s.tag} (+${s.overflowBy}px)</li>`).join("");t.innerHTML=`
            <div style="background:rgba(180,90,200,0.08);border:1px solid rgba(180,90,200,0.3);border-radius:8px;padding:10px;color:#fff;font-size:12px">
              ...
              ${o?`<div style="margin-top:6px;color:var(--ax-gold-deep)">Boutons cachés:</div><ul style="margin:2px 0 0 16px;padding:0">${o}</ul>`:""}
              ${c?`<div style="margin-top:6px;color:var(--ax-gold-deep)">Éléments overflow:</div><ul style="margin:2px 0 0 16px;padding:0">${c}</ul>`:""}
              <div style="margin-top:8px;font-size:11px;color:rgba(255,255,255,0.5)">→ Historique complet dans Admin (Apex Audits Live)</div>
            </div>
          `}catch(r){const a=r instanceof Error?r.message:String(r);t.innerHTML=`<div style="color:var(--ax-error)">❌ Erreur scan : ${u(a)}</div>`}}})()});const A=n.querySelector("#ax-cf-diagnostic-btn");A&&i&&i.bind(A,"click",()=>{(async()=>{const t=n.querySelector("#ax-cf-diagnostic-results");if(t){t.innerHTML='<div style="color:var(--ax-warning)">⏳ Test Cloudflare API en cours...</div>';try{const{apexCloudflareVaultDeploy:r}=await p(async()=>{const{apexCloudflareVaultDeploy:c}=await import("./apex-cloudflare-vault-deploy-C3W_pywN.js");return{apexCloudflareVaultDeploy:c}},__vite__mapDeps([1,0,2]),import.meta.url),a=await r.runDiagnostic(),e=(c,s,v)=>`
