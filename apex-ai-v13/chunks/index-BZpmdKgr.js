...
import{_ as g}from"./apex-kb-C132uqD8.js";import{a as i}from"./escape-html-DGIYNPKb.js";import{c as b}from"./listener-cleanup-Y2rGGxxX.js";import{C as v,q as f}from"./monitoring-BkJ_WZbY.js";import{c as m}from"./csp-style-helper-BisGRi53.js";import{g as h}from"./apex-tools-dispatch-core-BHo3mwX7.js";import{haptic as y}from"./haptic-CQFg2PXZ.js";import{r as w}from"./recharge-action-ZBY-wwbv.js";import"./credential-patterns-CLzI061R.js";import"./multi-source-analyze-DCjiC-qU.js";import"./apex-tools-dispatch-skills-sOPz5fsK.js";import"./apex-tools-dispatch-data-hMaZjSXL.js";import"./apex-tools-dispatch-finance-DoRAfEZC.js";import"./apex-tools-dispatch-misc-DoPKpcyI.js";import"./apex-tools-misc-C4sFgDRb.js";import"./apex-tools-registry-core-CQvgkOQw.js";import"./apex-tools-registry-skills-DRX4PPQ_.js";let p=null;function X(){p?.cleanup(),p=null}const $=[{id:"chat",icon:"💬",label:"Chat",description:"Conversation IA",route:"chat",color:"var(--ax-blue-bright)"},{id:"vault",icon:"🔐",label:"Coffre",description:"Clés API & secrets",route:"vault",color:"var(--ax-gold-deep)"},{id:"browser",icon:"🌐",label:"Browser",description:"Naviguer & embed",route:"browser",color:"var(--ax-green)"},{id:"studios",icon:"🎨",label:"Studios",description:"Créatif (musique/vidéo)",route:"studios",color:"var(--ax-purple)"},{id:"pro",icon:"🎓",label:"Pro",description:"Modules expert",route:"pro",color:"var(--ax-pink)"},{id:"self-diag",icon:"🩺",label:"Audit",description:"Auto-diagnostic",route:"self-diag",color:"var(--ax-cyan)"},{id:"settings",icon:"⚙️",label:"Réglages",description:"Configurer Apex",route:"settings",color:"var(--ax-text-dim)"},{id:"rgpd",icon:"🛡",label:"RGPD",description:"Mes données",route:"rgpd",color:"var(--ax-gold)"}];async function k(){let t=0,a=0,s=0,r=0,e=0,c=0;try{const l=localStorage.getItem("apex_v13_messages_24h")??"0",o=parseInt(l,10);Number.isNaN(o)||(t=o)}catch{}try{const l=localStorage.getItem("apex_v13_tokens_24h")??"0",o=parseInt(l,10);Number.isNaN(o)||(a=o)}catch{}try{const{kdmcProjectsRegistry:l}=await g(async()=>{const{kdmcProjectsRegistry:o}=await import("./kdmc-projects-registry-DPDrWNoc.js");return{kdmcProjectsRegistry:o}},__vite__mapDeps([0,1,2,3]),import.meta.url);s=l.countActive()}catch{}try{const{sentinels:l}=await g(async()=>{const{sentinels:n}=await import("./sentinels-xKTteFgY.js");return{sentinels:n}},__vite__mapDeps([2,0,1,3]),import.meta.url),o=l.list();e=o.length,r=o.filter(n=>n.lastResult?.ok).length}catch{}try{const l=localStorage.getItem("ax_claude_todo")??"[]",o=JSON.parse(l);Array.isArray(o)&&(c=o.filter(n=>n.status==="pending").length)}catch{}return[{id:"messages",icon:"💬",label:"Messages 24h",value:t,color:"var(--ax-blue-bright)",route:"chat"},{id:"tokens",icon:"🔢",label:"Tokens 24h",value:a.toLocaleString("fr-FR"),color:"var(--ax-gold-deep)",route:"self-diag"},{id:"projects",icon:"📦",label:"Projets actifs",value:s,color:"var(--ax-green)",route:"admin"},{id:"sentinels",icon:"🛡",label:"Sentinelles OK",value:`${r}/${e}`,color:r===e?"var(--ax-green)":"var(--ax-warning)",route:"sentinels"},{id:"todos",icon:"📋",label:"Todos en attente",value:c,color:c>0?"var(--ax-error)":"var(--ax-green)",route:"self-diag"}]}async function _(){const t=[];try{const{sentinels:a}=await g(async()=>{const{sentinels:e}=await import("./sentinels-xKTteFgY.js");return{sentinels:e}},__vite__mapDeps([2,0,1,3]),import.meta.url),r=a.list().filter(e=>e.lastResult&&!e.lastResult.ok);for(const e of r.slice(0,3))t.push({id:`sentinel_${e.id}`,level:"warn",title:`Sentinelle ${e.name}`,description:e.lastResult?.msg??"KO",ts:e.lastResult?.ts??Date.now(),action_route:"sentinels"})}catch{}try{const a=JSON.parse(localStorage.getItem("apex_v13_credentials_expiring")??"[]");if(Array.isArray(a))for(const s of a.slice(0,3))s.days_left<30&&t.push({id:`cred_${s.service}`,level:s.days_left<7?"error":"warn",title:`Credential ${s.service}`,description:`Expire dans ${s.days_left} jours`,ts:Date.now(),action_route:"vault"})}catch{}return t}async function z(){try{const{multiKeyVault:t}=await g(async()=>{const{multiKeyVault:r}=await import("./multi-key-vault-D3tnHGw6.js");return{multiKeyVault:r}},__vite__mapDeps([2,0,1,3]),import.meta.url),a=t.getKnownServices(),s=[];for(const r of a){const e=t.getStats(r),c=t.getServiceLight(r);s.push({service:r,light:c,totalKeys:e.total,activeKeys:e.active,failingKeys:e.failing,invalidKeys:e.invalid,lastSuccess:e.lastSuccess})}return s}catch{return[]}}function S(){const t=[];try{const a=localStorage.getItem("ax_claude_todo")??"[]",s=JSON.parse(a);if(Array.isArray(s))for(const r of s.filter(e=>e.status==="pending").slice(0,5))t.push({id:r.id??`todo_${Date.now()}`,source:"apex_todo",title:r.reason??"Todo sans description",severity:r.severity??"medium",ts_created:r.ts??Date.now()})}catch{}return t}function A(t,a=0){return`
    ...
    <button class="ax-shortcut-card ax-modernized-card ax-bounce-tap" data-route="${i(t.route)}"
      ...
      <div class="ax-modernized-card" style="padding:14px 16px;background:linear-gradient(135deg,rgba(20,20,35,0.6),rgba(14,14,28,0.4));backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
        ...
        <div class="ax-service-health-row ax-modernized-card"
          ...
          ${x}
        </div>`}).join("")}async function C(t){const a={};if(t.length===0)return a;try{const{linksRegistry:s}=await g(async()=>{const{linksRegistry:r}=await import("./multi-source-analyze-DCjiC-qU.js").then(e=>e.c);return{linksRegistry:r}},__vite__mapDeps([1,2,0,3]),import.meta.url);for(const r of t)a[r]={recharge:s.getRechargeLink(r),usage:s.getUsageLink(r),apiKeys:s.getApiKeysLink(r)}}catch{}return a}async function Q(t){p?.cleanup(),p=b("dashboard");const a=v.get("user");if(!h("admin.dashboard",t,a?.id))return;const s=a?.name?`Bonjour ${a.name}`:"Bonjour",[r,e,c]=await Promise.all([k(),_(),z()]),l=await C(c.map(n=>n.service)),o=S();t.innerHTML=m.withNonce(`
    <style>
  ...
  `),t.querySelectorAll("[data-route]").forEach(n=>{p.bind(n,"click",()=>{y.tap();const d=n.dataset.route;d&&(window.location.hash="#"+d)})}),f.info("feature-dashboard",`rendered (${r.length} kpis, ${e.length} alerts, ${o.length} todos)`)}export{k as computeKpis,X as dispose,i as escapeHtml,_ as loadAlerts,C as loadRechargeLinks,z as loadServiceHealth,S as loadTodos,Q as render,I as renderServiceHealthCard};
