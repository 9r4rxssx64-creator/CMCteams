...
import{_ as y}from"./apex-kb-D1VtWFD9.js";import{e as l}from"./escape-html-BlQj2yEF.js";import{c as v}from"./listener-cleanup-Y2rGGxxX.js";import{s as $,l as m}from"./monitoring-D2lWYrYo.js";import{capabilities as h}from"./capabilities-CjGRjjdP.js";import{haptic as u}from"./haptic-CQFg2PXZ.js";import{toast as T}from"./toast-CRdbcLoc.js";import"./credential-patterns-CLzI061R.js";import"./multi-source-analyze-Bg1HHfSC.js";let s=null;function H(){s?.cleanup(),s=null}function _(e,t){return e.filter(a=>{if(t.query){const n=t.query.toLowerCase();if(!(a.name.toLowerCase().includes(n)||a.description.toLowerCase().includes(n)))return!1}return!(t.tier&&t.tier!=="all"&&a.minTier!==t.tier||t.impactLevel&&t.impactLevel!=="all"&&a.impactLevel!==t.impactLevel)})}function z(e){const t={total:e.length,by_tier:{},by_impact:{}};for(const a of e)t.by_tier[a.minTier]=(t.by_tier[a.minTier]??0)+1,t.by_impact[a.impactLevel]=(t.by_impact[a.impactLevel]??0)+1;return t}const x={admin:{color:"#c9a227",label:"Admin"},laurence:{color:"#ff6b9d",label:"Laurence"},family:{color:"#a878ff",label:"Famille"},client_pro:{color:"#5aa8ff",label:"Pro"},client_free:{color:"#22cc77",label:"Free"}},A={A:{color:"#22cc77",label:"Auto",icon:"✅"},B:{color:"#ffaa00",label:"Notify",icon:"⚠️"},C:{color:"#ff5858",label:"Validate",icon:"🚨"}};let i={tier:"all",impactLevel:"all"},p="tools";function L(e){const t=x[e.minTier],a=A[e.impactLevel],n=e.inputSchema.properties??{},o=Object.keys(n).length;return`
    ...
    </li>`}function w(e){const t=e.enabled?"#22cc77":"#666",a=e.enabled?"✅":"⚪",n=e.enabled?"enabled":"disabled",o=e.examples.length>0?`<div style="margin-top:4px;color:#888;font-size:10px;font-style:italic">${l(e.examples[0]??"")}</div>`:"";return`
    ...
    </li>`}async function d(e){s?.cleanup(),s=v("apex-toolbox");const a=$.get("user")?.tier??"admin";let n=[];try{const{apexTools:b}=await y(async()=>{const{apexTools:g}=await import("./apex-tools-misc-DBbScgMK.js").then(f=>f.b);return{apexTools:g}},__vite__mapDeps([0,1,2,3]),import.meta.url);n=b.list()}catch(b){m.warn("feature-apex-toolbox","apex-tools load failed",{err:b})}const o=_(n,i),r=z(n),c=h.list();e.innerHTML=`
    <div class="ax-page" style="padding:16px;max-width:1100px;margin:0 auto">
      <header style="margin-bottom:20px">
        ...
        <p style="color:#a0a4c0;margin:0;font-size:13px">
          ${r.total} tools IA disponibles · ${c.length} capabilities device · Tier user : <strong style="color:${l(x[a].color)}">${l(x[a].label)}</strong>
        </p>
      </header>
      ...
      ${p==="tools"?`
        ...
        <section style="margin-bottom:24px">
          <ul style="list-style:none;padding:0;margin:0">
            ${o.length>0?o.map(L).join(""):'<li style="text-align:center;padding:30px;color:#888">Aucun tool pour ces filtres.</li>'}
          </ul>
        </section>
      `:`
        <section style="margin-bottom:24px">
          ...
          <ul style="list-style:none;padding:0;margin:0">
            ${c.length>0?c.map(w).join(""):'<li style="text-align:center;padding:30px;color:#888">Aucune capability détectée.</li>'}
          </ul>
        </section>
      `}
  ...
  `,C(e),m.info("feature-apex-toolbox",`rendered (tab=${p}, ${o.length}/${r.total} tools, ${c.length} caps)`)}function C(e){e.querySelectorAll("[data-tb-tab]").forEach(o=>{s.bind(o,"click",()=>{u.selection(),p=o.dataset.tbTab??"tools",d(e)})});const t=e.querySelector("#ax-tb-search");if(t){let o=null;s.bind(t,"input",()=>{o&&clearTimeout(o),o=setTimeout(()=>{i={...i,query:t.value},d(e)},250)})}const a=e.querySelector("#ax-tb-tier");a&&s&&s.bind(a,"change",o=>{const c=o.target.value;i={...i,tier:c},d(e)});const n=e.querySelector("#ax-tb-impact");n&&s&&s.bind(n,"change",o=>{const c=o.target.value;i={...i,impactLevel:c},d(e)}),e.querySelectorAll("[data-tool-test]").forEach(o=>{s.bind(o,"click",()=>{u.tap();const r=o.dataset.toolTest??"";T.info(`Tester ${r} : modal à implémenter (Jet 5)`)})})}export{z as computeStats,H as dispose,l as escapeHtml,_ as filterTools,d as render};
