...
import{_ as p}from"./apex-kb-BQ7RvhPe.js";import{e as l}from"./escape-html-BlQj2yEF.js";import{c as M}from"./listener-cleanup-Y2rGGxxX.js";import{l as D}from"./monitoring-3uBGKGRH.js";import"./credential-patterns-CLzI061R.js";let i=null;function O(){i?.cleanup(),i=null}async function m(s){i?.cleanup(),i=M("sentinels");const{sentinels:x}=await p(async()=>{const{sentinels:t}=await import("./sentinels-FIafT3XX.js");return{sentinels:t}},__vite__mapDeps([0,1,2]),import.meta.url),{sentinelsRegistry:g,bootstrapSentinelsRegistry:h}=await p(async()=>{const{sentinelsRegistry:t,bootstrapSentinelsRegistry:n}=await import("./sentinels-registry-DFDSAXl3.js");return{sentinelsRegistry:t,bootstrapSentinelsRegistry:n}},__vite__mapDeps([0,1,2]),import.meta.url);h();const a=x.list(),$=g.getStatus(),u=g.getMetrics(),y=a.filter(t=>t.lastResult?.ok).length,f=a.filter(t=>t.lastResult&&!t.lastResult.ok).length,R=a.filter(t=>!t.lastResult).length;s.innerHTML=`
