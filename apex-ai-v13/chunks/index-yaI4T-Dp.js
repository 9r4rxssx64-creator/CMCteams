...
(backup 30j conservé pour restauration)`))return;x.medium();let l=!1;if(c==="gn")l=f.remove(s);else if(c==="lg")try{const p=`apex_v13_recovery_backup_${s}_${Date.now()}`,v=localStorage.getItem(s);v&&localStorage.setItem(p,v),localStorage.removeItem(s),l=!0}catch{l=!1}else if(c==="mk")try{S.removeKey(s),l=!0}catch(p){w.warn("all-secrets","multi-key remove failed",{err:p})}l?(k.record("vault.secret_deleted",{details:{id:a,src:c}}),d.success("🗑 Supprimé (backup 30j)"),await m(e,t)):d.error("❌ Suppression échouée")}async function X(){try{const e={exported_at:new Date().toISOString(),version:1,multi_key_vault:S.listAll(!0),legacy:_.filter(i=>i.category!=="forbidden").map(i=>({key:i.storageKey,name:i.name,cipher:localStorage.getItem(i.storageKey)})).filter(i=>i.cipher),generic:f.list()},r=new Blob([JSON.stringify(e,null,2)],{type:"application/json"}),t=URL.createObjectURL(r),a=document.createElement("a");a.href=t,a.download=`apex-secrets-${new Date().toISOString().slice(0,10)}.json`,a.click(),URL.revokeObjectURL(t),k.record("vault.secrets_exported",{}),d.success("📤 Export téléchargé (chiffré)")}catch(e){w.warn("all-secrets","export failed",{err:e}),d.error("❌ Export échoué")}}async function ue(e){if(!(q.get("isAdmin")===!0)){e.innerHTML=`
      <div class="ax-gs-21">
        <h2 class="ax-gs-266">🔒 Accès admin uniquement</h2>
        <p>Cette section est réservée à Kevin.</p>
      </div>
    `;return}n?.cleanup(),n=I("all-secrets"),await m(e)}export{de as dispose,ue as render};
