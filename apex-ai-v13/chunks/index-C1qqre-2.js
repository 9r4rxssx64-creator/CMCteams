import{e as o}from"./escape-html-BlQj2yEF.js";import{c as g}from"./listener-cleanup-Y2rGGxxX.js";import{l as c}from"./monitoring-3uBGKGRH.js";import{s as f}from"../core/main-BHBzMEgW.js";import{a as u}from"./voice-CKlB4PWs.js";import{rgpd as x}from"./rgpd-BsMRXKNO.js";import{haptic as p}from"./haptic-CQFg2PXZ.js";import{toast as l}from"./toast-CRdbcLoc.js";import"./apex-kb-BUdkXo7X.js";import"./credential-patterns-CLzI061R.js";import"./multi-source-analyze-DfTeMbTC.js";let i=null;function H(){i?.cleanup(),i=null}function m(e){if(!e)return"—";try{return new Date(e).toLocaleString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"})}catch{return"—"}}function b(e){const t=e.scopes.includes("*")?'<span style="color:#ff6b6b;font-weight:600">Globale (tous scopes)</span>':e.scopes.map(s=>`<code style="background:rgba(255,255,255,0.05);padding:2px 6px;border-radius:4px;font-size:11px;margin-right:4px">${o(s)}</code>`).join("");return`
  ...
  `;try{const t=x.listRestrictedUsers(),s=t.length>0?t.map(b).join(""):'<p style="color:var(--ax-text-dim);text-align:center;padding:40px">✅ Aucune restriction RGPD active.<br><span style="font-size:12px">Tous les users peuvent traiter leurs données librement.</span></p>';e.innerHTML=`
    ...
    `}}function v(e){if(!i)return;const t=e.querySelector("#ax-rgpd-refresh");t&&i.bind(t,"click",()=>{p.tap(),a(e)}),e.querySelectorAll(".ax-rgpd-lift-btn").forEach(r=>{i&&i.bind(r,"click",async()=>{const n=r.dataset.uid;if(!(!n||!window.confirm(`Lever la restriction RGPD pour ${n} ?

L'utilisateur retrouvera l'accès complet (modif, AI, sync).`)))try{p.medium(),x.liftRestriction(n),await u.record("rgpd.admin.lift",{actor:"admin",details:{target_uid:n,via:"vRGPDAdmin"}}),l.success(`✅ Restriction levée pour ${n}`),await a(e)}catch(d){c.error("rgpd-admin","liftRestriction failed",{err:d,uid:n}),l.error(`❌ Échec : ${String(d).slice(0,100)}`)}})})}async function P(e){if(!(f.get("isAdmin")===!0)){e.innerHTML=`
      <div style="padding:40px;text-align:center;color:#999">
        <h2 style="color:#c9a227">Accès admin uniquement</h2>
        <p>Cette section est réservée à Kevin.</p>
      </div>
    `;return}i?.cleanup(),i=g("rgpd-admin"),await a(e)}export{H as dispose,P as render};
