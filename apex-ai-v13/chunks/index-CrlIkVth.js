...
import{_ as d,e as n,l as P}from"./monitoring-DIcR3-vs.js";import{c as q}from"./listener-cleanup-Y2rGGxxX.js";import"./multi-source-analyze-CeW95B3d.js";import"./credential-patterns-DUMYZEMu.js";import"./apex-kb-BYZgAGqg.js";let i=null;function M(){i?.cleanup(),i=null}const l={anthropic:"🧠",openai:"🤖",groq:"⚡",gemini:"✨",mistral:"🌊",cohere:"🔷",xai:"🔥",perplexity:"🔍",deepseek:"🐋",openrouter:"🌐"};function v(t){return t<0?"—":t<1e3?`${Math.round(t)}ms`:`${(t/1e3).toFixed(1)}s`}function $(t){return t<0?"—":`${Math.round(t)}%`}function w(t){if(t===0)return"jamais";const e=Date.now()-t;return e<6e4?"à l'instant":e<36e5?`il y a ${Math.round(e/6e4)} min`:e<864e5?`il y a ${Math.round(e/36e5)} h`:`il y a ${Math.round(e/864e5)} j`}function S(t){return!t||t.last_ping_ts===0?'<span class="ax-gs-64">⚪</span>':t.last_ping_ok?t.latency_avg_ms>3e3?'<span class="ax-gs-44">🟡</span>':'<span class="ax-gs-14">🟢</span>':'<span class="ax-gs-13">🔴</span>'}function m(t){return t>=80?"#22cc77":t>=60?"#c9a227":t>=40?"#f0c020":"#ff5b5b"}async function p(t){i?.cleanup(),i=q("smart-router");const{smartRouter:e}=await d(async()=>{const{smartRouter:s}=await import("./smart-router-Cm4XjKZS.js");return{smartRouter:s}},__vite__mapDeps([0,1,2,3]),import.meta.url),R=await e.rankProviders(),f=await e.getRecommendations(),g=e.getOverride(),r=R[0],x=e.getAllProviders(),o=r?await e.getStats(r.provider):null,b=await Promise.all(x.map(async s=>({provider:s,stats:await e.getStats(s),score:await e.scoreProvider(s)})));b.sort((s,a)=>a.score.total-s.score.total),t.innerHTML=`
