import{e as a}from"./escape-html-BlQj2yEF.js";import{c as u}from"./listener-cleanup-Y2rGGxxX.js";import{s as x,l as p,a as m}from"./monitoring-Bl85iYvy.js";import{rgpd as g}from"./rgpd-CCFFAEEd.js";import{haptic as o}from"./haptic-CQFg2PXZ.js";import{toast as l}from"./toast-CRdbcLoc.js";import"./multi-source-analyze-DfuDcC7m.js";import"./apex-kb-CAfWvMOn.js";import"./credential-patterns-CLzI061R.js";let s=null;function H(){s?.cleanup(),s=null}function f(i){if(!i)return"—";try{return new Date(i).toLocaleString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"})}catch{return"—"}}function v(i){const e=i.scopes.includes("*")?'<span style="color:#ff6b6b;font-weight:600">Globale (tous scopes)</span>':i.scopes.map(n=>`<code style="background:rgba(255,255,255,0.05);padding:2px 6px;border-radius:4px;font-size:11px;margin-right:4px">${a(n)}</code>`).join("");return`
...
L'utilisateur retrouvera l'accès complet (modif, AI, sync).`)))try{o.medium(),g.liftRestriction(t),await m.record("rgpd.admin.lift",{actor:"admin",details:{target_uid:t,via:"vRGPDAdmin"}}),l.success(`✅ Restriction levée pour ${t}`),await d(i)}catch(c){p.error("rgpd-admin","liftRestriction failed",{err:c,uid:t}),l.error(`❌ Échec : ${String(c).slice(0,100)}`)}})})}async function P(i){if(!(x.get("isAdmin")===!0)){i.innerHTML=`
      <div class="ax-gs-21">
        <h2 class="ax-gs-266">Accès admin uniquement</h2>
        <p>Cette section est réservée à Kevin.</p>
      </div>
    `;return}s?.cleanup(),s=u("rgpd-admin"),await d(i)}export{H as dispose,P as render};
