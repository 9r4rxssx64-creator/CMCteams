import{a as t}from"./escape-html-DGIYNPKb.js";import{q as p}from"./monitoring-BurevTV9.js";import{x as d}from"./apex-tools-dispatch-skills-DDiE-KLE.js";import{toast as r}from"./toast-CRdbcLoc.js";import"./multi-source-analyze-tqKS13-U.js";import"./apex-kb-W8GOXAde.js";import"./credential-patterns-CLzI061R.js";import"./haptic-CQFg2PXZ.js";function $(s){s.innerHTML=`
    <div class="ax-gs-115">
...
`).map(e=>e.trim()).filter(Boolean).map(e=>e.split(",").map(a=>{const l=a.trim(),i=Number(l);return!isNaN(i)&&l!==""?i:l}));if(n.length===0){r.error("Aucune donnée à exporter");return}r.info("Génération en cours...");try{const e=await d.generate({filename:x,sheets:[{name:o,data:n,freezeHeader:u}]}),a=s.querySelector("#xlsx-result");if(!a)return;e.success?(a.innerHTML=`
          ...
          </div>`,r.success(`✅ ${e.filename}`)):(a.innerHTML=`<p class="ax-gs-257">❌ ${t(e.error??"Erreur")}</p>`,r.error(`❌ ${e.error??"Erreur"}`))}catch(e){p.warn("studio-xlsx","failed",{err:e}),r.error(`❌ ${e instanceof Error?e.message:"Erreur"}`)}})}export{$ as render};
