import{l as u}from"./monitoring-B17vNBOa.js";import{s as T}from"../core/main-DqMShSUw.js";import{linksRegistry as w}from"./links-registry-DQfLEFc8.js";import{detectCredential as y,CREDENTIAL_PATTERNS as A}from"./credential-patterns-BybElwOv.js";import{multiKeyVault as g}from"./multi-key-vault-OtrREBIO.js";import{v}from"./apex-kb-BAhF50OC.js";import{h as d}from"./haptic-BUEqXK0N.js";import{toast as s}from"./toast-Dgg9rcIP.js";import"./apex-tools-dispatch-BEaNwuqd.js";import"./apex-tools-registry-7n7rHbkq.js";function n(t){return t.replace(/[&<>"']/g,a=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[a]??a)}const b=[{id:"ai",label:"🤖 IA & LLM",serviceMatchers:["anthropic","openai","groq","google","gemini","openrouter","cohere","mistral","perplexity","deepseek","xai","elevenlabs","replicate","huggingface","fireworks","togetherai","deepl"],patternCategories:["ai"]},{id:"finance",label:"💳 Paiements & Finance",serviceMatchers:["stripe","paypal","revolut","wise","lydia","n26","boursorama","fortuneo","ing","socgen","bnp","credit_agricole","credit_mutuel","banque_postale","lbp","bpce","shopify"],patternCategories:["finance"]},{id:"devops",label:"🛠 DevOps & Code",serviceMatchers:["github","gitlab","cloudflare","vercel","netlify","railway","aws","heroku","sentry","npm"],patternCategories:["devops"]},{id:"comms",label:"📨 Communications",serviceMatchers:["telegram","discord","slack","brevo","resend","twilio","sendgrid","mailchimp","whatsapp"],patternCategories:["comms"]},{id:"social",label:"🌐 Réseaux sociaux",serviceMatchers:["facebook","instagram","tiktok","youtube","twitter","linkedin"],patternCategories:[]},{id:"storage",label:"☁️ Stockage & Cloud",serviceMatchers:["firebase","supabase","airtable","notion","dropbox","pinecone","weaviate"],patternCategories:["storage"]},{id:"ecommerce",label:"🛒 E-commerce",serviceMatchers:["shopify","stripe_connect","paypal_business"],patternCategories:[]},{id:"crypto",label:"₿ Crypto",serviceMatchers:["coinbase","binance","crypto_com","kraken"],patternCategories:[]},{id:"identity",label:"🆔 Identité Kevin",serviceMatchers:["kevin","iban","siret","vat","bic","apple","microsoft"],patternCategories:["identity"]},{id:"other",label:"📦 Autres",serviceMatchers:[],patternCategories:["saas"]}];function $(t,a){const e=t.toLowerCase();let r=null;for(const i of b)if(i.id!=="other")for(const o of i.serviceMatchers)e.includes(o)&&(!r||o.length>r.matchLen)&&(r={catId:i.id,matchLen:o.length});if(r)return r.catId;if(a){for(const i of b)if(i.patternCategories.includes(a))return i.id}return"other"}function _(){return A.filter(t=>t.category!=="forbidden").map(t=>{const a=v.getKeyStatus(t.storageKey),e=(()=>{try{return localStorage.getItem(t.storageKey)??""}catch{return""}})(),r=e&&e.length>8&&!e.startsWith("AXENC1:")?v.maskKey(e):e.startsWith("AXENC1:")?"🔒 chiffré":"";return{pattern:t,status:a,masked:r}})}function ae(t,a){return t.filter(e=>{if(a.category&&e.pattern.category!==a.category||a.configuredOnly&&e.status==="empty")return!1;if(a.query){const r=a.query.toLowerCase();if(!(e.pattern.name.toLowerCase().includes(r)||e.pattern.storageKey.toLowerCase().includes(r)))return!1}return!0})}function z(){const t=[];let a=[];try{a=g.listAll(!0)}catch(e){u.warn("feature-vault","multiKeyVault.listAll failed",{err:e})}for(const e of a){const r=w.get(e.service),i=A.find(p=>p.storageKey.includes(e.service)),o={id:e.id,service:e.service,serviceName:r?.name??K(e.service),category:$(e.service,i?.category),status:e.status,source:"multi-key"};e.alias!==void 0&&(o.alias=e.alias),e.addedAt!==void 0&&(o.addedAt=e.addedAt),e.lastTestedAt!==void 0&&(o.lastTestedAt=e.lastTestedAt);const l=w.getRechargeLink(e.service);l&&(o.rechargeUrl=l),t.push(o)}return t}function j(){const t=z(),a={total:t.length,active:0,failing:0,invalid:0};for(const e of t)e.status==="active"?a.active+=1:e.status==="failing"||e.status==="rate-limited"?a.failing+=1:e.status==="invalid"&&(a.invalid+=1);return a}function I(t,a=""){const e=z(),r=a.trim().toLowerCase();return e.filter(i=>i.category!==t.id?!1:r?i.service.toLowerCase().includes(r)||i.serviceName.toLowerCase().includes(r)||(i.alias?.toLowerCase().includes(r)??!1):!0)}function K(t){return t.charAt(0).toUpperCase()+t.slice(1)}async function E(t){const a=t.trim();if(!a)return{ok:!1,reason:"Entrée vide"};const e=y(a);if(!e)return{ok:!1,reason:"Aucun pattern reconnu"};if(e.category==="forbidden")return{ok:!1,reason:"🚨 Type interdit (cartes/seed phrases jamais stockées)"};try{const r=await v.encryptAuto(a);return localStorage.setItem(e.storageKey,r),{ok:!0,pattern_name:e.name,storage_key:e.storageKey}}catch(r){return u.warn("vault-feature","autoDetectAndStore failed",{err:r}),{ok:!1,reason:"Erreur chiffrement"}}}function re(t){try{return localStorage.removeItem(t),!0}catch(a){return u.warn("vault-feature","remove failed",{err:a}),!1}}function R(t){const a={exported_at:new Date().toISOString(),version:1,entries:t.filter(e=>e.status!=="empty").map(e=>{const r=(()=>{try{return localStorage.getItem(e.pattern.storageKey)??""}catch{return""}})();return{storage_key:e.pattern.storageKey,name:e.pattern.name,value_encrypted:r}})};return JSON.stringify(a,null,2)}function O(t){const a=D[t.status]??"#888",e=U[t.status]??"⚪",r=(t.preview??"").slice(0,4)+"••••••"+(t.preview??"").slice(-4),i=t.preview?r:"••••••",o=t.rechargeUrl??"",l=t.alias?`<span style="color:#888;font-size:11px">— ${n(t.alias)}</span>`:"",p=t.logoUrl?`<img src="${n(t.logoUrl)}" alt="" style="width:24px;height:24px;border-radius:6px" onerror="this.style.display='none'">`:"",c=[];t.addedAt&&c.push(`Ajouté ${S(t.addedAt)}`),t.lastTestedAt&&c.push(`Testé ${S(t.lastTestedAt)}`);const m=c.length>0?`<div style="display:flex;gap:8px;font-size:11px;color:#888;margin-bottom:10px">${c.map(M=>`<span>${n(M)}</span>`).join("")}</div>`:"";return`
  ...
  `,L(t),F(t),u.info("feature-vault",`rendered (${e.total} entries)`)}function L(t){const a=t.querySelector("#ax-vault-categories");a&&(a.innerHTML=b.map(e=>{const r=I(e,h);if(r.length===0&&e.id!=="identity")return"";const i=r.length>0;return`
  ...
  `,e.querySelector("#ax-vault-modal-close")?.addEventListener("click",()=>x(t));const i=e.querySelector('[role="dialog"]');i?.addEventListener("click",o=>{o.target===i&&x(t)}),e.querySelector("#ax-vault-add-detect")?.addEventListener("click",()=>{(async()=>{d.tap();const o=e.querySelector("#ax-vault-add-value");if(!o)return;const l=y(o.value.trim());if(!l){s.warn("Aucun pattern reconnu");return}if(l.category==="forbidden"){s.error("🚨 Type interdit");return}const p=e.querySelector("#ax-vault-add-service"),c=e.querySelector("#ax-vault-add-cat");if(p){const m=l.storageKey.replace(/^(ax_|apex_v13_)/,"").replace(/_(?:key|token|pat|sk|pk|id|secret)$/,"");p.value=m}c&&(c.value=$(p?.value??"",l.category)),s.success(`Détecté: ${l.name}`)})()}),e.querySelector("#ax-vault-add-save")?.addEventListener("click",()=>{(async()=>{d.tap();const o=e.querySelector("#ax-vault-add-service")?.value.trim()??"",l=e.querySelector("#ax-vault-add-alias")?.value.trim()??"",p=e.querySelector("#ax-vault-add-value")?.value.trim()??"";if(!o||!p){s.warn("Service et valeur requis");return}try{const c={};l&&(c.alias=l),await g.addKey(o,p,c),s.success(`✅ Clé ${o} chiffrée + sauvegardée`),x(t),f(t)}catch(c){u.warn("feature-vault","add manual failed",{err:c}),s.error("Erreur pendant la sauvegarde")}})()})}function V(t,a){const e=k(t),r=g.listAll(!0).find(i=>i.id===a);if(!r){s.error("Clé introuvable");return}e.innerHTML=`
  ...
  `,e.querySelector("#ax-vault-modal-close")?.addEventListener("click",()=>x(t)),e.querySelector("#ax-vault-edit-cancel")?.addEventListener("click",()=>x(t)),e.querySelector("#ax-vault-edit-save")?.addEventListener("click",()=>{(async()=>{d.tap();const i=e.querySelector("#ax-vault-edit-value")?.value.trim()??"",o=e.querySelector("#ax-vault-edit-alias")?.value.trim()??"";if(!i){s.warn("Valeur requise");return}try{g.markInvalid(a,"replaced via edit");const l={};o&&(l.alias=o),await g.addKey(r.service,i,l),s.success("✅ Clé mise à jour"),x(t),f(t)}catch(l){u.warn("feature-vault","edit save failed",{err:l}),s.error("Erreur pendant la modification")}})()})}export{b as CATEGORIES,E as autoDetectAndStore,z as buildCredentialDisplays,$ as classifyService,j as computeStats,n as escapeHtml,R as exportVaultJson,ae as filterVaultEntries,S as formatRelativeTime,I as getCredentialsForCategory,_ as listVaultEntries,re as removeCredential,f as render,O as renderCredentialCard};
//# sourceMappingURL=index-Dn5xpCyu.js.map
