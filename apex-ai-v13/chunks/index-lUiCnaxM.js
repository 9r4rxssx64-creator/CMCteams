const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./monitoring-3uBGKGRH.js","./apex-kb-BUdkXo7X.js","./credential-patterns-CLzI061R.js","./multi-source-analyze-DfTeMbTC.js","./auth-BK9K-HVi.js","../assets/css/main-Bmb5ACYM.css"])))=>i.map(i=>d[i]);
import{_ as p}from"./apex-kb-BUdkXo7X.js";import{e as u}from"./escape-html-BlQj2yEF.js";import{c as E}from"./listener-cleanup-Y2rGGxxX.js";import{l as T}from"./monitoring-3uBGKGRH.js";import{s as I}from"../core/main-BHBzMEgW.js";import{c as O}from"./csp-style-helper-BisGRi53.js";import{r as D}from"./recharge-action-C73aOkPx.js";import"./voice-CKlB4PWs.js";import"./credential-patterns-CLzI061R.js";import"./multi-source-analyze-DfTeMbTC.js";let i=null;function G(){i?.cleanup(),i=null}const P="apex_v13_chat_auto_read";function V(){try{return localStorage.getItem(P)==="1"}catch{return!1}}function q(n){try{localStorage.setItem(P,n?"1":"0")}catch{}}async function R(n){try{const k=await p(()=>import("./voice-CKlB4PWs.js").then(l=>l.h),__vite__mapDeps([0]),import.meta.url),{listVoices:S,getActiveVoice:x,setActiveVoice:b,speak:v,stopAll:g}=k,_=n.querySelector("#ax-settings-auto-read"),A=n.querySelector("#ax-voice-current"),w=n.querySelector("#ax-voice-list"),L=n.querySelectorAll(".ax-voice-cat-btn");if(!w)return;_&&(_.checked=V(),i.bind(_,"change",()=>{q(_.checked),(async()=>{const{toast:l}=await p(async()=>{const{toast:d}=await import("./toast-CRdbcLoc.js");return{toast:d}},[],import.meta.url);l.success(_.checked?"Lecture auto activée":"Lecture auto désactivée")})()}));const z=()=>{if(!A)return;const l=x(),m=S().find(h=>h.id===l);A.textContent=m?`Voix active : ${m.emoji??"🔊"} ${m.name} (${m.category})`:`Voix active : ${l}`};z();const $=l=>{const d=S(),m=l==="all"?d:d.filter(t=>t.category===l),h=x();w.setAttribute("aria-busy","false"),w.innerHTML=m.map(t=>{const o=t.id===h,a=t.emoji??(t.category==="pro"?"🎙️":t.category==="fun"?"🎉":"🎨"),e=t.description?u(t.description):"";return`
            <div class="ax-voice-item${o?" is-active":""}" data-voice-id="${u(t.id)}">
              <span style="font-size:18px">${a}</span>
              <div style="flex:1;min-width:0">
                <div class="ax-voice-item__name">${u(t.name)}${o?' <span style="color:var(--ax-gold);font-size:11px">★ active</span>':""}</div>
  ...
  `),(async()=>{try{const{memoryBridge:t}=await p(async()=>{const{memoryBridge:r}=await import("./memory-bridge-BvcRBWq5.js");return{memoryBridge:r}},__vite__mapDeps([1,0,2]),import.meta.url),o=n.querySelector("#ax-memory-bridge-status"),a=n.querySelector("#ax-memory-bridge-sync"),e=()=>{if(!o)return;const r=t.getHealth(),c=t.getStatus(),s=c.filter(f=>f.last_success).length;o.textContent=`${r.backends_configured} backends configurés · ${s}/${c.length} dernier sync OK`};e(),a&&i&&i.bind(a,"click",()=>{(async()=>{a&&(a.disabled=!0);const r=await t.runAutoSync(),c=r.filter(f=>f.ok).length,{toast:s}=await p(async()=>{const{toast:f}=await import("./toast-CRdbcLoc.js");return{toast:f}},[],import.meta.url);r.length===0?s.warn("Aucun backend configuré"):c===r.length?s.success(`Sync OK (${c}/${r.length})`):s.warn(`Sync partielle (${c}/${r.length})`),e(),a&&(a.disabled=!1)})()})}catch(t){T.warn("feature-settings","memory-bridge wire failed",{err:t})}})();const _=n.querySelector("#ax-conso-scan");_&&i&&i.bind(_,"click",()=>{(async()=>{try{const{consumptionAnomalyDetector:t}=await p(async()=>{const{consumptionAnomalyDetector:e}=await import("./consumption-anomaly-detector-3KMtr7wS.js");return{consumptionAnomalyDetector:e}},__vite__mapDeps([0,3,1,2]),import.meta.url),o=t.scanAllVerbose(),a=n.querySelector("#ax-conso-results");if(!a)return;a.innerHTML=o.map(e=>{const r=e.severity==="critical"?"var(--ax-sev-critical)":e.severity==="high"?"var(--ax-sev-high)":e.severity==="medium"?"var(--ax-sev-medium)":e.severity==="low"?"var(--ax-sev-low)":"var(--ax-green)",c=e.severity==="critical"?"ax-sev-critical":e.severity==="high"?"ax-sev-high":e.severity==="medium"?"ax-sev-medium":(e.severity==="low","ax-sev-low"),s=e.severity==="critical"?"🚨":e.severity==="high"?"⚠️":e.severity==="medium"?"🟡":e.severity==="low"?"🔵":"✅";return`<div style="background:rgba(255,255,255,0.03);border-left:3px solid ${r};padding:8px 12px;margin-top:6px;border-radius:4px">
          ...
          </div>`}).join("")}catch(t){T.warn("feature-settings","conso scan failed",{err:t})}})()});const A=n.querySelector("#ax-zoom-inspector-btn");A&&i&&i.bind(A,"click",()=>{(async()=>{const{apexZoomInspector:t}=await p(async()=>{const{apexZoomInspector:o}=await import("./apex-zoom-inspector-DB-9-V8P.js");return{apexZoomInspector:o}},__vite__mapDeps([1,0,2]),import.meta.url);t.isVisible()?t.hide():t.show()})()});const w=n.querySelector("#ax-functional-test-btn");w&&i&&i.bind(w,"click",()=>{(async()=>{const t=n.querySelector("#ax-functional-test-results");if(t){t.innerHTML=`<div style="color:var(--ax-blue)">⏳ Test des boutons en cours (jusqu'à 40 boutons, ~30s)...</div>`;try{const{apexFunctionalTester:o}=await p(async()=>{const{apexFunctionalTester:y}=await import("./apex-functional-tester-CrDwBxuI.js");return{apexFunctionalTester:y}},__vite__mapDeps([1,0,2]),import.meta.url),{reportsHistory:a}=await p(async()=>{const{reportsHistory:y}=await import("./apex-reports-history-Cv_X_jWe.js");return{reportsHistory:y}},__vite__mapDeps([0]),import.meta.url),e=await o.testAndAutoFix({maxButtons:30});a.recordFunctional(e.before,e.fixes,e.after,e.improvement);const r=e.before.tested>0?Math.round(e.before.ok/e.before.tested*100):0,c=e.after?` → après fix : ${Math.round(e.after.ok/Math.max(1,e.after.tested)*100)}% OK (${e.improvement>0?"+":""}${Math.round(e.improvement*100)}%)`:"",s=e.before.details.filter(y=>y.status==="no_response"||y.status==="error").slice(0,5).map(y=>`<li style="color:var(--ax-sev-high);font-size:11px">${u(y.label||"(no label)")} → ${u(y.status)}</li>`).join(""),f=e.fixes.applied.map(y=>u(String(y))).join(", ");t.innerHTML=`
            <div style="background:rgba(106,138,255,0.08);border:1px solid rgba(106,138,255,0.3);border-radius:8px;padding:10px;color:#fff;font-size:12px">
              ...
              ${e.fixes.applied.length?`<div style="margin-top:6px;color:var(--ax-gold-deep)">🔧 Auto-fix appliqué : ${f}</div>`:""}
