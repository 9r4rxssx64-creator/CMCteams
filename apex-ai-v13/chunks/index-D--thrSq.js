...
import{_ as v}from"./apex-kb-C132uqD8.js";import{a as l}from"./escape-html-DGIYNPKb.js";import{c as y}from"./listener-cleanup-Y2rGGxxX.js";import{C as $,q as u}from"./monitoring-BkJ_WZbY.js";import{capabilities as h}from"./capabilities-OoJLhwXf.js";import{haptic as g}from"./haptic-CQFg2PXZ.js";import{toast as T}from"./toast-CRdbcLoc.js";import"./credential-patterns-CLzI061R.js";import"./multi-source-analyze-DCjiC-qU.js";let n=null;function H(){n?.cleanup(),n=null}function _(e,t){return e.filter(o=>{if(t.query){const r=t.query.toLowerCase();if(!(o.name.toLowerCase().includes(r)||o.description.toLowerCase().includes(r)))return!1}return!(t.tier&&t.tier!=="all"&&o.minTier!==t.tier||t.impactLevel&&t.impactLevel!=="all"&&o.impactLevel!==t.impactLevel)})}function A(e){const t={total:e.length,by_tier:{},by_impact:{}};for(const o of e)t.by_tier[o.minTier]=(t.by_tier[o.minTier]??0)+1,t.by_impact[o.impactLevel]=(t.by_impact[o.impactLevel]??0)+1;return t}const x={admin:{color:"#c9a227",label:"Admin"},laurence:{color:"#ff6b9d",label:"Laurence"},family:{color:"#a878ff",label:"Famille"},client_pro:{color:"#5aa8ff",label:"Pro"},client_free:{color:"#22cc77",label:"Free"}},L={A:{color:"#22cc77",label:"Auto",icon:"✅"},B:{color:"#ffaa00",label:"Notify",icon:"⚠️"},C:{color:"#ff5858",label:"Validate",icon:"🚨"}};let i={tier:"all",impactLevel:"all"},p="tools";function C(e){const t=x[e.minTier],o=L[e.impactLevel],r=e.inputSchema.properties??{},a=Object.keys(r).length;return`
    ...
    </li>`}function w(e){const t=e.enabled?"#22cc77":"#666",o=e.enabled?"✅":"⚪",r=e.enabled?"enabled":"disabled",a=e.examples.length>0?`<div style="margin-top:4px;color:#888;font-size:10px;font-style:italic">${l(e.examples[0]??"")}</div>`:"";return`
    ...
    </li>`}async function d(e){n?.cleanup(),n=y("apex-toolbox");const o=$.get("user")?.tier??"admin";let r=[];try{const{apexTools:b}=await v(async()=>{const{apexTools:m}=await import("./apex-tools-misc-C4sFgDRb.js").then(f=>f.c);return{apexTools:m}},__vite__mapDeps([0,1,2,3]),import.meta.url);r=b.list()}catch(b){u.warn("feature-apex-toolbox","apex-tools load failed",{err:b})}const a=_(r,i),s=A(r),c=h.list();e.innerHTML=`
    <div class="ax-page ax-gs-268">
      ...
      ${p==="tools"?`
        ...
        <section class="ax-gs-180">
          <ul class="ax-gs-286">
            ${a.length>0?a.map(C).join(""):'<li class="ax-gs-209">Aucun tool pour ces filtres.</li>'}
          </ul>
        </section>
      `:`
        <section class="ax-gs-180">
          ...
          <ul class="ax-gs-286">
            ${c.length>0?c.map(w).join(""):'<li class="ax-gs-209">Aucune capability détectée.</li>'}
          </ul>
        </section>
      `}
