import{l as b}from"./monitoring-BAiQJoxJ.js";import{s as y}from"../core/main-yvpYLU7C.js";import{iotRegistry as d}from"./iot-providers-registry-E-uZmzMu.js";import{toast as r}from"./toast-Dgg9rcIP.js";import"./apex-kb-0vuBuWYy.js";import"./apex-tools-registry-DFaDwWZQ.js";import"./credential-patterns-Dy6Wjk7e.js";import"./multi-source-analyze-DzTKkB_U.js";import"./broadlink-bridge-BBBULz8a.js";import"./haptic-BUEqXK0N.js";const h="kdmc_admin";function n(e){return String(e).replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]??t)}function v(){return y.get("user")?.id===h}async function g(e){if(e){if(!v()){e.innerHTML=`
  ...
  `;try{const t=await d.statusAll(),o=await d.listAllDevices().catch(()=>[]);e.innerHTML=k(t,o),_(e)}catch(t){b.warn("iot-providers-view","render fail",{err:String(t)}),e.innerHTML=`
  ...
  `}function $(e){const{provider:t,status:o}=e;let a="iot-status-no",i="⚪ Non configuré";o.ok?(a="iot-status-ok",i=`🟢 OK · ${o.devices_count??0} devices`):o.reason&&o.reason!=="no_credentials"&&(a="iot-status-ko",i=`🔴 ${n(o.reason)}`);const s=o.ok;return`
  ...
  `}function _(e){e.addEventListener("click",t=>{const o=t.target;if(!o)return;const a=o.closest("[data-action]");if(!a)return;const i=a.dataset.action??"",s=a.dataset.providerId??"",c=a.dataset.deviceId??"";i==="install"?q(s,e):i==="test"?z(s):i==="refresh-all"?C(e):i==="show-proxy"?L():i==="open-broadlink"?location.hash="#broadlink-setup":i==="device-on"&&s&&c&&I(s,c)})}async function q(e,t){const o=d.get(e);if(!o){r.error(`Provider ${e} inconnu`);return}const a=t.querySelector("#iot-modal-host");if(!a)return;const i=T(o);a.innerHTML=`
      ...
      <div class="iot-modal">
        ...
        <form id="iot-install-form" autocomplete="off">
          ${i.map(l=>`
            ...
            <input name="${n(l.name)}" type="${n(l.type)}" placeholder="${n(l.placeholder??"")}" ${l.required?"required":""} autocomplete="off" />
          `).join("")}
  ...
  `,a.querySelector('[data-modal-bg="1"]')?.addEventListener("click",l=>{const p=l.target;(p.dataset.modalBg==="1"||p.dataset.modalClose==="1")&&(a.innerHTML="")});const c=a.querySelector("#iot-install-form");c?.addEventListener("submit",l=>{l.preventDefault();const p=new FormData(c),f={};for(const u of i){const m=p.get(u.name);typeof m=="string"&&m&&(f[u.credentialKey??u.name]=m)}const x=p.get("region")||void 0;A(o.id,f,x,a,t)})}function T(e){switch(e.id){case"ewelink":return[{name:"email",label:"Email eWeLink",type:"email",required:!0},{name:"password",label:"Mot de passe",type:"password",required:!0},{name:"region",label:"Région (eu, us, as, cn)",type:"text",required:!1,placeholder:"eu"}];case"tuya":return[{name:"client_id",label:"Client ID Tuya Cloud",type:"text",required:!0},{name:"client_secret",label:"Client Secret",type:"password",required:!0},{name:"uid",label:"User UID Tuya",type:"text",required:!1,placeholder:"optionnel"},{name:"access_token",label:"Access Token (si déjà obtenu)",type:"password",required:!1},{name:"region",label:"Région (eu, us, cn, in)",type:"text",required:!1,placeholder:"eu"}];case"broadlink":return[{name:"email",label:"Email Broadlink",type:"email",required:!1},{name:"password",label:"Mot de passe",type:"password",required:!1},{name:"token",label:"Token (alternative — extrait via vision)",type:"password",required:!1}];case"hue":return[{name:"bridge_ip",label:"IP Hue Bridge (LAN)",type:"text",required:!1,placeholder:"192.168.1.X"},{name:"username",label:"Username (LAN, généré bouton bridge)",type:"text",required:!1},{name:"oauth_token",label:"OAuth token (cloud)",type:"password",required:!1}];case"sonos":return[{name:"token",label:"Access Token OAuth2 Sonos",type:"password",required:!0},{name:"household",label:"Household ID",type:"text",required:!1,placeholder:"optionnel — détecté auto"}];case"home-assistant":return[{name:"url",label:"URL Home Assistant",type:"url",required:!0,placeholder:"http://192.168.1.X:8123"},{name:"token",label:"Long-Lived Access Token",type:"password",required:!0}];default:return e.credential_keys.map(t=>({name:t.replace(/^ax_[a-z]+_/,""),credentialKey:t.replace(/^ax_[a-z]+_/,""),label:t,type:/password|secret|token/i.test(t)?"password":"text",required:!1}))}}async function A(e,t,o,a,i){r.info(`Installation ${e}…`);const s=await d.configureProvider(o?{provider_id:e,credentials:t,region:o}:{provider_id:e,credentials:t});s.ok?(r.success(`✅ ${e} installé · ${s.devices_found??0} devices`),a.innerHTML="",g(i)):r.error(`❌ Échec ${e} : ${s.error??"inconnu"}`)}async function z(e){r.info(`Test ${e}…`);const t=await d.testConnection(e);t.ok?r.success(`🟢 ${e} OK · ${t.devices_count??0} devices · ${t.latency_ms??"?"}ms`):r.warn(`⚠️ ${e} : ${t.error??t.reason??"inconnu"}`)}async function C(e){r.info("Refresh status providers…"),await g(e)}function L(){const e=localStorage.getItem("ax_iot_proxy_url")??"",t=window.prompt("URL Cloudflare Worker proxy CORS (vide = direct) :",e);if(t!==null)if(t.trim()){try{localStorage.setItem("ax_iot_proxy_url",t.trim())}catch{}r.success("Proxy configuré")}else{try{localStorage.removeItem("ax_iot_proxy_url")}catch{}r.info("Proxy retiré (mode direct)")}}async function I(e,t){const o=S(e,t);r.info(`▶ Envoi ${e}/${t.slice(0,12)}…`);const a=await d.sendCommand(e,t,o);a.ok?r.success("✅ Commande envoyée"):r.error(`❌ ${a.error??a.reason??"fail"}`)}function S(e,t){switch(e){case"ewelink":return{switch:"on"};case"tuya":return{commands:[{code:"switch_led",value:!0}]};case"hue":return{on:!0};case"sonos":return{action:"play"};case"home-assistant":{const o=t.split(".")[0]??"homeassistant";return{service:o==="light"||o==="switch"?"turn_on":"toggle"}}case"broadlink":return{ir_hex:""};default:return{on:!0}}}export{g as render};
