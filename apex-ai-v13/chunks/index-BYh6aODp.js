const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./monitoring-yJ4fpvfw.js","./multi-source-analyze-CutjfdEy.js","./credential-patterns-DUMYZEMu.js","./apex-kb-HxI2DoPN.js","./auth-Bdl-CqwN.js"])))=>i.map(i=>d[i]);
import{b as I,e as u,_ as p,l as z}from"./monitoring-yJ4fpvfw.js";import{c as E}from"./listener-cleanup-Y2rGGxxX.js";import{c as O}from"./csp-style-helper-BEHhIhzj.js";import{r as D}from"./recharge-action-B5bxXsSY.js";import"./multi-source-analyze-CutjfdEy.js";import"./credential-patterns-DUMYZEMu.js";import"./apex-kb-HxI2DoPN.js";let i=null;function K(){i?.cleanup(),i=null}const P="apex_v13_chat_auto_read";function V(){try{return localStorage.getItem(P)==="1"}catch{return!1}}function q(o){try{localStorage.setItem(P,o?"1":"0")}catch{}}async function R(o){try{const k=await p(()=>import("./voice-YR1AqS_B.js"),__vite__mapDeps([0,1,2,3]),import.meta.url),{listVoices:A,getActiveVoice:x,setActiveVoice:m,speak:v,stopAll:g}=k,_=o.querySelector("#ax-settings-auto-read"),S=o.querySelector("#ax-voice-current"),w=o.querySelector("#ax-voice-list"),L=o.querySelectorAll(".ax-voice-cat-btn");if(!w)return;_&&(_.checked=V(),i.bind(_,"change",()=>{q(_.checked),(async()=>{const{toast:l}=await p(async()=>{const{toast:d}=await import("./toast-BCPNzfMv.js");return{toast:d}},[],import.meta.url);l.success(_.checked?"Lecture auto activée":"Lecture auto désactivée")})()}));const T=()=>{if(!S)return;const l=x(),b=A().find(h=>h.id===l);S.textContent=b?`Voix active : ${b.emoji??"🔊"} ${b.name} (${b.category})`:`Voix active : ${l}`};T();const $=l=>{const d=A(),b=l==="all"?d:d.filter(t=>t.category===l),h=x();w.setAttribute("aria-busy","false"),w.innerHTML=b.map(t=>{const s=t.id===h,a=t.emoji??(t.category==="pro"?"🎙️":t.category==="fun"?"🎉":"🎨"),e=t.description?u(t.description):"";return`
    ...
    <div class="ax-page" style="padding:24px 16px max(24px, env(safe-area-inset-bottom)) 16px;max-width:680px;margin:0 auto;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif">
      ...
      <section class="ax-modernized-card" style="${x};animation-delay:220ms">
        ...
        <details style="margin-top:6px">
          ...
          <button class="ax-btn ax-btn-secondary" id="ax-functional-test-btn" style="${g};margin-top:10px;background:rgba(106,138,255,0.15);color:var(--ax-blue);border:1px solid rgba(106,138,255,0.35)">🧪 Tester tous les boutons + auto-fix</button>
  ...
  `),(async()=>{try{const{memoryBridge:t}=await p(async()=>{const{memoryBridge:r}=await import("./memory-bridge-ey9xKRlp.js");return{memoryBridge:r}},__vite__mapDeps([0,1,2,3]),import.meta.url),s=o.querySelector("#ax-memory-bridge-status"),a=o.querySelector("#ax-memory-bridge-sync"),e=()=>{if(!s)return;const r=t.getHealth(),c=t.getStatus(),n=c.filter(y=>y.last_success).length;s.textContent=`${r.backends_configured} backends configurés · ${n}/${c.length} dernier sync OK`};e(),a&&i&&i.bind(a,"click",()=>{(async()=>{a&&(a.disabled=!0);const r=await t.runAutoSync(),c=r.filter(y=>y.ok).length,{toast:n}=await p(async()=>{const{toast:y}=await import("./toast-BCPNzfMv.js");return{toast:y}},[],import.meta.url);r.length===0?n.warn("Aucun backend configuré"):c===r.length?n.success(`Sync OK (${c}/${r.length})`):n.warn(`Sync partielle (${c}/${r.length})`),e(),a&&(a.disabled=!1)})()})}catch(t){z.warn("feature-settings","memory-bridge wire failed",{err:t})}})();const _=o.querySelector("#ax-conso-scan");_&&i&&i.bind(_,"click",()=>{(async()=>{try{const{consumptionAnomalyDetector:t}=await p(async()=>{const{consumptionAnomalyDetector:e}=await import("./monitoring-yJ4fpvfw.js").then(r=>r.J);return{consumptionAnomalyDetector:e}},__vite__mapDeps([0,1,2,3]),import.meta.url),s=t.scanAllVerbose(),a=o.querySelector("#ax-conso-results");if(!a)return;a.innerHTML=s.map(e=>{const r=e.severity==="critical"?"var(--ax-sev-critical)":e.severity==="high"?"var(--ax-sev-high)":e.severity==="medium"?"var(--ax-sev-medium)":e.severity==="low"?"var(--ax-sev-low)":"var(--ax-green)",c=e.severity==="critical"?"ax-sev-critical":e.severity==="high"?"ax-sev-high":e.severity==="medium"?"ax-sev-medium":(e.severity==="low","ax-sev-low"),n=e.severity==="critical"?"🚨":e.severity==="high"?"⚠️":e.severity==="medium"?"🟡":e.severity==="low"?"🔵":"✅";return`<div style="background:rgba(255,255,255,0.03);border-left:3px solid ${r};padding:8px 12px;margin-top:6px;border-radius:4px">
          ...
          </div>`}).join("")}catch(t){z.warn("feature-settings","conso scan failed",{err:t})}})()});const S=o.querySelector("#ax-zoom-inspector-btn");S&&i&&i.bind(S,"click",()=>{(async()=>{const{apexZoomInspector:t}=await p(async()=>{const{apexZoomInspector:s}=await import("./apex-zoom-inspector-D71pRDX7.js");return{apexZoomInspector:s}},__vite__mapDeps([0,1,2,3]),import.meta.url);t.isVisible()?t.hide():t.show()})()});const w=o.querySelector("#ax-functional-test-btn");w&&i&&i.bind(w,"click",()=>{(async()=>{const t=o.querySelector("#ax-functional-test-results");if(t){t.innerHTML=`<div style="color:var(--ax-blue)">⏳ Test des boutons en cours (jusqu'à 40 boutons, ~30s)...</div>`;try{const{apexFunctionalTester:s}=await p(async()=>{const{apexFunctionalTester:f}=await import("./apex-functional-tester-DzzoCJ0s.js");return{apexFunctionalTester:f}},__vite__mapDeps([0,1,2,3]),import.meta.url),{reportsHistory:a}=await p(async()=>{const{reportsHistory:f}=await import("./apex-reports-history-C1bV2nEb.js");return{reportsHistory:f}},__vite__mapDeps([0,1,2,3]),import.meta.url),e=await s.testAndAutoFix({maxButtons:30});a.recordFunctional(e.before,e.fixes,e.after,e.improvement);const r=e.before.tested>0?Math.round(e.before.ok/e.before.tested*100):0,c=e.after?` → après fix : ${Math.round(e.after.ok/Math.max(1,e.after.tested)*100)}% OK (${e.improvement>0?"+":""}${Math.round(e.improvement*100)}%)`:"",n=e.before.details.filter(f=>f.status==="no_response"||f.status==="error").slice(0,5).map(f=>`<li class="ax-gs-250">${u(f.label||"(no label)")} → ${u(f.status)}</li>`).join(""),y=e.fixes.applied.map(f=>u(String(f))).join(", ");t.innerHTML=`
          ...
          `}catch(s){const a=s instanceof Error?s.message:String(s);t.innerHTML=`<div class="ax-gs-76">❌ Erreur test : ${u(a)}</div>`}}})()});const L=o.querySelector("#ax-layout-inspect-btn");L&&i&&i.bind(L,"click",()=>{(async()=>{const t=o.querySelector("#ax-layout-inspect-results");if(t){t.innerHTML='<div style="color:var(--ax-purple)">⏳ Scan layout...</div>';try{const{apexLayoutInspector:s}=await p(async()=>{const{apexLayoutInspector:n}=await import("./apex-layout-inspector-_moSSVD6.js");return{apexLayoutInspector:n}},__vite__mapDeps([0,1,2,3]),import.meta.url),{reportsHistory:a}=await p(async()=>{const{reportsHistory:n}=await import("./apex-reports-history-C1bV2nEb.js");return{reportsHistory:n}},__vite__mapDeps([0,1,2,3]),import.meta.url),e=s.scanDom();a.recordLayout(e);const r=e.hiddenButtons.slice(0,5).map(n=>`<li class="ax-gs-250">"${n.label}" → ${n.reason}</li>`).join(""),c=e.overflowingElements.slice(0,5).map(n=>`<li style="color:var(--ax-error);font-size:11px">${n.tag} (+${n.overflowBy}px)</li>`).join("");t.innerHTML=`
          ...
          `}catch(s){const a=s instanceof Error?s.message:String(s);t.innerHTML=`<div class="ax-gs-76">❌ Erreur scan : ${u(a)}</div>`}}})()});const T=o.querySelector("#ax-cf-diagnostic-btn");T&&i&&i.bind(T,"click",()=>{(async()=>{const t=o.querySelector("#ax-cf-diagnostic-results");if(t){t.innerHTML='<div class="ax-gs-168">⏳ Test Cloudflare API en cours...</div>';try{const{apexCloudflareVaultDeploy:s}=await p(async()=>{const{apexCloudflareVaultDeploy:c}=await import("./apex-cloudflare-vault-deploy-C4rLPm1u.js");return{apexCloudflareVaultDeploy:c}},__vite__mapDeps([0,1,2,3]),import.meta.url),a=await s.runDiagnostic(),e=(c,n,y)=>`
