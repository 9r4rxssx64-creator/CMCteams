import{e as a}from"./escape-html-BlQj2yEF.js";import{c as y}from"./listener-cleanup-Y2rGGxxX.js";import{s as h,l as b}from"./monitoring-D2lWYrYo.js";import{m,s as g}from"./multi-source-analyze-Bg1HHfSC.js";import{haptic as l}from"./haptic-CQFg2PXZ.js";import{toast as n}from"./toast-CRdbcLoc.js";import"./apex-kb-D1VtWFD9.js";import"./credential-patterns-CLzI061R.js";let u=null;function L(){u?.cleanup(),u=null}async function p(e){if(!(h.get("isAdmin")===!0)){e.innerHTML=`
      <div class="ax-gs-21">
        <h2 style="color:#c9a227">Accès admin uniquement</h2>
        <p>Multi-Source History — Kevin admin.</p>
      </div>
    `;return}u?.cleanup(),u=y("multi-source-history");const r=m.getHistory(),o=m.getStats(),d=g.listKnown();e.innerHTML=v(r,o,d.length),S(e,r),b.info("multi-source-history",`Render ${r.length} sources, ${o.items_total} items`)}function v(e,t,r){const o=t.items_total>0?Math.round(t.items_configured/t.items_total*100):0,d=t.items_configured>0?Math.round(t.items_tested_ok/t.items_configured*100):0;return`
    <div style="padding:20px;max-width:1100px;margin:0 auto;color:var(--ax-text,#eee)">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;margin-bottom:24px">
        ${c("📥 Sources",String(t.sources_total),"#c9a227")}
        ${c("🎯 Éléments",String(t.items_total),"#5c9eff")}
        ${c("✅ Configurés",`${t.items_configured} (${o}%)`,"#22cc77")}
        ${c("🟢 Testés OK",`${t.items_tested_ok} (${d}%)`,"#aaff77")}
        ${c("📚 Services étudiés",String(r),"#ff8855")}
      </div>
  ...
  `}function _(e){const t={credential:"#c9a227",site:"#5c9eff",identifier:"#aaff77",address:"#ff8855",device_id:"#ff66cc",metadata:"#888"}[e.type],r=e.test_result?e.test_result.ok?`<span class="ax-gs-14">🟢 OK${e.test_result.latency_ms?` (${e.test_result.latency_ms}ms)`:""}</span>`:`<span style="color:#ff6b6b">🔴 ${a(e.test_result.error??"fail")}</span>`:'<span class="ax-gs-64">—</span>',o=e.forbidden?'<span style="color:#ff6b6b;font-size:10px;margin-left:4px;background:rgba(255,107,107,0.1);padding:2px 4px;border-radius:3px">🚫 FORBIDDEN</span>':"";return`
    <tr style="border-bottom:1px solid #2a2a2a">
      <td style="padding:6px 8px;color:${t};font-weight:600">${a(e.type)}</td>
      <td style="padding:6px 8px">${a(e.service??"—")}${o}</td>
  ...
  `}function S(e,t){e.querySelector("#ax-msh-refresh")?.addEventListener("click",()=>{l.tap(),p(e)}),e.querySelector("#ax-msh-refresh-services")?.addEventListener("click",()=>{l.tap(),(async()=>{n.info("🔄 Refresh services en cours...",{duration:3e3});try{const s=await g.refreshAll();n.success(`✅ ${s.refreshed} services rafraîchis`,{duration:5e3}),s.errors.length>0&&n.warn(`⚠️ ${s.errors.length} erreur(s)`,{duration:5e3}),p(e)}catch(s){const i=s instanceof Error?s.message:String(s);n.error(`Refresh fail : ${i}`,{duration:5e3})}})()}),e.querySelector("#ax-msh-clear")?.addEventListener("click",()=>{if(l.warning(),!!confirm("Vider tout l'historique multi-source ? (irréversible)"))try{localStorage.removeItem("ax_multi_source_history"),n.success("🗑️ Historique vidé",{duration:3e3}),p(e)}catch(s){const i=s instanceof Error?s.message:String(s);n.error(`Clear fail : ${i}`)}}),e.querySelectorAll(".ax-msh-reanalyze").forEach(s=>{s.addEventListener("click",()=>{l.tap();const i=parseInt(s.dataset.idx??"-1",10),x=t[i];if(!x){n.warn("Source non trouvée");return}n.info(`Re-analyse demandée pour source #${i} (${x.source_type})`,{duration:4e3}),m.installAll(x,{test:!0}).then(f=>{n.success(`✅ ${f.installed} installés · ${f.tested_ok} testés OK`,{duration:5e3}),p(e)})})})}export{L as dispose,p as render};
