import{a as d}from"./escape-html-DGIYNPKb.js";import{c as v}from"./listener-cleanup-Y2rGGxxX.js";import{r as g}from"../core/main-B1tqK_pJ.js";import{s as h}from"./signup-dTjM3Rql.js";import{haptic as c}from"./haptic-CQFg2PXZ.js";import{toast as x}from"./toast-CRdbcLoc.js";import"./apex-kb-WlLpLTss.js";import"./monitoring-DMtdadhB.js";import"./credential-patterns-CLzI061R.js";import"./multi-source-analyze-C8YgDmlx.js";import"./auth-gate-eyyXLwOJ.js";import"./commerce-BPX7Ln9h.js";import"./kevin-alerts-DoDYdbK6.js";import"./whatsapp-_CKb_5rY.js";let t=null;function M(){t?.cleanup(),t=null}const k=[{id:"free",label:"🆓 Découverte",price:"0 €",features:["Chat IA basique","50 messages/jour"]},{id:"basic",label:"✨ Basic",price:"9,90 €/mois",features:["Chat illimité","Studios","Voix"]},{id:"pro",label:"💎 Pro",price:"29,90 €/mois",features:["Tout Basic","Modules pro","Coffre, IoT"]},{id:"family",label:"👨‍👩‍👧 Famille (sur invitation)",price:"Gratuit",features:["Accès complet","Validation Kevin"]}];function H(e){t?.cleanup(),t=v("signup"),e.innerHTML=`
            ...
            <div id="signup-plans" style="display:grid;gap:8px">
              ${k.map((a,l)=>`
                <label style="display:flex;align-items:center;gap:10px;padding:10px;background:rgba(0,0,0,0.25);border:1px solid rgba(255,255,255,0.08);border-radius:10px;cursor:pointer">
                  <input type="radio" name="signup-plan" aria-label="Plan ${d(a.label)}" value="${a.id}" ${l===0?"checked":""} style="accent-color:#c9a227">
                  <div style="flex:1">
                    <div style="display:flex;justify-content:space-between;align-items:center">
                      <strong style="color:#fff;font-size:14px">${d(a.label)}</strong>
                      <span style="color:#c9a227;font-size:13px">${d(a.price)}</span>
                    </div>
                    <div style="color:var(--ax-text-dim,#888);font-size:11px;margin-top:2px">${d(a.features.join(" · "))}</div>
                  </div>
                </label>
              `).join("")}
            </div>
  ...
  `,q(e)}function q(e){if(!t)return;const a=e.querySelector("#signup-form");a&&t.bind(a,"submit",n=>{n.preventDefault(),c.tap(),S(e)});const l=e.querySelector("#signup-go-login");l&&t.bind(l,"click",()=>{c.tap(),g.navigate("landing")});const u=e.querySelector("#signup-link-cgu");u&&t.bind(u,"click",n=>{n.preventDefault(),g.navigate("legal")});const m=e.querySelector("#signup-link-privacy");m&&t.bind(m,"click",n=>{n.preventDefault(),g.navigate("legal")})}async function S(e){const a=(e.querySelector("#signup-prenom")?.value??"").trim(),l=(e.querySelector("#signup-nom")?.value??"").trim(),u=(e.querySelector("#signup-email")?.value??"").trim(),m=(e.querySelector("#signup-whatsapp")?.value??"").trim(),n=e.querySelector('input[name="signup-plan"]:checked')?.value??"free",f=e.querySelector("#signup-cgu")?.checked??!1,y=e.querySelector("#signup-rgpd")?.checked??!1,r=e.querySelector("#signup-submit"),o=r?.querySelector(".ax-btn-label"),p=r?.querySelector(".ax-spinner"),s=e.querySelector("#signup-error");s&&(s.innerHTML=""),r&&(r.disabled=!0),o&&(o.textContent="Envoi en cours..."),p&&(p.style.display="inline-block");try{const i=await h.requestSignup({prenom:a,nom:l,email:u,whatsapp:m,plan:n,consent:{cgu:f,rgpd:y,ts:Date.now()}});if(!i.ok){c.error(),s&&(s.innerHTML=`<div class="ax-alert ax-alert-warn">⚠️ ${d(i.reason??"Erreur")}</div>`),x.error(i.reason??"Erreur"),r&&(r.disabled=!1),o&&(o.textContent="📤 Envoyer ma demande via WhatsApp"),p&&(p.style.display="none");return}c.success();try{i.requestId&&localStorage.setItem("apex_v13_signup_pending_id",i.requestId),i.inviteLink&&localStorage.setItem("apex_v13_signup_invite_link",i.inviteLink)}catch{}if(i.inviteLink)try{window.open(i.inviteLink,"_blank")}catch{}x.success("✅ Demande envoyée — ouvre WhatsApp pour valider"),setTimeout(()=>g.navigate("waiting-approval"),500)}catch(i){c.error();const b=i instanceof Error?i.message:"Erreur inattendue";s&&(s.innerHTML=`<div class="ax-alert ax-alert-error">${d(b)}</div>`),x.error(b),r&&(r.disabled=!1),o&&(o.textContent="📤 Envoyer ma demande via WhatsApp"),p&&(p.style.display="none")}}export{M as dispose,H as render};
