...
import{_ as d}from"./apex-kb-Bv3UHTHR.js";import{e as r}from"./escape-html-BlQj2yEF.js";import{c as v}from"./listener-cleanup-Y2rGGxxX.js";import{l as b}from"./monitoring-3uBGKGRH.js";import"./credential-patterns-CLzI061R.js";let s=null;function I(){s?.cleanup(),s=null}const y={"ai-provider":"🤖 IA Provider","lib-npm":"📦 Lib npm","api-service":"🌐 API Service","browser-api":"🌍 Browser API","tts-stt":"🎙 TTS/STT",vision:"👁 Vision","image-gen":"🎨 Image Gen","video-gen":"🎬 Video Gen","vector-db":"🗄 Vector DB",auth:"🔐 Auth","mobile-framework":"📱 Mobile",bundler:"⚙ Bundler"};function h(i){const e=i.estimatedGain;if(!e)return"—";const n=[];return e.perf!==void 0&&n.push(`perf +${e.perf}%`),e.cost!==void 0&&n.push(`cost +${e.cost}%`),e.capabilities!==void 0&&n.push(`cap +${e.capabilities}%`),e.bundleSize!==void 0&&n.push(`bundle -${e.bundleSize}%`),n.join(", ")||"—"}function S(i){return{"upgrade-asap":'<span style="color:#22cc77">⚡ ASAP</span>',"upgrade-soon":'<span style="color:#f0c020">⏳ SOON</span>',monitor:'<span style="color:#888">👀 MONITOR</span>',skip:'<span style="color:#666">— SKIP</span>',"breaking-changes":'<span style="color:#ff5b5b">⚠ BREAKING</span>'}[i]}async function p(i){s?.cleanup(),s=v("innovation");const{innovationWatch:e}=await d(async()=>{const{innovationWatch:t}=await import("./innovation-watch-BV8hFenX.js");return{innovationWatch:t}},__vite__mapDeps([0,1,2]),import.meta.url),n=e.getStats(),c=e.getUpdates().filter(t=>(t.status??"pending")==="pending"),f=n.lastScan===0?"jamais":new Date(n.lastScan).toLocaleString("fr-FR"),l=new Map;for(const t of c){const a=l.get(t.category)??[];a.push(t),l.set(t.category,a)}i.innerHTML=`
    <div class="ax-page" style="padding:16px;max-width:1000px;margin:0 auto">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:8px;margin-bottom:16px">
        <div style="background:rgba(20,20,35,0.5);padding:12px;border-radius:8px">
          ...
          <div style="font-size:13px;color:#fff">${r(f)}</div>
        </div>
        <div style="background:rgba(20,20,35,0.5);padding:12px;border-radius:8px">
                ...
                <tbody>
                  ${a.map(o=>`
                    <tr style="border-bottom:1px solid rgba(255,255,255,0.05)">
                      <td style="padding:8px;font-size:12px"><strong>${r(o.name)}</strong>${o.details?`<div style="font-size:10px;color:var(--ax-text-dim)">${r(o.details)}</div>`:""}</td>
                      <td style="padding:8px;font-size:11px;color:var(--ax-text-dim)">${r(o.currentVersion??"—")} → ${r(o.latestVersion??"—")}</td>
                      <td style="padding:8px;font-size:11px;color:#22cc77">${r(h(o))}</td>
                      <td style="padding:8px;font-size:11px">${S(o.recommendation)}</td>
                      <td style="padding:8px;text-align:right">
                        <button class="ax-btn ax-btn-sm ax-inno-apply" data-id="${r(o.id)}" style="padding:4px 8px;font-size:11px;background:#22cc77">Apply</button>
                        <button class="ax-btn ax-btn-sm ax-inno-skip" data-id="${r(o.id)}" style="padding:4px 8px;font-size:11px">Skip</button>
                      </td>
                    </tr>
                  `).join("")}
                </tbody>
  ...
  `;const u=i.querySelector("#ax-inno-scan");u&&s&&s.bind(u,"click",()=>{(async()=>{const{toast:t}=await d(async()=>{const{toast:a}=await import("./toast-CRdbcLoc.js");return{toast:a}},[],import.meta.url);t.info("Scan en cours…");try{const a=await e.runScan();t.success(`✅ ${a.summary}`),await p(i)}catch(a){t.error("Scan failed: "+(a instanceof Error?a.message:String(a)))}})()});const g=i.querySelector("#ax-inno-refresh");g&&s&&s.bind(g,"click",()=>{p(i)});const m=i.querySelector("#ax-inno-reset");m&&s&&s.bind(m,"click",()=>{(async()=>{const{toast:t}=await d(async()=>{const{toast:a}=await import("./toast-CRdbcLoc.js");return{toast:a}},[],import.meta.url);typeof confirm=="function"&&!confirm("Reset historique innovation watch ?")||(e.reset(),t.success("Historique réinitialisé"),await p(i))})()}),i.querySelectorAll(".ax-inno-apply").forEach(t=>{s.bind(t,"click",()=>{(async()=>{const a=t.dataset.id;if(!a)return;const{toast:o}=await d(async()=>{const{toast:x}=await import("./toast-CRdbcLoc.js");return{toast:x}},[],import.meta.url);e.markUpdate(a,"applied"),o.success("Marqué comme appliqué"),await p(i)})()})}),i.querySelectorAll(".ax-inno-skip").forEach(t=>{s.bind(t,"click",()=>{(async()=>{const a=t.dataset.id;if(!a)return;const{toast:o}=await d(async()=>{const{toast:x}=await import("./toast-CRdbcLoc.js");return{toast:x}},[],import.meta.url);e.markUpdate(a,"skipped"),o.info("Skippé"),await p(i)})()})}),b.info("feature-innovation",`rendered ${c.length} pending updates (${n.totalUpdatesDetected} total)`)}export{I as dispose,p as render};
