const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./monitoring-3uBGKGRH.js","./apex-kb-Bv3UHTHR.js","./credential-patterns-CLzI061R.js","./multi-source-analyze-C6-Y5ORB.js"])))=>i.map(i=>d[i]);
import{_ as g}from"./apex-kb-Bv3UHTHR.js";import{e as i}from"./escape-html-BlQj2yEF.js";import{c as f}from"./listener-cleanup-Y2rGGxxX.js";import{l as m}from"./monitoring-3uBGKGRH.js";import{s as b}from"../core/main-nnFoIOkm.js";import{c as v}from"./csp-style-helper-BisGRi53.js";import{g as h}from"./apex-tools-dispatch-core-BXRLjua5.js";import{haptic as y}from"./haptic-CQFg2PXZ.js";import{r as w}from"./recharge-action-C73aOkPx.js";import"./credential-patterns-CLzI061R.js";import"./multi-source-analyze-C6-Y5ORB.js";import"./apex-tools-dispatch-skills-B9CCcyTS.js";import"./apex-tools-dispatch-data-CiyYnBGc.js";import"./apex-tools-dispatch-finance-D84Ce07W.js";import"./apex-tools-dispatch-misc-P7OCW2w5.js";import"./apex-tools-misc-OP0KiSxw.js";import"./apex-tools-registry-core-BMhHY4vU.js";import"./apex-tools-registry-skills-DRX4PPQ_.js";import"./voice-DwaF0v2N.js";let d=null;function W(){d?.cleanup(),d=null}const k=[{id:"chat",icon:"💬",label:"Chat",description:"Conversation IA",route:"chat",color:"var(--ax-blue-bright)"},{id:"vault",icon:"🔐",label:"Coffre",description:"Clés API & secrets",route:"vault",color:"var(--ax-gold-deep)"},{id:"browser",icon:"🌐",label:"Browser",description:"Naviguer & embed",route:"browser",color:"var(--ax-green)"},{id:"studios",icon:"🎨",label:"Studios",description:"Créatif (musique/vidéo)",route:"studios",color:"var(--ax-purple)"},{id:"pro",icon:"🎓",label:"Pro",description:"Modules expert",route:"pro",color:"var(--ax-pink)"},{id:"self-diag",icon:"🩺",label:"Audit",description:"Auto-diagnostic",route:"self-diag",color:"var(--ax-cyan)"},{id:"settings",icon:"⚙️",label:"Réglages",description:"Configurer Apex",route:"settings",color:"var(--ax-text-dim)"},{id:"rgpd",icon:"🛡",label:"RGPD",description:"Mes données",route:"rgpd",color:"var(--ax-gold)"}];async function $(){let t=0,r=0,o=0,a=0,e=0,c=0;try{const l=localStorage.getItem("apex_v13_messages_24h")??"0",n=parseInt(l,10);Number.isNaN(n)||(t=n)}catch{}try{const l=localStorage.getItem("apex_v13_tokens_24h")??"0",n=parseInt(l,10);Number.isNaN(n)||(r=n)}catch{}try{const{kdmcProjectsRegistry:l}=await g(async()=>{const{kdmcProjectsRegistry:n}=await import("./kdmc-projects-registry-DdTqW2gy.js");return{kdmcProjectsRegistry:n}},__vite__mapDeps([0]),import.meta.url);o=l.countActive()}catch{}try{const{sentinels:l}=await g(async()=>{const{sentinels:s}=await import("./sentinels-C0MFTmUG.js");return{sentinels:s}},__vite__mapDeps([1,0,2]),import.meta.url),n=l.list();e=n.length,a=n.filter(s=>s.lastResult?.ok).length}catch{}try{const l=localStorage.getItem("ax_claude_todo")??"[]",n=JSON.parse(l);Array.isArray(n)&&(c=n.filter(s=>s.status==="pending").length)}catch{}return[{id:"messages",icon:"💬",label:"Messages 24h",value:t,color:"var(--ax-blue-bright)",route:"chat"},{id:"tokens",icon:"🔢",label:"Tokens 24h",value:r.toLocaleString("fr-FR"),color:"var(--ax-gold-deep)",route:"self-diag"},{id:"projects",icon:"📦",label:"Projets actifs",value:o,color:"var(--ax-green)",route:"admin"},{id:"sentinels",icon:"🛡",label:"Sentinelles OK",value:`${a}/${e}`,color:a===e?"var(--ax-green)":"var(--ax-warning)",route:"sentinels"},{id:"todos",icon:"📋",label:"Todos en attente",value:c,color:c>0?"var(--ax-error)":"var(--ax-green)",route:"self-diag"}]}async function z(){const t=[];try{const{sentinels:r}=await g(async()=>{const{sentinels:e}=await import("./sentinels-C0MFTmUG.js");return{sentinels:e}},__vite__mapDeps([1,0,2]),import.meta.url),a=r.list().filter(e=>e.lastResult&&!e.lastResult.ok);for(const e of a.slice(0,3))t.push({id:`sentinel_${e.id}`,level:"warn",title:`Sentinelle ${e.name}`,description:e.lastResult?.msg??"KO",ts:e.lastResult?.ts??Date.now(),action_route:"sentinels"})}catch{}try{const r=JSON.parse(localStorage.getItem("apex_v13_credentials_expiring")??"[]");if(Array.isArray(r))for(const o of r.slice(0,3))o.days_left<30&&t.push({id:`cred_${o.service}`,level:o.days_left<7?"error":"warn",title:`Credential ${o.service}`,description:`Expire dans ${o.days_left} jours`,ts:Date.now(),action_route:"vault"})}catch{}return t}async function _(){try{const{multiKeyVault:t}=await g(async()=>{const{multiKeyVault:a}=await import("./multi-key-vault-l7rh4xaH.js");return{multiKeyVault:a}},__vite__mapDeps([1,0,2]),import.meta.url),r=t.getKnownServices(),o=[];for(const a of r){const e=t.getStats(a),c=t.getServiceLight(a);o.push({service:a,light:c,totalKeys:e.total,activeKeys:e.active,failingKeys:e.failing,invalidKeys:e.invalid,lastSuccess:e.lastSuccess})}return o}catch{return[]}}function S(){const t=[];try{const r=localStorage.getItem("ax_claude_todo")??"[]",o=JSON.parse(r);if(Array.isArray(o))for(const a of o.filter(e=>e.status==="pending").slice(0,5))t.push({id:a.id??`todo_${Date.now()}`,source:"apex_todo",title:a.reason??"Todo sans description",severity:a.severity??"medium",ts_created:a.ts??Date.now()})}catch{}return t}function A(t,r=0){return`
    ...
    </button>`}function R(t,r=0){return`
    <button class="ax-shortcut-card ax-modernized-card ax-bounce-tap" data-route="${i(t.route)}"
      style="position:relative;background:linear-gradient(135deg,rgba(20,20,35,0.7),rgba(14,14,28,0.55));backdrop-filter:blur(16px) saturate(140%);-webkit-backdrop-filter:blur(16px) saturate(140%);border:1px solid rgba(255,255,255,0.08);border-radius:14px;padding:16px;cursor:pointer;text-align:left;transition:all 240ms cubic-bezier(0.16,1,0.3,1);display:flex;align-items:center;gap:14px;min-height:72px;overflow:hidden;animation:ax-fade-up 320ms cubic-bezier(0.34,1.56,0.64,1) ${30+r*20}ms backwards;-webkit-tap-highlight-color:transparent">
      ...
      </div>`:t.map((r,o)=>{const a=r.level==="error"?"var(--ax-sev-critical)":r.level==="warn"?"var(--ax-sev-high)":"var(--ax-sev-low)",e=r.level==="error"?"255,91,91":r.level==="warn"?"255,153,68":"106,138,255",c=r.level==="error"?"🚨":r.level==="warn"?"⚠️":"ℹ️";return`
        <div class="ax-alert-row ax-modernized-card ax-bounce-tap" ${r.action_route?`data-route="${i(r.action_route)}" role="button" tabindex="0" aria-label="${i(r.title)} — ouvrir"`:""}
      ...
      <div style="display:flex;align-items:center;gap:12px;padding:16px;background:linear-gradient(135deg,rgba(34,204,119,0.08),rgba(34,204,119,0.03));border:1px solid rgba(34,204,119,0.15);border-radius:12px">
      ...
      </div>`:t.map((r,o)=>{const a=r.severity==="critical"?"var(--ax-sev-critical)":r.severity==="high"?"var(--ax-sev-high)":r.severity==="medium"?"var(--ax-sev-medium)":"var(--ax-sev-low)",e=new Date(r.ts_created).toLocaleString("fr-FR");return`
        ...
        </div>`}).join("")}function I(t,r={}){if(t.length===0)return`
      <div class="ax-modernized-card" style="padding:14px 16px;background:linear-gradient(135deg,rgba(20,20,35,0.6),rgba(14,14,28,0.4));backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.06);border-radius:12px">
      ...
      </div>`;const o={green:"var(--ax-green)",yellow:"var(--ax-warning)",red:"var(--ax-error)",gray:"var(--ax-text-muted)"},a={green:"OK",yellow:"Partiel",red:"Panne",gray:"Non testé"};return t.map((e,c)=>{const l=o[e.light],n=a[e.light],s=`${e.activeKeys}/${e.totalKeys} active${e.failingKeys>0?` · ${e.failingKeys} failing`:""}${e.invalidKeys>0?` · ${e.invalidKeys} invalid`:""}`,p=r[e.service]??{recharge:null,usage:null},x=p.recharge&&(e.light==="yellow"||e.light==="red"||e.light==="gray"),u=w({rechargeUrl:x?p.recharge??void 0:void 0,rotateUrl:p.usage??void 0,variant:"button",label:"Recharge"});return`
        <div class="ax-service-health-row ax-modernized-card"
          ...
          ${u}
        </div>`}).join("")}async function T(t){const r={};if(t.length===0)return r;try{const{linksRegistry:o}=await g(async()=>{const{linksRegistry:a}=await import("./multi-source-analyze-C6-Y5ORB.js").then(e=>e.c);return{linksRegistry:a}},__vite__mapDeps([3,1,0,2]),import.meta.url);for(const a of t)r[a]={recharge:o.getRechargeLink(a),usage:o.getUsageLink(a),apiKeys:o.getApiKeysLink(a)}}catch{}return r}async function Z(t){d?.cleanup(),d=f("dashboard");const r=b.get("user");if(!h("admin.dashboard",t,r?.id))return;const o=r?.name?`Bonjour ${r.name}`:"Bonjour",[a,e,c]=await Promise.all([$(),z(),_()]),l=await T(c.map(s=>s.service)),n=S();t.innerHTML=v.withNonce(`
    <style>
    ...
    <div class="ax-page" style="padding:24px 16px max(24px, env(safe-area-inset-bottom)) 16px;max-width:1140px;margin:0 auto;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif">
      ...
      <section style="margin-bottom:32px">
        <h2 style="font-size:12px;color:rgba(232,184,48,0.85);margin:0 0 14px;text-transform:uppercase;letter-spacing:0.12em;font-weight:700;display:flex;align-items:center;gap:8px">
          <span style="font-size:14px;-webkit-text-fill-color:initial">🚥</span> Statut services IA ${c.length>0?`<span style="display:inline-block;padding:2px 10px;background:rgba(106,138,255,0.15);color:var(--ax-blue);border-radius:24px;font-size:11px;font-weight:700">${c.length}</span>`:""}
        </h2>
        ${I(c,l)}
      </section>
  ...
  `),t.querySelectorAll("[data-route]").forEach(s=>{d.bind(s,"click",()=>{y.tap();const p=s.dataset.route;p&&(window.location.hash="#"+p)})}),m.info("feature-dashboard",`rendered (${a.length} kpis, ${e.length} alerts, ${n.length} todos)`)}export{$ as computeKpis,W as dispose,i as escapeHtml,z as loadAlerts,T as loadRechargeLinks,_ as loadServiceHealth,S as loadTodos,Z as render,I as renderServiceHealthCard};
