...
import{_ as d}from"./apex-kb-KUpdSyu5.js";import{e as n}from"./escape-html-BlQj2yEF.js";import{c as P}from"./listener-cleanup-Y2rGGxxX.js";import{l as q}from"./monitoring-glbGSPAY.js";import"./credential-patterns-CLzI061R.js";import"./multi-source-analyze-BKtnCZaJ.js";let i=null;function I(){i?.cleanup(),i=null}const l={anthropic:"🧠",openai:"🤖",groq:"⚡",gemini:"✨",mistral:"🌊",cohere:"🔷",xai:"🔥",perplexity:"🔍",deepseek:"🐋",openrouter:"🌐"};function v(t){return t<0?"—":t<1e3?`${Math.round(t)}ms`:`${(t/1e3).toFixed(1)}s`}function $(t){return t<0?"—":`${Math.round(t)}%`}function w(t){if(t===0)return"jamais";const e=Date.now()-t;return e<6e4?"à l'instant":e<36e5?`il y a ${Math.round(e/6e4)} min`:e<864e5?`il y a ${Math.round(e/36e5)} h`:`il y a ${Math.round(e/864e5)} j`}function S(t){return!t||t.last_ping_ts===0?'<span class="ax-gs-64">⚪</span>':t.last_ping_ok?t.latency_avg_ms>3e3?'<span class="ax-gs-44">🟡</span>':'<span class="ax-gs-14">🟢</span>':'<span class="ax-gs-13">🔴</span>'}function m(t){return t>=80?"#22cc77":t>=60?"#c9a227":t>=40?"#f0c020":"#ff5b5b"}async function p(t){i?.cleanup(),i=P("smart-router");const{smartRouter:e}=await d(async()=>{const{smartRouter:s}=await import("./smart-router-CJBuaFG2.js");return{smartRouter:s}},__vite__mapDeps([0,1,2,3]),import.meta.url),R=await e.rankProviders(),f=await e.getRecommendations(),g=e.getOverride(),r=R[0],x=e.getAllProviders(),o=r?await e.getStats(r.provider):null,b=await Promise.all(x.map(async s=>({provider:s,stats:await e.getStats(s),score:await e.scoreProvider(s)})));b.sort((s,a)=>a.score.total-s.score.total),t.innerHTML=`
  ...
  `;const c=t.querySelector("#ax-sr-retest");c&&i&&i.bind(c,"click",()=>{(async()=>{const{toast:s}=await d(async()=>{const{toast:a}=await import("./toast-CRdbcLoc.js");return{toast:a}},[],import.meta.url);c.disabled=!0,c.textContent="⏳ Test en cours…",s.info("Ping de 10 providers en parallèle…");try{await e.pingAllProviders(),s.success("✅ Tous les providers re-testés"),await p(t)}catch(a){s.error("Erreur ping: "+(a instanceof Error?a.message:String(a))),c.disabled=!1,c.textContent="🔄 Re-tester tous maintenant"}})()});const y=t.querySelector("#ax-sr-refresh");y&&i&&i.bind(y,"click",()=>{p(t)});const _=t.querySelector("#ax-sr-reset");_&&i&&i.bind(_,"click",()=>{(async()=>{const{toast:s}=await d(async()=>{const{toast:a}=await import("./toast-CRdbcLoc.js");return{toast:a}},[],import.meta.url);typeof confirm=="function"&&!confirm("Reset toutes les stats Smart Router ? (samples + scores + override)")||(e.resetAll(),s.success("Stats reset"),await p(t))})()});const h=t.querySelector("#ax-sr-clear-override");h&&i&&i.bind(h,"click",()=>{(async()=>{const{toast:s}=await d(async()=>{const{toast:a}=await import("./toast-CRdbcLoc.js");return{toast:a}},[],import.meta.url);e.setOverride(null),s.success("Override retiré — auto-routing actif"),await p(t)})()}),t.querySelectorAll(".ax-sr-force").forEach(s=>{i.bind(s,"click",()=>{(async()=>{const a=s.dataset.provider;if(!a)return;const{toast:u}=await d(async()=>{const{toast:k}=await import("./toast-CRdbcLoc.js");return{toast:k}},[],import.meta.url);e.setOverride(a),u.success(`📌 ${a} forcé pour tous les calls`),await p(t)})()})}),q.info("feature-smart-router",`rendered ${x.length} providers, best=${r?.provider??"none"}`)}export{I as dispose,p as render};
