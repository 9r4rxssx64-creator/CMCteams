...
import{v as B,_ as C}from"./apex-kb-DRERxXuJ.js";import{e as g}from"./escape-html-BlQj2yEF.js";import{c as X}from"./listener-cleanup-Y2rGGxxX.js";import{l as m}from"./monitoring-3uBGKGRH.js";import{s as P,a as Y}from"../core/main-CxoKTJ9r.js";import{autoDiscoverLinks as Z}from"./auto-discover-links-I6DasFuO.js";import{detectCredential as R,CREDENTIAL_PATTERNS as V}from"./credential-patterns-CLzI061R.js";import{c as ee}from"./csp-style-helper-BisGRi53.js";import{g as te}from"./apex-tools-dispatch-core-D6SzUGOU.js";import{g as re}from"./generic-secrets-BLnv2dMw.js";import{l as O}from"./multi-source-analyze-Bfq2osax.js";import{multiKeyVault as S}from"./multi-key-vault-BEpo4ZDn.js";import{haptic as c}from"./haptic-CQFg2PXZ.js";import{toast as l}from"./toast-CRdbcLoc.js";import"./apex-tools-dispatch-skills-BsY-IUiM.js";import"./apex-tools-dispatch-data-CyoUD8a0.js";import"./apex-tools-dispatch-finance-D84Ce07W.js";import"./apex-tools-dispatch-misc-CIy4pfP_.js";import"./apex-tools-misc-lkYrWQIJ.js";import"./apex-tools-registry-core-BMhHY4vU.js";import"./apex-tools-registry-skills-DRX4PPQ_.js";import"./voice-627WxwAa.js";let s=null;function Fe(){s?.cleanup(),s=null}const L=[{id:"ai",label:"🤖 IA & LLM",serviceMatchers:["anthropic","openai","groq","google","gemini","openrouter","cohere","mistral","perplexity","deepseek","xai","elevenlabs","replicate","huggingface","fireworks","togetherai","deepl"],patternCategories:["ai"]},{id:"finance",label:"💳 Paiements & Finance",serviceMatchers:["stripe","paypal","revolut","wise","lydia","n26","boursorama","fortuneo","ing","socgen","bnp","credit_agricole","credit_mutuel","banque_postale","lbp","bpce","shopify"],patternCategories:["finance"]},{id:"devops",label:"🛠 DevOps & Code",serviceMatchers:["github","gitlab","cloudflare","vercel","netlify","railway","aws","heroku","sentry","npm"],patternCategories:["devops"]},{id:"comms",label:"📨 Communications",serviceMatchers:["telegram","discord","slack","brevo","resend","twilio","sendgrid","mailchimp","whatsapp"],patternCategories:["comms"]},{id:"social",label:"🌐 Réseaux sociaux",serviceMatchers:["facebook","instagram","tiktok","youtube","twitter","linkedin"],patternCategories:[]},{id:"storage",label:"☁️ Stockage & Cloud",serviceMatchers:["firebase","supabase","airtable","notion","dropbox","pinecone","weaviate"],patternCategories:["storage"]},{id:"ecommerce",label:"🛒 E-commerce",serviceMatchers:["shopify","stripe_connect","paypal_business"],patternCategories:[]},{id:"crypto",label:"₿ Crypto",serviceMatchers:["coinbase","binance","crypto_com","kraken"],patternCategories:[]},{id:"identity",label:"🆔 Identité Kevin",serviceMatchers:["kevin","iban","siret","vat","bic","apple","microsoft"],patternCategories:["identity"]},{id:"other",label:"📦 Autres",serviceMatchers:[],patternCategories:["saas"]}];function E(e,r){const t=e.toLowerCase();let a=null;for(const i of L)if(i.id!=="other")for(const o of i.serviceMatchers)t.includes(o)&&(!a||o.length>a.matchLen)&&(a={catId:i.id,matchLen:o.length});if(a)return a.catId;if(r){for(const i of L)if(i.patternCategories.includes(r))return i.id}return"other"}function I(){return V.filter(e=>e.category!=="forbidden").map(e=>{const r=B.getKeyStatus(e.storageKey),t=(()=>{try{return localStorage.getItem(e.storageKey)??""}catch{return""}})(),a=t&&t.length>8&&!t.startsWith("AXENC1:")?B.maskKey(t):t.startsWith("AXENC1:")?"🔒 chiffré":"";return{pattern:e,status:r,masked:a}})}function je(e,r){return e.filter(t=>{if(r.category&&t.pattern.category!==r.category||r.configuredOnly&&t.status==="empty")return!1;if(r.query){const a=r.query.toLowerCase();if(!(t.pattern.name.toLowerCase().includes(a)||t.pattern.storageKey.toLowerCase().includes(a)))return!1}return!0})}function D(){const e=[];let r=[];try{r=S.listAll(!0)}catch(t){m.warn("feature-vault","multiKeyVault.listAll failed",{err:t})}for(const t of r){const a=O.get(t.service),i=V.find(x=>x.storageKey.includes(t.service)),o={id:t.id,service:t.service,serviceName:a?.name??ie(t.service),category:E(t.service,i?.category),status:t.status,source:"multi-key"};t.alias!==void 0&&(o.alias=t.alias),t.addedAt!==void 0&&(o.addedAt=t.addedAt),t.lastTestedAt!==void 0&&(o.lastTestedAt=t.lastTestedAt);const b=O.getRechargeLink(t.service);b&&(o.rechargeUrl=b),e.push(o)}return e}function G(){const e=D(),r={total:e.length,active:0,failing:0,invalid:0};for(const t of e)t.status==="active"?r.active+=1:t.status==="failing"||t.status==="rate-limited"?r.failing+=1:t.status==="invalid"&&(r.invalid+=1);return r}function ae(e,r=""){const t=D(),a=r.trim().toLowerCase();return t.filter(i=>i.category!==e.id?!1:a?i.service.toLowerCase().includes(a)||i.serviceName.toLowerCase().includes(a)||(i.alias?.toLowerCase().includes(a)??!1):!0)}function ie(e){return e.charAt(0).toUpperCase()+e.slice(1)}async function oe(e){const r=e.trim();if(!r)return{ok:!1,reason:"Entrée vide"};const t=R(r);if(t&&t.category==="forbidden")return{ok:!1,reason:"🚨 Type interdit (cartes/seed phrases jamais stockées)"};if(t)try{const a=await B.encryptAuto(r);return localStorage.setItem(t.storageKey,a),{ok:!0,pattern_name:t.name,storage_key:t.storageKey}}catch(a){return m.warn("vault-feature","autoDetectAndStore failed",{err:a}),{ok:!1,reason:"Erreur chiffrement"}}if(r.length>=20){const a=await re.add(r,void 0,"Auto-détecté (pattern inconnu)");return a.ok?{ok:!0,generic:!0,pattern_name:"Secret générique",storage_key:"apex_v13_generic_secrets",generic_id:a.id}:{ok:!1,reason:a.reason}}return{ok:!1,reason:"Aucun pattern reconnu (trop court pour secret générique)"}}function Pe(e){try{return localStorage.removeItem(e),!0}catch(r){return m.warn("vault-feature","remove failed",{err:r}),!1}}function U(e){const r={exported_at:new Date().toISOString(),version:1,entries:e.filter(t=>t.status!=="empty").map(t=>{const a=(()=>{try{return localStorage.getItem(t.pattern.storageKey)??""}catch{return""}})();return{storage_key:t.pattern.storageKey,name:t.pattern.name,value_encrypted:a}})};return JSON.stringify(r,null,2)}function ne(e){const r=se[e.status]??"#888",t=le[e.status]??"⚪",a=(e.preview??"").slice(0,4)+"••••••"+(e.preview??"").slice(-4),i=e.preview?a:"••••••",o=e.rechargeUrl??"",b=e.alias?`<span style="color:#888;font-size:11px">— ${g(e.alias)}</span>`:"",x=e.logoUrl?`<img src="${g(e.logoUrl)}" alt="" loading="lazy" decoding="async" style="width:24px;height:24px;border-radius:6px" onerror="this.style.display='none'">`:"",v=[];e.addedAt&&v.push(`Ajouté ${H(e.addedAt)}`),e.lastTestedAt&&v.push(`Testé ${H(e.lastTestedAt)}`);const y=v.length>0?`<div style="display:flex;gap:8px;font-size:11px;color:#888;margin-bottom:10px">${v.map(h=>`<span>${g(h)}</span>`).join("")}</div>`:"";return`
    <div class="ax-cred-card" data-cred-id="${g(e.id)}" data-service="${g(e.service)}"
      ...
      <div style="display:flex;gap:6px;flex-wrap:wrap">
        ...
        <button data-action="recharge" data-service="${g(e.service)}" data-recharge-url="${g(o)}" ${o?"":"disabled"}
          style="flex:1;min-width:80px;padding:6px 10px;background:rgba(201,162,39,0.1);color:var(--ax-gold-deep);border:1px solid rgba(201,162,39,0.3);border-radius:6px;cursor:pointer;font-size:11px;min-height:32px;${o?"":"opacity:0.4;cursor:not-allowed"}">💰 Recharger</button>
  ...
  `}const se={active:"var(--ax-green)",failing:"var(--ax-warning)","rate-limited":"var(--ax-warning)",invalid:"var(--ax-error)",unknown:"#888"},le={active:"🟢",failing:"🟡","rate-limited":"🟡",invalid:"🔴",unknown:"⚪"};function H(e){const r=Date.now()-e;if(r<0||!Number.isFinite(r))return"à l'instant";const t=Math.floor(r/6e4);if(t<1)return"à l'instant";if(t<60)return`il y a ${t}min`;const a=Math.floor(t/60);if(a<24)return`il y a ${a}h`;const i=Math.floor(a/24);return i<30?`il y a ${i}j`:`il y a ${Math.floor(i/30)} mois`}let K="";function w(e){if(s?.cleanup(),s=X("vault"),!P.get("isAdmin")){e.innerHTML=`<div style="padding:40px;text-align:center"><h2 style="color:var(--ax-gold-deep)">🔒 Coffre admin</h2><p style="color:var(--ax-text-dim)">Cette section est réservée à l'admin Kevin.</p></div>`;return}const t=P.get("user")?.id??"anon";if(!te("admin.vault",e,t))return;const a=G();e.innerHTML=ee.withNonce(`
    <style>
    ...
    <div class="ax-vault-page" style="padding:env(safe-area-inset-top,16px) 16px calc(env(safe-area-inset-bottom,16px) + 96px);max-width:1140px;margin:0 auto;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif">
      ...
      <div class="ax-vault-sticky-wrap">
        <header style="padding:12px 0">
          ...
          <div class="ax-vault-stats" style="display:flex;gap:14px;padding:8px 0 0;font-size:12px;color:#aaa;flex-wrap:wrap">
            ...
            <span style="color:var(--ax-green)">🟢 ${a.active} actifs</span>
            <span style="color:var(--ax-warning)">🟡 ${a.failing} dégradés</span>
            <span style="color:var(--ax-error)">🔴 ${a.invalid} invalides</span>
          </div>
        </header>
      ...
      <section id="ax-vault-empty-rescue" style="background:linear-gradient(135deg,rgba(255,91,91,0.12),rgba(232,184,48,0.08));border:1px solid rgba(255,91,91,0.4);border-radius:14px;padding:14px;margin-bottom:14px">
        <h3 style="margin:0 0 6px;font-size:14px;color:var(--ax-error);font-weight:700">${a.total===0?"🆘 Coffre vide — Restauration possible":`🚨 ${a.invalid} clé(s) illisible(s) — récupération ou cleanup`}</h3>
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap">
          ...
          ${a.invalid>0?`<button id="ax-vault-cleanup-invalid" data-action="cleanup-invalid" style="padding:10px 16px;background:rgba(255,91,91,0.15);color:var(--ax-error);border:1px solid rgba(255,91,91,0.4);border-radius:10px;cursor:pointer;font-size:13px;min-height:40px">🗑 Supprimer ${a.invalid} illisibles</button>`:""}
        </div>
  ...
  `),F(e),ce(e),de(e),m.info("feature-vault",`rendered (${a.total} entries)`)}function de(e){const r=e.querySelector(".ax-vault-page"),t=e.querySelector("#ax-vault-fab");if(!r)return;let a=0,i=0;const o=()=>{i||(i=requestAnimationFrame(()=>{i=0;const b=window.scrollY||document.documentElement.scrollTop||0;b!==a&&(a=b,b>80?r.classList.add("ax-vault-scrolled"):r.classList.remove("ax-vault-scrolled"))}))};s?s.bind(window,"scroll",o,{passive:!0}):window.addEventListener("scroll",o,{passive:!0}),o(),t&&s&&s.bind(t,"click",()=>{c.tap(),e.querySelector("#ax-vault-test-all")?.click()})}function F(e){const r=e.querySelector("#ax-vault-categories");if(!r)return;if(G().total===0&&!r.dataset.axInitialized){r.dataset.axInitialized="1";const o=document.createElement("div");o.className="ax-skel-vault-wrapper",r.appendChild(o);const b=Y(o,"vault-cards");setTimeout(()=>{b(),o.remove(),F(e)},250)}const a=L.map(o=>{const b=ae(o,K);if(b.length===0&&o.id!=="identity")return"";const x=b.length>0;return`
      <details class="ax-cat" data-cat-id="${g(o.id)}" ${x?"open":""}
        ...
        <summary style="padding:14px 16px;cursor:pointer;display:flex;justify-content:space-between;align-items:center;font-weight:600;list-style:none;-webkit-tap-highlight-color:transparent;min-height:44px">
          <span>${g(o.label)} <span style="color:#888;font-weight:400;font-size:13px">(${b.length})</span></span>
    ...
    `}).join("");let i=r.querySelector(".ax-vault-cats-wrapper");i||(i=document.createElement("div"),i.className="ax-vault-cats-wrapper",r.appendChild(i)),i.innerHTML=a}function ce(e){const r=e.querySelector("#ax-vault-search");if(r){let n=null;s.bind(r,"input",()=>{n&&clearTimeout(n),n=setTimeout(()=>{K=r.value.trim(),F(e),N(e)},240)})}const t=e.querySelector("#ax-vault-add-manual");t&&s&&s.bind(t,"click",()=>{c.tap(),Q(e)});const a=e.querySelector("#ax-vault-test-all");a&&s&&s.bind(a,"click",()=>{(async()=>{c.tap(),l.info("Test de toutes les clés en cours…");try{const n=await S.healthCheckAll();l.success(`✅ ${n.tested} testées · ${n.recovered} récupérées · ${n.stillDown} HS`),w(e)}catch(n){m.warn("feature-vault","testAll failed",{err:n}),l.error("Erreur pendant le test global")}})()});const i=e.querySelector("#ax-vault-rescue-fb");i&&s&&s.bind(i,"click",()=>{(async()=>{c.tap();const n=e.querySelector("#ax-vault-rescue-result");n&&(n.innerHTML="⏳ Lecture Firebase backup chiffré…");try{const{vaultFirebaseBackup:d}=await C(async()=>{const{vaultFirebaseBackup:p}=await import("./vault-firebase-backup-D_EKoZIa.js");return{vaultFirebaseBackup:p}},__vite__mapDeps([0,1,2,3,4]),import.meta.url),u=await d.restoreAllFromFirebaseBackup();if(n){n.textContent="";const p=document.createElement("div");p.style.cssText="padding:8px;background:rgba(34,204,119,.1);color:var(--ax-green);border-radius:8px",p.textContent=`🔓 ${u.restored} clés restaurées · ${u.failed} échouées · ${u.skipped} ignorées`,n.append(p)}u.restored>0?(l.success(`🔓 ${u.restored} clés restaurées depuis Firebase backup`),c.success(),setTimeout(()=>w(e),600)):l.info("Aucune clé trouvée dans Firebase backup")}catch(d){m.warn("feature-vault","rescueFb failed",{err:d}),n&&(n.innerHTML=`<div style="padding:8px;background:rgba(255,88,88,.1);color:var(--ax-error);border-radius:8px">⚠ ${g(String(d).slice(0,120))}</div>`),l.error("Erreur lecture Firebase backup"),c.error()}})()});const o=e.querySelector("#ax-vault-rescue-all");o&&s&&s.bind(o,"click",()=>{(async()=>{c.tap();const n=e.querySelector("#ax-vault-rescue-result");n&&(n.innerHTML="⏳ Scan 4 sources : alias, IDB, Firebase, pattern…");try{const{autoRestoreCredentials:d}=await C(async()=>{const{autoRestoreCredentials:p}=await import("./auto-restore-credentials-CPKpTwGk.js");return{autoRestoreCredentials:p}},__vite__mapDeps([1,2,3,4]),import.meta.url),u=await d.restoreAutomatically();if(n){n.textContent="";const p=document.createElement("div");p.style.cssText="padding:8px;background:rgba(34,204,119,.1);color:var(--ax-green);border-radius:8px",p.textContent=`🔓 ${u.restored} restaurées · ${u.failed} échouées`,n.append(p)}u.restored>0?(l.success(`🔓 ${u.restored} clés restaurées (4 sources)`),c.success(),setTimeout(()=>w(e),600)):l.info("Aucune clé trouvable dans les 4 sources. Colle une clé manuellement ci-dessous.")}catch(d){m.warn("feature-vault","rescueAll failed",{err:d}),n&&(n.innerHTML=`<div style="padding:8px;background:rgba(255,88,88,.1);color:var(--ax-error);border-radius:8px">⚠ ${g(String(d).slice(0,120))}</div>`),l.error("Erreur scan multi-sources"),c.error()}})()});const b=e.querySelector("#ax-vault-cleanup-invalid");b&&s&&s.bind(b,"click",()=>{(async()=>{c.tap();const n=e.querySelector("#ax-vault-rescue-result");if(confirm(`Supprimer définitivement toutes les clés illisibles (decrypt fail) ?

Ces clés sont chiffrées avec une passphrase perdue. Tu devras les recoller pour les retrouver.`)){n&&(n.innerHTML="⏳ Suppression des entrées illisibles…");try{const u=D().filter(f=>f.status==="invalid");let p=0;for(const f of u)try{if(f.id.startsWith("mkv_")||f.id.includes("_")){S.removeKey(f.id),p++;continue}const _=f.id.startsWith("ax_")||f.id.startsWith("apex_v13_")?f.id:`ax_${f.service}_key`;localStorage.removeItem(_);const A=indexedDB.open("apex_v13_vault_shadow",1);A.onsuccess=()=>{try{A.result.transaction("keys","readwrite").objectStore("keys").delete(_),A.result.close()}catch{}},p++}catch{}if(n){n.textContent="";const f=document.createElement("div");f.style.cssText="padding:8px;background:rgba(34,204,119,.1);color:var(--ax-green);border-radius:8px",f.textContent=`🗑 ${p} clé(s) illisibles supprimées. Recolle tes clés via "Détecter & stocker" ci-dessous.`,n.append(f)}l.success(`🗑 ${p} clés illisibles supprimées`),c.success(),setTimeout(()=>w(e),800)}catch(d){m.warn("feature-vault","cleanupInvalid failed",{err:d}),l.error("Erreur suppression"),c.error()}}})()});const x=e.querySelector("#ax-vault-paste-clipboard-btn");x&&s&&s.bind(x,"click",()=>{(async()=>{c.tap();const n=e.querySelector("#ax-vault-paste"),d=e.querySelector("#ax-vault-paste-result");if(n)try{if(!navigator.clipboard?.readText)throw new Error("Clipboard API non supportée");const u=await navigator.clipboard.readText();if(!u){d&&(d.innerHTML='<div style="padding:8px;background:rgba(240,192,32,.1);color:var(--ax-gold);border-radius:8px">⚠ Presse-papier vide</div>');return}n.value=u,n.dispatchEvent(new Event("input",{bubbles:!0})),c.success(),l.success(`📋 ${u.length} caractères collés — clique "Détecter & stocker"`),d&&(d.innerHTML='<div style="padding:8px;background:rgba(106,138,255,.1);color:var(--ax-blue);border-radius:8px">📋 Collé — clique "Détecter & stocker" pour analyser</div>'),n.focus()}catch(u){const p=u instanceof Error?u.message:"unknown";l.error(`Clipboard refusé : ${p}. Utilise long-press → Coller manuellement.`),d&&(d.innerHTML='<div style="padding:8px;background:rgba(255,88,88,.1);color:var(--ax-error);border-radius:8px">⚠ Permission refusée. Long-press dans le champ → Coller.</div>')}})()});const v=e.querySelector("#ax-vault-paste-btn");v&&s&&s.bind(v,"click",()=>{(async()=>{c.tap();const n=e.querySelector("#ax-vault-paste"),d=e.querySelector("#ax-vault-paste-result");if(!n||!d)return;const u=n.value.trim();if(!u){d.innerHTML=`<div style="padding:8px;background:rgba(240,192,32,.1);color:var(--ax-gold);border-radius:8px">⚠ Colle quelque chose d'abord</div>`;return}const p=await oe(u);if(p.ok){c.success(),l.success(`✅ ${p.pattern_name} stocké`),d.innerHTML=`<div style="padding:8px;background:rgba(34,204,119,.1);color:var(--ax-green);border-radius:8px">✅ ${g(p.pattern_name)} → ${g(p.storage_key)}</div>`;const f=R(u);if(f){const _=f.storageKey.replace(/^(ax_|apex_v13_)/,"").replace(/_(?:key|token|pat|sk|pk|id|secret)$/,"");try{await S.addKey(_,u)}catch{}}n.value="",w(e)}else c.error(),l.error(p.reason),d.innerHTML=`<div style="padding:8px;background:rgba(255,88,88,.1);color:var(--ax-error);border-radius:8px">⚠ ${g(p.reason)}</div>`})()});const y=e.querySelector("#ax-vault-import");y&&s&&s.bind(y,"click",()=>{(async()=>{c.tap();try{const{apexVaultImport:n}=await C(async()=>{const{apexVaultImport:u}=await import("./apex-vault-import-BEXmr5ze.js");return{apexVaultImport:u}},__vite__mapDeps([2,1,3]),import.meta.url),d=await n.promptAndImport();if(d.cancelled){l.info("Import annulé",{duration:2e3});return}d.ok&&d.restored>0?(l.success(`🔓 ${d.restored} clés restaurées depuis JSON Drive${d.failed>0?` · ${d.failed} échouées`:""}`,{duration:8e3}),setTimeout(()=>location.reload(),1500)):d.decrypt_failed>0?l.error(`🔒 ${d.decrypt_failed} clés non déchiffrables. PIN admin différent ? Vérifie ton PIN actuel.`,{duration:1e4}):d.error?l.error(`Import échoué : ${d.error.slice(0,80)}`,{duration:8e3}):l.warn("Aucune clé restaurée depuis ce JSON",{duration:5e3})}catch(n){const d=n instanceof Error?n.message:String(n);l.error(`Import erreur : ${d.slice(0,80)}`,{duration:8e3})}})()});const h=e.querySelector("#ax-vault-export");h&&s&&s.bind(h,"click",()=>{c.tap();const n=U(I()),d=new Blob([n],{type:"application/json"}),u=URL.createObjectURL(d),p=document.createElement("a");p.href=u,p.download=`apex-vault-${new Date().toISOString().slice(0,10)}.json`,document.body.appendChild(p),p.click(),document.body.removeChild(p),URL.revokeObjectURL(u),l.success("Coffre exporté (chiffré)")});const q=e.querySelector("#ax-vault-qr-backup");q&&s&&s.bind(q,"click",()=>{(async()=>{c.tap();try{const n=U(I()),d=(n.length/1024).toFixed(1),u=I().length,p=2500;let f="";try{f=(await C(()=>import("https://cdn.jsdelivr.net/npm/lz-string@1.5.0/+esm"),[],import.meta.url)).compressToEncodedURIComponent(n);const k=(f.length/1024).toFixed(1);m.info("vault-qr-backup",`LZ compress ${d}KB → ${k}KB (${Math.round((1-f.length/n.length)*100)}% gain)`)}catch($){m.warn("vault-qr-backup","LZ-string load failed",{err:$})}if(f&&f.length<p){const{apexQrBackup:$}=await C(async()=>{const{apexQrBackup:k}=await import("./apex-qr-backup-CIm0jwws.js");return{apexQrBackup:k}},__vite__mapDeps([1,2,3]),import.meta.url);await $.showQrBackupModal({text:`APEXVAULT_LZ:${f}`,title:"📦 Backup Vault Compressé — Photos iCloud",description:`${u} clés compressées LZ (${(f.length/1024).toFixed(1)}KB vs ${d}KB orig). JSON chiffré AES-GCM-256. Sauvegarde dans Photos iCloud — au reinstall, scan = restore complet.`,filename:`apex-vault-backup-${new Date().toISOString().slice(0,10)}.png`});return}l.info(`Vault compressé ${f.length}B encore > QR max. Upload Gist privé chiffré...`,{duration:4e3});try{const{apexGithubGistBackup:$}=await C(async()=>{const{apexGithubGistBackup:M}=await import("./apex-github-gist-backup-XNwmpXJj.js");return{apexGithubGistBackup:M}},__vite__mapDeps([2,5,1,3,4,6]),import.meta.url),k=await $.pushBackup({force:!0});if(k.ok&&k.gist_id){const M=`https://gist.github.com/${k.gist_id}`,{apexQrBackup:J}=await C(async()=>{const{apexQrBackup:W}=await import("./apex-qr-backup-CIm0jwws.js");return{apexQrBackup:W}},__vite__mapDeps([1,2,3]),import.meta.url);await J.showQrBackupModal({text:`APEXVAULT_GIST:${k.gist_id}`,title:"📦 Backup Vault → Gist URL — Photos iCloud",description:`${u} clés uploadées Gist privé chiffré (${(k.bytes??0)/1024}KB). QR contient juste l'ID Gist. Au reinstall, scan + PAT GitHub = pull Gist + restore complet. URL : ${M}`,filename:`apex-vault-gist-${new Date().toISOString().slice(0,10)}.png`});return}l.warn(`Gist upload échoué : ${k.error??"?"}. Fallback download JSON.`,{duration:6e3})}catch($){m.warn("vault-qr-backup","gist push failed",{err:$})}const _=new Blob([n],{type:"application/json"}),A=URL.createObjectURL(_),T=document.createElement("a");T.href=A,T.download=`apex-vault-backup-${new Date().toISOString().slice(0,10)}.json`,document.body.appendChild(T),T.click(),document.body.removeChild(T),URL.revokeObjectURL(A),l.success(`📥 Backup JSON téléchargé (${d}KB chiffré). Sauvegarde dans iCloud Drive / Notes.`,{duration:8e3})}catch(n){const d=n instanceof Error?n.message:String(n);l.error(`Backup QR échoué : ${d.slice(0,60)}`,{duration:6e3})}})()}),N(e)}function N(e){e.querySelectorAll('[data-action="test"]').forEach(r=>{s.bind(r,"click",t=>{t.stopPropagation();const a=r.dataset.credId??"";pe(e,a,r)})}),e.querySelectorAll('[data-action="recharge"]').forEach(r=>{s.bind(r,"click",t=>{t.stopPropagation();const a=r.dataset.rechargeUrl??"",i=r.dataset.service??"";ue(a,i)})}),e.querySelectorAll('[data-action="discover-links"]').forEach(r=>{s.bind(r,"click",t=>{t.stopPropagation();const a=r.dataset.service??"";xe(e,a,r)})}),e.querySelectorAll('[data-action="edit"]').forEach(r=>{s.bind(r,"click",t=>{t.stopPropagation();const a=r.dataset.credId??"";be(e,a)})}),e.querySelectorAll('[data-action="delete"]').forEach(r=>{s.bind(r,"click",t=>{t.stopPropagation();const a=r.dataset.credId??"";ge(e,a)})}),e.querySelectorAll('[data-action="add-to-cat"]').forEach(r=>{s.bind(r,"click",t=>{t.stopPropagation();const a=r.dataset.catId??"";Q(e,a)})})}async function pe(e,r,t){if(!r)return;c.tap();const a=t.textContent;t.textContent="⏳ Test…",t.setAttribute("disabled","true");try{const i=await S.testKey(r);i.ok?(c.success(),l.success(`✅ Active (${i.latencyMs}ms)`)):(c.error(),l.error(`❌ ${i.reason??"Test échoué"}`)),w(e)}catch(i){m.warn("feature-vault","testKey failed",{err:i}),c.error(),l.error("Erreur pendant le test"),t.textContent=a,t.removeAttribute("disabled")}}function ue(e,r){if(c.tap(),!e){l.warn(`Aucune page recharge connue pour ${r}`);return}try{window.open(e,"_blank","noopener,noreferrer")}catch(t){m.warn("feature-vault","recharge open failed",{err:t}),l.error("Impossible d'ouvrir le lien")}}async function xe(e,r,t){if(!r)return;c.tap();const a=t.textContent;t.textContent="⏳ Recherche…",t.setAttribute("disabled","true");try{const i=await Z.discover(r,{force:!0}),o=[];i.login&&o.push("login"),i.dashboard&&o.push("dashboard"),i.billing&&o.push("billing"),i.api_keys&&o.push("api_keys"),i.usage&&o.push("usage"),i.docs&&o.push("docs"),i.password_reset&&o.push("reset_pw"),i.account_settings&&o.push("settings"),i.support&&o.push("support"),i.status_page&&o.push("status"),i.alive&&o.length>0?(c.success(),l.success(`🔗 ${o.length} liens trouvés (${i.source}) : ${o.join(", ")}`)):(c.error(),l.warn(`Aucun lien validé pour ${r} — réessaie plus tard`)),w(e)}catch(i){m.warn("feature-vault","discoverLinks failed",{err:i}),c.error(),l.error("Erreur pendant la recherche de liens")}finally{t.textContent=a,t.removeAttribute("disabled")}}function ge(e,r){if(r&&(c.tap(),!!window.confirm("Supprimer cette clé définitivement ? Elle sera retirée du Coffre + ne sera plus restaurée auto.")))try{S.removeKey(r),c.success(),l.success("Clé supprimée définitivement ✓"),w(e)}catch(t){m.warn("feature-vault","delete failed",{err:t}),c.error(),l.error("Suppression échouée")}}function j(e){let r=e.querySelector("#ax-vault-modal-root");return r||(r=document.createElement("div"),r.id="ax-vault-modal-root",e.appendChild(r)),r}function z(e){const r=j(e);r.innerHTML=""}function Q(e,r){const t=j(e),a=L.filter(x=>x.id!=="other").map(x=>`<option value="${g(x.id)}" ${r===x.id?"selected":""}>${g(x.label)}</option>`).join("");t.innerHTML=`
  ...
  `,(()=>{const x=t.querySelector("#ax-vault-modal-close");x&&s&&s.bind(x,"click",()=>z(e))})();const i=t.querySelector('[role="dialog"]');i&&s&&s.bind(i,"click",x=>{x.target===i&&z(e)});const o=t.querySelector("#ax-vault-add-detect");o&&s&&s.bind(o,"click",()=>{(async()=>{c.tap();const x=t.querySelector("#ax-vault-add-value");if(!x)return;const v=R(x.value.trim());if(!v){l.warn("Aucun pattern reconnu");return}if(v.category==="forbidden"){l.error("🚨 Type interdit");return}const y=t.querySelector("#ax-vault-add-service"),h=t.querySelector("#ax-vault-add-cat");if(y){const q=v.storageKey.replace(/^(ax_|apex_v13_)/,"").replace(/_(?:key|token|pat|sk|pk|id|secret)$/,"");y.value=q}h&&(h.value=E(y?.value??"",v.category)),l.success(`Détecté: ${v.name}`)})()});const b=t.querySelector("#ax-vault-add-save");b&&s&&s.bind(b,"click",()=>{(async()=>{c.tap();const x=t.querySelector("#ax-vault-add-service")?.value.trim()??"",v=t.querySelector("#ax-vault-add-alias")?.value.trim()??"",y=t.querySelector("#ax-vault-add-value")?.value.trim()??"";if(!x||!y){l.warn("Service et valeur requis");return}try{const h={};v&&(h.alias=v),await S.addKey(x,y,h),l.success(`✅ Clé ${x} chiffrée + sauvegardée`),z(e),w(e)}catch(h){m.warn("feature-vault","add manual failed",{err:h}),l.error("Erreur pendant la sauvegarde")}})()})}function be(e,r){const t=j(e),a=S.listAll(!0).find(o=>o.id===r);if(!a){l.error("Clé introuvable");return}t.innerHTML=`
    <div role="dialog" aria-modal="true" aria-label="Modifier une clé"
      ...
      <div style="background:var(--ax-bg-flat);border:1px solid rgba(255,255,255,0.1);border-radius:16px;padding:20px;max-width:440px;width:100%">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
          <h2 style="margin:0;font-size:18px;color:var(--ax-gold)">✏️ Modifier ${g(a.service)}</h2>
          <button id="ax-vault-modal-close" aria-label="Fermer"
            style="background:transparent;border:0;color:#aaa;font-size:24px;cursor:pointer;min-height:32px;min-width:32px">×</button>
        </div>
  ...
  `,(()=>{const o=t.querySelector("#ax-vault-modal-close");o&&s&&s.bind(o,"click",()=>z(e))})(),(()=>{const o=t.querySelector("#ax-vault-edit-cancel");o&&s&&s.bind(o,"click",()=>z(e))})();const i=t.querySelector("#ax-vault-edit-save");i&&s&&s.bind(i,"click",()=>{(async()=>{c.tap();const o=t.querySelector("#ax-vault-edit-value")?.value.trim()??"",b=t.querySelector("#ax-vault-edit-alias")?.value.trim()??"";if(!o){l.warn("Valeur requise");return}try{S.markInvalid(r,"replaced via edit");const x={};b&&(x.alias=b),await S.addKey(a.service,o,x),l.success("✅ Clé mise à jour"),z(e),w(e)}catch(x){m.warn("feature-vault","edit save failed",{err:x}),l.error("Erreur pendant la modification")}})()})}export{L as CATEGORIES,oe as autoDetectAndStore,D as buildCredentialDisplays,E as classifyService,G as computeStats,Fe as dispose,g as escapeHtml,U as exportVaultJson,je as filterVaultEntries,H as formatRelativeTime,ae as getCredentialsForCategory,I as listVaultEntries,Pe as removeCredential,w as render,ne as renderCredentialCard};
