import{c as I}from"./listener-cleanup-Y2rGGxxX.js";import{l as f}from"./monitoring-3uBGKGRH.js";import{s as A,a as D}from"../core/main-B2dYl_oo.js";import{autoDiscoverLinks as R}from"./auto-discover-links-Dx9ziyqS.js";import{c as B}from"./csp-style-helper-BisGRi53.js";import{detectCredential as k,CREDENTIAL_PATTERNS as _}from"./credential-patterns-Dy6Wjk7e.js";import{g as F}from"./apex-tools-dispatch--7BaBVAR.js";import{l as z}from"./multi-source-analyze-DHezvtlA.js";import{multiKeyVault as b}from"./multi-key-vault-wsvGvLzC.js";import{v as h}from"./apex-kb-Cc3uNflI.js";import{h as u}from"./haptic-BUEqXK0N.js";import{toast as c}from"./toast-Dgg9rcIP.js";import"./apex-tools-registry-swuInW0E.js";import"./voice-DKzoXzL8.js";let n=null;function fe(){n?.cleanup(),n=null}function d(e){return e.replace(/[&<>"']/g,a=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[a]??a)}const y=[{id:"ai",label:"🤖 IA & LLM",serviceMatchers:["anthropic","openai","groq","google","gemini","openrouter","cohere","mistral","perplexity","deepseek","xai","elevenlabs","replicate","huggingface","fireworks","togetherai","deepl"],patternCategories:["ai"]},{id:"finance",label:"💳 Paiements & Finance",serviceMatchers:["stripe","paypal","revolut","wise","lydia","n26","boursorama","fortuneo","ing","socgen","bnp","credit_agricole","credit_mutuel","banque_postale","lbp","bpce","shopify"],patternCategories:["finance"]},{id:"devops",label:"🛠 DevOps & Code",serviceMatchers:["github","gitlab","cloudflare","vercel","netlify","railway","aws","heroku","sentry","npm"],patternCategories:["devops"]},{id:"comms",label:"📨 Communications",serviceMatchers:["telegram","discord","slack","brevo","resend","twilio","sendgrid","mailchimp","whatsapp"],patternCategories:["comms"]},{id:"social",label:"🌐 Réseaux sociaux",serviceMatchers:["facebook","instagram","tiktok","youtube","twitter","linkedin"],patternCategories:[]},{id:"storage",label:"☁️ Stockage & Cloud",serviceMatchers:["firebase","supabase","airtable","notion","dropbox","pinecone","weaviate"],patternCategories:["storage"]},{id:"ecommerce",label:"🛒 E-commerce",serviceMatchers:["shopify","stripe_connect","paypal_business"],patternCategories:[]},{id:"crypto",label:"₿ Crypto",serviceMatchers:["coinbase","binance","crypto_com","kraken"],patternCategories:[]},{id:"identity",label:"🆔 Identité Kevin",serviceMatchers:["kevin","iban","siret","vat","bic","apple","microsoft"],patternCategories:["identity"]},{id:"other",label:"📦 Autres",serviceMatchers:[],patternCategories:["saas"]}];function M(e,a){const t=e.toLowerCase();let r=null;for(const i of y)if(i.id!=="other")for(const o of i.serviceMatchers)t.includes(o)&&(!r||o.length>r.matchLen)&&(r={catId:i.id,matchLen:o.length});if(r)return r.catId;if(a){for(const i of y)if(i.patternCategories.includes(a))return i.id}return"other"}function U(){return _.filter(e=>e.category!=="forbidden").map(e=>{const a=h.getKeyStatus(e.storageKey),t=(()=>{try{return localStorage.getItem(e.storageKey)??""}catch{return""}})(),r=t&&t.length>8&&!t.startsWith("AXENC1:")?h.maskKey(t):t.startsWith("AXENC1:")?"🔒 chiffré":"";return{pattern:e,status:a,masked:r}})}function be(e,a){return e.filter(t=>{if(a.category&&t.pattern.category!==a.category||a.configuredOnly&&t.status==="empty")return!1;if(a.query){const r=a.query.toLowerCase();if(!(t.pattern.name.toLowerCase().includes(r)||t.pattern.storageKey.toLowerCase().includes(r)))return!1}return!0})}function T(){const e=[];let a=[];try{a=b.listAll(!0)}catch(t){f.warn("feature-vault","multiKeyVault.listAll failed",{err:t})}for(const t of a){const r=z.get(t.service),i=_.find(l=>l.storageKey.includes(t.service)),o={id:t.id,service:t.service,serviceName:r?.name??H(t.service),category:M(t.service,i?.category),status:t.status,source:"multi-key"};t.alias!==void 0&&(o.alias=t.alias),t.addedAt!==void 0&&(o.addedAt=t.addedAt),t.lastTestedAt!==void 0&&(o.lastTestedAt=t.lastTestedAt);const s=z.getRechargeLink(t.service);s&&(o.rechargeUrl=s),e.push(o)}return e}function L(){const e=T(),a={total:e.length,active:0,failing:0,invalid:0};for(const t of e)t.status==="active"?a.active+=1:t.status==="failing"||t.status==="rate-limited"?a.failing+=1:t.status==="invalid"&&(a.invalid+=1);return a}function O(e,a=""){const t=T(),r=a.trim().toLowerCase();return t.filter(i=>i.category!==e.id?!1:r?i.service.toLowerCase().includes(r)||i.serviceName.toLowerCase().includes(r)||(i.alias?.toLowerCase().includes(r)??!1):!0)}function H(e){return e.charAt(0).toUpperCase()+e.slice(1)}async function N(e){const a=e.trim();if(!a)return{ok:!1,reason:"Entrée vide"};const t=k(a);if(!t)return{ok:!1,reason:"Aucun pattern reconnu"};if(t.category==="forbidden")return{ok:!1,reason:"🚨 Type interdit (cartes/seed phrases jamais stockées)"};try{const r=await h.encryptAuto(a);return localStorage.setItem(t.storageKey,r),{ok:!0,pattern_name:t.name,storage_key:t.storageKey}}catch(r){return f.warn("vault-feature","autoDetectAndStore failed",{err:r}),{ok:!1,reason:"Erreur chiffrement"}}}function ve(e){try{return localStorage.removeItem(e),!0}catch(a){return f.warn("vault-feature","remove failed",{err:a}),!1}}function P(e){const a={exported_at:new Date().toISOString(),version:1,entries:e.filter(t=>t.status!=="empty").map(t=>{const r=(()=>{try{return localStorage.getItem(t.pattern.storageKey)??""}catch{return""}})();return{storage_key:t.pattern.storageKey,name:t.pattern.name,value_encrypted:r}})};return JSON.stringify(a,null,2)}function V(e){const a=Y[e.status]??"#888",t=J[e.status]??"⚪",r=(e.preview??"").slice(0,4)+"••••••"+(e.preview??"").slice(-4),i=e.preview?r:"••••••",o=e.rechargeUrl??"",s=e.alias?`<span style="color:#888;font-size:11px">— ${d(e.alias)}</span>`:"",l=e.logoUrl?`<img src="${d(e.logoUrl)}" alt="" loading="lazy" decoding="async" style="width:24px;height:24px;border-radius:6px" onerror="this.style.display='none'">`:"",p=[];e.addedAt&&p.push(`Ajouté ${$(e.addedAt)}`),e.lastTestedAt&&p.push(`Testé ${$(e.lastTestedAt)}`);const x=p.length>0?`<div style="display:flex;gap:8px;font-size:11px;color:#888;margin-bottom:10px">${p.map(g=>`<span>${d(g)}</span>`).join("")}</div>`:"";return`
  ...
  `}const Y={active:"#22cc77",failing:"#ffaa00","rate-limited":"#ffaa00",invalid:"#ff5b5b",unknown:"#888"},J={active:"🟢",failing:"🟡","rate-limited":"🟡",invalid:"🔴",unknown:"⚪"};function $(e){const a=Date.now()-e;if(a<0||!Number.isFinite(a))return"à l'instant";const t=Math.floor(a/6e4);if(t<1)return"à l'instant";if(t<60)return`il y a ${t}min`;const r=Math.floor(t/60);if(r<24)return`il y a ${r}h`;const i=Math.floor(r/24);return i<30?`il y a ${i}j`:`il y a ${Math.floor(i/30)} mois`}let w="";function v(e){if(n?.cleanup(),n=I("vault"),!A.get("isAdmin")){e.innerHTML=`<div style="padding:40px;text-align:center"><h2 style="color:#c9a227">🔒 Coffre admin</h2><p style="color:#a0a4c0">Cette section est réservée à l'admin Kevin.</p></div>`;return}const t=A.get("user")?.id??"anon";if(!F("admin.vault",e,t))return;const r=L();e.innerHTML=B.withNonce(`
    <style>
  ...
  `),S(e),G(e),X(e),f.info("feature-vault",`rendered (${r.total} entries)`)}function X(e){const a=e.querySelector(".ax-vault-page"),t=e.querySelector("#ax-vault-fab");if(!a)return;let r=0,i=0;const o=()=>{i||(i=requestAnimationFrame(()=>{i=0;const s=window.scrollY||document.documentElement.scrollTop||0;s!==r&&(r=s,s>80?a.classList.add("ax-vault-scrolled"):a.classList.remove("ax-vault-scrolled"))}))};n?n.bind(window,"scroll",o,{passive:!0}):window.addEventListener("scroll",o,{passive:!0}),o(),t&&n&&n.bind(t,"click",()=>{u.tap(),e.querySelector("#ax-vault-test-all")?.click()})}function S(e){const a=e.querySelector("#ax-vault-categories");if(!a)return;if(L().total===0&&!a.dataset.axInitialized){a.dataset.axInitialized="1";const r=D(a,"vault-cards");setTimeout(()=>{r(),S(e)},250);return}a.innerHTML=y.map(r=>{const i=O(r,w);if(i.length===0&&r.id!=="identity")return"";const o=i.length>0;return`
    ...
    `}).join("")}function G(e){const a=e.querySelector("#ax-vault-search");if(a){let s=null;n.bind(a,"input",()=>{s&&clearTimeout(s),s=setTimeout(()=>{w=a.value.trim(),S(e),q(e)},240)})}const t=e.querySelector("#ax-vault-add-manual");t&&n&&n.bind(t,"click",()=>{u.tap(),j(e)});const r=e.querySelector("#ax-vault-test-all");r&&n&&n.bind(r,"click",()=>{(async()=>{u.tap(),c.info("Test de toutes les clés en cours…");try{const s=await b.healthCheckAll();c.success(`✅ ${s.tested} testées · ${s.recovered} récupérées · ${s.stillDown} HS`),v(e)}catch(s){f.warn("feature-vault","testAll failed",{err:s}),c.error("Erreur pendant le test global")}})()});const i=e.querySelector("#ax-vault-paste-btn");i&&n&&n.bind(i,"click",()=>{(async()=>{u.tap();const s=e.querySelector("#ax-vault-paste"),l=e.querySelector("#ax-vault-paste-result");if(!s||!l)return;const p=await N(s.value);if(p.ok){u.success(),c.success(`✅ ${p.pattern_name} stocké`),l.innerHTML=`<div style="padding:8px;background:rgba(34,204,119,.1);color:#22cc77;border-radius:8px">✅ ${d(p.pattern_name)} → ${d(p.storage_key)}</div>`,s.value="";const x=k(s.value.trim());if(x){const g=x.storageKey.replace(/^(ax_|apex_v13_)/,"").replace(/_(?:key|token|pat|sk|pk|id|secret)$/,"");try{await b.addKey(g,s.value.trim())}catch{}}v(e)}else u.error(),c.error(p.reason),l.innerHTML=`<div style="padding:8px;background:rgba(255,88,88,.1);color:#ff5858;border-radius:8px">⚠ ${d(p.reason)}</div>`})()});const o=e.querySelector("#ax-vault-export");o&&n&&n.bind(o,"click",()=>{u.tap();const s=P(U()),l=new Blob([s],{type:"application/json"}),p=URL.createObjectURL(l),x=document.createElement("a");x.href=p,x.download=`apex-vault-${new Date().toISOString().slice(0,10)}.json`,document.body.appendChild(x),x.click(),document.body.removeChild(x),URL.revokeObjectURL(p),c.success("Coffre exporté (chiffré)")}),q(e)}function q(e){e.querySelectorAll('[data-action="test"]').forEach(a=>{n.bind(a,"click",t=>{t.stopPropagation();const r=a.dataset.credId??"";W(e,r,a)})}),e.querySelectorAll('[data-action="recharge"]').forEach(a=>{n.bind(a,"click",t=>{t.stopPropagation();const r=a.dataset.rechargeUrl??"",i=a.dataset.service??"";E(r,i)})}),e.querySelectorAll('[data-action="discover-links"]').forEach(a=>{n.bind(a,"click",t=>{t.stopPropagation();const r=a.dataset.service??"";Q(e,r,a)})}),e.querySelectorAll('[data-action="edit"]').forEach(a=>{n.bind(a,"click",t=>{t.stopPropagation();const r=a.dataset.credId??"";ee(e,r)})}),e.querySelectorAll('[data-action="delete"]').forEach(a=>{n.bind(a,"click",t=>{t.stopPropagation();const r=a.dataset.credId??"";Z(e,r)})}),e.querySelectorAll('[data-action="add-to-cat"]').forEach(a=>{n.bind(a,"click",t=>{t.stopPropagation();const r=a.dataset.catId??"";j(e,r)})})}async function W(e,a,t){if(!a)return;u.tap();const r=t.textContent;t.textContent="⏳ Test…",t.setAttribute("disabled","true");try{const i=await b.testKey(a);i.ok?(u.success(),c.success(`✅ Active (${i.latencyMs}ms)`)):(u.error(),c.error(`❌ ${i.reason??"Test échoué"}`)),v(e)}catch(i){f.warn("feature-vault","testKey failed",{err:i}),u.error(),c.error("Erreur pendant le test"),t.textContent=r,t.removeAttribute("disabled")}}function E(e,a){if(u.tap(),!e){c.warn(`Aucune page recharge connue pour ${a}`);return}try{window.open(e,"_blank","noopener,noreferrer")}catch(t){f.warn("feature-vault","recharge open failed",{err:t}),c.error("Impossible d'ouvrir le lien")}}async function Q(e,a,t){if(!a)return;u.tap();const r=t.textContent;t.textContent="⏳ Recherche…",t.setAttribute("disabled","true");try{const i=await R.discover(a,{force:!0}),o=[];i.login&&o.push("login"),i.dashboard&&o.push("dashboard"),i.billing&&o.push("billing"),i.api_keys&&o.push("api_keys"),i.usage&&o.push("usage"),i.docs&&o.push("docs"),i.password_reset&&o.push("reset_pw"),i.account_settings&&o.push("settings"),i.support&&o.push("support"),i.status_page&&o.push("status"),i.alive&&o.length>0?(u.success(),c.success(`🔗 ${o.length} liens trouvés (${i.source}) : ${o.join(", ")}`)):(u.error(),c.warn(`Aucun lien validé pour ${a} — réessaie plus tard`)),v(e)}catch(i){f.warn("feature-vault","discoverLinks failed",{err:i}),u.error(),c.error("Erreur pendant la recherche de liens")}finally{t.textContent=r,t.removeAttribute("disabled")}}function Z(e,a){if(a&&(u.tap(),!!window.confirm("Supprimer cette clé définitivement ? Elle sera retirée du Coffre + ne sera plus restaurée auto.")))try{b.removeKey(a),u.success(),c.success("Clé supprimée définitivement ✓"),v(e)}catch(t){f.warn("feature-vault","delete failed",{err:t}),u.error(),c.error("Suppression échouée")}}function C(e){let a=e.querySelector("#ax-vault-modal-root");return a||(a=document.createElement("div"),a.id="ax-vault-modal-root",e.appendChild(a)),a}function m(e){const a=C(e);a.innerHTML=""}function j(e,a){const t=C(e),r=y.filter(l=>l.id!=="other").map(l=>`<option value="${d(l.id)}" ${a===l.id?"selected":""}>${d(l.label)}</option>`).join("");t.innerHTML=`
  ...
  `,(()=>{const l=t.querySelector("#ax-vault-modal-close");l&&n&&n.bind(l,"click",()=>m(e))})();const i=t.querySelector('[role="dialog"]');i&&n&&n.bind(i,"click",l=>{l.target===i&&m(e)});const o=t.querySelector("#ax-vault-add-detect");o&&n&&n.bind(o,"click",()=>{(async()=>{u.tap();const l=t.querySelector("#ax-vault-add-value");if(!l)return;const p=k(l.value.trim());if(!p){c.warn("Aucun pattern reconnu");return}if(p.category==="forbidden"){c.error("🚨 Type interdit");return}const x=t.querySelector("#ax-vault-add-service"),g=t.querySelector("#ax-vault-add-cat");if(x){const K=p.storageKey.replace(/^(ax_|apex_v13_)/,"").replace(/_(?:key|token|pat|sk|pk|id|secret)$/,"");x.value=K}g&&(g.value=M(x?.value??"",p.category)),c.success(`Détecté: ${p.name}`)})()});const s=t.querySelector("#ax-vault-add-save");s&&n&&n.bind(s,"click",()=>{(async()=>{u.tap();const l=t.querySelector("#ax-vault-add-service")?.value.trim()??"",p=t.querySelector("#ax-vault-add-alias")?.value.trim()??"",x=t.querySelector("#ax-vault-add-value")?.value.trim()??"";if(!l||!x){c.warn("Service et valeur requis");return}try{const g={};p&&(g.alias=p),await b.addKey(l,x,g),c.success(`✅ Clé ${l} chiffrée + sauvegardée`),m(e),v(e)}catch(g){f.warn("feature-vault","add manual failed",{err:g}),c.error("Erreur pendant la sauvegarde")}})()})}function ee(e,a){const t=C(e),r=b.listAll(!0).find(o=>o.id===a);if(!r){c.error("Clé introuvable");return}t.innerHTML=`
