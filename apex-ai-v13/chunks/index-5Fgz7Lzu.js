const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./apex-kb-DRP-rQ5c.js","./monitoring-3uBGKGRH.js","./credential-patterns-CLzI061R.js"])))=>i.map(i=>d[i]);
import{_ as g}from"./apex-kb-DRP-rQ5c.js";import{l as M}from"./monitoring-3uBGKGRH.js";import{m as b}from"../core/main-BxMVCELi.js";import{g as L}from"./apex-tools-dispatch-core-D-SClRAj.js";import{toast as p}from"./toast-ClsF1KRZ.js";import"./credential-patterns-CLzI061R.js";import"./multi-source-analyze-DFRy0PZC.js";import"./apex-tools-dispatch-skills-B3P-0xbA.js";import"./apex-tools-dispatch-data-CzTAok8u.js";import"./apex-tools-dispatch-finance-D84Ce07W.js";import"./apex-tools-dispatch-misc-CkMZqq-7.js";import"./apex-tools-registry-p2nVY1Xn.js";import"./voice-B7u_Xmm6.js";import"./haptic-CQFg2PXZ.js";const S="kdmc_admin";function c(r){return r.replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]??t)}function T(){try{const r=localStorage.getItem("apex_v13_user");return r?JSON.parse(r):null}catch{return null}}function x(r){if(!r)return"—";try{return new Date(r).toLocaleString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"})}catch{return"—"}}function h(r){if(!r)return"jamais";const t=Date.now()-r,e=t/36e5;return e<1?`${Math.floor(t/6e4)}min`:e<24?`${Math.floor(e)}h`:`${Math.floor(e/24)}j`}function B(r){const t=T(),e=t?.id===S,o=t?.name??"Anonyme";L("admin.kb",r,t?.id)&&(r.innerHTML=`
  ...
  `,w(r,t,e),H(r,t,e))}async function w(r,t,e){await Promise.all([v(r,t),e?C(r):Promise.resolve(),D(r),_(r),E(r)])}async function v(r,t){const e=r.querySelector("#my-facts-content");if(e){if(!t){e.innerHTML="<em>Pas de user connecté.</em>";return}try{const{persistentMemory:o}=await g(async()=>{const{persistentMemory:s}=await import("./persistent-memory-store-gSf9sg_2.js");return{persistentMemory:s}},__vite__mapDeps([0,1,2]),import.meta.url),d=(await o.list()).filter(s=>s.scope===t.id).sort((s,n)=>n.importance-s.importance);if(d.length===0){e.innerHTML="<em>Aucun fact mémorisé pour ton compte. Les facts seront extraits automatiquement de tes messages chat.</em>";return}const l=d.slice(0,100).map((s,n)=>`
      <tr data-fact-idx="${n}" style="cursor:pointer">
        <td><span class="ax-tag">${c(s.category)}</span></td>
        <td>${c(s.text)}</td>
        <td><span class="ax-importance" style="color:${s.importance>=80?"#ff6b6b":s.importance>=60?"#ffa94d":"#888"};">${s.importance}</span></td>
        <td><time>${h(s.ts)}</time></td>
      </tr>
    `).join("");e.innerHTML=`
    ...
    `,e.querySelectorAll("tr[data-fact-idx]").forEach(s=>{s.addEventListener("click",()=>{const n=Number(s.dataset.factIdx??"-1"),a=d[n];a&&(async()=>{const{drillDown:y}=await g(async()=>{const{drillDown:f}=await import("./drilldown-1SlvGToi.js");return{drillDown:f}},[],import.meta.url),m="ax-drilldown-mount-knowledge";let u=document.getElementById(m);u||(u=document.createElement("div"),u.id=m,document.body.appendChild(u)),y.open({id:`fact-${n}`,title:`🧠 Fact ${c(a.category)}`,content:()=>`
            ...
            `,data:{factIdx:n}},u)})()})})}catch(o){e.innerHTML=`<em style="color:#c00;">Erreur chargement : ${c(String(o))}</em>`}}}async function C(r){const t=r.querySelector("#cross-user-content");if(t)try{const{persistentMemory:e}=await g(async()=>{const{persistentMemory:n}=await import("./persistent-memory-store-gSf9sg_2.js");return{persistentMemory:n}},__vite__mapDeps([0,1,2]),import.meta.url),o=await e.list(),i=new Map;for(const n of o){const a=i.get(n.scope)??[];a.push(n),i.set(n.scope,a)}if(i.size===0){t.innerHTML="<em>Aucun user n'a encore de facts mémorisés.</em>";return}const d=[];for(const[n,a]of i){const y=a.sort((m,u)=>u.importance-m.importance).slice(0,3);d.push(`
      ...
      `)}t.textContent="";const l=document.createElement("p");l.textContent=`${i.size} user(s), ${o.length} fact(s) total`,t.append(l);const s=document.createElement("div");s.innerHTML=d.join(""),t.append(s)}catch(e){t.innerHTML=`<em style="color:#c00;">Erreur : ${c(String(e))}</em>`}}async function D(r){const t=r.querySelector("#lessons-content");if(t)try{const e=localStorage.getItem("ax_lessons_learned_struct");if(!e){t.innerHTML="<em>Aucune lesson cross-app encore.</em>";return}const o=JSON.parse(e);if(o.length===0){t.innerHTML="<em>Liste vide.</em>";return}const d=[...o].sort((n,a)=>a.ts-n.ts).slice(0,30).map(n=>{const a=n.severity==="critical"?"#ff4444":n.severity==="warn"?"#ffa94d":"#888",y=n.resolved?"✅":"⏳";return`
      ...
      `});t.textContent="";const l=document.createElement("p");l.textContent=`${o.length} lesson(s) (30 plus récentes affichées)`,t.append(l);const s=document.createElement("ul");s.style.cssText="list-style:none;padding:0;",s.innerHTML=d.join(""),t.append(s)}catch(e){t.innerHTML=`<em style="color:#c00;">Erreur : ${c(String(e))}</em>`}}async function _(r){const t=r.querySelector("#docs-content");if(!t)return;const e=b.getDocsContext(),o=Object.keys(e);if(o.length===0){t.innerHTML='<em>Aucun doc synchronisé. Clique "Force re-sync docs" pour fetcher depuis GitHub.</em>';return}const i=o.map(d=>{const l=e[d];return l?`
      <tr>
  ...
  `}async function E(r){const t=r.querySelector("#audit-content");if(t)try{const e=localStorage.getItem("ax_memory_audit_log");if(!e){t.innerHTML="<em>Sentinelle memory-watch n'a pas encore tourné (1×/jour).</em>";return}const o=JSON.parse(e);if(o.length===0){t.innerHTML="<em>Log vide.</em>";return}const i=o[o.length-1];if(!i){t.innerHTML="<em>Log vide.</em>";return}t.innerHTML=`
    ...
    `}catch(e){t.innerHTML=`<em style="color:#c00;">Erreur : ${c(String(e))}</em>`}}function H(r,t,e){const o=r.querySelector("#btn-resync-docs");o?.addEventListener("click",async()=>{o.disabled=!0,o.textContent="⏳ Sync en cours…";try{const s=await b.syncDocsAtBoot({forceRefresh:!0});p.show(`✅ Docs sync : ${s.synced} OK · ${s.failed} fails`,"success"),await _(r)}catch(s){p.show(`❌ Sync fail : ${String(s)}`,"error")}finally{o.disabled=!1,o.textContent="🔄 Force re-sync docs"}});const i=r.querySelector("#btn-compress-mem");i?.addEventListener("click",async()=>{if(confirm("Compresser la mémoire ? Garde top 100 facts par importance par user, supprime le reste. Action irréversible.")){i.disabled=!0;try{const{sentinels:s}=await g(async()=>{const{sentinels:a}=await import("./sentinels-dK6jKFwy.js");return{sentinels:a}},__vite__mapDeps([0,1,2]),import.meta.url),n=await s.runOne("memory-watch");p.show(`✅ ${n?.msg??"Done"}`,"success"),await w(r,t,e)}catch(s){p.show(`❌ Compress fail : ${String(s)}`,"error")}finally{i.disabled=!1}}}),r.querySelector("#btn-export-json")?.addEventListener("click",async()=>{try{const{persistentMemory:s}=await g(async()=>{const{persistentMemory:f}=await import("./persistent-memory-store-gSf9sg_2.js");return{persistentMemory:f}},__vite__mapDeps([0,1,2]),import.meta.url),n=await s.list(),a={ts:Date.now(),user_id:t?.id??"anonymous",is_admin:e,facts:e?n:n.filter(f=>f.scope===t?.id),lessons:JSON.parse(localStorage.getItem("ax_lessons_learned_struct")??"[]"),docs_meta:Object.fromEntries(Object.entries(b.getDocsContext()).map(([f,$])=>[f,{ts:$.ts,size:$.size}]))},y=new Blob([JSON.stringify(a,null,2)],{type:"application/json"}),m=URL.createObjectURL(y),u=document.createElement("a");u.href=m,u.download=`apex-memory-${new Date().toISOString().slice(0,10)}.json`,u.click(),URL.revokeObjectURL(m),p.show("💾 Export téléchargé","success")}catch(s){p.show(`❌ Export fail : ${String(s)}`,"error")}}),r.querySelector("#btn-extract-test")?.addEventListener("click",async()=>{const s=prompt(`Tape une phrase (ex: "j'habite Monaco et j'ai 35 ans, je suis allergique aux fruits de mer") :`,"");if(!(!s||!t))try{const n=await b.extractFactsFromMessage(s,t.id);p.show(`✅ ${n.extracted} fact(s) extrait(s)`,"success"),await v(r,t)}catch(n){p.show(`❌ Extract fail : ${String(n)}`,"error")}}),M.info("knowledge","render + handlers wired",{isAdmin:e,user:t?.id})}export{B as render};
