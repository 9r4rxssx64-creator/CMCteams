const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./multi-source-analyze-DHezvtlA.js","./apex-kb-Cc3uNflI.js","./monitoring-3uBGKGRH.js","./credential-patterns-Dy6Wjk7e.js","../assets/css/main-BBcx83Ub.css"])))=>i.map(i=>d[i]);
import{b as h,_ as k}from"./apex-kb-Cc3uNflI.js";import{l as $}from"./monitoring-3uBGKGRH.js";import{c as L}from"./listener-cleanup-Y2rGGxxX.js";import{s as P,a as H}from"../core/main-BVow9fOV.js";import{apexExecute as v}from"./apex-execute-DqhiwnwD.js";import{auth as T}from"./auth-C-_2t28H.js";import{commerce as S}from"./commerce-DfT_W7V6.js";import{c as I}from"./csp-style-helper-BisGRi53.js";import{i as A,r as j}from"./voice-PFJ6HXkn.js";import{kdmcProjectsRegistry as w}from"./kdmc-projects-registry-DdTqW2gy.js";import{w as _}from"./whatsapp-CCgrtVEc.js";import{h as d}from"./haptic-BUEqXK0N.js";import{toast as u}from"./toast-Dgg9rcIP.js";import"./credential-patterns-Dy6Wjk7e.js";import"./multi-source-analyze-DHezvtlA.js";import"./claude-bridge-CrT4Tb0u.js";let f="commerce";const q={commerce:"admin.commerce",users:"admin.users",pending:"admin.users",health:null,projects:null,executions:"admin.executions",knowledge:"admin.kb",bilan:"admin.bilan",consumption:"admin.consumption","audit-log":"admin.audit-log"};function n(e){return e.replace(/[&<>"']/g,i=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[i]??i)}function R(){const e=S.isEnabled();return`
  ...
  `}function M(){const e=T.listUsers(),i=new Set(["admin","family","client_pro","client_free"]),c=e.map(s=>{const o=i.has(s.tier)?s.tier:"client_free";return`
  ...
  `}function B(){const e=w.list(),i=w.count(),c=w.countActive(),s=["active","wip","archived"],o=e.map(r=>{const t=s.includes(r.status)?r.status:"archived",a=r.tech_stack.slice(0,4).map(l=>n(l)).join(", ");return`
  ...
  `}function U(){return[["commerce","💳 Commerce"],["users","👥 Comptes"],["pending","📨 Attente"],["health","🩺 Santé"],["projects","📦 Projets"],["executions","⚙️ Exec"],["knowledge","📚 KB"],["bilan","📊 Bilan"],["consumption","💰 Conso"],["audit-log","🔒 Audit"]].filter(([c])=>{const s=q[c];return!s||A(s)}).map(([c,s])=>{const o=f===c;return`
      ...
      `}).join("")}function K(){const e=v.listPendingExecutions({limit:30}),i=v.getStats(),c=v.listAllowedTasks(),s=v.listForbiddenTasks(),o={pending:"pending",dispatched:"wip",running:"wip",completed:"ok",failed:"error",cancelled:"archived",timeout:"error"},r=e.map(t=>{const a=o[t.status]??"archived",l=new Date(t.ts_created).toLocaleString(),m=t.duration_ms?`${Math.round(t.duration_ms/1e3)}s`:"—",b=t.workflow_run_url?`<a href="${n(t.workflow_run_url)}" target="_blank" rel="noopener">🔗 Workflow</a>`:'<span class="ax-muted">—</span>',g=t.status==="pending"||t.status==="dispatched";return`
  ...
  `}function G(){const e=q[f];if(e&&!A(e))return`<div class="ax-admin-section">${j(e)}</div>`;switch(f){case"commerce":return R();case"users":return M();case"pending":return z();case"health":return D();case"projects":return B();case"executions":return K();case"knowledge":return O();case"bilan":return'<div id="ax-admin-mount-bilan" class="ax-admin-section"><p class="ax-muted">Chargement du bilan financier…</p></div>';case"consumption":return'<div id="ax-admin-mount-consumption" class="ax-admin-section"><p class="ax-muted">Chargement consommation IA…</p></div>';case"audit-log":return'<div id="ax-admin-mount-audit-log" class="ax-admin-section"><p class="ax-muted">Chargement audit log immuable…</p></div>'}}async function C(e){const i=e.querySelector("#ax-admin-mount-bilan");if(i)try{(await k(()=>import("./financial-bilan-Df_9Fglj.js"),__vite__mapDeps([0,1,2,3,4]),import.meta.url)).render(i)}catch(o){$.warn("admin","financial-bilan render failed",{err:o}),i.innerHTML='<p class="ax-muted">Bilan indisponible (module ko)</p>'}const c=e.querySelector("#ax-admin-mount-consumption");if(c)try{(await k(()=>import("./consumption-dashboard-Zp5ed6fJ.js"),__vite__mapDeps([1,2,3,0,4]),import.meta.url)).render(c)}catch(o){$.warn("admin","consumption-dashboard render failed",{err:o}),c.innerHTML='<p class="ax-muted">Consommation indisponible (module ko)</p>'}const s=e.querySelector("#ax-admin-mount-audit-log");if(s){if(!A("admin.audit-log")){s.innerHTML=j("admin.audit-log");return}try{const{auditLog:o}=await k(async()=>{const{auditLog:a}=await import("./apex-kb-Cc3uNflI.js").then(l=>l.c);return{auditLog:a}},__vite__mapDeps([1,2,3]),import.meta.url);o.init();const r=o.getEntries().slice(-100).reverse(),t=r.map(a=>`<li><code>${n(new Date(a.ts).toLocaleString())}</code> · <strong>${n(a.action)}</strong> · ${n(a.actor||"system")}</li>`).join("");s.innerHTML=`
      ...
      `}catch(o){$.warn("admin","audit-log render failed",{err:o}),s.innerHTML='<p class="ax-muted">Audit log indisponible (module ko)</p>'}}}function O(){const e=h.listRepos(),i=h.getStats(),c=e.map(s=>`
  ...
  `}function V(e){e.querySelectorAll("[data-nav-route]").forEach(t=>{p.bind(t,"click",()=>{d.tap();const a=t.dataset.navRoute??"chat";window.location.hash="#"+a})}),e.querySelectorAll('[data-action="select-all"]').forEach(t=>{p.bind(t,"click",()=>{t.select()})}),e.querySelectorAll("[data-tab]").forEach(t=>{p.bind(t,"click",()=>{d.selection(),f=t.dataset.tab,x(e)})});const i=e.querySelector("#commerce-toggle");i&&p.bind(i,"change",()=>{d.medium(),S.setEnabled(i.checked),u.success(`Commercialisation ${i.checked?"activée":"désactivée"}`),x(e)});const c=e.querySelector("#create-user-form");c&&p.bind(c,"submit",t=>{t.preventDefault(),F(e)}),e.querySelectorAll("[data-user-plan]").forEach(t=>{p.bind(t,"change",()=>{const a=t.dataset.userPlan??"";a&&(S.setUserPlan(a,t.value),$.info("admin",`Plan ${t.value} → ${a}`))})}),e.querySelectorAll("[data-project-save]").forEach(t=>{p.bind(t,"click",()=>{d.tap();const a=t.dataset.projectSave??"";if(!a)return;const l=e.querySelector(`[data-project-version="${CSS.escape(a)}"]`),m=e.querySelector(`[data-project-status="${CSS.escape(a)}"]`),b=(l?.value??"").trim(),g=m?.value??"active";if(!b){u.warn("Version requise");return}w.update(a,{version:b,status:g})?(d.success(),u.success(`${a} → ${b} (${g})`),$.info("admin",`Project ${a} updated`,{version:b,status:g}),x(e)):(d.error(),u.error("Update échoué"))})}),e.querySelectorAll("[data-confirm-otp]").forEach(t=>{p.bind(t,"click",()=>{d.tap();const a=t.dataset.confirmOtp??"";if(!a)return;const l=_.confirm(a);l.ok?(d.success(),u.success("Compte activé"),$.info("admin",`Confirmed user ${l.uid}`),x(e)):(d.error(),u.error("Code OTP invalide ou expiré"))})}),e.querySelectorAll("[data-exec-cancel]").forEach(t=>{p.bind(t,"click",()=>{d.tap();const a=t.dataset.execCancel??"";if(!a)return;v.cancelExecution(a)?(d.success(),u.success("Exécution annulée"),x(e)):(d.warning(),u.warn("Annulation impossible"))})}),e.querySelectorAll("[data-exec-poll]").forEach(t=>{p.bind(t,"click",()=>{d.tap();const a=t.dataset.execPoll??"";a&&v.pollResult(a).then(l=>{l?(u.success(`Statut : ${l.status}`),x(e)):u.warn("Exécution introuvable")})})});const s=e.querySelector("#add-repo-form");s&&p.bind(s,"submit",t=>{t.preventDefault(),d.tap();const l=e.querySelector("#kb-add-repo")?.value.trim()??"";if(!l){u.warn("Indique un repo");return}const m=h.addRepo(l);m.ok?(d.success(),u.success(`Repo ajouté : ${l}`),x(e)):(d.error(),u.error(m.reason??"Erreur ajout repo"))}),e.querySelectorAll("[data-remove-repo]").forEach(t=>{p.bind(t,"click",()=>{d.tap();const a=t.dataset.removeRepo??"";a&&(h.removeRepo(a),u.success(`Repo retiré : ${a}`),x(e))})});const o=e.querySelector("#kb-search-form");o&&p.bind(o,"submit",t=>{t.preventDefault(),d.tap();const a=e.querySelector("#kb-search-query"),l=e.querySelector("#kb-search-results"),m=a?.value.trim()??"";!m||!l||(l.innerHTML='<p class="ax-muted">Recherche en cours...</p>',h.searchCode(m).then(b=>{if(b.length===0){l.innerHTML='<p class="ax-muted">Aucun résultat (configure ax_github_token pour augmenter la limite).</p>';return}const g=b.slice(0,20).map(y=>`
          ...
          `).join("");l.innerHTML=`<ul class="ax-kb-results-list">${g}</ul>`}))});const r=e.querySelector("#kb-clear-cache");r&&p.bind(r,"click",()=>{d.tap();const t=h.clearCache();u.success(`Cache vidé : ${t.cleared} entrées`),x(e)})}async function F(e){const i=e.querySelector("#cu-name")?.value.trim()??"",c=e.querySelector("#cu-tier")?.value??"family",s=e.querySelector("#cu-email")?.value.trim()??"",o=e.querySelector("#cu-whatsapp")?.value.trim()??"",r=e.querySelector("#cu-pin")?.value??"";if(!i||i.length<2){d.warning(),u.warn("Nom complet requis (min 2 caractères)");return}const t=await T.createUser({name:i,tier:c,...s&&{email:s},...o&&{whatsappPhone:o},...r&&{initialPin:r}}),a=e.querySelector("#create-user-result");if(!a)return;if(!t.ok||!t.uid){d.error();const m=t.reason??"Erreur création";a.innerHTML=`<div class="ax-error">${n(m)}</div>`,u.error(m);return}d.success(),u.success(`Compte ${i} créé`);let l="";if(o){const m=await _.requestConfirmation({uid:t.uid,name:i,whatsappPhone:o});m.ok&&m.inviteLink&&(l=`
  ...
  `),V(e),f==="bilan"||f==="consumption"){const c=f==="bilan"?"ax-admin-mount-bilan":"ax-admin-mount-consumption",s=e.querySelector(`#${c}`);if(s){const o=s.querySelector("p.ax-muted"),r=o?H(o,"admin-table"):()=>{};C(e).finally(()=>{r()})}else C(e)}}export{ce as dispose,x as render};
