import{a as n}from"./escape-html-DGIYNPKb.js";import{c as y}from"./listener-cleanup-Y2rGGxxX.js";import{l as h}from"./monitoring-DMtdadhB.js";import{i as b}from"../core/main-BNvOFLLb.js";import{m,s as g}from"./multi-source-analyze-D2F3mBsQ.js";import{haptic as c}from"./haptic-CQFg2PXZ.js";import{toast as a}from"./toast-CRdbcLoc.js";import"./apex-kb-OPrvxVPi.js";import"./credential-patterns-CLzI061R.js";let u=null;function M(){u?.cleanup(),u=null}async function p(e){if(!(b.get("isAdmin")===!0)){e.innerHTML=`
      <div style="padding:40px;text-align:center;color:#999">
        <h2 style="color:#c9a227">Accès admin uniquement</h2>
        <p>Multi-Source History — Kevin admin.</p>
      </div>
    `;return}u?.cleanup(),u=y("multi-source-history");const s=m.getHistory(),o=m.getStats(),d=g.listKnown();e.innerHTML=v(s,o,d.length),S(e,s),h.info("multi-source-history",`Render ${s.length} sources, ${o.items_total} items`)}function v(e,t,s){const o=t.items_total>0?Math.round(t.items_configured/t.items_total*100):0,d=t.items_configured>0?Math.round(t.items_tested_ok/t.items_configured*100):0;return`
    <div style="padding:20px;max-width:1100px;margin:0 auto;color:var(--ax-text,#eee)">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;margin-bottom:24px">
        ...
        ${l("✅ Configurés",`${t.items_configured} (${o}%)`,"#22cc77")}
  ...
  `}function _(e){const t={credential:"#c9a227",site:"#5c9eff",identifier:"#aaff77",address:"#ff8855",device_id:"#ff66cc",metadata:"#888"}[e.type],s=e.test_result?e.test_result.ok?`<span style="color:#22cc77">🟢 OK${e.test_result.latency_ms?` (${e.test_result.latency_ms}ms)`:""}</span>`:`<span style="color:#ff6b6b">🔴 ${n(e.test_result.error??"fail")}</span>`:'<span style="color:#666">—</span>',o=e.forbidden?'<span style="color:#ff6b6b;font-size:10px;margin-left:4px;background:rgba(255,107,107,0.1);padding:2px 4px;border-radius:3px">🚫 FORBIDDEN</span>':"";return`
    <tr style="border-bottom:1px solid #2a2a2a">
      <td style="padding:6px 8px;color:${t};font-weight:600">${n(e.type)}</td>
      <td style="padding:6px 8px">${n(e.service??"—")}${o}</td>
  ...
  `}function S(e,t){e.querySelector("#ax-msh-refresh")?.addEventListener("click",()=>{c.tap(),p(e)}),e.querySelector("#ax-msh-refresh-services")?.addEventListener("click",()=>{c.tap(),(async()=>{a.info("🔄 Refresh services en cours...",{duration:3e3});try{const r=await g.refreshAll();a.success(`✅ ${r.refreshed} services rafraîchis`,{duration:5e3}),r.errors.length>0&&a.warn(`⚠️ ${r.errors.length} erreur(s)`,{duration:5e3}),p(e)}catch(r){const i=r instanceof Error?r.message:String(r);a.error(`Refresh fail : ${i}`,{duration:5e3})}})()}),e.querySelector("#ax-msh-clear")?.addEventListener("click",()=>{if(c.warning(),!!confirm("Vider tout l'historique multi-source ? (irréversible)"))try{localStorage.removeItem("ax_multi_source_history"),a.success("🗑️ Historique vidé",{duration:3e3}),p(e)}catch(r){const i=r instanceof Error?r.message:String(r);a.error(`Clear fail : ${i}`)}}),e.querySelectorAll(".ax-msh-reanalyze").forEach(r=>{r.addEventListener("click",()=>{c.tap();const i=parseInt(r.dataset.idx??"-1",10),x=t[i];if(!x){a.warn("Source non trouvée");return}a.info(`Re-analyse demandée pour source #${i} (${x.source_type})`,{duration:4e3}),m.installAll(x,{test:!0}).then(f=>{a.success(`✅ ${f.installed} installés · ${f.tested_ok} testés OK`,{duration:5e3}),p(e)})})})}export{M as dispose,p as render};
