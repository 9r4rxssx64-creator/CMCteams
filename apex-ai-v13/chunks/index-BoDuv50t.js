...
import{_ as u}from"./apex-kb-Cc3uNflI.js";import{c as M}from"./listener-cleanup-Y2rGGxxX.js";import{l as D}from"./monitoring-3uBGKGRH.js";import"./credential-patterns-Dy6Wjk7e.js";let i=null;function z(){i?.cleanup(),i=null}async function g(a){i?.cleanup(),i=M("sentinels");const{sentinels:d}=await u(async()=>{const{sentinels:t}=await import("./sentinels-D6sOY4VL.js");return{sentinels:t}},__vite__mapDeps([0,1,2]),import.meta.url),{sentinelsRegistry:m,bootstrapSentinelsRegistry:h}=await u(async()=>{const{sentinelsRegistry:t,bootstrapSentinelsRegistry:n}=await import("./sentinels-registry-DHdMuAO3.js");return{sentinelsRegistry:t,bootstrapSentinelsRegistry:n}},__vite__mapDeps([0,1,2]),import.meta.url);h();const s=d.list(),$=m.getStatus(),x=m.getMetrics(),y=s.filter(t=>t.lastResult?.ok).length,f=s.filter(t=>t.lastResult&&!t.lastResult.ok).length,R=s.filter(t=>!t.lastResult).length;a.innerHTML=`
  ...
  `;const b=a.querySelector("#ax-sent-run-all");b&&i.bind(b,"click",()=>{(async()=>{const{toast:t}=await u(async()=>{const{toast:n}=await import("./toast-Dgg9rcIP.js");return{toast:n}},[],import.meta.url);t.info(`Exécution de ${s.length} sentinelles...`),await Promise.all(s.map(n=>d.runOne(n.id))),t.success("✅ Tous les sentinelles re-exécutés"),await g(a)})()});const v=a.querySelector("#ax-sent-refresh");v&&i.bind(v,"click",()=>{g(a)}),a.querySelectorAll(".ax-sent-run").forEach(t=>{i.bind(t,"click",n=>{n.stopPropagation(),(async()=>{const e=t.dataset.sentId;if(!e)return;const{toast:p}=await u(async()=>{const{toast:l}=await import("./toast-Dgg9rcIP.js");return{toast:l}},[],import.meta.url),r=await d.runOne(e);p[r?.ok?"success":"warn"](`${e}: ${r?.msg??"KO"}`),await g(a)})()})}),a.querySelectorAll(".ax-sent-row").forEach(t=>{i.bind(t,"click",()=>{(async()=>{const n=t.dataset.sentId;if(!n)return;const e=s.find(c=>c.id===n);if(!e)return;const{drillDown:p}=await u(async()=>{const{drillDown:c}=await import("./drilldown-1SlvGToi.js");return{drillDown:c}},[],import.meta.url),r="ax-drilldown-mount-sentinels";let l=document.getElementById(r);l||(l=document.createElement("div"),l.id=r,document.body.appendChild(l)),p.open({id:`sent-${n}`,title:`🛡 ${e.name}`,content:()=>{const c=e.lastResult?.ok,w=e.lastResult?.msg??"—",_=e.lastResult?new Date(e.lastResult.ts).toLocaleString("fr-FR"):"—";return`
