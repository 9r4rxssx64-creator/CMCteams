import{a as d}from"./escape-html-DGIYNPKb.js";import{c as y}from"./listener-cleanup-Y2rGGxxX.js";import{q as b}from"./monitoring-BurevTV9.js";import{rgpd as p}from"./rgpd-xdBngC4l.js";import{c as k}from"./csp-style-helper-BisGRi53.js";import"./multi-source-analyze-tqKS13-U.js";import"./apex-kb-W8GOXAde.js";import"./credential-patterns-CLzI061R.js";let i=null;function z(){i?.cleanup(),i=null}const x=[{id:"cgu",label:"CGU",file:"cgu",emoji:"📜"},{id:"cgv",label:"CGV",file:"cgv",emoji:"💳"},{id:"privacy",label:"Confidentialité",file:"privacy-policy",emoji:"🔒"},{id:"cookies",label:"Cookies",file:"cookie-policy",emoji:"🍪"},{id:"dpa",label:"DPA",file:"data-processing-agreement",emoji:"🤝"},{id:"mentions",label:"Mentions légales",file:"mentions-legales",emoji:"🏛️"},{id:"rgpd",label:"Mes données",file:"",emoji:"⚙️"}];function $(t){let e=d(t);return e=e.replace(/^###### (.+)$/gm,"<h6>$1</h6>"),e=e.replace(/^##### (.+)$/gm,"<h5>$1</h5>"),e=e.replace(/^#### (.+)$/gm,"<h4>$1</h4>"),e=e.replace(/^### (.+)$/gm,"<h3>$1</h3>"),e=e.replace(/^## (.+)$/gm,"<h2>$1</h2>"),e=e.replace(/^# (.+)$/gm,"<h1>$1</h1>"),e=e.replace(/^&gt; (.+)$/gm,"<blockquote>$1</blockquote>"),e=e.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),e=e.replace(/\*([^*]+)\*/g,"<em>$1</em>"),e=e.replace(/`([^`]+)`/g,"<code>$1</code>"),e=e.replace(/\[([^\]]+)\]\(([^)]+)\)/g,'<a href="$2" target="_blank" rel="noopener noreferrer">$1</a>'),e=e.replace(/^- (.+)$/gm,"<li>$1</li>"),e=e.replace(/(<li>.*?<\/li>(\n|$))+/gs,a=>`<ul>${a.replace(/\n/g,"")}</ul>`),e=e.replace(/^\|(.+)\|\s*\n\|[\s|:-]+\|\s*\n((?:\|.+\|\s*\n?)+)/gm,(a,o,s)=>{const c=o.split("|").map(n=>n.trim()).filter(Boolean),g=s.trim().split(`
`).map(n=>n.split("|").map(m=>m.trim()).filter(Boolean)),r=c.map(n=>`<th>${n}</th>`).join(""),l=g.map(n=>`<tr>${n.map(m=>`<td>${m}</td>`).join("")}</tr>`).join("");return`<table><thead><tr>${r}</tr></thead><tbody>${l}</tbody></table>`}),e=e.replace(/^---$/gm,"<hr/>"),e=e.replace(/\n\n+/g,"</p><p>"),e=`<p>${e}</p>`,e=e.replace(/<p>(<h[1-6]>)/g,"$1"),e=e.replace(/(<\/h[1-6]>)<\/p>/g,"$1"),e=e.replace(/<p>(<ul>|<table>|<blockquote>|<hr\/>)/g,"$1"),e=e.replace(/(<\/ul>|<\/table>|<\/blockquote>)<\/p>/g,"$1"),e}async function w(t){if(!t)return"";try{const e=await fetch(`/docs/legal/${t}.md`,{signal:AbortSignal.timeout(5e3)});if(e.ok)return await e.text()}catch(e){b.warn("feature-legal",`Failed to load doc ${t}`,{err:e})}return`# Document indisponible

Le document **${t}** est temporairement indisponible. Réessaie plus tard ou contacte kevin.desarzens@gmail.com.`}function f(t){return x.map(e=>{const a=e.id===t,o=`padding:10px 14px;font-size:13px;font-weight:600;border-radius:8px;cursor:pointer;background:${a?"rgba(232,184,48,0.18)":"rgba(255,255,255,0.03)"};color:${a?"#e8b830":"rgba(255,255,255,0.7)"};border:1px solid ${a?"rgba(232,184,48,0.4)":"rgba(255,255,255,0.05)"};transition:all 180ms`;return`<button class="ax-legal-tab" data-tab="${d(e.id)}" style="${o}">${e.emoji} ${d(e.label)}</button>`}).join("")}function v(){const t=p.showCookieBanner(),e=p.hasConsent("analytics"),a=p.hasConsent("marketing");return`
    ...
    <p>Cookies marketing : <strong>${a?"✅ Acceptés":"❌ Refusés"}</strong></p>
  ...
  `}function u(){try{const t=localStorage.getItem("apex_v13_user");return t?JSON.parse(t).id??"":""}catch{return""}}function S(t,e){const a=URL.createObjectURL(t),o=document.createElement("a");o.href=a,o.download=e,o.style.display="none",document.body.appendChild(o),o.click(),setTimeout(()=>{URL.revokeObjectURL(a),o.remove()},1e3)}function A(t){const e=t.querySelector("#ax-rgpd-registry");if(e){const r=p.getProcessingRegistry();e.innerHTML=r.map(l=>`
      <div style="margin-bottom:10px;padding:8px;background:rgba(255,255,255,0.03);border-radius:6px">
        ...
        <div style="font-size:12px;color:rgba(255,255,255,0.6);margin-top:4px">
          Données : ${l.donnees.map(n=>d(n)).join(", ")}<br/>
          Base légale : ${d(l.baseLegale)}<br/>
          Durée : ${d(l.duree)}<br/>
          Destinataires : ${l.destinataires.map(n=>d(n)).join(", ")}
        </div>
      </div>
    `).join("")}const a=t.querySelector("#ax-rgpd-export");a&&i&&i.bind(a,"click",()=>{(async()=>{try{const r=u();if(!r){alert("Tu dois être connecté pour exporter tes données.");return}const l=await p.portableExport(r);S(l,`apex-data-${r}-${Date.now()}.json`)}catch(r){b.warn("feature-legal","Export failed",{err:r}),alert("Export échoué. Réessaie ou contacte le support.")}})()});const o=t.querySelector("#ax-rgpd-delete");o&&i&&i.bind(o,"click",()=>{(async()=>{const r=u();if(!r){alert("Tu dois être connecté pour supprimer ton compte.");return}if(confirm("Confirmer la suppression définitive de ton compte ? Cette action est IRRÉVERSIBLE."))try{const n=await p.deleteUserData(r,!0);n.ok?(alert(`Compte supprimé. ${n.deletedKeys.length} clés effacées. Audit log conservé.`),location.hash="#login"):alert(`Suppression partielle : ${n.failures.join(", ")}`)}catch(n){b.warn("feature-legal","Delete failed",{err:n}),alert("Suppression échouée. Contacte le support.")}})()});const s=t.querySelector("#ax-rgpd-cookies-customize");s&&i&&i.bind(s,"click",()=>{const r=confirm("Cookies analytics (anonymisés) — accepter ?"),l=confirm("Cookies marketing — accepter ? (non utilisés actuellement)");p.setConsent({analytics:r,marketing:l,preferences:!0}),alert("Préférences cookies enregistrées. Recharge la page pour voir l'effet.")});const c=t.querySelector("#ax-rgpd-optout-training");c&&i&&i.bind(c,"click",()=>{const r=u();if(!r){alert("Tu dois être connecté.");return}p.optOutAITraining(r,!0),alert("Opt-out IA training enregistré.")});const g=t.querySelector("#ax-rgpd-optout-automation");g&&i&&i.bind(g,"click",()=>{const r=u();if(!r){alert("Tu dois être connecté.");return}p.optOutAutomation(r,!0),alert("Opt-out automation Art. 22 enregistré.")})}async function h(t,e){const a=x.find(c=>c.id===e);if(!a)return;const o=t.querySelector("#ax-legal-content");if(!o)return;if(a.id==="rgpd"){o.innerHTML=v(),A(o);return}o.innerHTML='<p style="color:rgba(255,255,255,0.5);font-style:italic">Chargement…</p>';const s=await w(a.file);o.innerHTML=$(s)}function B(t){i?.cleanup(),i=y("legal");const e=location.hash.replace(/^#legal\/?/,"").split("/")[0]||"cgu",a=x.find(o=>o.id===e)?e:"cgu";t.innerHTML=k.withNonce(`
    <style>
      .ax-legal-content {
  ...
  `),h(t,a),t.querySelectorAll(".ax-legal-tab").forEach(o=>{i.bind(o,"click",()=>{const s=o.getAttribute("data-tab");if(!s)return;const c=t.querySelector("#ax-legal-tabs");c&&(c.innerHTML=f(s),t.querySelectorAll(".ax-legal-tab").forEach(g=>{i.bind(g,"click",()=>{const r=g.getAttribute("data-tab");r&&(location.hash=`#legal/${r}`)})})),h(t,s);try{history.replaceState(null,"",`#legal/${s}`)}catch{}})}),b.info("feature-legal","rendered",{tab:a})}export{z as dispose,B as render};
