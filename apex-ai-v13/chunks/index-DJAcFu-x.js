const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./monitoring-BAwFG35Q.js","./multi-source-analyze-DRqrcvIZ.js","./credential-patterns-DUMYZEMu.js","./apex-kb-CGmn9N6T.js","./auth-9SpRE9Vg.js"])))=>i.map(i=>d[i]);
import{b as L,e as x,_ as l,l as z}from"./monitoring-BAwFG35Q.js";import{c as P}from"./listener-cleanup-Y2rGGxxX.js";import{c as O}from"./csp-style-helper-BEHhIhzj.js";import{r as D}from"./recharge-action-CoI11wkT.js";import"./multi-source-analyze-DRqrcvIZ.js";import"./credential-patterns-DUMYZEMu.js";import"./apex-kb-CGmn9N6T.js";let i=null;function K(){i?.cleanup(),i=null}const I="apex_v13_chat_auto_read";function R(){try{return localStorage.getItem(I)==="1"}catch{return!1}}function V(o){try{localStorage.setItem(I,o?"1":"0")}catch{}}async function q(o){try{const k=await l(()=>import("./voice-BwaSQ8L2.js"),__vite__mapDeps([0,1,2,3]),import.meta.url),{listVoices:A,getActiveVoice:g,setActiveVoice:b,speak:v,stopAll:m}=k,_=o.querySelector("#ax-settings-auto-read"),S=o.querySelector("#ax-voice-current"),$=o.querySelector("#ax-voice-list"),E=o.querySelectorAll(".ax-voice-cat-btn"),w=o.querySelector("#ax-settings-rag");if(w&&(l(async()=>{const{apexMemoryRag:c}=await import("./apex-memory-rag-DYGtb1OV.js");return{apexMemoryRag:c}},__vite__mapDeps([0,1,2,3]),import.meta.url).then(({apexMemoryRag:c})=>{w.checked=c.isEnabled()}),i.bind(w,"change",()=>{(async()=>{const{apexMemoryRag:c}=await l(async()=>{const{apexMemoryRag:u}=await import("./apex-memory-rag-DYGtb1OV.js");return{apexMemoryRag:u}},__vite__mapDeps([0,1,2,3]),import.meta.url);c.enable(w.checked);const{toast:p}=await l(async()=>{const{toast:u}=await import("./toast-BCPNzfMv.js");return{toast:u}},[],import.meta.url);p.success(w.checked?"🧠 Mémoire long terme activée":"Mémoire long terme désactivée")})()})),!$)return;_&&(_.checked=R(),i.bind(_,"change",()=>{V(_.checked),(async()=>{const{toast:c}=await l(async()=>{const{toast:p}=await import("./toast-BCPNzfMv.js");return{toast:p}},[],import.meta.url);c.success(_.checked?"Lecture auto activée":"Lecture auto désactivée")})()}));const T=()=>{if(!S)return;const c=g(),u=A().find(a=>a.id===c);S.textContent=u?`Voix active : ${u.emoji??"🔊"} ${u.name} (${u.category})`:`Voix active : ${c}`};T();const h=c=>{const p=A(),u=c==="all"?p:p.filter(t=>t.category===c),a=g();$.setAttribute("aria-busy","false"),$.innerHTML=u.map(t=>{const s=t.id===a,e=t.emoji??(t.category==="pro"?"🎙️":t.category==="fun"?"🎉":"🎨"),r=t.description?x(t.description):"";return`
  ...
  `),(async()=>{try{const{memoryBridge:a}=await l(async()=>{const{memoryBridge:r}=await import("./memory-bridge-CCDqcwYR.js");return{memoryBridge:r}},__vite__mapDeps([0,1,2,3]),import.meta.url),t=o.querySelector("#ax-memory-bridge-status"),s=o.querySelector("#ax-memory-bridge-sync"),e=()=>{if(!t)return;const r=a.getHealth(),d=a.getStatus(),n=d.filter(f=>f.last_success).length;t.textContent=`${r.backends_configured} backends configurés · ${n}/${d.length} dernier sync OK`};e(),s&&i&&i.bind(s,"click",()=>{(async()=>{s&&(s.disabled=!0);const r=await a.runAutoSync(),d=r.filter(f=>f.ok).length,{toast:n}=await l(async()=>{const{toast:f}=await import("./toast-BCPNzfMv.js");return{toast:f}},[],import.meta.url);r.length===0?n.warn("Aucun backend configuré"):d===r.length?n.success(`Sync OK (${d}/${r.length})`):n.warn(`Sync partielle (${d}/${r.length})`),e(),s&&(s.disabled=!1)})()})}catch(a){z.warn("feature-settings","memory-bridge wire failed",{err:a})}})();const _=o.querySelector("#ax-conso-scan");_&&i&&i.bind(_,"click",()=>{(async()=>{try{const{consumptionAnomalyDetector:a}=await l(async()=>{const{consumptionAnomalyDetector:e}=await import("./monitoring-BAwFG35Q.js").then(r=>r.J);return{consumptionAnomalyDetector:e}},__vite__mapDeps([0,1,2,3]),import.meta.url),t=a.scanAllVerbose(),s=o.querySelector("#ax-conso-results");if(!s)return;s.innerHTML=t.map(e=>{const r=e.severity==="critical"?"var(--ax-sev-critical)":e.severity==="high"?"var(--ax-sev-high)":e.severity==="medium"?"var(--ax-sev-medium)":e.severity==="low"?"var(--ax-sev-low)":"var(--ax-green)",d=e.severity==="critical"?"ax-sev-critical":e.severity==="high"?"ax-sev-high":e.severity==="medium"?"ax-sev-medium":(e.severity==="low","ax-sev-low"),n=e.severity==="critical"?"🚨":e.severity==="high"?"⚠️":e.severity==="medium"?"🟡":e.severity==="low"?"🔵":"✅";return`<div style="background:rgba(255,255,255,0.03);border-left:3px solid ${r};padding:8px 12px;margin-top:6px;border-radius:4px">
          ...
          </div>`}).join("")}catch(a){z.warn("feature-settings","conso scan failed",{err:a})}})()});const S=o.querySelector("#ax-zoom-inspector-btn");S&&i&&i.bind(S,"click",()=>{(async()=>{const{apexZoomInspector:a}=await l(async()=>{const{apexZoomInspector:t}=await import("./apex-zoom-inspector-Uk8WqT3b.js");return{apexZoomInspector:t}},__vite__mapDeps([0,1,2,3]),import.meta.url);a.isVisible()?a.hide():a.show()})()});const $=o.querySelector("#ax-functional-test-btn");$&&i&&i.bind($,"click",()=>{(async()=>{const a=o.querySelector("#ax-functional-test-results");if(a){a.innerHTML=`<div style="color:var(--ax-blue)">⏳ Test des boutons en cours (jusqu'à 40 boutons, ~30s)...</div>`;try{const{apexFunctionalTester:t}=await l(async()=>{const{apexFunctionalTester:y}=await import("./apex-functional-tester-C5cquJQx.js");return{apexFunctionalTester:y}},__vite__mapDeps([0,1,2,3]),import.meta.url),{reportsHistory:s}=await l(async()=>{const{reportsHistory:y}=await import("./apex-reports-history-Dhi0ixmm.js");return{reportsHistory:y}},__vite__mapDeps([0,1,2,3]),import.meta.url),e=await t.testAndAutoFix({maxButtons:30});s.recordFunctional(e.before,e.fixes,e.after,e.improvement);const r=e.before.tested>0?Math.round(e.before.ok/e.before.tested*100):0,d=e.after?` → après fix : ${Math.round(e.after.ok/Math.max(1,e.after.tested)*100)}% OK (${e.improvement>0?"+":""}${Math.round(e.improvement*100)}%)`:"",n=e.before.details.filter(y=>y.status==="no_response"||y.status==="error").slice(0,5).map(y=>`<li class="ax-gs-250">${x(y.label||"(no label)")} → ${x(y.status)}</li>`).join(""),f=e.fixes.applied.map(y=>x(String(y))).join(", ");a.innerHTML=`
