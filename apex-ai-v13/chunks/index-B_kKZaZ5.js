...
>📊 Usage</a>`:"";return`
        <div class="ax-service-health-row ax-modernized-card"
          ...
          ${x}
          ${f}
        </div>`}).join("")}async function K(t){const r={};if(t.length===0)return r;try{const{linksRegistry:o}=await g(async()=>{const{linksRegistry:a}=await import("./multi-source-analyze-BefgB4O2.js").then(e=>e.c);return{linksRegistry:a}},[],import.meta.url);for(const a of t)r[a]={recharge:o.getRechargeLink(a),usage:o.getUsageLink(a),apiKeys:o.getApiKeysLink(a)}}catch{}return r}async function N(t){d?.cleanup(),d=b("dashboard");const r=m.get("user"),o=r?.name?`Bonjour ${r.name}`:"Bonjour",[a,e,c]=await Promise.all([v(),w(),k()]),l=await K(c.map(s=>s.service)),n=$();t.innerHTML=`
    <style>
  ...
  `,t.querySelectorAll("[data-route]").forEach(s=>{d.bind(s,"click",()=>{h.tap();const p=s.dataset.route;p&&(window.location.hash="#"+p)})}),u.info("feature-dashboard",`rendered (${a.length} kpis, ${e.length} alerts, ${n.length} todos)`)}export{v as computeKpis,E as dispose,i as escapeHtml,w as loadAlerts,K as loadRechargeLinks,k as loadServiceHealth,$ as loadTodos,N as render,R as renderServiceHealthCard};
