...
`)}}const l=new k,E={ai:"🧠 Intelligence Artificielle",saas:"💼 SaaS",comms:"✉️ Communications",infra:"☁️ Infrastructure",finance:"💳 Finance & Paiements",other:"🔧 Autres"};function w(t){if(t.length<2)return'<span class="ax-spark-empty">—</span>';const e=Math.max(...t.map(o=>o.eur),1),n=120,s=32,r=n/(t.length-1),a=t.map((o,i)=>`${i===0?"M":"L"}${(i*r).toFixed(1)} ${(s-o.eur/e*s).toFixed(1)}`).join(" ");return`
  ...
  `}function T(t){const e=l.getSummary(),n=l.getSparklineMonth(),s=l.getHourlyHeatmap(),r=new Map;for(const u of e.services){const c=r.get(u.category)??[];c.push(u),r.set(u.category,c)}const o=["ai","comms","infra","finance","saas","other"].map(u=>j(u,r.get(u)??[])).join(""),i=e.projection_end_month_eur-e.total_month_eur;t.innerHTML=`
  ...
  `}export{T as render};
//# sourceMappingURL=financial-bilan-B1MK45HU.js.map
