import{a as l}from"./escape-html-DGIYNPKb.js";import{q as x}from"./monitoring-DqzsxXYC.js";import{v as g}from"./apex-tools-dispatch-skills-CSqzJm_J.js";import{toast as o}from"./toast-CRdbcLoc.js";import"./multi-source-analyze-CfUsGw_c.js";import"./apex-kb-AropY-xd.js";import"./credential-patterns-CLzI061R.js";import"./haptic-CQFg2PXZ.js";const b=[{id:"pitch-startup",label:"Pitch startup",emoji:"🚀"},{id:"business-quarterly",label:"Review trimestrielle",emoji:"📊"},{id:"lecture-academic",label:"Cours académique",emoji:"🎓"},{id:"wedding-anniversary",label:"Mariage / Anniv",emoji:"💍"},{id:"birthday-party",label:"Anniversaire fun",emoji:"🎂"},{id:"casino-training",label:"Formation casino",emoji:"🎰"},{id:"product-launch",label:"Lancement produit",emoji:"📢"}];let r=[{title:"Slide 1",content:`• Point 1
• Point 2
• Point 3`}];function j(t){function p(){return r.map((a,s)=>`<div style="background:#0f172a;border:1px solid #1e293b;border-radius:8px;padding:12px;margin-bottom:10px">
            <div class="ax-gs-22">
          ...
          </div>`).join("")}function n(){t.innerHTML=`
      <div class="ax-gs-59">
    ...
    `,t.querySelector("#pptx-add-slide")?.addEventListener("click",()=>{r.push({title:`Slide ${r.length+1}`,content:"• Point 1"}),n()}),t.querySelectorAll("[data-rm]").forEach(a=>{a.addEventListener("click",()=>{const s=parseInt(a.getAttribute("data-rm")??"-1",10);s>=0&&r.length>1&&(r.splice(s,1),n())})}),t.querySelector("#pptx-generate")?.addEventListener("click",async()=>{const a=t.querySelector("#pptx-template")?.value??"pitch-startup",s=t.querySelector("#pptx-title")?.value||"Présentation",d=t.querySelector("#pptx-author")?.value||"Apex",c=t.querySelector("#pptx-mode")?.value,u=r.map((e,i)=>({title:t.querySelector(`[data-slide-title="${i}"]`)?.value??`Slide ${i+1}`,content:t.querySelector(`[data-slide-content="${i}"]`)?.value??""}));o.info("Génération en cours...");try{const e=await g.generate({template:a,title:s,author:d,slides:u,mode:c}),i=t.querySelector("#pptx-result");if(!i)return;e.success?(i.innerHTML=`
            <div class="ax-gs-47">
            ...
            </div>`,o.success(`✅ ${e.filename}`)):(i.innerHTML=`<p class="ax-gs-257">❌ ${l(e.error??"Erreur")}</p>`,o.error(`❌ ${e.error??"Erreur"}`))}catch(e){x.warn("studio-pptx","failed",{err:e}),o.error(`❌ ${e instanceof Error?e.message:"Erreur"}`)}})}n()}export{j as render};
