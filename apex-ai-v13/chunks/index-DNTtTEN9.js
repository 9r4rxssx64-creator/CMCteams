const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./monitoring-3uBGKGRH.js","./apex-kb-DRP-rQ5c.js","./credential-patterns-CLzI061R.js","./multi-source-analyze-DFRy0PZC.js","./auth-JGQ_KFuU.js","../assets/css/main-BUIq5pfg.css"])))=>i.map(i=>d[i]);
import{_ as u}from"./apex-kb-DRP-rQ5c.js";import{c as P}from"./listener-cleanup-Y2rGGxxX.js";import{l as A}from"./monitoring-3uBGKGRH.js";import{s as L}from"../core/main-BxMVCELi.js";import{c as E}from"./csp-style-helper-BisGRi53.js";import"./credential-patterns-CLzI061R.js";import"./multi-source-analyze-DFRy0PZC.js";let n=null;function j(){n?.cleanup(),n=null}function v(i){return i.replace(/[&<>"']/g,h=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[h]??h)}const I="apex_v13_chat_auto_read";function O(){try{return localStorage.getItem(I)==="1"}catch{return!1}}function D(i){try{localStorage.setItem(I,i?"1":"0")}catch{}}async function V(i){try{const h=await u(()=>import("./voice-B7u_Xmm6.js").then(l=>l.c),__vite__mapDeps([0,1,2]),import.meta.url),{listVoices:k,getActiveVoice:x,setActiveVoice:f,speak:m,stopAll:b}=h,_=i.querySelector("#ax-settings-auto-read"),S=i.querySelector("#ax-voice-current"),w=i.querySelector("#ax-voice-list"),T=i.querySelectorAll(".ax-voice-cat-btn");if(!w)return;_&&(_.checked=O(),n.bind(_,"change",()=>{D(_.checked),(async()=>{const{toast:l}=await u(async()=>{const{toast:d}=await import("./toast-ClsF1KRZ.js");return{toast:d}},[],import.meta.url);l.success(_.checked?"Lecture auto activée":"Lecture auto désactivée")})()}));const z=()=>{if(!S)return;const l=x(),g=k().find(y=>y.id===l);S.textContent=g?`Voix active : ${g.emoji??"🔊"} ${g.name} (${g.category})`:`Voix active : ${l}`};z();const $=l=>{const d=k(),g=l==="all"?d:d.filter(e=>e.category===l),y=x();w.innerHTML=g.map(e=>{const a=e.id===y,o=e.emoji??(e.category==="pro"?"🎙️":e.category==="fun"?"🎉":"🎨"),t=e.description?v(e.description):"",r=a?"background:rgba(232,184,48,0.15);border-color:rgba(232,184,48,0.45)":"background:rgba(255,255,255,0.03);border-color:rgba(255,255,255,0.06)";return`
            <div class="ax-voice-item" data-voice-id="${v(e.id)}" style="display:flex;align-items:center;gap:8px;padding:10px;margin-bottom:6px;border:1px solid;border-radius:8px;${r}">
              <span style="font-size:18px">${o}</span>
              <div style="flex:1;min-width:0">
                <div style="color:#fff;font-size:13px;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${v(e.name)}${a?' <span style="color:#e8b830;font-size:11px">★ active</span>':""}</div>
                <div style="color:rgba(255,255,255,0.5);font-size:11px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${v(e.category)}${t?" · "+t:""}</div>
              </div>
              <button class="ax-voice-test-btn" data-test-voice="${v(e.id)}" title="Tester cette voix" aria-label="Tester ${v(e.name)}" style="min-width:44px;min-height:44px;width:44px;height:44px;border-radius:8px;background:rgba(34,204,119,0.15);color:#22cc77;border:1px solid rgba(34,204,119,0.3);cursor:pointer;font-size:14px">▶</button>
              <button class="ax-voice-set-btn" data-set-voice="${v(e.id)}" title="Définir comme voix par défaut" aria-label="Définir ${v(e.name)} par défaut" style="min-width:44px;min-height:44px;width:44px;height:44px;border-radius:8px;background:rgba(232,184,48,0.15);color:#e8b830;border:1px solid rgba(232,184,48,0.3);cursor:pointer;font-size:14px">★</button>
            </div>
          `}).join("")};$("all"),T.forEach(l=>{n.bind(l,"click",()=>{const d=l.getAttribute("data-cat");d&&$(d)})}),n.bind(w,"click",l=>{const d=l.target,g=d.closest("[data-test-voice]"),y=d.closest("[data-set-voice]");if(g){const e=g.getAttribute("data-test-voice");if(!e)return;(async()=>{b();const a=await m("Bonjour Kevin, je suis ta voix.",e);if(!a.ok){const{toast:o}=await u(async()=>{const{toast:t}=await import("./toast-ClsF1KRZ.js");return{toast:t}},[],import.meta.url);o.warn(`Test échoué : ${a.reason??"erreur"}`)}})();return}if(y){const e=y.getAttribute("data-set-voice");if(!e)return;(async()=>{await f(e),z();const a=i.querySelector(".ax-voice-cat-btn[data-cat]:focus")?.getAttribute("data-cat")??"all";$(a);const{toast:o}=await u(async()=>{const{toast:s}=await import("./toast-ClsF1KRZ.js");return{toast:s}},[],import.meta.url),r=k().find(s=>s.id===e);o.success(r?`Voix par défaut : ${r.name}`:"Voix mise à jour")})()}})}catch(h){A.warn("feature-settings","wireVoiceSection failed",{err:h})}}function F(i){n?.cleanup(),n=P("settings");const h=L.get("user"),k=L.get("isAdmin")??!1,x="background:linear-gradient(135deg,rgba(20,20,35,0.7),rgba(14,14,28,0.5));backdrop-filter:blur(16px) saturate(140%);-webkit-backdrop-filter:blur(16px) saturate(140%);border:1px solid rgba(255,255,255,0.08);border-radius:16px;padding:20px;margin-top:14px;transition:all 240ms cubic-bezier(0.16,1,0.3,1)",f="margin:0 0 12px;font-size:15px;font-weight:700;color:#fff;letter-spacing:-0.01em;display:flex;align-items:center;gap:10px",m="display:inline-flex;align-items:center;justify-content:center;width:32px;height:32px;background:linear-gradient(135deg,rgba(232,184,48,0.2),rgba(201,162,39,0.08));border:1px solid rgba(232,184,48,0.25);border-radius:10px;font-size:16px",b="width:100%;min-height:44px;padding:12px 16px;font-size:14px;font-weight:600;border-radius:10px;cursor:pointer;-webkit-tap-highlight-color:transparent;transition:all 180ms cubic-bezier(0.16,1,0.3,1)";i.innerHTML=E.withNonce(`
    <style>
    ...
    <div class="ax-page" style="padding:24px 16px max(24px, env(safe-area-inset-bottom)) 16px;max-width:680px;margin:0 auto;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif">
      <header style="margin-bottom:24px;animation:ax-fade-up 360ms cubic-bezier(0.16,1,0.3,1) backwards">
        ...
        <p style="color:rgba(255,255,255,0.55);margin:0;font-size:14px">Utilisateur : <strong style="color:rgba(255,255,255,0.9)">${v(h?.name??"inconnu")}</strong> ${k?'<span style="color:#e8b830">👑 Admin</span>':""}</p>
      </header>
  ...
  `),(async()=>{try{const{memoryBridge:e}=await u(async()=>{const{memoryBridge:r}=await import("./memory-bridge-B9zKArZD.js");return{memoryBridge:r}},__vite__mapDeps([1,0,2]),import.meta.url),a=i.querySelector("#ax-memory-bridge-status"),o=i.querySelector("#ax-memory-bridge-sync"),t=()=>{if(!a)return;const r=e.getHealth(),s=e.getStatus(),c=s.filter(p=>p.last_success).length;a.textContent=`${r.backends_configured} backends configurés · ${c}/${s.length} dernier sync OK`};t(),o&&n&&n.bind(o,"click",()=>{(async()=>{o&&(o.disabled=!0);const r=await e.runAutoSync(),s=r.filter(p=>p.ok).length,{toast:c}=await u(async()=>{const{toast:p}=await import("./toast-ClsF1KRZ.js");return{toast:p}},[],import.meta.url);r.length===0?c.warn("Aucun backend configuré"):s===r.length?c.success(`Sync OK (${s}/${r.length})`):c.warn(`Sync partielle (${s}/${r.length})`),t(),o&&(o.disabled=!1)})()})}catch(e){A.warn("feature-settings","memory-bridge wire failed",{err:e})}})();const _=i.querySelector("#ax-conso-scan");_&&n&&n.bind(_,"click",()=>{(async()=>{try{const{consumptionAnomalyDetector:e}=await u(async()=>{const{consumptionAnomalyDetector:t}=await import("./consumption-anomaly-detector-RhnDbJY5.js");return{consumptionAnomalyDetector:t}},__vite__mapDeps([0,1,2,3]),import.meta.url),a=e.scanAllVerbose(),o=i.querySelector("#ax-conso-results");if(!o)return;o.innerHTML=a.map(t=>{const r=t.severity==="critical"?"#ff4444":t.severity==="high"?"#ff8844":t.severity==="medium"?"#ffaa00":t.severity==="low"?"#88aaff":"#22cc77",s=t.severity==="critical"?"🚨":t.severity==="high"?"⚠️":t.severity==="medium"?"🟡":t.severity==="low"?"🔵":"✅";return`<div style="background:rgba(255,255,255,0.03);border-left:3px solid ${r};padding:8px 12px;margin-top:6px;border-radius:4px">
          ...
          </div>`}).join("")}catch(e){A.warn("feature-settings","conso scan failed",{err:e})}})()});const S=i.querySelector("#ax-zoom-inspector-btn");S&&n&&n.bind(S,"click",()=>{(async()=>{const{apexZoomInspector:e}=await u(async()=>{const{apexZoomInspector:a}=await import("./apex-zoom-inspector-B3cbSE7P.js");return{apexZoomInspector:a}},__vite__mapDeps([1,0,2]),import.meta.url);e.isVisible()?e.hide():e.show()})()});const w=i.querySelector("#ax-functional-test-btn");w&&n&&n.bind(w,"click",()=>{(async()=>{const e=i.querySelector("#ax-functional-test-results");if(e){e.innerHTML=`<div style="color:#6a8aff">⏳ Test des boutons en cours (jusqu'à 40 boutons, ~30s)...</div>`;try{const{apexFunctionalTester:a}=await u(async()=>{const{apexFunctionalTester:p}=await import("./apex-functional-tester-BmMXFm4n.js");return{apexFunctionalTester:p}},__vite__mapDeps([1,0,2]),import.meta.url),{reportsHistory:o}=await u(async()=>{const{reportsHistory:p}=await import("./apex-reports-history-Cv_X_jWe.js");return{reportsHistory:p}},__vite__mapDeps([0]),import.meta.url),t=await a.testAndAutoFix({maxButtons:30});o.recordFunctional(t.before,t.fixes,t.after,t.improvement);const r=t.before.tested>0?Math.round(t.before.ok/t.before.tested*100):0,s=t.after?` → après fix : ${Math.round(t.after.ok/Math.max(1,t.after.tested)*100)}% OK (${t.improvement>0?"+":""}${Math.round(t.improvement*100)}%)`:"",c=t.before.details.filter(p=>p.status==="no_response"||p.status==="error").slice(0,5).map(p=>`<li style="color:#ffaa66;font-size:11px">${p.label||"(no label)"} → ${p.status}</li>`).join("");e.innerHTML=`
            <div style="background:rgba(106,138,255,0.08);border:1px solid rgba(106,138,255,0.3);border-radius:8px;padding:10px;color:#fff;font-size:12px">
              ...
              ${t.fixes.applied.length?`<div style="margin-top:6px;color:#c9a227">🔧 Auto-fix appliqué : ${t.fixes.applied.join(", ")}</div>`:""}
          ...
          `}catch(a){const o=a instanceof Error?a.message:String(a);e.innerHTML=`<div style="color:#ff5b5b">❌ Erreur test : ${o}</div>`}}})()});const T=i.querySelector("#ax-layout-inspect-btn");T&&n&&n.bind(T,"click",()=>{(async()=>{const e=i.querySelector("#ax-layout-inspect-results");if(e){e.innerHTML='<div style="color:#c97aff">⏳ Scan layout...</div>';try{const{apexLayoutInspector:a}=await u(async()=>{const{apexLayoutInspector:c}=await import("./apex-layout-inspector-D7L5Bohc.js");return{apexLayoutInspector:c}},__vite__mapDeps([1,0,2]),import.meta.url),{reportsHistory:o}=await u(async()=>{const{reportsHistory:c}=await import("./apex-reports-history-Cv_X_jWe.js");return{reportsHistory:c}},__vite__mapDeps([0]),import.meta.url),t=a.scanDom();o.recordLayout(t);const r=t.hiddenButtons.slice(0,5).map(c=>`<li style="color:#ffaa66;font-size:11px">"${c.label}" → ${c.reason}</li>`).join(""),s=t.overflowingElements.slice(0,5).map(c=>`<li style="color:#ff5b5b;font-size:11px">${c.tag} (+${c.overflowBy}px)</li>`).join("");e.innerHTML=`
          ...
          `}catch(a){const o=a instanceof Error?a.message:String(a);e.innerHTML=`<div style="color:#ff5b5b">❌ Erreur scan : ${o}</div>`}}})()});const z=i.querySelector("#ax-cf-diagnostic-btn");z&&n&&n.bind(z,"click",()=>{(async()=>{const e=i.querySelector("#ax-cf-diagnostic-results");if(e){e.innerHTML='<div style="color:#f78322">⏳ Test Cloudflare API en cours...</div>';try{const{apexCloudflareVaultDeploy:a}=await u(async()=>{const{apexCloudflareVaultDeploy:s}=await import("./apex-cloudflare-vault-deploy-Bp8By_9v.js");return{apexCloudflareVaultDeploy:s}},__vite__mapDeps([1,0,2]),import.meta.url),o=await a.runDiagnostic(),t=(s,c,p)=>`
            <div style="display:flex;align-items:center;gap:8px;padding:6px 0;font-size:13px">
              ...
              ${p?`<span style="color:rgba(255,255,255,0.5);font-size:11px">${p}</span>`:""}
            </div>`;let r='<div style="background:rgba(15,15,25,0.8);border:1px solid rgba(247,131,34,0.3);border-radius:10px;padding:12px;margin-top:10px">';r+='<div style="color:#f78322;font-weight:700;margin-bottom:8px">☁️ Diagnostic Cloudflare</div>',r+=t("Token Cloudflare présent",o.token_present),r+=t("Token valide (HTTP 200)",o.token_valid,o.http_status?`HTTP ${o.http_status}`:""),r+=t("Account ID accessible",!!o.account_id,o.account_name??o.account_id??""),r+=t("Permission KV (Workers KV Storage:Edit)",o.kv_permission,o.namespace_id?`ns ${o.namespace_id.slice(0,8)}…`:""),r+=t("Permission Workers (auto-deploy futur)",o.workers_permission),r+=t("Namespace apex-vault-kevin existe",o.namespace_exists),o.error_reason&&(r+=`<div style="margin-top:10px;padding:8px;background:rgba(255,91,91,0.1);border-left:3px solid #ff5b5b;color:#ff5b5b;font-size:12px;border-radius:4px">${o.error_reason}</div>`),o.fix_url&&(r+=`<div style="margin-top:8px"><a href="${o.fix_url}" target="_blank" rel="noopener" style="color:#6a8aff;font-size:12px">🔗 Fix : ${o.fix_url}</a></div>`),r+="</div>",e.innerHTML=r,A.info("cf-diag-manual",`Diagnostic result : token=${o.token_valid} kv=${o.kv_permission} workers=${o.workers_permission}`)}catch(a){const o=a instanceof Error?a.message:String(a);e.textContent="";const t=document.createElement("div");t.style.color="#ff5b5b",t.textContent=`❌ Erreur : ${o.slice(0,100)}`,e.append(t)}}})()}),i.querySelectorAll("[data-nav-route]").forEach(e=>{n.bind(e,"click",()=>{const a=e.getAttribute("data-nav-route");a&&(location.hash="#"+a)})});const $=i.querySelector("#ax-settings-logout");$&&n&&n.bind($,"click",()=>{(async()=>{const{auth:e}=await u(async()=>{const{auth:a}=await import("./auth-JGQ_KFuU.js");return{auth:a}},__vite__mapDeps([4,1,0,2,3,5]),import.meta.url);e.logout(),location.hash="#login"})()});const l=i.querySelector("#ax-settings-tools-auto-embed");if(l&&n){try{const e=JSON.parse(localStorage.getItem("ax_settings")??"{}");l.checked=e.tools_auto_embed!==!1}catch{l.checked=!0}n.bind(l,"change",()=>{try{const e=JSON.parse(localStorage.getItem("ax_settings")??"{}");e.tools_auto_embed=l.checked,localStorage.setItem("ax_settings",JSON.stringify(e))}catch{}})}const d=i.querySelector("#ax-force-update-btn"),g=i.querySelector("#ax-force-update-status");d&&n&&n.bind(d,"click",()=>{(async()=>{const e=a=>{g&&(g.textContent=a)};d.disabled=!0,d.textContent="⏳ Reset en cours…";try{if(e("🔍 Désinstallation Service Workers…"),"serviceWorker"in navigator){const o=await navigator.serviceWorker.getRegistrations();for(const t of o)await t.unregister();e(`✅ ${o.length} SW désinstallés`)}if(e("🔍 Vidage caches PWA…"),"caches"in window){const o=await caches.keys();for(const t of o)await caches.delete(t);e(`✅ ${o.length} caches vidés`)}e("✅ Reset terminé. Rechargement dans 2s…");const{toast:a}=await u(async()=>{const{toast:o}=await import("./toast-ClsF1KRZ.js");return{toast:o}},[],import.meta.url);a.info("🔄 Reset OK — reload imminent"),setTimeout(()=>{location.href=location.pathname+"?_forceupd=1&_reset="+Date.now()},2e3)}catch(a){e(`❌ Erreur : ${String(a)}`),d.disabled=!1,d.textContent="🔄 Force reset PWA + reload"}})()}),V(i);const y=i.querySelector("#ax-settings-notif-test");y&&n&&n.bind(y,"click",()=>{(async()=>{try{if("Notification"in window&&Notification.permission==="granted")new Notification("Test Apex",{body:"Si tu vois ça, push notif fonctionne ✅"});else if("Notification"in window)if(await Notification.requestPermission()==="granted")new Notification("Test Apex",{body:"Push activé ✅"});else{const{toast:a}=await u(async()=>{const{toast:o}=await import("./toast-ClsF1KRZ.js");return{toast:o}},[],import.meta.url);a.warn("Permission notifications refusée")}else{const{toast:e}=await u(async()=>{const{toast:a}=await import("./toast-ClsF1KRZ.js");return{toast:a}},[],import.meta.url);e.warn("Notifications non supportées par ce navigateur")}}catch{const{toast:e}=await u(async()=>{const{toast:a}=await import("./toast-ClsF1KRZ.js");return{toast:a}},[],import.meta.url);e.warn("Test notification échoué")}})()}),A.info("feature-settings","rendered")}export{j as dispose,F as render,V as wireVoiceSection};
