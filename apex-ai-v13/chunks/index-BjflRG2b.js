const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./smart-router-CBXWppR9.js","./toast-Dgg9rcIP.js","./haptic-BUEqXK0N.js"])))=>i.map(i=>d[i]);
import{_ as l}from"./apex-kb-CLN2FaYC.js";import{l as R}from"./monitoring-WiO5ZBU9.js";import{c as q}from"./listener-cleanup-Y2rGGxxX.js";import"./apex-tools-registry-_6Box58P.js";import"./credential-patterns-DqicUg9o.js";let o=null;function M(){o?.cleanup(),o=null}const p={anthropic:"🧠",openai:"🤖",groq:"⚡",gemini:"✨",mistral:"🌊",cohere:"🔷",xai:"🔥",perplexity:"🔍",deepseek:"🐋",openrouter:"🌐"};function s(t){return t.replace(/[&<>"']/g,a=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[a]??a)}function f(t){return t<0?"—":t<1e3?`${Math.round(t)}ms`:`${(t/1e3).toFixed(1)}s`}function $(t){return t<0?"—":`${Math.round(t)}%`}function z(t){if(t===0)return"jamais";const a=Date.now()-t;return a<6e4?"à l'instant":a<36e5?`il y a ${Math.round(a/6e4)} min`:a<864e5?`il y a ${Math.round(a/36e5)} h`:`il y a ${Math.round(a/864e5)} j`}function w(t){return!t||t.last_ping_ts===0?'<span style="color:#666">⚪</span>':t.last_ping_ok?t.latency_avg_ms>3e3?'<span style="color:#f0c020">🟡</span>':'<span style="color:#22cc77">🟢</span>':'<span style="color:#ff5b5b">🔴</span>'}function v(t){return t>=80?"#22cc77":t>=60?"#c9a227":t>=40?"#f0c020":"#ff5b5b"}async function c(t){o?.cleanup(),o=q("smart-router");const{smartRouter:a}=await l(async()=>{const{smartRouter:e}=await import("./smart-router-CBXWppR9.js");return{smartRouter:e}},__vite__mapDeps([0]),import.meta.url),S=await a.rankProviders(),m=await a.getRecommendations(),u=a.getOverride(),i=S[0],g=a.getAllProviders(),n=i?await a.getStats(i.provider):null,y=await Promise.all(g.map(async e=>({provider:e,stats:await a.getStats(e),score:await a.scoreProvider(e)})));y.sort((e,r)=>r.score.total-e.score.total),t.innerHTML=`
  ...
  `;const d=t.querySelector("#ax-sr-retest");d&&o&&o.bind(d,"click",()=>{(async()=>{const{toast:e}=await l(async()=>{const{toast:r}=await import("./toast-Dgg9rcIP.js");return{toast:r}},__vite__mapDeps([1,2]),import.meta.url);d.disabled=!0,d.textContent="⏳ Test en cours…",e.info("Ping de 10 providers en parallèle…");try{await a.pingAllProviders(),e.success("✅ Tous les providers re-testés"),await c(t)}catch(r){e.error("Erreur ping: "+(r instanceof Error?r.message:String(r))),d.disabled=!1,d.textContent="🔄 Re-tester tous maintenant"}})()});const b=t.querySelector("#ax-sr-refresh");b&&o&&o.bind(b,"click",()=>{c(t)});const h=t.querySelector("#ax-sr-reset");h&&o&&o.bind(h,"click",()=>{(async()=>{const{toast:e}=await l(async()=>{const{toast:r}=await import("./toast-Dgg9rcIP.js");return{toast:r}},__vite__mapDeps([1,2]),import.meta.url);typeof confirm=="function"&&!confirm("Reset toutes les stats Smart Router ? (samples + scores + override)")||(a.resetAll(),e.success("Stats reset"),await c(t))})()});const _=t.querySelector("#ax-sr-clear-override");_&&o&&o.bind(_,"click",()=>{(async()=>{const{toast:e}=await l(async()=>{const{toast:r}=await import("./toast-Dgg9rcIP.js");return{toast:r}},__vite__mapDeps([1,2]),import.meta.url);a.setOverride(null),e.success("Override retiré — auto-routing actif"),await c(t)})()}),t.querySelectorAll(".ax-sr-force").forEach(e=>{o.bind(e,"click",()=>{(async()=>{const r=e.dataset.provider;if(!r)return;const{toast:x}=await l(async()=>{const{toast:k}=await import("./toast-Dgg9rcIP.js");return{toast:k}},__vite__mapDeps([1,2]),import.meta.url);a.setOverride(r),x.success(`📌 ${r} forcé pour tous les calls`),await c(t)})()})}),R.info("feature-smart-router",`rendered ${g.length} providers, best=${i?.provider??"none"}`)}export{M as dispose,c as render};
