import{o as n}from"./monitoring-BEF8D60L.js";import"./multi-source-analyze-GHL4EzEF.js";import"./apex-kb-CYEMVHK9.js";import"./credential-patterns-DNv8q-dc.js";const d={ai:"🧠 Intelligence Artificielle",saas:"💼 SaaS",comms:"✉️ Communications",infra:"☁️ Infrastructure",finance:"💳 Finance & Paiements",other:"🔧 Autres"};function u(a){if(a.length<2)return'<span class="ax-spark-empty">—</span>';const s=Math.max(...a.map(i=>i.eur),1),e=120,r=32,t=e/(a.length-1),o=a.map((i,l)=>`${l===0?"M":"L"}${(l*t).toFixed(1)} ${(r-i.eur/s*r).toFixed(1)}`).join(" ");return`
    <svg class="ax-sparkline" viewBox="0 0 ${e} ${r}" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <path d="${o}" fill="none" stroke="#c9a227" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
      <path d="${o} L${e} ${r} L0 ${r} Z" fill="rgba(201,162,39,0.15)"/>
    </svg>
  `}function f(a){const s=Math.max(...a,.01);return`<div class="ax-heatmap-24h" aria-label="Heatmap 24h">${a.map((r,t)=>`<div class="ax-heat-cell" style="background:rgba(201,162,39,${(.1+r/s*.9).toFixed(2)})" title="${t}h : ${r.toFixed(2)}€" aria-label="${t}h"></div>`).join("")}</div>`}function x(a){const s=a.trend_7d==="up"?"↗":a.trend_7d==="down"?"↘":"→",e=a.trend_7d==="up"?"trend-up":a.trend_7d==="down"?"trend-down":"trend-stable",r=a.status==="critical"?"🔴":a.status==="warn"?"🟡":"🟢",t=Math.min(100,a.pct_budget),o=a.status==="critical"?"#ff4444":a.status==="warn"?"#ffaa00":"#22cc77",i=a.is_free_tier?'<span class="ax-badge-free">FREE</span>':"";return`
    <article class="ax-fin-service-card" data-service="${a.service}" data-status="${a.status}">
      <header class="ax-fin-card-head">
        ...
        <span class="ax-fin-status">${r}</span>
      </header>
      ...
      <div class="ax-fin-bar-container">
        <div class="ax-fin-bar-bg">
          <div class="ax-fin-bar-fill" style="width:${t}%;background:${o}"></div>
        </div>
        <span class="ax-fin-bar-pct">${a.pct_budget}%</span>
      </div>
      <div class="ax-fin-budget-label">Budget : ${n.formatEur(a.budget_eur_month)}/mois</div>
    </article>
  `}function v(a,s){if(s.length===0)return"";const e=s.reduce((r,t)=>r+t.used_eur_month,0);return`
    <section class="ax-fin-category">
      ...
      <div class="ax-fin-cards-grid">${s.map(x).join("")}</div>
    </section>
  `}function m(a){const s=a.monthly_profit_eur>0?"profit-positive":"profit-negative";return`
  ...
  `}function k(a){const s=n.getSummary(),e=n.getSparklineMonth(),r=n.getHourlyHeatmap(),t=new Map;for(const c of s.services){const p=t.get(c.category)??[];p.push(c),t.set(c.category,p)}const i=["ai","comms","infra","finance","saas","other"].map(c=>v(c,t.get(c)??[])).join(""),l=s.projection_end_month_eur-s.total_month_eur;a.innerHTML=`
