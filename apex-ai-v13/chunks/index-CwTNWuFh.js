const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./monitoring-CslxfvgC.js","./multi-source-analyze-B40uKK57.js","./credential-patterns-DUMYZEMu.js","./apex-kb-CbrdV0-V.js"])))=>i.map(i=>d[i]);
import{b as H,f as _,l as h,e as s,_ as k}from"./monitoring-CslxfvgC.js";import{c as M}from"./listener-cleanup-Y2rGGxxX.js";import{apexExecute as f}from"./apex-execute-Bct6wMAp.js";import{a as y}from"./apex-kb-CbrdV0-V.js";import{kdmcProjectsRegistry as A}from"./kdmc-projects-registry-BTybQCln.js";import{auth as q}from"./auth-HrVkX6wx.js";import{i as T,r as R}from"./apex-tools-dispatch-core-32566Kmd.js";import{c as D}from"./csp-style-helper-BEHhIhzj.js";import{w as j}from"./whatsapp-BH5Uq4TK.js";import{haptic as d}from"./haptic-CQFg2PXZ.js";import{s as z}from"../core/main-CMqgR0va.js";import{toast as p}from"./toast-BCPNzfMv.js";import"./multi-source-analyze-B40uKK57.js";import"./credential-patterns-DUMYZEMu.js";import"./apex-tools-dispatch-skills-BB49lDk2.js";import"./apex-tools-dispatch-data-DXlfNHgx.js";import"./apex-tools-dispatch-finance-D84Ce07W.js";import"./apex-tools-dispatch-misc-HvkBYyv5.js";import"./apex-tools-misc-B_5p9dSL.js";import"./apex-tools-registry-core-BMhHY4vU.js";import"./apex-tools-registry-skills-x-mAWYry.js";import"./memory-C3YZzQiz.js";let v="commerce";const I={commerce:"admin.commerce",users:"admin.users",pending:"admin.users",health:null,projects:null,executions:"admin.executions",knowledge:"admin.kb",bilan:"admin.bilan",consumption:"admin.consumption","audit-log":"admin.audit-log"};function E(){const t=_.isEnabled();return`
  ...
  `}function B(){const t=j.listPending();return t.length?`
    <div class="ax-admin-section">
      ...
      <li class="ax-pending-row">
        ...
        <span class="ax-muted">${s(c.whatsapp)}</span>
  ...
  `}function N(){const t=A.list(),o=A.count(),c=A.countActive(),r=["active","wip","archived"],l=t.map(n=>{const e=r.includes(n.status)?n.status:"archived",a=n.tech_stack.slice(0,4).map(i=>s(i)).join(", ");return`
        <li class="ax-project-row" data-project-id="${s(n.id)}">
          <div class="ax-project-head">
            <strong>${s(n.name)}</strong>
            <span class="ax-badge ax-badge-${e}">${s(e)}</span>
            <code class="ax-project-version">${s(n.version)}</code>
          </div>
          <p class="ax-muted">${s(n.description)}</p>
          <p class="ax-project-stack"><em>stack:</em> ${a||"—"}</p>
          <p class="ax-project-links">
            <a href="${s(n.deploy_url)}" target="_blank" rel="noopener">🚀 Live</a>
            ·
            <a href="${s(n.repo_url)}" target="_blank" rel="noopener">📦 Repo</a>
            · <span class="ax-muted">🛡 ${n.sentinels_count} sentinelles</span>
          </p>
          <div class="ax-project-edit">
            <input type="text" data-project-version="${s(n.id)}"
                   aria-label="Version du projet ${s(n.id)}"
                   value="${s(n.version)}" placeholder="vX.Y" maxlength="20"
                   class="ax-input-sm" autocomplete="off">
            <select data-project-status="${s(n.id)}" class="ax-select-sm">
            ...
            <button class="ax-btn ax-btn-sm" data-project-save="${s(n.id)}">💾</button>
          </div>
        </li>
      `}).join("");return`
    <div class="ax-admin-section">
  ...
  `}function U(){return[["commerce","💳 Commerce"],["users","👥 Comptes"],["pending","📨 Attente"],["health","🩺 Santé"],["projects","📦 Projets"],["executions","⚙️ Exec"],["knowledge","📚 KB"],["bilan","📊 Bilan"],["consumption","💰 Conso"],["audit-log","🔒 Audit"]].filter(([c])=>{const r=I[c];return!r||T(r)}).map(([c,r])=>{const l=v===c;return`
      ...
      `}).join("")}function K(){const t=f.listPendingExecutions({limit:30}),o=f.getStats(),c=f.listAllowedTasks(),r=f.listForbiddenTasks(),l={pending:"pending",dispatched:"wip",running:"wip",completed:"ok",failed:"error",cancelled:"archived",timeout:"error"},n=t.map(e=>{const a=l[e.status]??"archived",i=new Date(e.ts_created).toLocaleString(),u=e.duration_ms?`${Math.round(e.duration_ms/1e3)}s`:"—",x=e.workflow_run_url?`<a href="${s(e.workflow_run_url)}" target="_blank" rel="noopener">🔗 Workflow</a>`:'<span class="ax-muted">—</span>',b=e.status==="pending"||e.status==="dispatched";return`
        <li class="ax-execution-row" data-exec-id="${s(e.id)}">
          <div class="ax-exec-head">
            <code class="ax-exec-id">${s(e.id.slice(0,18))}</code>
            <span class="ax-badge ax-badge-${a}">${s(e.status)}</span>
            <strong>${s(e.task)}</strong>
          </div>
          <p class="ax-muted">📅 ${s(i)} · ⏱ ${s(u)} · 🚀 ${s(e.src)} · 👤 ${s(e.initiated_by)}</p>
          ${e.error?`<p class="ax-error">⚠ ${s(e.error.slice(0,200))}</p>`:""}
          <p class="ax-exec-actions">
            ${x}
            ${b?` · <button class="ax-btn ax-btn-sm" data-exec-cancel="${s(e.id)}">✕ Annuler</button>`:""}
    ...
    <div class="ax-admin-section">
      ...
      <div class="ax-info-card">
        ...
        <p>${c.map(e=>`<code>${s(e)}</code>`).join(" · ")}</p>
  ...
  `}function O(){const t=I[v];if(t&&!T(t))return`<div class="ax-admin-section">${R(t)}</div>`;switch(v){case"commerce":return E();case"users":return V();case"pending":return B();case"health":return F();case"projects":return N();case"executions":return K();case"knowledge":return G();case"bilan":return'<div id="ax-admin-mount-bilan" class="ax-admin-section"><p class="ax-muted">Chargement du bilan financier…</p></div>';case"consumption":return'<div id="ax-admin-mount-consumption" class="ax-admin-section"><p class="ax-muted">Chargement consommation IA…</p></div>';case"audit-log":return'<div id="ax-admin-mount-audit-log" class="ax-admin-section"><p class="ax-muted">Chargement audit log immuable…</p></div>'}}async function P(t){const o=t.querySelector("#ax-admin-audits-summary");if(o)try{const{reportsHistory:n}=await k(async()=>{const{reportsHistory:b}=await import("./apex-reports-history-CPVBZzO3.js");return{reportsHistory:b}},__vite__mapDeps([0,1,2,3]),import.meta.url),e=n.getStats(),a=e.lastLayoutTs?new Date(e.lastLayoutTs).toLocaleString("fr-FR",{day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit"}):"jamais",i=e.lastFunctionalTs?new Date(e.lastFunctionalTs).toLocaleString("fr-FR",{day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit"}):"jamais",u=e.recentBugs>0?"var(--ax-error)":"var(--ax-green)",x=e.recentEscalations>0?"var(--ax-sev-high)":"var(--ax-green)";o.innerHTML=`
        <div class="ax-bounce-tap" data-nav-route="apex-audits-live" style="background:linear-gradient(135deg,rgba(106,138,255,0.10),rgba(180,90,200,0.06));border:1px solid rgba(106,138,255,0.3);border-radius:12px;padding:14px;cursor:pointer">
          ...
          <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:8px;font-size:12px;color:rgba(255,255,255,0.85)">
            ...
            <div>🧪 Fonctionnel : <b>${e.functionalCount}</b> tests<br><span class="ax-gs-23">${i}</span></div>
            <div>🐛 Bugs 24h : <b style="color:${u}">${e.recentBugs}</b></div>
            <div>📤 Escaladés 24h : <b style="color:${x}">${e.recentEscalations}</b></div>
          </div>
        </div>
      `}catch(n){h.warn("admin","audits summary render failed",{err:n})}const c=t.querySelector("#ax-admin-mount-bilan");if(c)try{(await k(()=>import("./financial-bilan-RWGnLEYF.js"),__vite__mapDeps([0,1,2,3]),import.meta.url)).render(c)}catch(n){h.warn("admin","financial-bilan render failed",{err:n}),c.innerHTML='<p class="ax-muted">Bilan indisponible (module ko)</p>'}const r=t.querySelector("#ax-admin-mount-consumption");if(r)try{(await k(()=>import("./consumption-dashboard-BYDp_Li7.js"),__vite__mapDeps([0,1,2,3]),import.meta.url)).render(r)}catch(n){h.warn("admin","consumption-dashboard render failed",{err:n}),r.innerHTML='<p class="ax-muted">Consommation indisponible (module ko)</p>'}const l=t.querySelector("#ax-admin-mount-audit-log");if(l){if(!T("admin.audit-log")){l.innerHTML=R("admin.audit-log");return}try{const{auditLog:n}=await k(async()=>{const{auditLog:i}=await import("./monitoring-CslxfvgC.js").then(u=>u.x);return{auditLog:i}},__vite__mapDeps([0,1,2,3]),import.meta.url);n.init();const e=n.getEntries().slice(-100).reverse(),a=e.map(i=>`<li><code>${s(new Date(i.ts).toLocaleString())}</code> · <strong>${s(i.action)}</strong> · ${s(i.actor||"system")}</li>`).join("");l.innerHTML=`
      ...
      `}catch(n){h.warn("admin","audit-log render failed",{err:n}),l.innerHTML='<p class="ax-muted">Audit log indisponible (module ko)</p>'}}}function G(){const t=y.listRepos(),o=y.getStats(),c=t.map(r=>`
  ...
  `}function W(t){t.querySelectorAll("[data-nav-route]").forEach(e=>{m.bind(e,"click",()=>{d.tap();const a=e.dataset.navRoute??"chat";window.location.hash="#"+a})}),t.querySelectorAll('[data-action="select-all"]').forEach(e=>{m.bind(e,"click",()=>{e.select()})}),t.querySelectorAll("[data-tab]").forEach(e=>{m.bind(e,"click",()=>{d.selection(),v=e.dataset.tab,g(t)})});const o=t.querySelector("#commerce-toggle");o&&m.bind(o,"change",()=>{const e=o.checked;d.medium();const a=t.querySelector(".ax-toggle-label");if(a){a.textContent="",a.append("Commercialisation ");const i=document.createElement("strong");i.textContent=e?"ACTIVÉE":"désactivée",a.append(i)}_.setEnabled(e),p.success(`Commercialisation ${e?"activée":"désactivée"}`),g(t)});const c=t.querySelector("#create-user-form");c&&m.bind(c,"submit",e=>{e.preventDefault(),Y(t)}),t.querySelectorAll("[data-user-plan]").forEach(e=>{m.bind(e,"change",()=>{const a=e.dataset.userPlan??"";a&&(_.setUserPlan(a,e.value),h.info("admin",`Plan ${e.value} → ${a}`))})}),t.querySelectorAll("[data-project-save]").forEach(e=>{m.bind(e,"click",()=>{d.tap();const a=e.dataset.projectSave??"";if(!a)return;const i=t.querySelector(`[data-project-version="${CSS.escape(a)}"]`),u=t.querySelector(`[data-project-status="${CSS.escape(a)}"]`),x=(i?.value??"").trim(),b=u?.value??"active";if(!x){p.warn("Version requise");return}A.update(a,{version:x,status:b})?(d.success(),p.success(`${a} → ${x} (${b})`),h.info("admin",`Project ${a} updated`,{version:x,status:b}),g(t)):(d.error(),p.error("Update échoué"))})}),t.querySelectorAll("[data-confirm-otp]").forEach(e=>{m.bind(e,"click",()=>{d.tap();const a=e.dataset.confirmOtp??"";if(!a)return;const i=j.confirm(a);i.ok?(d.success(),p.success("Compte activé"),h.info("admin",`Confirmed user ${i.uid}`),g(t)):(d.error(),p.error("Code OTP invalide ou expiré"))})}),t.querySelectorAll("[data-exec-cancel]").forEach(e=>{m.bind(e,"click",()=>{d.tap();const a=e.dataset.execCancel??"";if(!a)return;f.cancelExecution(a)?(d.success(),p.success("Exécution annulée"),g(t)):(d.warning(),p.warn("Annulation impossible"))})}),t.querySelectorAll("[data-exec-poll]").forEach(e=>{m.bind(e,"click",()=>{d.tap();const a=e.dataset.execPoll??"";a&&f.pollResult(a).then(i=>{i?(p.success(`Statut : ${i.status}`),g(t)):p.warn("Exécution introuvable")})})});const r=t.querySelector("#add-repo-form");r&&m.bind(r,"submit",e=>{e.preventDefault(),d.tap();const i=t.querySelector("#kb-add-repo")?.value.trim()??"";if(!i){p.warn("Indique un repo");return}const u=y.addRepo(i);u.ok?(d.success(),p.success(`Repo ajouté : ${i}`),g(t)):(d.error(),p.error(u.reason??"Erreur ajout repo"))}),t.querySelectorAll("[data-remove-repo]").forEach(e=>{m.bind(e,"click",()=>{d.tap();const a=e.dataset.removeRepo??"";a&&(y.removeRepo(a),p.success(`Repo retiré : ${a}`),g(t))})});const l=t.querySelector("#kb-search-form");l&&m.bind(l,"submit",e=>{e.preventDefault(),d.tap();const a=t.querySelector("#kb-search-query"),i=t.querySelector("#kb-search-results"),u=a?.value.trim()??"";!u||!i||(i.innerHTML='<p class="ax-muted">Recherche en cours...</p>',y.searchCode(u).then(x=>{if(x.length===0){i.innerHTML='<p class="ax-muted">Aucun résultat (configure ax_github_token pour augmenter la limite).</p>';return}i.textContent="";const b=document.createElement("ul");b.className="ax-kb-results-list",x.slice(0,20).forEach($=>{const S=document.createElement("li");S.className="ax-kb-result";const w=document.createElement("a");w.href=$.htmlUrl,w.target="_blank",w.rel="noopener";const L=document.createElement("code");L.textContent=$.path,w.append(L);const C=document.createElement("span");C.className="ax-muted",C.textContent=`${$.repo} · score ${$.score.toFixed(2)}`,S.append(w," ",C),b.append(S)}),i.append(b)}))});const n=t.querySelector("#kb-clear-cache");n&&m.bind(n,"click",()=>{d.tap();const e=y.clearCache();p.success(`Cache vidé : ${e.cleared} entrées`),g(t)})}async function Y(t){const o=t.querySelector("#cu-name")?.value.trim()??"",c=t.querySelector("#cu-tier")?.value??"family",r=t.querySelector("#cu-email")?.value.trim()??"",l=t.querySelector("#cu-whatsapp")?.value.trim()??"",n=t.querySelector("#cu-pin")?.value??"";if(!o||o.length<2){d.warning(),p.warn("Nom complet requis (min 2 caractères)");return}const e=await q.createUser({name:o,tier:c,...r&&{email:r},...l&&{whatsappPhone:l},...n&&{initialPin:n}}),a=t.querySelector("#create-user-result");if(!a)return;if(!e.ok||!e.uid){d.error();const u=e.reason??"Erreur création";a.innerHTML=`<div class="ax-error">${s(u)}</div>`,p.error(u);return}d.success(),p.success(`Compte ${o} créé`);let i="";if(l){const u=await j.requestConfirmation({uid:e.uid,name:o,whatsappPhone:l});u.ok&&u.inviteLink&&(i=`
    ...
    <div class="ax-success">
      Compte créé : <strong>${s(o)}</strong> (${c})
  ...
  `,g(t)}let m=null;function ve(){m?.cleanup(),m=null}function g(t){if(m?.cleanup(),m=M("admin"),!H.get("isAdmin")){t.innerHTML=`
      <div class="ax-empty">
        <h2>Accès réservé</h2>
  ...
  `),W(t),v==="bilan"||v==="consumption"){const c=v==="bilan"?"ax-admin-mount-bilan":"ax-admin-mount-consumption",r=t.querySelector(`#${c}`);if(r){const l=r.querySelector("p.ax-muted"),n=l?z(l,"admin-table"):()=>{};P(t).finally(()=>{n()})}else P(t)}}export{ve as dispose,g as render};
