import{e as s}from"./escape-html-BlQj2yEF.js";import{c as k}from"./listener-cleanup-Y2rGGxxX.js";import{s as w,l as f}from"./monitoring-D2lWYrYo.js";import{apexAutonomousMode as p}from"./apex-autonomous-mode-YTXBqgvP.js";import{autonomousWatch as h}from"./autonomous-watch-W-Y9T9o6.js";import{haptic as z}from"./haptic-CQFg2PXZ.js";import{toast as n}from"./toast-CRdbcLoc.js";import"./multi-source-analyze-Bg1HHfSC.js";import"./apex-kb-D1VtWFD9.js";import"./credential-patterns-CLzI061R.js";import"./ai-router-Dx5c2JPW.js";import"../core/main-B4DExc5S.js";import"./chat-fallback-Dl4OzVEq.js";import"./apex-tools-dispatch-core-C_k5h2yM.js";import"./apex-tools-dispatch-skills-DOw4cI4G.js";import"./apex-tools-dispatch-data-DHUpGBCD.js";import"./apex-tools-dispatch-finance-D84Ce07W.js";import"./apex-tools-dispatch-misc-fNevKu6E.js";import"./apex-tools-misc-DBbScgMK.js";import"./apex-tools-registry-core-BMhHY4vU.js";import"./apex-tools-registry-skills-DRX4PPQ_.js";import"./economy-mode-BeAoy3PT.js";import"./firebase-queue-CmN5eOOe.js";let l=null,a=null;function Y(){a&&(clearInterval(a),a=null),l?.cleanup(),l=null}function y(t){const e={running:{color:"#0a0",bg:"rgba(60,200,80,0.18)",emoji:"🟢"},paused:{color:"#cc9000",bg:"rgba(255,200,60,0.15)",emoji:"⏸"},stopped:{color:"#aaa",bg:"rgba(255,255,255,0.08)",emoji:"🛑"},quota_exhausted:{color:"#c50",bg:"rgba(220,90,30,0.15)",emoji:"🪫"},completed:{color:"#08c",bg:"rgba(70,140,210,0.15)",emoji:"🏁"},failed:{color:"#c33",bg:"rgba(220,60,60,0.15)",emoji:"❌"}}[t];return`<span class="ax-badge" style="display:inline-flex;align-items:center;gap:4px;padding:3px 10px;border-radius:14px;background:${e.bg};color:${e.color};font-weight:600;font-size:12px">${e.emoji} ${t}</span>`}function A(t){if(!t)return`
    ...
    `;const o=t.tasksCompleted.filter(r=>r.status==="done").length,e=t.tasksCompleted.filter(r=>r.status==="failed").length,i=Math.round((Date.now()-t.startedAt)/6e4),c=t.maxIterations??50,x=t.quotaLimit??5e4,g=Math.min(100,Math.round(t.tokensConsumed/x*100)),v=Math.min(100,Math.round(t.iterations/c*100));return`
    <div class="ax-card" data-session-id="${s(t.id)}" style="padding:18px;border-radius:16px;background:linear-gradient(135deg,rgba(232,184,48,0.08),rgba(60,200,80,0.04));border:1px solid rgba(232,184,48,0.18)">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:10px;margin-bottom:14px">
        <div class="ax-gs-38">
          ...
          <p style="margin:4px 0 0;font-size:18px;font-weight:700;color:#e8b830">${i} min</p>
        </div>
        <div class="ax-gs-38">
          <p style="margin:0;font-size:11px;text-transform:uppercase;color:rgba(255,255,255,0.5);letter-spacing:0.5px">Itérations</p>
          <p style="margin:4px 0 0;font-size:18px;font-weight:700">${t.iterations}/${c}</p>
          <div class="ax-gs-90"><div style="width:${v}%;background:#e8b830;height:100%"></div></div>
        </div>
        <div class="ax-gs-38">
  ...
  `}function j(){const t=p.getHistory(10);if(t.length===0)return"";const o=t.map(e=>{const i=Math.round((Date.now()-e.startedAt)/36e5),c=e.endedAt?Math.round((e.endedAt-e.startedAt)/6e4):null,x=e.tasksCompleted.filter(g=>g.status==="done").length;return`<tr>
        ...
        <td style="padding:8px 6px;font-size:11px;color:rgba(255,255,255,0.6)">il y a ${i}h</td>
        ...
        <td style="padding:8px 6px;font-size:11px;text-align:right">${c??"–"} min</td>
      </tr>`}).join("");return`
  ...
  `}function m(){const t=p.getActiveSession();return`
  ...
  `}function b(t){t.querySelectorAll("[data-action]").forEach(o=>{l.bind(o,"click",async()=>{z.tap();const e=o.dataset.action??"";try{if(e==="back")window.location.hash="#admin";else if(e==="refresh")d(t),n.info("Rafraîchi");else if(e==="stop"){if(!confirm("Arrêter le mode autonome ?"))return;p.stop(void 0,"admin-ui-stop"),n.success("🛑 Session arrêtée"),d(t)}else e==="pause"?(p.pause(),n.info("⏸ Session pausée"),d(t)):e==="resume"?(p.resume(),n.success("▶️ Session reprise"),d(t)):e==="force-tick"&&(n.info("⚡ Tick forcé…"),await h.forceTick(),d(t))}catch(i){f.warn("admin-autonomous",`action ${e} failed`,{err:i}),n.error(`Erreur : ${i instanceof Error?i.message:String(i)}`)}})})}function d(t){if(l?.cleanup(),l=k("admin-autonomous"),!w.get("isAdmin")){t.innerHTML=`
    ...
    `;return}t.innerHTML=m(),b(t),a&&(clearInterval(a),a=null);const e=p.getActiveSession();e&&(e.status==="running"||e.status==="paused")&&(a=setInterval(()=>{try{t.innerHTML=m(),b(t)}catch(i){f.warn("admin-autonomous","auto-refresh failed",{err:i})}},5e3))}export{Y as dispose,d as render};
