const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./monitoring-BAiQJoxJ.js","./apex-kb-BrxoW6ip.js","./credential-patterns-Dy6Wjk7e.js","./multi-source-analyze-BefgB4O2.js"])))=>i.map(i=>d[i]);
import{_ as g}from"./apex-kb-BrxoW6ip.js";import{l as u}from"./monitoring-BAiQJoxJ.js";import{c as b}from"./listener-cleanup-Y2rGGxxX.js";import{s as m}from"../core/main-C9h0PGqO.js";import{h}from"./haptic-BUEqXK0N.js";import{g as y}from"./apex-tools-dispatch-Dmryj_FW.js";import"./credential-patterns-Dy6Wjk7e.js";import"./multi-source-analyze-BefgB4O2.js";import"./apex-tools-registry-B37X3vXu.js";import"./voice-D_1ntYe5.js";let d=null;function H(){d?.cleanup(),d=null}function i(e){return e.replace(/[&<>"']/g,r=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[r]??r)}const v=[{id:"chat",icon:"💬",label:"Chat",description:"Conversation IA",route:"chat",color:"#5aa8ff"},{id:"vault",icon:"🔐",label:"Coffre",description:"Clés API & secrets",route:"vault",color:"#c9a227"},{id:"browser",icon:"🌐",label:"Browser",description:"Naviguer & embed",route:"browser",color:"#22cc77"},{id:"studios",icon:"🎨",label:"Studios",description:"Créatif (musique/vidéo)",route:"studios",color:"#a878ff"},{id:"pro",icon:"🎓",label:"Pro",description:"Modules expert",route:"pro",color:"#ff6b9d"},{id:"self-diag",icon:"🩺",label:"Audit",description:"Auto-diagnostic",route:"self-diag",color:"#38d8c8"},{id:"settings",icon:"⚙️",label:"Réglages",description:"Configurer Apex",route:"settings",color:"#a0a4c0"},{id:"rgpd",icon:"🛡",label:"RGPD",description:"Mes données",route:"rgpd",color:"#e8b830"}];async function w(){let e=0,r=0,o=0,a=0,t=0,c=0;try{const l=localStorage.getItem("apex_v13_messages_24h")??"0",n=parseInt(l,10);Number.isNaN(n)||(e=n)}catch{}try{const l=localStorage.getItem("apex_v13_tokens_24h")??"0",n=parseInt(l,10);Number.isNaN(n)||(r=n)}catch{}try{const{kdmcProjectsRegistry:l}=await g(async()=>{const{kdmcProjectsRegistry:n}=await import("./kdmc-projects-registry-sRmoyobO.js");return{kdmcProjectsRegistry:n}},__vite__mapDeps([0]),import.meta.url);o=l.countActive()}catch{}try{const{sentinels:l}=await g(async()=>{const{sentinels:s}=await import("./sentinels-BmTesbRF.js");return{sentinels:s}},__vite__mapDeps([1,0,2]),import.meta.url),n=l.list();t=n.length,a=n.filter(s=>s.lastResult?.ok).length}catch{}try{const l=localStorage.getItem("ax_claude_todo")??"[]",n=JSON.parse(l);Array.isArray(n)&&(c=n.filter(s=>s.status==="pending").length)}catch{}return[{id:"messages",icon:"💬",label:"Messages 24h",value:e,color:"#5aa8ff",route:"chat"},{id:"tokens",icon:"🔢",label:"Tokens 24h",value:r.toLocaleString("fr-FR"),color:"#c9a227",route:"self-diag"},{id:"projects",icon:"📦",label:"Projets actifs",value:o,color:"#22cc77",route:"admin"},{id:"sentinels",icon:"🛡",label:"Sentinelles OK",value:`${a}/${t}`,color:a===t?"#22cc77":"#ffaa00",route:"sentinels"},{id:"todos",icon:"📋",label:"Todos en attente",value:c,color:c>0?"#ff5858":"#22cc77",route:"self-diag"}]}async function k(){const e=[];try{const{sentinels:r}=await g(async()=>{const{sentinels:t}=await import("./sentinels-BmTesbRF.js");return{sentinels:t}},__vite__mapDeps([1,0,2]),import.meta.url),a=r.list().filter(t=>t.lastResult&&!t.lastResult.ok);for(const t of a.slice(0,3))e.push({id:`sentinel_${t.id}`,level:"warn",title:`Sentinelle ${t.name}`,description:t.lastResult?.msg??"KO",ts:t.lastResult?.ts??Date.now(),action_route:"sentinels"})}catch{}try{const r=JSON.parse(localStorage.getItem("apex_v13_credentials_expiring")??"[]");if(Array.isArray(r))for(const o of r.slice(0,3))o.days_left<30&&e.push({id:`cred_${o.service}`,level:o.days_left<7?"error":"warn",title:`Credential ${o.service}`,description:`Expire dans ${o.days_left} jours`,ts:Date.now(),action_route:"vault"})}catch{}return e}async function $(){try{const{multiKeyVault:e}=await g(async()=>{const{multiKeyVault:a}=await import("./multi-key-vault-DyQXGzqs.js");return{multiKeyVault:a}},__vite__mapDeps([1,0,2]),import.meta.url),r=e.getKnownServices(),o=[];for(const a of r){const t=e.getStats(a),c=e.getServiceLight(a);o.push({service:a,light:c,totalKeys:t.total,activeKeys:t.active,failingKeys:t.failing,invalidKeys:t.invalid,lastSuccess:t.lastSuccess})}return o}catch{return[]}}function z(){const e=[];try{const r=localStorage.getItem("ax_claude_todo")??"[]",o=JSON.parse(r);if(Array.isArray(o))for(const a of o.filter(t=>t.status==="pending").slice(0,5))e.push({id:a.id??`todo_${Date.now()}`,source:"apex_todo",title:a.reason??"Todo sans description",severity:a.severity??"medium",ts_created:a.ts??Date.now()})}catch{}return e}function _(e,r=0){return`
    ...
    </button>`}function S(e,r=0){return`
    <button class="ax-shortcut-card ax-modernized-card ax-bounce-tap" data-route="${i(e.route)}"
    ...
    </button>`}function A(e){return e.length===0?`
      <div style="display:flex;align-items:center;gap:12px;padding:16px;background:linear-gradient(135deg,rgba(34,204,119,0.08),rgba(34,204,119,0.03));border:1px solid rgba(34,204,119,0.15);border-radius:12px">
        ...
        </div>`}).join("")}function R(e){return e.length===0?`
      <div style="display:flex;align-items:center;gap:12px;padding:16px;background:linear-gradient(135deg,rgba(34,204,119,0.08),rgba(34,204,119,0.03));border:1px solid rgba(34,204,119,0.15);border-radius:12px">
        <span style="font-size:20px;filter:drop-shadow(0 2px 6px rgba(34,204,119,0.4))">🎉</span>
        ...
        </div>`}).join("")}function K(e,r={}){if(e.length===0)return`
...
>📊 Usage</a>`:"";return`
        <div class="ax-service-health-row ax-modernized-card"
          ...
          ${f}
          ${x}
        </div>`}).join("")}async function P(e){const r={};if(e.length===0)return r;try{const{linksRegistry:o}=await g(async()=>{const{linksRegistry:a}=await import("./multi-source-analyze-BefgB4O2.js").then(t=>t.c);return{linksRegistry:a}},__vite__mapDeps([3,1,0,2]),import.meta.url);for(const a of e)r[a]={recharge:o.getRechargeLink(a),usage:o.getUsageLink(a),apiKeys:o.getApiKeysLink(a)}}catch{}return r}async function M(e){d?.cleanup(),d=b("dashboard");const r=m.get("user");if(!y("admin.dashboard",e,r?.id))return;const o=r?.name?`Bonjour ${r.name}`:"Bonjour",[a,t,c]=await Promise.all([w(),k(),$()]),l=await P(c.map(s=>s.service)),n=z();e.innerHTML=`
    <style>
  ...
  `,e.querySelectorAll("[data-route]").forEach(s=>{d.bind(s,"click",()=>{h.tap();const p=s.dataset.route;p&&(window.location.hash="#"+p)})}),u.info("feature-dashboard",`rendered (${a.length} kpis, ${t.length} alerts, ${n.length} todos)`)}export{w as computeKpis,H as dispose,i as escapeHtml,k as loadAlerts,P as loadRechargeLinks,$ as loadServiceHealth,z as loadTodos,M as render,K as renderServiceHealthCard};
