...
`),a={};for(const i of o){const s=i.indexOf("=");s>0&&(a[i.slice(0,s)]=i.slice(s+1))}const n=(a.loc??"").toUpperCase();if(n){const i={country:n,countryCode:n};return a.ip&&(i.ip=a.ip),i}}}catch(e){p.debug("geolocation","Cloudflare trace failed",{err:e})}try{const e=await fetch("https://ipapi.co/json/",{headers:{Accept:"application/json"}});if(e.ok){const t=await e.json(),o={country:t.country_name??"",countryCode:(t.country_code??"").toUpperCase()};return t.city&&(o.city=t.city),t.region&&(o.region=t.region),t.ip&&(o.ip=t.ip),o}}catch(e){p.debug("geolocation","ipapi failed",{err:e})}return{country:"Monaco",countryCode:"MC"}}saveFavoriteLocation(e){const t={id:`fav_${Date.now().toString(36)}_${Math.random().toString(36).slice(2,7)}`,name:e.name,lat:e.lat,lng:e.lng,type:e.type??"other",createdAt:Date.now()},o=this.getFavoriteLocations();o.push(t);try{localStorage.setItem(x,JSON.stringify(o))}catch(a){p.warn("geolocation","saveFavoriteLocation storage failed",{err:a})}return t}getFavoriteLocations(){try{const e=localStorage.getItem(x);if(!e)return[];const t=JSON.parse(e);return Array.isArray(t)?t:[]}catch{return[]}}removeFavoriteLocation(e){const t=this.getFavoriteLocations(),o=t.filter(a=>a.id!==e);if(o.length===t.length)return!1;try{return localStorage.setItem(x,JSON.stringify(o)),!0}catch{return!1}}watchGeofence(e){const t={id:`gf_${Date.now().toString(36)}_${Math.random().toString(36).slice(2,7)}`,name:e.name,lat:e.lat,lng:e.lng,radius:e.radius};e.onEnter&&(t.onEnter=e.onEnter),e.onExit&&(t.onExit=e.onExit),this.fenceCallbacks.set(t.id,t);const o=this.getStoredGeofences();o.push({id:t.id,name:t.name,lat:t.lat,lng:t.lng,radius:t.radius});try{localStorage.setItem(y,JSON.stringify(o))}catch(a){p.warn("geolocation","watchGeofence storage failed",{err:a})}return t.id}removeGeofence(e){const t=this.getStoredGeofences().filter(o=>o.id!==e);this.fenceCallbacks.delete(e);try{return localStorage.setItem(y,JSON.stringify(t)),!0}catch{return!1}}getGeofences(){return this.getStoredGeofences()}checkGeofences(e){const t=this.getStoredGeofences();if(t.length)for(const o of t){const n=this.distanceBetween({lat:e.latitude,lng:e.longitude},{lat:o.lat,lng:o.lng})*1e3<=o.radius,i=`apex_v13_geo_inside_${o.id}`;let s=!1;try{s=localStorage.getItem(i)==="true"}catch{}if(n!==s){try{localStorage.setItem(i,String(n))}catch{}const c=this.fenceCallbacks.get(o.id);if(c)try{n&&c.onEnter?c.onEnter():!n&&c.onExit&&c.onExit()}catch(d){p.warn("geolocation","geofence callback threw",{id:o.id,err:d})}}}}getLastKnownPosition(){try{const e=localStorage.getItem(w);return e?JSON.parse(e):null}catch{return null}}getHistory(){try{const e=localStorage.getItem(v);if(!e)return[];const t=JSON.parse(e);return Array.isArray(t)?t:[]}catch{return[]}}clearHistory(){try{localStorage.removeItem(v)}catch{}}fenceCallbacks=new Map;lastHistoryAppend=0;saveLastPosition(e){try{localStorage.setItem(w,JSON.stringify(e))}catch(t){p.debug("geolocation","saveLastPosition skipped (quota?)",{err:t})}}appendHistory(e){try{const t=this.getHistory();t.push(e);const o=t.length>_?t.slice(-_):t;localStorage.setItem(v,JSON.stringify(o))}catch(t){p.debug("geolocation","appendHistory skipped",{err:t})}}appendHistoryThrottled(e){const t=Date.now();t-this.lastHistoryAppend<6e4||(this.lastHistoryAppend=t,this.appendHistory(e))}getStoredGeofences(){try{const e=localStorage.getItem(y);if(!e)return[];const t=JSON.parse(e);return Array.isArray(t)?t:[]}catch{return[]}}}const g=new k;let f=-1;function L(r){return r<=10?{label:"Excellente",color:"#4ade80"}:r<=50?{label:"Bonne",color:"#60a5fa"}:r<=500?{label:"Moyenne",color:"#facc15"}:{label:"Approximative",color:"#f87171"}}function O(r){const e=L(r.accuracy),t=r.latitude.toFixed(6),o=r.longitude.toFixed(6);let a="";return r.altitude!==null&&r.altitude!==void 0&&(a+=`<br><span class="ax-gs-10">Altitude:</span> ${Math.round(r.altitude)}m`),r.speed!==null&&r.speed!==void 0&&r.speed>0&&(a+=`<br><span class="ax-gs-10">Vitesse:</span> ${Math.round(r.speed*3.6)} km/h`),`
  ...
  `}function G(r){return r.length?r.map(e=>`
        <div class="ax-gs-135">
          <div>
            ...
            <span class="ax-gs-136"> · ${u(e.type??"other")}</span>
      ...
      `).join(""):'<div class="ax-gs-5">Aucun lieu favori. Cliquez "Ajouter ma position" pour en créer un.</div>'}function H(r){const e=r.forecast7d.slice(0,7).map(t=>{const a=new Date(t.date).toLocaleDateString("fr-FR",{weekday:"short",day:"2-digit",month:"2-digit"});return`
        <div style="display:flex;justify-content:space-between;padding:4px 0;font-size:12px;border-bottom:1px solid var(--ax-border)">
          <span>${u(a)}</span>
          <span class="ax-gs-10">${u(t.condition)}</span>
    ...
    ${e}
  `}function m(r){const e=g.getLastKnownPosition(),t=g.getFavoriteLocations(),o=g.getGeofences(),a=g.getHistory(),n=f!==-1,i=e?O(e):'<div class="ax-gs-5">Aucune position enregistrée</div>',s=e?`
        <div class="ax-gs-129">
  ...
  `,T(r)}function T(r){const e=(t,o)=>{if(t==="refresh-position")g.getCurrentPosition().then(()=>m(r)).catch(a=>{p.warn("geo-view","getCurrentPosition failed",{err:a}),alert("Impossible d'obtenir votre position. Vérifiez les autorisations GPS.")});else if(t==="share-position"){const a=g.getLastKnownPosition();if(!a)return;const n=`https://maps.google.com/?q=${a.latitude},${a.longitude}`,i=navigator;i.share?i.share({title:"Ma position",url:n}).catch(()=>{}):navigator.clipboard&&navigator.clipboard.writeText(n).catch(()=>{})}else if(t==="toggle-tracking")f!==-1?(g.clearWatch(f),f=-1):f=g.watchPosition(()=>{m(r)}),m(r);else if(t==="add-fav"){const a=g.getLastKnownPosition();if(!a){alert("Obtenez d'abord votre position.");return}const n=prompt("Nom du lieu favori (ex: Maison, Bureau) :");if(!n)return;g.saveFavoriteLocation({name:n,lat:a.latitude,lng:a.longitude,type:"other"}),m(r)}else if(t==="remove-fav"){const a=o.dataset.favId;a&&g.removeFavoriteLocation(a)&&m(r)}else if(t==="add-fence"){const a=g.getLastKnownPosition();if(!a){alert("Obtenez d'abord votre position.");return}const n=prompt("Nom de la zone (ex: Casino, Domicile) :");if(!n)return;const i=prompt("Rayon en mètres (défaut 100) :","100"),s=Math.max(10,parseInt(i??"100",10)||100);g.watchGeofence({name:n,lat:a.latitude,lng:a.longitude,radius:s}),m(r)}else if(t==="remove-fence"){const a=o.dataset.fenceId;a&&g.removeGeofence(a)&&m(r)}else if(t==="load-weather"){const a=r.querySelector("#ax-geo-weather");if(!a)return;a.innerHTML='<div class="ax-gs-5">Chargement…</div>';const n=g.getLastKnownPosition();(n?g.getLocalWeather(n.latitude,n.longitude):g.getLocalWeather()).then(s=>{a.innerHTML=H(s)}).catch(s=>{p.warn("geo-view","weather load failed",{err:s}),a.innerHTML='<div style="font-size:12px;color:#f87171">Impossible de charger la météo.</div>'})}};r.querySelectorAll("[data-action]").forEach(t=>{t.addEventListener("click",()=>{const o=t.dataset.action;o&&e(o,t)})})}export{u as escapeHtml,m as render};
