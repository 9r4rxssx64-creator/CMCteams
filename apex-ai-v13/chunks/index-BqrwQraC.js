import{e as p}from"./escape-html-BlQj2yEF.js";import{c as f}from"./listener-cleanup-Y2rGGxxX.js";import{l as x}from"./monitoring-3uBGKGRH.js";import{s as b}from"../core/main-DfBAVUzk.js";import{g as h}from"./apex-tools-dispatch-core-CDO8VC2i.js";import{haptic as u}from"./haptic-CQFg2PXZ.js";import"./apex-kb-DNf_z_6Q.js";import"./credential-patterns-CLzI061R.js";import"./multi-source-analyze-BhbXkTSb.js";import"./apex-tools-dispatch-skills-DXNQRXOS.js";import"./apex-tools-dispatch-data-CXmoy5wJ.js";import"./apex-tools-dispatch-finance-D84Ce07W.js";import"./apex-tools-dispatch-misc-DyMSQAy8.js";import"./apex-tools-misc-DvXoGjTJ.js";import"./apex-tools-registry-core-BMhHY4vU.js";import"./apex-tools-registry-skills-DRX4PPQ_.js";import"./voice-Dkn5kX58.js";let r=null;function O(){r?.cleanup(),r=null}const l=[{id:"chien",name:"Chien",emoji:"🐕",weight_range_kg:{min:1,max:80},kcal_per_kg_normal:70,vaccinations:["CHPPiL (carré, hépatite, parvo, parainfluenza, leptospirose)","Rage","Toux du chenil","Maladie de Lyme (zone à risque)"],red_flags:["Vomissements répétés > 24h","Diarrhée sanglante","Léthargie soudaine","Refus de manger > 24h","Difficulté à respirer","Convulsions"],age_coefficient:5,base_year_offset:16},{id:"chat",name:"Chat",emoji:"🐈",weight_range_kg:{min:2,max:12},kcal_per_kg_normal:60,vaccinations:["Typhus, coryza, leucose","Rage","Chlamydiose"],red_flags:["Léthargie","Refus de boire/manger > 24h","Vomissements répétés","Mictions difficiles ou absentes","Boiterie soudaine","Respiration rapide au repos"],age_coefficient:4,base_year_offset:15},{id:"lapin",name:"Lapin",emoji:"🐇",weight_range_kg:{min:1,max:6},kcal_per_kg_normal:55,vaccinations:["Myxomatose","VHD (maladie hémorragique virale)"],red_flags:["Arrêt total alimentation > 12h (URGENT)","Pas de selles > 12h","Tête inclinée","Boiterie","Difficulté respiratoire"],age_coefficient:7,base_year_offset:0},{id:"hamster",name:"Hamster",emoji:"🐹",weight_range_kg:{min:.05,max:.2},kcal_per_kg_normal:100,vaccinations:[],red_flags:["Léthargie inhabituelle","Diarrhée (mortelle rapide)","Boiterie","Croûtes / perte poils","Difficulté respiratoire"],age_coefficient:25,base_year_offset:0},{id:"oiseau",name:"Oiseau (perroquet, canari)",emoji:"🦜",weight_range_kg:{min:.01,max:1.5},kcal_per_kg_normal:120,vaccinations:["Polyomavirus (perroquets élevage)","Maladie de Pacheco"],red_flags:["Plumes ébouriffées au repos","Fientes anormales","Respiration bouche ouverte","Repli au fond cage","Refus chant"],age_coefficient:6,base_year_offset:0},{id:"poisson",name:"Poisson (aquarium)",emoji:"🐟",weight_range_kg:{min:.001,max:.5},kcal_per_kg_normal:30,vaccinations:[],red_flags:["Nage de travers","Plaies / champignons visibles","Ouïes pâles ou rouges","Refus alimentation","Reste isolé en surface"],age_coefficient:3,base_year_offset:0}],v={sedentaire:.85,normal:1,actif:1.2,"tres-actif":1.5};function y(e,a,t){const n=l.find(s=>s.id===e);if(!n||!isFinite(a)||a<=0||a<n.weight_range_kg.min||a>n.weight_range_kg.max)return null;const i=70*Math.pow(a,.75),o=v[t];return Math.round(i*o)}function _(e,a){const t=l.find(n=>n.id===e);return!t||!isFinite(a)||a<0?null:Math.round(t.base_year_offset+a*t.age_coefficient)}function V(e){r?.cleanup(),r=f("studios-pet");const a=b.get("user")?.id??"anon";if(!h("studio.pet",e,a))return;const t=l.map(i=>`<option value="${i.id}">${i.emoji} ${p(i.name)}</option>`).join(""),n=l.map(i=>`
    <div style="background:rgba(255,255,255,0.03);border:1px solid #333;border-radius:8px;padding:10px;margin-bottom:8px">
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
        ...
        <strong style="color:#c9a227">${p(i.name)}</strong>
      </div>
      ${i.vaccinations.length>0?`<div style="font-size:12px;color:#ddd;margin-bottom:4px"><strong>💉 Vaccins :</strong> ${i.vaccinations.map(o=>p(o)).join(", ")}</div>`:""}
      <div style="font-size:11px;color:#ff8866"><strong>⚠ Urgence véto si :</strong> ${i.red_flags.slice(0,3).map(o=>p(o)).join(" · ")}</div>
    </div>
  `).join("");e.innerHTML=`
  ...
  `,k(e)}function k(e){const a=e.querySelector("#ax-pet-calc"),t=e.querySelector("#ax-pet-out");a&&t&&r&&r.bind(a,"click",()=>{u.tap();const o=e.querySelector("#ax-pet-species")?.value??"",s=parseFloat(e.querySelector("#ax-pet-weight")?.value??""),c=e.querySelector("#ax-pet-activity")?.value??"normal",d=y(o,s,c);if(d===null){t.textContent="Poids hors plage pour cette espèce.";return}const m=l.find(g=>g.id===o);t.innerHTML=`Ration journalière estimée : <strong>${d} kcal</strong> ${m?`(${m.emoji} ${p(m.name)} · ${s} kg · ${p(c)})`:""}<br><span style="font-size:12px;color:var(--ax-text-dim)">Diviser en 2-3 repas. Adapter selon avis véto.</span>`});const n=e.querySelector("#ax-pet-age-btn"),i=e.querySelector("#ax-pet-age-out");n&&i&&r&&r.bind(n,"click",()=>{u.tap();const o=e.querySelector("#ax-pet-species")?.value??"",s=parseFloat(e.querySelector("#ax-pet-age")?.value??""),c=_(o,s);if(c===null){i.textContent="Âge invalide.";return}i.textContent="",i.append("Équivalent humain : ");const d=document.createElement("strong");d.textContent=`≈ ${c} ans`,i.append(d)}),x.info("studios-pet","rendered")}export{l as PET_SPECIES,_ as animalToHumanAge,y as calcDailyCalories,O as dispose,V as render};
