...
>📊 Usage</a>`:"";return`
        <div class="ax-service-health-row ax-modernized-card"
          ...
          ${f}
          ${x}
        </div>`}).join("")}async function I(e){const r={};if(e.length===0)return r;try{const{linksRegistry:o}=await g(async()=>{const{linksRegistry:a}=await import("./multi-source-analyze-BZQ4jND3.js").then(t=>t.c);return{linksRegistry:a}},__vite__mapDeps([3,1,0,2]),import.meta.url);for(const a of e)r[a]={recharge:o.getRechargeLink(a),usage:o.getUsageLink(a),apiKeys:o.getApiKeysLink(a)}}catch{}return r}async function F(e){d?.cleanup(),d=b("dashboard");const r=m.get("user");if(!v("admin.dashboard",e,r?.id))return;const o=r?.name?`Bonjour ${r.name}`:"Bonjour",[a,t,c]=await Promise.all([k(),$(),z()]),l=await I(c.map(s=>s.service)),n=_();e.innerHTML=y.withNonce(`
    <style>
