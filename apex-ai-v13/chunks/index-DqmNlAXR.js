const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./monitoring-3uBGKGRH.js","./apex-kb-Ss-LQHUo.js","./credential-patterns-CLzI061R.js","./multi-source-analyze-Cb_4xPK7.js"])))=>i.map(i=>d[i]);
import{_ as g}from"./apex-kb-Ss-LQHUo.js";import{c as u}from"./listener-cleanup-Y2rGGxxX.js";import{l as b}from"./monitoring-3uBGKGRH.js";import{s as m}from"../core/main-Chf9Kx4D.js";import{c as h}from"./csp-style-helper-BisGRi53.js";import{g as y}from"./apex-tools-dispatch-core-CscT1PrL.js";import{haptic as v}from"./haptic-CQFg2PXZ.js";import"./credential-patterns-CLzI061R.js";import"./multi-source-analyze-Cb_4xPK7.js";import"./apex-tools-dispatch-skills-0fh-89Jk.js";import"./apex-tools-dispatch-data-C9w8sOql.js";import"./apex-tools-dispatch-finance-D84Ce07W.js";import"./apex-tools-dispatch-misc-BT0loxBS.js";import"./apex-tools-registry-CTvFo_GP.js";import"./voice-DMlxiGcD.js";let d=null;function Y(){d?.cleanup(),d=null}function i(e){return e.replace(/[&<>"']/g,r=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[r]??r)}const w=[{id:"chat",icon:"💬",label:"Chat",description:"Conversation IA",route:"chat",color:"#5aa8ff"},{id:"vault",icon:"🔐",label:"Coffre",description:"Clés API & secrets",route:"vault",color:"#c9a227"},{id:"browser",icon:"🌐",label:"Browser",description:"Naviguer & embed",route:"browser",color:"#22cc77"},{id:"studios",icon:"🎨",label:"Studios",description:"Créatif (musique/vidéo)",route:"studios",color:"#a878ff"},{id:"pro",icon:"🎓",label:"Pro",description:"Modules expert",route:"pro",color:"#ff6b9d"},{id:"self-diag",icon:"🩺",label:"Audit",description:"Auto-diagnostic",route:"self-diag",color:"#38d8c8"},{id:"settings",icon:"⚙️",label:"Réglages",description:"Configurer Apex",route:"settings",color:"#a0a4c0"},{id:"rgpd",icon:"🛡",label:"RGPD",description:"Mes données",route:"rgpd",color:"#e8b830"}];async function k(){let e=0,r=0,o=0,a=0,t=0,c=0;try{const l=localStorage.getItem("apex_v13_messages_24h")??"0",n=parseInt(l,10);Number.isNaN(n)||(e=n)}catch{}try{const l=localStorage.getItem("apex_v13_tokens_24h")??"0",n=parseInt(l,10);Number.isNaN(n)||(r=n)}catch{}try{const{kdmcProjectsRegistry:l}=await g(async()=>{const{kdmcProjectsRegistry:n}=await import("./kdmc-projects-registry-DdTqW2gy.js");return{kdmcProjectsRegistry:n}},__vite__mapDeps([0]),import.meta.url);o=l.countActive()}catch{}try{const{sentinels:l}=await g(async()=>{const{sentinels:s}=await import("./sentinels-Cx1WWffn.js");return{sentinels:s}},__vite__mapDeps([1,0,2]),import.meta.url),n=l.list();t=n.length,a=n.filter(s=>s.lastResult?.ok).length}catch{}try{const l=localStorage.getItem("ax_claude_todo")??"[]",n=JSON.parse(l);Array.isArray(n)&&(c=n.filter(s=>s.status==="pending").length)}catch{}return[{id:"messages",icon:"💬",label:"Messages 24h",value:e,color:"#5aa8ff",route:"chat"},{id:"tokens",icon:"🔢",label:"Tokens 24h",value:r.toLocaleString("fr-FR"),color:"#c9a227",route:"self-diag"},{id:"projects",icon:"📦",label:"Projets actifs",value:o,color:"#22cc77",route:"admin"},{id:"sentinels",icon:"🛡",label:"Sentinelles OK",value:`${a}/${t}`,color:a===t?"#22cc77":"#ffaa00",route:"sentinels"},{id:"todos",icon:"📋",label:"Todos en attente",value:c,color:c>0?"#ff5858":"#22cc77",route:"self-diag"}]}async function $(){const e=[];try{const{sentinels:r}=await g(async()=>{const{sentinels:t}=await import("./sentinels-Cx1WWffn.js");return{sentinels:t}},__vite__mapDeps([1,0,2]),import.meta.url),a=r.list().filter(t=>t.lastResult&&!t.lastResult.ok);for(const t of a.slice(0,3))e.push({id:`sentinel_${t.id}`,level:"warn",title:`Sentinelle ${t.name}`,description:t.lastResult?.msg??"KO",ts:t.lastResult?.ts??Date.now(),action_route:"sentinels"})}catch{}try{const r=JSON.parse(localStorage.getItem("apex_v13_credentials_expiring")??"[]");if(Array.isArray(r))for(const o of r.slice(0,3))o.days_left<30&&e.push({id:`cred_${o.service}`,level:o.days_left<7?"error":"warn",title:`Credential ${o.service}`,description:`Expire dans ${o.days_left} jours`,ts:Date.now(),action_route:"vault"})}catch{}return e}async function z(){try{const{multiKeyVault:e}=await g(async()=>{const{multiKeyVault:a}=await import("./multi-key-vault-DeKrwMzr.js");return{multiKeyVault:a}},__vite__mapDeps([1,0,2]),import.meta.url),r=e.getKnownServices(),o=[];for(const a of r){const t=e.getStats(a),c=e.getServiceLight(a);o.push({service:a,light:c,totalKeys:t.total,activeKeys:t.active,failingKeys:t.failing,invalidKeys:t.invalid,lastSuccess:t.lastSuccess})}return o}catch{return[]}}function _(){const e=[];try{const r=localStorage.getItem("ax_claude_todo")??"[]",o=JSON.parse(r);if(Array.isArray(o))for(const a of o.filter(t=>t.status==="pending").slice(0,5))e.push({id:a.id??`todo_${Date.now()}`,source:"apex_todo",title:a.reason??"Todo sans description",severity:a.severity??"medium",ts_created:a.ts??Date.now()})}catch{}return e}function S(e,r=0){return`
...
>📊 Usage</a>`:"";return`
        <div class="ax-service-health-row ax-modernized-card"
          ...
          ${f}
          ${x}
        </div>`}).join("")}async function I(e){const r={};if(e.length===0)return r;try{const{linksRegistry:o}=await g(async()=>{const{linksRegistry:a}=await import("./multi-source-analyze-Cb_4xPK7.js").then(t=>t.c);return{linksRegistry:a}},__vite__mapDeps([3,1,0,2]),import.meta.url);for(const a of e)r[a]={recharge:o.getRechargeLink(a),usage:o.getUsageLink(a),apiKeys:o.getApiKeysLink(a)}}catch{}return r}async function G(e){d?.cleanup(),d=u("dashboard");const r=m.get("user");if(!y("admin.dashboard",e,r?.id))return;const o=r?.name?`Bonjour ${r.name}`:"Bonjour",[a,t,c]=await Promise.all([k(),$(),z()]),l=await I(c.map(s=>s.service)),n=_();e.innerHTML=h.withNonce(`
    <style>
