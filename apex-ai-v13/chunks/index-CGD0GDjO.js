...
>📊 Usage</a>`:"";return`
        <div class="ax-service-health-row ax-modernized-card"
          ...
          ${x}
          ${f}
        </div>`}).join("")}async function K(e){const r={};if(e.length===0)return r;try{const{linksRegistry:o}=await g(async()=>{const{linksRegistry:a}=await import("./links-registry-DZQcmt41.js");return{linksRegistry:a}},[],import.meta.url);for(const a of e)r[a]={recharge:o.getRechargeLink(a),usage:o.getUsageLink(a),apiKeys:o.getApiKeysLink(a)}}catch{}return r}async function V(e){d?.cleanup(),d=b("dashboard");const r=m.get("user"),o=r?.name?`Bonjour ${r.name}`:"Bonjour",[a,t,c]=await Promise.all([v(),w(),k()]),l=await K(c.map(s=>s.service)),n=$();e.innerHTML=`
    <style>
