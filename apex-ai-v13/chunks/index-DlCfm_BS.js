...
import{_ as u}from"./apex-kb-D-Wwv6M3.js";import{c as M}from"./listener-cleanup-Y2rGGxxX.js";import{l as D}from"./monitoring-3uBGKGRH.js";import"./credential-patterns-Dy6Wjk7e.js";let i=null;function z(){i?.cleanup(),i=null}async function g(a){i?.cleanup(),i=M("sentinels");const{sentinels:d}=await u(async()=>{const{sentinels:t}=await import("./sentinels-N9YZEHFn.js");return{sentinels:t}},__vite__mapDeps([0,1,2]),import.meta.url),{sentinelsRegistry:m,bootstrapSentinelsRegistry:h}=await u(async()=>{const{sentinelsRegistry:t,bootstrapSentinelsRegistry:n}=await import("./sentinels-registry-NyZ8GWxK.js");return{sentinelsRegistry:t,bootstrapSentinelsRegistry:n}},__vite__mapDeps([0,1,2]),import.meta.url);h();const s=d.list(),$=m.getStatus(),x=m.getMetrics(),y=s.filter(t=>t.lastResult?.ok).length,f=s.filter(t=>t.lastResult&&!t.lastResult.ok).length,R=s.filter(t=>!t.lastResult).length;a.innerHTML=`
