...
import{_ as l}from"./apex-kb-DndcZFMK.js";import{l as R}from"./monitoring-3uBGKGRH.js";import{c as q}from"./listener-cleanup-Y2rGGxxX.js";import"./credential-patterns-qcw7Brjr.js";let o=null;function T(){o?.cleanup(),o=null}const p={anthropic:"🧠",openai:"🤖",groq:"⚡",gemini:"✨",mistral:"🌊",cohere:"🔷",xai:"🔥",perplexity:"🔍",deepseek:"🐋",openrouter:"🌐"};function s(t){return t.replace(/[&<>"']/g,a=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[a]??a)}function f(t){return t<0?"—":t<1e3?`${Math.round(t)}ms`:`${(t/1e3).toFixed(1)}s`}function $(t){return t<0?"—":`${Math.round(t)}%`}function z(t){if(t===0)return"jamais";const a=Date.now()-t;return a<6e4?"à l'instant":a<36e5?`il y a ${Math.round(a/6e4)} min`:a<864e5?`il y a ${Math.round(a/36e5)} h`:`il y a ${Math.round(a/864e5)} j`}function w(t){return!t||t.last_ping_ts===0?'<span style="color:#666">⚪</span>':t.last_ping_ok?t.latency_avg_ms>3e3?'<span style="color:#f0c020">🟡</span>':'<span style="color:#22cc77">🟢</span>':'<span style="color:#ff5b5b">🔴</span>'}function v(t){return t>=80?"#22cc77":t>=60?"#c9a227":t>=40?"#f0c020":"#ff5b5b"}async function c(t){o?.cleanup(),o=q("smart-router");const{smartRouter:a}=await l(async()=>{const{smartRouter:e}=await import("./smart-router-DDnWSyOX.js");return{smartRouter:e}},__vite__mapDeps([0,1,2]),import.meta.url),S=await a.rankProviders(),m=await a.getRecommendations(),u=a.getOverride(),i=S[0],g=a.getAllProviders(),n=i?await a.getStats(i.provider):null,y=await Promise.all(g.map(async e=>({provider:e,stats:await a.getStats(e),score:await a.scoreProvider(e)})));y.sort((e,r)=>r.score.total-e.score.total),t.innerHTML=`
