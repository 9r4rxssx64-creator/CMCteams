import{e as g}from"./escape-html-BlQj2yEF.js";import{c as E}from"./listener-cleanup-Y2rGGxxX.js";import{l,a as k,s as q}from"./monitoring-D2lWYrYo.js";import{r as H}from"../core/main-B4DExc5S.js";import{aiRouter as $}from"./ai-router-Dx5c2JPW.js";import{_ as j}from"./apex-kb-D1VtWFD9.js";import{haptic as C}from"./haptic-CQFg2PXZ.js";import{modalSheet as x}from"./modal-sheet-oR7SW-wv.js";import{toast as y}from"./toast-CRdbcLoc.js";import"./multi-source-analyze-Bg1HHfSC.js";import"./credential-patterns-CLzI061R.js";import"./chat-fallback-Dl4OzVEq.js";import"./apex-tools-dispatch-core-C_k5h2yM.js";import"./apex-tools-dispatch-skills-DOw4cI4G.js";import"./apex-tools-dispatch-data-DHUpGBCD.js";import"./apex-tools-dispatch-finance-D84Ce07W.js";import"./apex-tools-dispatch-misc-fNevKu6E.js";import"./apex-tools-misc-DBbScgMK.js";import"./apex-tools-registry-core-BMhHY4vU.js";import"./apex-tools-registry-skills-DRX4PPQ_.js";import"./economy-mode-BeAoy3PT.js";const T="apex_v13_agent_browser_history",N=20;class J{async analyze(t,e){const r=(t||"").trim(),a=(e||"").trim();if(!r)throw new Error("URL vide");if(!a)throw new Error("Objectif vide");if(!/^https?:\/\//i.test(r))throw new Error("URL doit commencer par http(s)://");const c=Date.now(),s=`agent_${Date.now()}_${Math.random().toString(36).slice(2,6)}`;let n="",m=!1;try{const o=await fetch(r,{method:"GET",mode:"cors",credentials:"omit"});o.ok&&(n=await o.text(),m=!0)}catch(o){l.info("agent-browser","fetch CORS blocked, fallback skeleton",{err:o})}const h=this.extractRelevantDom(n).slice(0,3e4),d=`Tu es un agent pilote browser. L'utilisateur te donne une URL et un objectif.
...
- Pas de \\n littéraux dans JSON (échappe correctement)`;let s="",n;try{await $.stream([{role:"user",content:`Compose une animation pour : ${e}`}],c,d=>{d.text&&(s+=d.text)},d=>{n=d})}catch(d){n=d instanceof Error?d:new Error(String(d))}if(n||!s)return l.warn("hyperframes","IA failed, fallback",{err:n?.message}),this.fallbackComposition(a,e,r);let m;try{const d=s.match(/\{[\s\S]*"frames"[\s\S]*"html"[\s\S]*\}/);if(!d)throw new Error("JSON manquant");const u=JSON.parse(d[0]),p=typeof u.frames=="number"?Math.max(3,Math.min(30,u.frames)):5,f=typeof u.duration=="number"?u.duration:p*(1e3/v),o=typeof u.html=="string"?u.html:"";if(!o)throw new Error("html vide");m={frames:p,duration:f,html:o}}catch(d){return l.warn("hyperframes","parse failed, fallback",{err:d}),this.fallbackComposition(a,e,r)}const h={id:a,prompt:e,html:this.sanitizeHtml(m.html),frames:m.frames,duration:m.duration,fps:v,generatedAt:Date.now(),durationMs:Date.now()-r};return this.persist(h),k.record("hyperframes.compose",{details:{id:a,frames:h.frames,durationMs:h.durationMs}}),l.info("hyperframes",`Composed ${h.frames} frames (${h.durationMs}ms)`),h}buildPreviewSrcdoc(t){return`<!DOCTYPE html>
<html lang="fr">
<head>
<meta charset="utf-8">
...
</html>`}history(){try{const t=localStorage.getItem(O);if(!t)return[];const e=JSON.parse(t);return Array.isArray(e)?e:[]}catch{return[]}}sanitizeHtml(t){return t.replace(/<iframe[\s\S]*?<\/iframe>/gi,"").slice(0,5e4)}fallbackComposition(t,e,r){const c=`<div data-composition-id="${t}" style="position:relative;width:300px;height:200px;background:linear-gradient(135deg,#1a1a2e,#0e0e1c);border-radius:12px;overflow:hidden">
...
<script>
(function(){
  var id="${t}";var frames=6;var fps=${v};
  window.__timelines = window.__timelines || {};
...
Trigger Cialdini : ${a}`}],d,o=>{o.text&&(u+=o.text)},o=>{l.warn("marketing-psy","stream err",{err:o})})}catch(o){l.warn("marketing-psy","stream throw",{err:o})}let p;try{const o=u.match(/\{[\s\S]*"copy"[\s\S]*\}/);if(!o)throw new Error("JSON manquant");const b=JSON.parse(o[0]);if(p={copy:typeof b.copy=="string"?b.copy.slice(0,1e3):"",rationale:typeof b.rationale=="string"?b.rationale.slice(0,500):""},!p.copy)throw new Error("copy vide")}catch(o){l.warn("marketing-psy","parse failed, fallback",{err:o}),p=this.fallbackCopy(e,a)}const f={id:m,copy:p.copy,trigger:a,rationale:p.rationale||h,product:e,audience:r,format:c,tone:s,generatedAt:Date.now(),durationMs:Date.now()-n};return this.persist(f),k.record("marketing-psy.generate",{details:{id:m,trigger:a,product:e.slice(0,50),durationMs:f.durationMs}}),l.info("marketing-psy",`Generated copy (${a}, ${f.durationMs}ms)`),f}history(){try{const t=localStorage.getItem(R);if(!t)return[];const e=JSON.parse(t);return Array.isArray(e)?e:[]}catch{return[]}}listTriggers(){return Object.keys(S).map(t=>({id:t,description:S[t]}))}fallbackCopy(t,e){return{copy:{reciprocity:`Essaie ${t} gratuitement — sans engagement.`,scarcity:`${t} : seulement 100 places cette semaine.`,authority:`${t}, recommandé par les experts du domaine.`,consistency:`Tu cherches X depuis longtemps ? ${t} est l'étape logique.`,liking:`On a créé ${t} pour les gens comme toi.`,"social-proof":`Rejoins 10 000+ utilisateurs satisfaits de ${t}.`,unity:`Entre nous, ${t} change vraiment la donne.`}[e],rationale:S[e]}}persist(t){try{const e=this.history();e.unshift(t);const r=e.slice(0,Q);localStorage.setItem(R,JSON.stringify(r))}catch(e){l.warn("marketing-psy","persist failed",{err:e})}}}const z=new Z;let w=null;function _e(){w?.cleanup(),w=null}const ee=[{id:"hyperframes",emoji:"🎞",name:"HyperFrames",description:"Compose une animation HTML/CSS/JS multi-frames depuis un prompt. Preview dans iframe sandbox.",buttonLabel:"Composer"},{id:"agent-browser",emoji:"🌐",name:"Agent Browser",description:"Analyse une URL + objectif, retourne actions structurées (click/fill/extract).",buttonLabel:"Analyser URL"},{id:"marketing-psy",emoji:"🧠",name:"Marketing Psy",description:"Génère copies marketing avec triggers Cialdini (scarcity/social-proof/authority/...).",buttonLabel:"Générer copy"},{id:"impeccable-design",emoji:"✨",name:"Impeccable Design",description:"23 commandes design fluency (make-it-pop / tighten-spacing / improve-typography / ...).",buttonLabel:"Polir un design"},{id:"ios-simulator",emoji:"📱",name:"iOS Simulator",description:"Preview HTML/URL dans frame iPhone 15 Pro simulé (visuel only, pas runtime iOS).",buttonLabel:"Lancer preview"}];function te(i){return`
  ...
  `}async function ae(){const i=window.prompt("Décris l'animation à composer :","Logo APEX qui pulse en doré sur fond noir");if(i){y.info("🎞 Composition en cours...");try{const t=await M.compose(i),r=M.buildPreviewSrcdoc(t).replace(/"/g,"&quot;").replace(/</g,"&lt;");x.open({title:`🎞 HyperFrames — ${t.frames} frames`,content:`
        <div class="ax-gs-12">
      ...
      </li>`).join("");x.open({title:`🌐 Agent Browser — ${e.actions.length} actions`,content:`
        <div class="ax-gs-12">
          <p style="color:rgba(255,255,255,0.7);font-size:13px;margin-bottom:10px">
      ...
      `,actions:[{label:"Fermer",variant:"ghost",onClick:()=>x.closeAll()}]})}catch(e){l.warn("shubham-skills","agent-browser failed",{err:e}),y.error("Analyse échouée")}}}async function oe(){const i=window.prompt("Produit :","Apex AI");if(!i)return;const t=window.prompt("Audience :","Développeurs freelance");if(!t)return;const e=z.listTriggers(),r=e.map((n,m)=>`${m+1}. ${n.id}`).join(`
`),a=window.prompt(`Trigger Cialdini ? (1-${e.length})

${r}`,"6"),c=parseInt(a??"6",10)-1,s=e[c]?.id??"social-proof";y.info("🧠 Génération copy...");try{const n=await z.generate({product:i,audience:t,trigger:s});x.open({title:`🧠 Marketing Psy — ${n.trigger}`,content:`
        <div class="ax-gs-12">
...
${t}`,"1"),r=parseInt(e??"1",10)-1,a=i[r]?.id??"make-it-pop",c=window.prompt("Design actuel (HTML/CSS) :","<button>CTA</button>");if(c){y.info("✨ Polissage en cours...");try{const s=await I.applyCommand(a,c),n=s.changes.map(m=>`
      ...
      `,actions:[{label:"Fermer",variant:"ghost",onClick:()=>x.closeAll()}]})}catch(s){l.warn("shubham-skills","impeccable-design failed",{err:s}),y.error("Polissage échoué")}}}async function ne(){const i=window.prompt("HTML à preview iPhone :",'<h1 style="color:#c9a227;font-family:Georgia">Hello Apex</h1>');if(i)try{await W.openPreview(i)}catch(t){l.warn("shubham-skills","ios-simulator failed",{err:t}),y.error("Preview échouée")}}function ce(i){if(!w)return;i.querySelectorAll("[data-launch]").forEach(e=>{w.bind(e,"click",()=>{C.tap();const r=e.dataset.launch??"";switch(r){case"hyperframes":ae();break;case"agent-browser":ie();break;case"marketing-psy":oe();break;case"impeccable-design":se();break;case"ios-simulator":ne();break;default:y.warn(`Skill ${r} non implémenté`)}})});const t=i.querySelector("[data-back-admin]");t&&w.bind(t,"click",()=>{C.tap(),H.navigate("admin")})}function De(i){if(w?.cleanup(),w=E("admin-shubham-skills"),!q.get("isAdmin")){i.innerHTML=`
    ...
    `;return}i.innerHTML=re(),ce(i)}export{_e as dispose,De as render};
