const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./multi-source-analyze-Cakw0eHL.js","./apex-kb-CaHQJ4Rs.js","./monitoring-3uBGKGRH.js","./credential-patterns-z3lBBSNT.js","../assets/css/main-CYVYL7bN.css"])))=>i.map(i=>d[i]);
import{b as h,_ as k}from"./apex-kb-CaHQJ4Rs.js";import{l as $}from"./monitoring-3uBGKGRH.js";import{c as L}from"./listener-cleanup-Y2rGGxxX.js";import{s as P,a as H}from"../core/main-CUeElrox.js";import{apexExecute as v}from"./apex-execute-CqZ0ONV1.js";import{auth as T}from"./auth-BDVJxf47.js";import{commerce as S}from"./commerce-yfb9eev8.js";import{c as I}from"./csp-style-helper-BisGRi53.js";import{i as A,r as j}from"./voice-mGqaxeo4.js";import{kdmcProjectsRegistry as w}from"./kdmc-projects-registry-DdTqW2gy.js";import{w as _}from"./whatsapp-7OTK1zvB.js";import{haptic as d}from"./haptic-CQFg2PXZ.js";import{toast as u}from"./toast-ClsF1KRZ.js";import"./credential-patterns-z3lBBSNT.js";import"./multi-source-analyze-Cakw0eHL.js";import"./claude-bridge-zmXYETKR.js";let f="commerce";const q={commerce:"admin.commerce",users:"admin.users",pending:"admin.users",health:null,projects:null,executions:"admin.executions",knowledge:"admin.kb",bilan:"admin.bilan",consumption:"admin.consumption","audit-log":"admin.audit-log"};function n(e){return e.replace(/[&<>"']/g,i=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[i]??i)}function R(){const e=S.isEnabled();return`
  ...
  `}function B(){const e=w.list(),i=w.count(),c=w.countActive(),s=["active","wip","archived"],r=e.map(o=>{const t=s.includes(o.status)?o.status:"archived",a=o.tech_stack.slice(0,4).map(l=>n(l)).join(", ");return`
  ...
  `}function V(){const e=q[f];if(e&&!A(e))return`<div class="ax-admin-section">${j(e)}</div>`;switch(f){case"commerce":return R();case"users":return M();case"pending":return z();case"health":return D();case"projects":return B();case"executions":return U();case"knowledge":return G();case"bilan":return'<div id="ax-admin-mount-bilan" class="ax-admin-section"><p class="ax-muted">Chargement du bilan financier…</p></div>';case"consumption":return'<div id="ax-admin-mount-consumption" class="ax-admin-section"><p class="ax-muted">Chargement consommation IA…</p></div>';case"audit-log":return'<div id="ax-admin-mount-audit-log" class="ax-admin-section"><p class="ax-muted">Chargement audit log immuable…</p></div>'}}async function C(e){const i=e.querySelector("#ax-admin-mount-bilan");if(i)try{(await k(()=>import("./financial-bilan-CaVl8fRE.js"),__vite__mapDeps([0,1,2,3,4]),import.meta.url)).render(i)}catch(r){$.warn("admin","financial-bilan render failed",{err:r}),i.innerHTML='<p class="ax-muted">Bilan indisponible (module ko)</p>'}const c=e.querySelector("#ax-admin-mount-consumption");if(c)try{(await k(()=>import("./consumption-dashboard-Nd26T33I.js"),__vite__mapDeps([1,2,3,0,4]),import.meta.url)).render(c)}catch(r){$.warn("admin","consumption-dashboard render failed",{err:r}),c.innerHTML='<p class="ax-muted">Consommation indisponible (module ko)</p>'}const s=e.querySelector("#ax-admin-mount-audit-log");if(s){if(!A("admin.audit-log")){s.innerHTML=j("admin.audit-log");return}try{const{auditLog:r}=await k(async()=>{const{auditLog:a}=await import("./apex-kb-CaHQJ4Rs.js").then(l=>l.d);return{auditLog:a}},__vite__mapDeps([1,2,3]),import.meta.url);r.init();const o=r.getEntries().slice(-100).reverse(),t=o.map(a=>`<li><code>${n(new Date(a.ts).toLocaleString())}</code> · <strong>${n(a.action)}</strong> · ${n(a.actor||"system")}</li>`).join("");s.innerHTML=`
      ...
      `}catch(r){$.warn("admin","audit-log render failed",{err:r}),s.innerHTML='<p class="ax-muted">Audit log indisponible (module ko)</p>'}}}function G(){const e=h.listRepos(),i=h.getStats(),c=e.map(s=>`
  ...
  `}function K(e){e.querySelectorAll("[data-nav-route]").forEach(t=>{p.bind(t,"click",()=>{d.tap();const a=t.dataset.navRoute??"chat";window.location.hash="#"+a})}),e.querySelectorAll('[data-action="select-all"]').forEach(t=>{p.bind(t,"click",()=>{t.select()})}),e.querySelectorAll("[data-tab]").forEach(t=>{p.bind(t,"click",()=>{d.selection(),f=t.dataset.tab,x(e)})});const i=e.querySelector("#commerce-toggle");i&&p.bind(i,"change",()=>{d.medium(),S.setEnabled(i.checked),u.success(`Commercialisation ${i.checked?"activée":"désactivée"}`),x(e)});const c=e.querySelector("#create-user-form");c&&p.bind(c,"submit",t=>{t.preventDefault(),O(e)}),e.querySelectorAll("[data-user-plan]").forEach(t=>{p.bind(t,"change",()=>{const a=t.dataset.userPlan??"";a&&(S.setUserPlan(a,t.value),$.info("admin",`Plan ${t.value} → ${a}`))})}),e.querySelectorAll("[data-project-save]").forEach(t=>{p.bind(t,"click",()=>{d.tap();const a=t.dataset.projectSave??"";if(!a)return;const l=e.querySelector(`[data-project-version="${CSS.escape(a)}"]`),m=e.querySelector(`[data-project-status="${CSS.escape(a)}"]`),b=(l?.value??"").trim(),g=m?.value??"active";if(!b){u.warn("Version requise");return}w.update(a,{version:b,status:g})?(d.success(),u.success(`${a} → ${b} (${g})`),$.info("admin",`Project ${a} updated`,{version:b,status:g}),x(e)):(d.error(),u.error("Update échoué"))})}),e.querySelectorAll("[data-confirm-otp]").forEach(t=>{p.bind(t,"click",()=>{d.tap();const a=t.dataset.confirmOtp??"";if(!a)return;const l=_.confirm(a);l.ok?(d.success(),u.success("Compte activé"),$.info("admin",`Confirmed user ${l.uid}`),x(e)):(d.error(),u.error("Code OTP invalide ou expiré"))})}),e.querySelectorAll("[data-exec-cancel]").forEach(t=>{p.bind(t,"click",()=>{d.tap();const a=t.dataset.execCancel??"";if(!a)return;v.cancelExecution(a)?(d.success(),u.success("Exécution annulée"),x(e)):(d.warning(),u.warn("Annulation impossible"))})}),e.querySelectorAll("[data-exec-poll]").forEach(t=>{p.bind(t,"click",()=>{d.tap();const a=t.dataset.execPoll??"";a&&v.pollResult(a).then(l=>{l?(u.success(`Statut : ${l.status}`),x(e)):u.warn("Exécution introuvable")})})});const s=e.querySelector("#add-repo-form");s&&p.bind(s,"submit",t=>{t.preventDefault(),d.tap();const l=e.querySelector("#kb-add-repo")?.value.trim()??"";if(!l){u.warn("Indique un repo");return}const m=h.addRepo(l);m.ok?(d.success(),u.success(`Repo ajouté : ${l}`),x(e)):(d.error(),u.error(m.reason??"Erreur ajout repo"))}),e.querySelectorAll("[data-remove-repo]").forEach(t=>{p.bind(t,"click",()=>{d.tap();const a=t.dataset.removeRepo??"";a&&(h.removeRepo(a),u.success(`Repo retiré : ${a}`),x(e))})});const r=e.querySelector("#kb-search-form");r&&p.bind(r,"submit",t=>{t.preventDefault(),d.tap();const a=e.querySelector("#kb-search-query"),l=e.querySelector("#kb-search-results"),m=a?.value.trim()??"";!m||!l||(l.innerHTML='<p class="ax-muted">Recherche en cours...</p>',h.searchCode(m).then(b=>{if(b.length===0){l.innerHTML='<p class="ax-muted">Aucun résultat (configure ax_github_token pour augmenter la limite).</p>';return}const g=b.slice(0,20).map(y=>`
          ...
          `).join("");l.innerHTML=`<ul class="ax-kb-results-list">${g}</ul>`}))});const o=e.querySelector("#kb-clear-cache");o&&p.bind(o,"click",()=>{d.tap();const t=h.clearCache();u.success(`Cache vidé : ${t.cleared} entrées`),x(e)})}async function O(e){const i=e.querySelector("#cu-name")?.value.trim()??"",c=e.querySelector("#cu-tier")?.value??"family",s=e.querySelector("#cu-email")?.value.trim()??"",r=e.querySelector("#cu-whatsapp")?.value.trim()??"",o=e.querySelector("#cu-pin")?.value??"";if(!i||i.length<2){d.warning(),u.warn("Nom complet requis (min 2 caractères)");return}const t=await T.createUser({name:i,tier:c,...s&&{email:s},...r&&{whatsappPhone:r},...o&&{initialPin:o}}),a=e.querySelector("#create-user-result");if(!a)return;if(!t.ok||!t.uid){d.error();const m=t.reason??"Erreur création";a.innerHTML=`<div class="ax-error">${n(m)}</div>`,u.error(m);return}d.success(),u.success(`Compte ${i} créé`);let l="";if(r){const m=await _.requestConfirmation({uid:t.uid,name:i,whatsappPhone:r});m.ok&&m.inviteLink&&(l=`
  ...
  `,x(e)}let p=null;function ce(){p?.cleanup(),p=null}function x(e){if(p?.cleanup(),p=L("admin"),!P.get("isAdmin")){e.innerHTML=`
    ...
    <div class="ax-admin ax-modernized-card" style="padding:max(20px, env(safe-area-inset-top)) 16px max(20px, env(safe-area-inset-bottom)) 16px;max-width:1200px;margin:0 auto;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif">
      ...
      <div class="ax-admin-content">${V()}</div>
    </div>
  `),K(e),f==="bilan"||f==="consumption"){const c=f==="bilan"?"ax-admin-mount-bilan":"ax-admin-mount-consumption",s=e.querySelector(`#${c}`);if(s){const r=s.querySelector("p.ax-muted"),o=r?H(r,"admin-table"):()=>{};C(e).finally(()=>{o()})}else C(e)}}export{ce as dispose,x as render};
