const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./persistent-memory-store-Dusx9jRB.js"])))=>i.map(i=>d[i]);
import{_ as y}from"./apex-kb-By2L93Xh.js";import{l as v}from"./monitoring-WiO5ZBU9.js";import{m as p}from"../core/main-3fOQAofu.js";import{toast as d}from"./toast-Dgg9rcIP.js";import"./apex-tools-registry-CVvnzBqX.js";import"./credential-patterns-DqicUg9o.js";import"./apex-tools-dispatch-Cn-bcah1.js";import"./haptic-BUEqXK0N.js";const L="kdmc_admin";function c(s){return s.replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]??t)}function S(){try{const s=localStorage.getItem("apex_v13_user");return s?JSON.parse(s):null}catch{return null}}function f(s){if(!s)return"—";try{return new Date(s).toLocaleString("fr-FR",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"})}catch{return"—"}}function g(s){if(!s)return"jamais";const t=Date.now()-s,e=t/36e5;return e<1?`${Math.floor(t/6e4)}min`:e<24?`${Math.floor(e)}h`:`${Math.floor(e/24)}j`}function R(s){const t=S(),e=t?.id===L,n=t?.name??"Anonyme";s.innerHTML=`
  ...
  `,w(s,t,e),D(s,t,e)}async function w(s,t,e){await Promise.all([_(s,t),e?T(s):Promise.resolve(),H(s),M(s),j(s)])}async function _(s,t){const e=s.querySelector("#my-facts-content");if(e){if(!t){e.innerHTML="<em>Pas de user connecté.</em>";return}try{const{persistentMemory:n}=await y(async()=>{const{persistentMemory:r}=await import("./persistent-memory-store-Dusx9jRB.js");return{persistentMemory:r}},__vite__mapDeps([0]),import.meta.url),i=(await n.list()).filter(r=>r.scope===t.id).sort((r,l)=>l.importance-r.importance);if(i.length===0){e.innerHTML="<em>Aucun fact mémorisé pour ton compte. Les facts seront extraits automatiquement de tes messages chat.</em>";return}const o=i.slice(0,100).map(r=>`
      <tr>
