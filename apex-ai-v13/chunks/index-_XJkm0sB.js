const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./kdmc-projects-registry-D9nrHhTK.js","./monitoring-WiO5ZBU9.js","./sentinels-DpjrNx1D.js","./apex-kb-DuwHLl9O.js","./apex-tools-registry-DPQHcZUW.js","./credential-patterns-DqicUg9o.js","./multi-key-vault-CeFvI4S5.js","./links-registry-D6-Ben0k.js"])))=>i.map(i=>d[i]);
import{_ as g}from"./apex-kb-DuwHLl9O.js";import{l as u}from"./monitoring-WiO5ZBU9.js";import{c as b}from"./listener-cleanup-Y2rGGxxX.js";import{s as m}from"../core/main-g0Xshv5V.js";import{h}from"./haptic-BUEqXK0N.js";import"./apex-tools-registry-DPQHcZUW.js";import"./credential-patterns-DqicUg9o.js";import"./apex-tools-dispatch-BvSZ_r4_.js";let d=null;function N(){d?.cleanup(),d=null}function i(e){return e.replace(/[&<>"']/g,r=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[r]??r)}const y=[{id:"chat",icon:"💬",label:"Chat",description:"Conversation IA",route:"chat",color:"#5aa8ff"},{id:"vault",icon:"🔐",label:"Coffre",description:"Clés API & secrets",route:"vault",color:"#c9a227"},{id:"browser",icon:"🌐",label:"Browser",description:"Naviguer & embed",route:"browser",color:"#22cc77"},{id:"studios",icon:"🎨",label:"Studios",description:"Créatif (musique/vidéo)",route:"studios",color:"#a878ff"},{id:"pro",icon:"🎓",label:"Pro",description:"Modules expert",route:"pro",color:"#ff6b9d"},{id:"self-diag",icon:"🩺",label:"Audit",description:"Auto-diagnostic",route:"self-diag",color:"#38d8c8"},{id:"settings",icon:"⚙️",label:"Réglages",description:"Configurer Apex",route:"settings",color:"#a0a4c0"},{id:"rgpd",icon:"🛡",label:"RGPD",description:"Mes données",route:"rgpd",color:"#e8b830"}];async function v(){let e=0,r=0,o=0,a=0,t=0,c=0;try{const l=localStorage.getItem("apex_v13_messages_24h")??"0",n=parseInt(l,10);Number.isNaN(n)||(e=n)}catch{}try{const l=localStorage.getItem("apex_v13_tokens_24h")??"0",n=parseInt(l,10);Number.isNaN(n)||(r=n)}catch{}try{const{kdmcProjectsRegistry:l}=await g(async()=>{const{kdmcProjectsRegistry:n}=await import("./kdmc-projects-registry-D9nrHhTK.js");return{kdmcProjectsRegistry:n}},__vite__mapDeps([0,1]),import.meta.url);o=l.countActive()}catch{}try{const{sentinels:l}=await g(async()=>{const{sentinels:s}=await import("./sentinels-DpjrNx1D.js");return{sentinels:s}},__vite__mapDeps([2,3,1,4,5]),import.meta.url),n=l.list();t=n.length,a=n.filter(s=>s.lastResult?.ok).length}catch{}try{const l=localStorage.getItem("ax_claude_todo")??"[]",n=JSON.parse(l);Array.isArray(n)&&(c=n.filter(s=>s.status==="pending").length)}catch{}return[{id:"messages",icon:"💬",label:"Messages 24h",value:e,color:"#5aa8ff",route:"chat"},{id:"tokens",icon:"🔢",label:"Tokens 24h",value:r.toLocaleString("fr-FR"),color:"#c9a227",route:"self-diag"},{id:"projects",icon:"📦",label:"Projets actifs",value:o,color:"#22cc77",route:"admin"},{id:"sentinels",icon:"🛡",label:"Sentinelles OK",value:`${a}/${t}`,color:a===t?"#22cc77":"#ffaa00",route:"sentinels"},{id:"todos",icon:"📋",label:"Todos en attente",value:c,color:c>0?"#ff5858":"#22cc77",route:"self-diag"}]}async function w(){const e=[];try{const{sentinels:r}=await g(async()=>{const{sentinels:t}=await import("./sentinels-DpjrNx1D.js");return{sentinels:t}},__vite__mapDeps([2,3,1,4,5]),import.meta.url),a=r.list().filter(t=>t.lastResult&&!t.lastResult.ok);for(const t of a.slice(0,3))e.push({id:`sentinel_${t.id}`,level:"warn",title:`Sentinelle ${t.name}`,description:t.lastResult?.msg??"KO",ts:t.lastResult?.ts??Date.now(),action_route:"sentinels"})}catch{}try{const r=JSON.parse(localStorage.getItem("apex_v13_credentials_expiring")??"[]");if(Array.isArray(r))for(const o of r.slice(0,3))o.days_left<30&&e.push({id:`cred_${o.service}`,level:o.days_left<7?"error":"warn",title:`Credential ${o.service}`,description:`Expire dans ${o.days_left} jours`,ts:Date.now(),action_route:"vault"})}catch{}return e}async function k(){try{const{multiKeyVault:e}=await g(async()=>{const{multiKeyVault:a}=await import("./multi-key-vault-CeFvI4S5.js");return{multiKeyVault:a}},__vite__mapDeps([6,3,1,4,5]),import.meta.url),r=e.getKnownServices(),o=[];for(const a of r){const t=e.getStats(a),c=e.getServiceLight(a);o.push({service:a,light:c,totalKeys:t.total,activeKeys:t.active,failingKeys:t.failing,invalidKeys:t.invalid,lastSuccess:t.lastSuccess})}return o}catch{return[]}}function $(){const e=[];try{const r=localStorage.getItem("ax_claude_todo")??"[]",o=JSON.parse(r);if(Array.isArray(o))for(const a of o.filter(t=>t.status==="pending").slice(0,5))e.push({id:a.id??`todo_${Date.now()}`,source:"apex_todo",title:a.reason??"Todo sans description",severity:a.severity??"medium",ts_created:a.ts??Date.now()})}catch{}return e}function z(e,r=0){return`
...
>📊 Usage</a>`:"";return`
        <div class="ax-service-health-row ax-modernized-card"
          ...
          ${x}
          ${f}
        </div>`}).join("")}async function K(e){const r={};if(e.length===0)return r;try{const{linksRegistry:o}=await g(async()=>{const{linksRegistry:a}=await import("./links-registry-D6-Ben0k.js");return{linksRegistry:a}},__vite__mapDeps([7,1,3,4,5]),import.meta.url);for(const a of e)r[a]={recharge:o.getRechargeLink(a),usage:o.getUsageLink(a),apiKeys:o.getApiKeysLink(a)}}catch{}return r}async function V(e){d?.cleanup(),d=b("dashboard");const r=m.get("user"),o=r?.name?`Bonjour ${r.name}`:"Bonjour",[a,t,c]=await Promise.all([v(),w(),k()]),l=await K(c.map(s=>s.service)),n=$();e.innerHTML=`
    <style>
...
//# sourceMappingURL=index-_XJkm0sB.js.map
