const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./kdmc-projects-registry-Bc-QS4so.js","./monitoring-B17vNBOa.js","./sentinels-CGiSKL0u.js","./apex-kb-BZEQo2pJ.js","./apex-tools-registry-Duck4KzY.js","./credential-patterns-BybElwOv.js","./multi-key-vault-IPd8qiav.js","./links-registry-Ccej85hR.js"])))=>i.map(i=>d[i]);
import{_ as d}from"./apex-kb-BZEQo2pJ.js";import{l as f}from"./monitoring-B17vNBOa.js";import{s as u}from"../core/main-BHzORHz8.js";import{h as b}from"./haptic-BUEqXK0N.js";import"./apex-tools-registry-Duck4KzY.js";import"./credential-patterns-BybElwOv.js";import"./apex-tools-dispatch-CVI00iKM.js";function i(e){return e.replace(/[&<>"']/g,r=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[r]??r)}const m=[{id:"chat",icon:"💬",label:"Chat",description:"Conversation IA",route:"chat",color:"#5aa8ff"},{id:"vault",icon:"🔐",label:"Coffre",description:"Clés API & secrets",route:"vault",color:"#c9a227"},{id:"browser",icon:"🌐",label:"Browser",description:"Naviguer & embed",route:"browser",color:"#22cc77"},{id:"studios",icon:"🎨",label:"Studios",description:"Créatif (musique/vidéo)",route:"studios",color:"#a878ff"},{id:"pro",icon:"🎓",label:"Pro",description:"Modules expert",route:"pro",color:"#ff6b9d"},{id:"self-diag",icon:"🩺",label:"Audit",description:"Auto-diagnostic",route:"self-diag",color:"#38d8c8"},{id:"settings",icon:"⚙️",label:"Réglages",description:"Configurer Apex",route:"settings",color:"#a0a4c0"},{id:"rgpd",icon:"🛡",label:"RGPD",description:"Mes données",route:"rgpd",color:"#e8b830"}];async function h(){let e=0,r=0,o=0,a=0,t=0,c=0;try{const l=localStorage.getItem("apex_v13_messages_24h")??"0",n=parseInt(l,10);Number.isNaN(n)||(e=n)}catch{}try{const l=localStorage.getItem("apex_v13_tokens_24h")??"0",n=parseInt(l,10);Number.isNaN(n)||(r=n)}catch{}try{const{kdmcProjectsRegistry:l}=await d(async()=>{const{kdmcProjectsRegistry:n}=await import("./kdmc-projects-registry-Bc-QS4so.js");return{kdmcProjectsRegistry:n}},__vite__mapDeps([0,1]),import.meta.url);o=l.countActive()}catch{}try{const{sentinels:l}=await d(async()=>{const{sentinels:s}=await import("./sentinels-CGiSKL0u.js");return{sentinels:s}},__vite__mapDeps([2,3,1,4,5]),import.meta.url),n=l.list();t=n.length,a=n.filter(s=>s.lastResult?.ok).length}catch{}try{const l=localStorage.getItem("ax_claude_todo")??"[]",n=JSON.parse(l);Array.isArray(n)&&(c=n.filter(s=>s.status==="pending").length)}catch{}return[{id:"messages",icon:"💬",label:"Messages 24h",value:e,color:"#5aa8ff",route:"chat"},{id:"tokens",icon:"🔢",label:"Tokens 24h",value:r.toLocaleString("fr-FR"),color:"#c9a227",route:"self-diag"},{id:"projects",icon:"📦",label:"Projets actifs",value:o,color:"#22cc77",route:"admin"},{id:"sentinels",icon:"🛡",label:"Sentinelles OK",value:`${a}/${t}`,color:a===t?"#22cc77":"#ffaa00",route:"sentinels"},{id:"todos",icon:"📋",label:"Todos en attente",value:c,color:c>0?"#ff5858":"#22cc77",route:"self-diag"}]}async function y(){const e=[];try{const{sentinels:r}=await d(async()=>{const{sentinels:t}=await import("./sentinels-CGiSKL0u.js");return{sentinels:t}},__vite__mapDeps([2,3,1,4,5]),import.meta.url),a=r.list().filter(t=>t.lastResult&&!t.lastResult.ok);for(const t of a.slice(0,3))e.push({id:`sentinel_${t.id}`,level:"warn",title:`Sentinelle ${t.name}`,description:t.lastResult?.msg??"KO",ts:t.lastResult?.ts??Date.now(),action_route:"sentinels"})}catch{}try{const r=JSON.parse(localStorage.getItem("apex_v13_credentials_expiring")??"[]");if(Array.isArray(r))for(const o of r.slice(0,3))o.days_left<30&&e.push({id:`cred_${o.service}`,level:o.days_left<7?"error":"warn",title:`Credential ${o.service}`,description:`Expire dans ${o.days_left} jours`,ts:Date.now(),action_route:"vault"})}catch{}return e}async function v(){try{const{multiKeyVault:e}=await d(async()=>{const{multiKeyVault:a}=await import("./multi-key-vault-IPd8qiav.js");return{multiKeyVault:a}},__vite__mapDeps([6,3,1,4,5]),import.meta.url),r=e.getKnownServices(),o=[];for(const a of r){const t=e.getStats(a),c=e.getServiceLight(a);o.push({service:a,light:c,totalKeys:t.total,activeKeys:t.active,failingKeys:t.failing,invalidKeys:t.invalid,lastSuccess:t.lastSuccess})}return o}catch{return[]}}function w(){const e=[];try{const r=localStorage.getItem("ax_claude_todo")??"[]",o=JSON.parse(r);if(Array.isArray(o))for(const a of o.filter(t=>t.status==="pending").slice(0,5))e.push({id:a.id??`todo_${Date.now()}`,source:"apex_todo",title:a.reason??"Todo sans description",severity:a.severity??"medium",ts_created:a.ts??Date.now()})}catch{}return e}function k(e,r=0){return`
    ...
    <button class="ax-shortcut-card ax-modernized-card ax-bounce-tap" data-route="${i(e.route)}"
    ...
    </button>`}function z(e){return e.length===0?`
      <div style="display:flex;align-items:center;gap:12px;padding:16px;background:linear-gradient(135deg,rgba(34,204,119,0.08),rgba(34,204,119,0.03));border:1px solid rgba(34,204,119,0.15);border-radius:12px">
        ...
        </div>`}).join("")}function _(e){return e.length===0?`
      <div style="display:flex;align-items:center;gap:12px;padding:16px;background:linear-gradient(135deg,rgba(34,204,119,0.08),rgba(34,204,119,0.03));border:1px solid rgba(34,204,119,0.15);border-radius:12px">
        <span style="font-size:20px;filter:drop-shadow(0 2px 6px rgba(34,204,119,0.4))">🎉</span>
        ...
        </div>`}).join("")}function A(e,r={}){if(e.length===0)return`
...
>📊 Usage</a>`:"";return`
        <div class="ax-service-health-row ax-modernized-card"
          ...
          ${g}
          ${x}
        </div>`}).join("")}async function S(e){const r={};if(e.length===0)return r;try{const{linksRegistry:o}=await d(async()=>{const{linksRegistry:a}=await import("./links-registry-Ccej85hR.js");return{linksRegistry:a}},__vite__mapDeps([7,1,3,4,5]),import.meta.url);for(const a of e)r[a]={recharge:o.getRechargeLink(a),usage:o.getUsageLink(a),apiKeys:o.getApiKeysLink(a)}}catch{}return r}async function O(e){const r=u.get("user"),o=r?.name?`Bonjour ${r.name}`:"Bonjour",[a,t,c]=await Promise.all([h(),y(),v()]),l=await S(c.map(s=>s.service)),n=w();e.innerHTML=`
    <style>
  ...
  `,e.querySelectorAll("[data-route]").forEach(s=>{s.addEventListener("click",()=>{b.tap();const p=s.dataset.route;p&&(window.location.hash="#"+p)})}),f.info("feature-dashboard",`rendered (${a.length} kpis, ${t.length} alerts, ${n.length} todos)`)}export{h as computeKpis,i as escapeHtml,y as loadAlerts,S as loadRechargeLinks,v as loadServiceHealth,w as loadTodos,O as render,A as renderServiceHealthCard};
//# sourceMappingURL=index-enaMCqMg.js.map
