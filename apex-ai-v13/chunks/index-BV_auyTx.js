...
import{_ as u}from"./apex-kb-GVSoM0ek.js";import{e as o}from"./escape-html-BlQj2yEF.js";import{c as M}from"./listener-cleanup-Y2rGGxxX.js";import{l as D}from"./monitoring-CDiFHg4q.js";import"./credential-patterns-ClK5OHi0.js";import"./multi-source-analyze-KNqxsI4v.js";let i=null;function E(){i?.cleanup(),i=null}async function g(n){i?.cleanup(),i=M("sentinels");const{sentinels:x}=await u(async()=>{const{sentinels:t}=await import("./sentinels-CQKjfAL5.js");return{sentinels:t}},__vite__mapDeps([0,1,2,3]),import.meta.url),{sentinelsRegistry:m,bootstrapSentinelsRegistry:b}=await u(async()=>{const{sentinelsRegistry:t,bootstrapSentinelsRegistry:a}=await import("./sentinels-registry-BcmFscZj.js");return{sentinelsRegistry:t,bootstrapSentinelsRegistry:a}},__vite__mapDeps([0,1,2,3]),import.meta.url);b();const e=x.list(),y=m.getStatus(),p=m.getMetrics(),v=e.filter(t=>t.lastResult?.ok).length,$=e.filter(t=>t.lastResult&&!t.lastResult.ok).length,R=e.filter(t=>!t.lastResult).length;n.innerHTML=`
    <div class="ax-page ax-gs-376">
      ...
      <p style="color:var(--ax-text-dim);margin:0 0 12px;font-size:12px">
        📊 Métriques : ${p.totalRuns} runs · avg ${p.avgDurationMs}ms · auto-fix ${p.totalAutoFixSuccess}✅ / ${p.totalAutoFixFailures}❌
      </p>
  ...
  `;const f=n.querySelector("#ax-sent-run-all");f&&i.bind(f,"click",()=>{(async()=>{const{toast:t}=await u(async()=>{const{toast:a}=await import("./toast-CRdbcLoc.js");return{toast:a}},[],import.meta.url);t.info(`Exécution de ${e.length} sentinelles...`),await Promise.all(e.map(a=>x.runOne(a.id))),t.success("✅ Tous les sentinelles re-exécutés"),await g(n)})()});const h=n.querySelector("#ax-sent-refresh");h&&i.bind(h,"click",()=>{g(n)}),n.querySelectorAll(".ax-sent-run").forEach(t=>{i.bind(t,"click",a=>{a.stopPropagation(),(async()=>{const s=t.dataset.sentId;if(!s)return;const{toast:d}=await u(async()=>{const{toast:r}=await import("./toast-CRdbcLoc.js");return{toast:r}},[],import.meta.url),l=await x.runOne(s);d[l?.ok?"success":"warn"](`${s}: ${l?.msg??"KO"}`),await g(n)})()})}),n.querySelectorAll(".ax-sent-row").forEach(t=>{i.bind(t,"click",()=>{(async()=>{const a=t.dataset.sentId;if(!a)return;const s=e.find(c=>c.id===a);if(!s)return;const{drillDown:d}=await u(async()=>{const{drillDown:c}=await import("./drilldown-1SlvGToi.js");return{drillDown:c}},[],import.meta.url),l="ax-drilldown-mount-sentinels";let r=document.getElementById(l);r||(r=document.createElement("div"),r.id=l,document.body.appendChild(r)),d.open({id:`sent-${a}`,title:`🛡 ${s.name}`,content:()=>{const c=s.lastResult?.ok,_=s.lastResult?.msg??"—",w=s.lastResult?new Date(s.lastResult.ts).toLocaleString("fr-FR"):"—";return`
            ...
            `},data:{sentinelId:a}},r)})()})}),D.info("feature-sentinels",`rendered ${e.length} sentinels (${v}OK / ${$}WARN)`)}export{E as dispose,g as render};
