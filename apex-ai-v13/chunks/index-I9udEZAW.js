const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./multi-source-analyze-Dai8QiTQ.js","./apex-kb-Dx00IPob.js","./monitoring-3uBGKGRH.js","./credential-patterns-CLzI061R.js","../assets/css/main-CHYznJwT.css"])))=>i.map(i=>d[i]);
import{b as f,_ as C}from"./apex-kb-Dx00IPob.js";import{c as M}from"./listener-cleanup-Y2rGGxxX.js";import{l as y}from"./monitoring-3uBGKGRH.js";import{s as R,a as H}from"../core/main-BdyYp559.js";import{apexExecute as v}from"./apex-execute-CQdUS9Ad.js";import{auth as P}from"./auth-XHB-4sdo.js";import{commerce as _}from"./commerce-C2LFuNfy.js";import{c as D}from"./csp-style-helper-BisGRi53.js";import{i as T,r as z}from"./voice-8szRM3YO.js";import{kdmcProjectsRegistry as $}from"./kdmc-projects-registry-DdTqW2gy.js";import{w as j}from"./whatsapp-BFeKj9oS.js";import{haptic as d}from"./haptic-CQFg2PXZ.js";import{toast as u}from"./toast-ClsF1KRZ.js";import"./credential-patterns-CLzI061R.js";import"./multi-source-analyze-Dai8QiTQ.js";import"./claude-bridge-Ivx1Entd.js";let h="commerce";const I={commerce:"admin.commerce",users:"admin.users",pending:"admin.users",health:null,projects:null,executions:"admin.executions",knowledge:"admin.kb",bilan:"admin.bilan",consumption:"admin.consumption","audit-log":"admin.audit-log"};function n(t){return t.replace(/[&<>"']/g,i=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[i]??i)}function E(){const t=_.isEnabled();return`
  ...
  `}function N(){const t=P.listUsers(),i=new Set(["admin","family","client_pro","client_free"]),c=t.map(s=>{const r=i.has(s.tier)?s.tier:"client_free";return`
  ...
  `}function V(){const t=j.listPending();return t.length?`
    <div class="ax-admin-section">
  ...
  `}function B(){const t=$.list(),i=$.count(),c=$.countActive(),s=["active","wip","archived"],r=t.map(o=>{const e=s.includes(o.status)?o.status:"archived",a=o.tech_stack.slice(0,4).map(l=>n(l)).join(", ");return`
  ...
  `}function G(){return[["commerce","💳 Commerce"],["users","👥 Comptes"],["pending","📨 Attente"],["health","🩺 Santé"],["projects","📦 Projets"],["executions","⚙️ Exec"],["knowledge","📚 KB"],["bilan","📊 Bilan"],["consumption","💰 Conso"],["audit-log","🔒 Audit"]].filter(([c])=>{const s=I[c];return!s||T(s)}).map(([c,s])=>{const r=h===c;return`
      ...
      `}).join("")}function K(){const t=v.listPendingExecutions({limit:30}),i=v.getStats(),c=v.listAllowedTasks(),s=v.listForbiddenTasks(),r={pending:"pending",dispatched:"wip",running:"wip",completed:"ok",failed:"error",cancelled:"archived",timeout:"error"},o=t.map(e=>{const a=r[e.status]??"archived",l=new Date(e.ts_created).toLocaleString(),m=e.duration_ms?`${Math.round(e.duration_ms/1e3)}s`:"—",b=e.workflow_run_url?`<a href="${n(e.workflow_run_url)}" target="_blank" rel="noopener">🔗 Workflow</a>`:'<span class="ax-muted">—</span>',g=e.status==="pending"||e.status==="dispatched";return`
  ...
  `}function O(){const t=I[h];if(t&&!T(t))return`<div class="ax-admin-section">${z(t)}</div>`;switch(h){case"commerce":return E();case"users":return N();case"pending":return V();case"health":return U();case"projects":return B();case"executions":return K();case"knowledge":return F();case"bilan":return'<div id="ax-admin-mount-bilan" class="ax-admin-section"><p class="ax-muted">Chargement du bilan financier…</p></div>';case"consumption":return'<div id="ax-admin-mount-consumption" class="ax-admin-section"><p class="ax-muted">Chargement consommation IA…</p></div>';case"audit-log":return'<div id="ax-admin-mount-audit-log" class="ax-admin-section"><p class="ax-muted">Chargement audit log immuable…</p></div>'}}async function L(t){const i=t.querySelector("#ax-admin-mount-bilan");if(i)try{(await C(()=>import("./financial-bilan-Bz2z7vtj.js"),__vite__mapDeps([0,1,2,3,4]),import.meta.url)).render(i)}catch(r){y.warn("admin","financial-bilan render failed",{err:r}),i.innerHTML='<p class="ax-muted">Bilan indisponible (module ko)</p>'}const c=t.querySelector("#ax-admin-mount-consumption");if(c)try{(await C(()=>import("./consumption-dashboard-BqXHZau6.js"),__vite__mapDeps([1,2,3,0,4]),import.meta.url)).render(c)}catch(r){y.warn("admin","consumption-dashboard render failed",{err:r}),c.innerHTML='<p class="ax-muted">Consommation indisponible (module ko)</p>'}const s=t.querySelector("#ax-admin-mount-audit-log");if(s){if(!T("admin.audit-log")){s.innerHTML=z("admin.audit-log");return}try{const{auditLog:r}=await C(async()=>{const{auditLog:a}=await import("./apex-kb-Dx00IPob.js").then(l=>l.d);return{auditLog:a}},__vite__mapDeps([1,2,3]),import.meta.url);r.init();const o=r.getEntries().slice(-100).reverse(),e=o.map(a=>`<li><code>${n(new Date(a.ts).toLocaleString())}</code> · <strong>${n(a.action)}</strong> · ${n(a.actor||"system")}</li>`).join("");s.innerHTML=`
      ...
      `}catch(r){y.warn("admin","audit-log render failed",{err:r}),s.innerHTML='<p class="ax-muted">Audit log indisponible (module ko)</p>'}}}function F(){const t=f.listRepos(),i=f.getStats(),c=t.map(s=>`
  ...
  `}function W(t){t.querySelectorAll("[data-nav-route]").forEach(e=>{p.bind(e,"click",()=>{d.tap();const a=e.dataset.navRoute??"chat";window.location.hash="#"+a})}),t.querySelectorAll('[data-action="select-all"]').forEach(e=>{p.bind(e,"click",()=>{e.select()})}),t.querySelectorAll("[data-tab]").forEach(e=>{p.bind(e,"click",()=>{d.selection(),h=e.dataset.tab,x(t)})});const i=t.querySelector("#commerce-toggle");i&&p.bind(i,"change",()=>{const e=i.checked;d.medium();const a=t.querySelector(".ax-toggle-label");if(a){a.textContent="",a.append("Commercialisation ");const l=document.createElement("strong");l.textContent=e?"ACTIVÉE":"désactivée",a.append(l)}_.setEnabled(e),u.success(`Commercialisation ${e?"activée":"désactivée"}`),x(t)});const c=t.querySelector("#create-user-form");c&&p.bind(c,"submit",e=>{e.preventDefault(),Y(t)}),t.querySelectorAll("[data-user-plan]").forEach(e=>{p.bind(e,"change",()=>{const a=e.dataset.userPlan??"";a&&(_.setUserPlan(a,e.value),y.info("admin",`Plan ${e.value} → ${a}`))})}),t.querySelectorAll("[data-project-save]").forEach(e=>{p.bind(e,"click",()=>{d.tap();const a=e.dataset.projectSave??"";if(!a)return;const l=t.querySelector(`[data-project-version="${CSS.escape(a)}"]`),m=t.querySelector(`[data-project-status="${CSS.escape(a)}"]`),b=(l?.value??"").trim(),g=m?.value??"active";if(!b){u.warn("Version requise");return}$.update(a,{version:b,status:g})?(d.success(),u.success(`${a} → ${b} (${g})`),y.info("admin",`Project ${a} updated`,{version:b,status:g}),x(t)):(d.error(),u.error("Update échoué"))})}),t.querySelectorAll("[data-confirm-otp]").forEach(e=>{p.bind(e,"click",()=>{d.tap();const a=e.dataset.confirmOtp??"";if(!a)return;const l=j.confirm(a);l.ok?(d.success(),u.success("Compte activé"),y.info("admin",`Confirmed user ${l.uid}`),x(t)):(d.error(),u.error("Code OTP invalide ou expiré"))})}),t.querySelectorAll("[data-exec-cancel]").forEach(e=>{p.bind(e,"click",()=>{d.tap();const a=e.dataset.execCancel??"";if(!a)return;v.cancelExecution(a)?(d.success(),u.success("Exécution annulée"),x(t)):(d.warning(),u.warn("Annulation impossible"))})}),t.querySelectorAll("[data-exec-poll]").forEach(e=>{p.bind(e,"click",()=>{d.tap();const a=e.dataset.execPoll??"";a&&v.pollResult(a).then(l=>{l?(u.success(`Statut : ${l.status}`),x(t)):u.warn("Exécution introuvable")})})});const s=t.querySelector("#add-repo-form");s&&p.bind(s,"submit",e=>{e.preventDefault(),d.tap();const l=t.querySelector("#kb-add-repo")?.value.trim()??"";if(!l){u.warn("Indique un repo");return}const m=f.addRepo(l);m.ok?(d.success(),u.success(`Repo ajouté : ${l}`),x(t)):(d.error(),u.error(m.reason??"Erreur ajout repo"))}),t.querySelectorAll("[data-remove-repo]").forEach(e=>{p.bind(e,"click",()=>{d.tap();const a=e.dataset.removeRepo??"";a&&(f.removeRepo(a),u.success(`Repo retiré : ${a}`),x(t))})});const r=t.querySelector("#kb-search-form");r&&p.bind(r,"submit",e=>{e.preventDefault(),d.tap();const a=t.querySelector("#kb-search-query"),l=t.querySelector("#kb-search-results"),m=a?.value.trim()??"";!m||!l||(l.innerHTML='<p class="ax-muted">Recherche en cours...</p>',f.searchCode(m).then(b=>{if(b.length===0){l.innerHTML='<p class="ax-muted">Aucun résultat (configure ax_github_token pour augmenter la limite).</p>';return}l.textContent="";const g=document.createElement("ul");g.className="ax-kb-results-list",b.slice(0,20).forEach(w=>{const S=document.createElement("li");S.className="ax-kb-result";const k=document.createElement("a");k.href=w.htmlUrl,k.target="_blank",k.rel="noopener";const q=document.createElement("code");q.textContent=w.path,k.append(q);const A=document.createElement("span");A.className="ax-muted",A.textContent=`${w.repo} · score ${w.score.toFixed(2)}`,S.append(k," ",A),g.append(S)}),l.append(g)}))});const o=t.querySelector("#kb-clear-cache");o&&p.bind(o,"click",()=>{d.tap();const e=f.clearCache();u.success(`Cache vidé : ${e.cleared} entrées`),x(t)})}async function Y(t){const i=t.querySelector("#cu-name")?.value.trim()??"",c=t.querySelector("#cu-tier")?.value??"family",s=t.querySelector("#cu-email")?.value.trim()??"",r=t.querySelector("#cu-whatsapp")?.value.trim()??"",o=t.querySelector("#cu-pin")?.value??"";if(!i||i.length<2){d.warning(),u.warn("Nom complet requis (min 2 caractères)");return}const e=await P.createUser({name:i,tier:c,...s&&{email:s},...r&&{whatsappPhone:r},...o&&{initialPin:o}}),a=t.querySelector("#create-user-result");if(!a)return;if(!e.ok||!e.uid){d.error();const m=e.reason??"Erreur création";a.innerHTML=`<div class="ax-error">${n(m)}</div>`,u.error(m);return}d.success(),u.success(`Compte ${i} créé`);let l="";if(r){const m=await j.requestConfirmation({uid:e.uid,name:i,whatsappPhone:r});m.ok&&m.inviteLink&&(l=`
  ...
  `,x(t)}let p=null;function pe(){p?.cleanup(),p=null}function x(t){if(p?.cleanup(),p=M("admin"),!R.get("isAdmin")){t.innerHTML=`
      <div class="ax-empty">
        <h2>Accès réservé</h2>
  ...
  `),W(t),h==="bilan"||h==="consumption"){const c=h==="bilan"?"ax-admin-mount-bilan":"ax-admin-mount-consumption",s=t.querySelector(`#${c}`);if(s){const r=s.querySelector("p.ax-muted"),o=r?H(r,"admin-table"):()=>{};L(t).finally(()=>{o()})}else L(t)}}export{pe as dispose,x as render};
