...
`)}getFacts(){return this.facts}getLessons(){return this.lessons}getProjects(){return y}persist(){try{localStorage.setItem("apex_v13_facts",JSON.stringify(this.facts)),localStorage.setItem("apex_v13_lessons",JSON.stringify(this.lessons))}catch(e){u.warn("memory","persist failed (quota?)",{err:e})}}}const R=new x;class O{state={};listeners=new Map;initialized=!1;init(e){if(this.initialized)return;this.initialized=!0;const r=this.loadPersisted();this.state={user:null,view:"landing",isStreaming:!1,online:navigator.onLine,appVer:"",isAdmin:!1,commerceEnabled:!0,theme:"dark",...e,...r}}get(e){return this.state[e]}set(e,r){const c=this.state[e];c!==r&&(this.state[e]=r,this.persistKey(e,r),f.emit("store:change",{key:e,value:r}),this.notify(e,r,c))}subscribe(e,r){const c=this.listeners.get(e)??new Set;return c.add(r),this.listeners.set(e,c),()=>c.delete(r)}snapshot(){return{...this.state}}notify(e,r,c){this.listeners.get(e)?.forEach(i=>{try{i(r,c)}catch{}})}PERSISTED_KEYS=new Set(["theme","commerceEnabled"]);persistKey(e,r){if(this.PERSISTED_KEYS.has(e))try{localStorage.setItem(`apex_v13_${e}`,JSON.stringify(r))}catch{}}loadPersisted(){const e={};for(const r of this.PERSISTED_KEYS)try{const c=localStorage.getItem(`apex_v13_${r}`);c&&(e[r]=JSON.parse(c))}catch{}return e}}const _=new O;class L{routes=new Map;currentRoute="";rootEl=null;initialized=!1;register(e,r){this.routes.set(e,r)}init(){this.initialized||(this.initialized=!0,this.rootEl=document.getElementById("apex-root"),window.addEventListener("hashchange",()=>void this.dispatch()))}navigate(e){if(location.hash===`#${e}`){this.dispatch();return}location.hash=`#${e}`}async dispatch(){if(!this.rootEl){u.warn("router","dispatch called before init");return}const e=location.hash.replace(/^#\/?/,"")||this.defaultRoute(),r=this.routes.get(e);if(!r){u.warn("router",`Unknown route: ${e} → fallback`),this.renderNotFound(e);return}const c=_.get("user")!==null,i=_.get("isAdmin");if(r.requiresAuth&&!c){this.navigate("login");return}if(r.requiresAdmin&&!i){this.renderForbidden();return}const l=this.currentRoute;this.currentRoute=e,f.emit("route:change",{from:l,to:e}),_.set("view",e);try{await(await r.loader()).render(this.rootEl)}catch(a){w.capture(a,{source:"manual"}),this.renderError(w.toUserMessage(a))}}defaultRoute(){return _.get("user")?"chat":"landing"}renderNotFound(e){this.rootEl&&(this.rootEl.innerHTML=`
    ...
    `)}escape(e){return e.replace(/[&<>"']/g,r=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[r]??r)}}const n=new L,v="v13.3.7",S="kdmc_admin";async function D(){const m={startedAt:performance.now(),online:navigator.onLine,pwaInstalled:window.matchMedia("(display-mode: standalone)").matches,isAdmin:!1};w.installGlobalHandlers(),u.info("boot",`APEX ${v} starting`,{ctx:m});const e=async(t,o)=>{try{await o()}catch(d){u.warn("boot",`Service init failed: ${t} (continuing degraded)`,{err:d})}};await e("sentry",async()=>{const{sentryBridge:t}=await s(async()=>{const{sentryBridge:o}=await import("../chunks/monitoring-B17vNBOa.js").then(d=>d.s);return{sentryBridge:o}},[],import.meta.url);await t.init()}),await e("bodyguard",async()=>{const{bodyguard:t}=await s(async()=>{const{bodyguard:o}=await import("../chunks/bodyguard-Dov7nvCD.js");return{bodyguard:o}},__vite__mapDeps([0,1,2]),import.meta.url);t.install()}),await e("audit-log",async()=>{const{auditLog:t}=await s(async()=>{const{auditLog:o}=await import("../chunks/apex-tools-registry-DloDnFZi.js").then(d=>d.c);return{auditLog:o}},__vite__mapDeps([2,1]),import.meta.url);t.init(),await t.record("boot.start",{details:{ver:v}})}),await e("auto-backup",async()=>{const{autoBackup:t}=await s(async()=>{const{autoBackup:o}=await import("../chunks/auto-backup-B42LFeti.js");return{autoBackup:o}},__vite__mapDeps([3,4,1,2,5]),import.meta.url);await t.init()}),await e("observability",async()=>{const{observability:t}=await s(async()=>{const{observability:o}=await import("../chunks/monitoring-B17vNBOa.js").then(d=>d.b);return{observability:o}},[],import.meta.url);t.init()}),await e("firebase-queue",async()=>{const{firebaseQueue:t}=await s(async()=>{const{firebaseQueue:o}=await import("../chunks/firebase-queue-DtAS5gqp.js");return{firebaseQueue:o}},__vite__mapDeps([6,4,1,2,5]),import.meta.url);t.init()}),await e("sentinels",async()=>{const{sentinels:t}=await s(async()=>{const{sentinels:d}=await import("../chunks/sentinels-CAT1LSPa.js");return{sentinels:d}},__vite__mapDeps([7,4,1,2,5]),import.meta.url),{bootstrapSentinelsRegistry:o}=await s(async()=>{const{bootstrapSentinelsRegistry:d}=await import("../chunks/sentinels-registry-BhR_47kr.js");return{bootstrapSentinelsRegistry:d}},__vite__mapDeps([8,4,1,2,5,7]),import.meta.url);o(),t.init()}),"serviceWorker"in navigator||u.warn("boot","Service Worker not supported — degraded mode"),(!("crypto"in window)||!window.crypto.subtle)&&u.error("boot","Web Crypto API not available — vault DISABLED"),_.init({user:null,view:"landing",isStreaming:!1,online:m.online,appVer:v}),await R.init().catch(t=>{u.error("boot","Memory init failed (degraded)",{err:t})});const{firebase:r}=await s(async()=>{const{firebase:t}=await import("../chunks/apex-tools-dispatch-oP9vJ1P5.js").then(o=>o.a);return{firebase:t}},__vite__mapDeps([4,1,2,5]),import.meta.url);if(await r.init().catch(t=>{u.error("boot","Firebase init failed (degraded offline mode)",{err:t})}),!localStorage.getItem("apex_v13_migrated"))try{const{migrate:t}=await s(async()=>{const{migrate:o}=await import("../chunks/migrate-v12-to-v13-BH7vN9c4.js");return{migrate:o}},__vite__mapDeps([9,1]),import.meta.url);await t(),localStorage.setItem("apex_v13_migrated",new Date().toISOString()),u.info("boot","Migration v12→v13 completed")}catch(t){u.error("boot","Migration failed (continuing with empty state)",{err:t})}const{auth:i}=await s(async()=>{const{auth:t}=await import("../chunks/auth-5WcpAsEf.js");return{auth:t}},__vite__mapDeps([10,4,1,2,5]),import.meta.url);if(m.isAdmin=await i.isAdmin().catch(()=>!1),_.set("isAdmin",m.isAdmin),n.register("landing",{loader:()=>s(()=>import("../chunks/index-BrpVkJZp.js"),__vite__mapDeps([11,4,1,2,5,10,12,13]),import.meta.url)}),n.register("login",{loader:()=>s(()=>import("../chunks/index-BrpVkJZp.js"),__vite__mapDeps([11,4,1,2,5,10,12,13]),import.meta.url)}),n.register("chat",{loader:()=>s(()=>import("../chunks/index-DrPWaUg8.js"),__vite__mapDeps([14,4,1,2,5,15,16,17,18,12,19,13]),import.meta.url),requiresAuth:!0}),n.register("admin",{loader:()=>s(()=>import("../chunks/index-DtCPgSxy.js"),__vite__mapDeps([20,4,1,2,5,21,22,23,10,18,24,12,13]),import.meta.url),requiresAdmin:!0}),n.register("credentials",{loader:()=>s(()=>import("../chunks/index-DPVwGSQi.js"),__vite__mapDeps([25,4,1,2,5,21,26,12,13]),import.meta.url),requiresAdmin:!0}),n.register("studios",{loader:()=>s(()=>import("../chunks/index-DK2hHaqo.js"),__vite__mapDeps([27,4,1,2,5]),import.meta.url),requiresAuth:!0}),n.register("pro",{loader:()=>s(()=>import("../chunks/index-BYOap72R.js"),__vite__mapDeps([28,4,1,2,5]),import.meta.url),requiresAuth:!0}),n.register("laurence",{loader:()=>s(()=>import("../chunks/index-CjaeIBPc.js"),__vite__mapDeps([29,4,1,2,5,30]),import.meta.url),requiresAuth:!0}),n.register("settings",{loader:()=>s(()=>import("../chunks/index-CtExR5cT.js"),__vite__mapDeps([31,4,1,2,5]),import.meta.url),requiresAuth:!0}),n.register("sentinels",{loader:()=>s(()=>import("../chunks/index-DldBNrVP.js"),__vite__mapDeps([32,4,1,2,5,21]),import.meta.url),requiresAdmin:!0}),n.register("browser",{loader:()=>s(()=>import("../chunks/index-CtUMFZld.js"),__vite__mapDeps([33,4,1,2,5,21,34]),import.meta.url),requiresAuth:!0}),n.register("crypto",{loader:()=>s(()=>import("../chunks/index-NPYRSQTs.js"),__vite__mapDeps([35,1]),import.meta.url),requiresAuth:!0}),n.register("domotique",{loader:()=>s(()=>import("../chunks/index-BtnZR27Y.js"),__vite__mapDeps([36,1]),import.meta.url),requiresAuth:!0}),n.register("workflow",{loader:()=>s(()=>import("../chunks/index-Dl_IgUgo.js"),__vite__mapDeps([37,1]),import.meta.url),requiresAuth:!0}),n.register("remote",{loader:()=>s(()=>import("../chunks/index-BOUNP8Z7.js"),__vite__mapDeps([38,4,1,2,5,26]),import.meta.url),requiresAuth:!0}),n.register("notes",{loader:()=>s(()=>import("../chunks/index-CzWXWMA3.js"),__vite__mapDeps([39,1,21,4,2,5]),import.meta.url),requiresAuth:!0}),n.register("calendar",{loader:()=>s(()=>import("../chunks/index-DybbMkXt.js"),__vite__mapDeps([40,1,21,4,2,5]),import.meta.url),requiresAuth:!0}),n.register("billing",{loader:()=>s(()=>import("../chunks/index-BjNPlzmR.js"),__vite__mapDeps([41,1,4,2,5]),import.meta.url),requiresAuth:!0}),n.register("calculators",{loader:()=>s(()=>import("../chunks/index-CUrrHf9M.js"),__vite__mapDeps([42,1]),import.meta.url),requiresAuth:!0}),n.register("archive",{loader:()=>s(()=>import("../chunks/index-Bbcu8pmy.js"),__vite__mapDeps([43,1,4,2,5]),import.meta.url),requiresAuth:!0}),n.register("studio-music",{loader:()=>s(()=>import("../chunks/index-TCA6Zakb.js"),__vite__mapDeps([44,1,34,4,2,5]),import.meta.url),requiresAuth:!0}),n.register("studio-video",{loader:()=>s(()=>import("../chunks/index-Dq_UxZX0.js"),__vite__mapDeps([45,1,34,4,2,5]),import.meta.url),requiresAuth:!0}),n.register("studio-cv",{loader:()=>s(()=>import("../chunks/index-CEElhQR6.js"),__vite__mapDeps([46,4,1,2,5]),import.meta.url),requiresAuth:!0}),n.register("studio-invoice",{loader:()=>s(()=>import("../chunks/index-Qsl9S5w6.js"),__vite__mapDeps([47,1,4,2,5]),import.meta.url),requiresAuth:!0}),n.register("studio-contract",{loader:()=>s(()=>import("../chunks/index-DfBPPAz9.js"),__vite__mapDeps([48,1,4,2,5]),import.meta.url),requiresAuth:!0}),n.register("studio-logo",{loader:()=>s(()=>import("../chunks/index-YDtVUslb.js"),__vite__mapDeps([49,1,4,2,5]),import.meta.url),requiresAuth:!0}),n.register("studio-presentation",{loader:()=>s(()=>import("../chunks/index-1sCdrwDl.js"),__vite__mapDeps([50,1,4,2,5]),import.meta.url),requiresAuth:!0}),n.register("studio-prefecture",{loader:()=>s(()=>import("../chunks/index-CX3fsWnd.js"),__vite__mapDeps([51,1,4,2,5]),import.meta.url),requiresAuth:!0}),n.register("studio-clip",{loader:()=>s(()=>import("../chunks/index-B5L5muxN.js"),__vite__mapDeps([52,1,4,2,5]),import.meta.url),requiresAuth:!0}),n.register("studio-photo",{loader:()=>s(()=>import("../chunks/index-BeZ_XK-R.js"),__vite__mapDeps([53,1,4,2,5]),import.meta.url),requiresAuth:!0}),n.register("pro-business",{loader:()=>s(()=>import("../chunks/index-D_cBQ5Vj.js"),__vite__mapDeps([54,1]),import.meta.url),requiresAuth:!0}),n.register("pro-education",{loader:()=>s(()=>import("../chunks/index-Cnmdxqv0.js"),__vite__mapDeps([55,1]),import.meta.url),requiresAuth:!0}),n.register("pro-certifications",{loader:()=>s(()=>import("../chunks/index-D6B42LM7.js"),__vite__mapDeps([56,1]),import.meta.url),requiresAuth:!0}),n.register("dashboard",{loader:()=>s(()=>import("../chunks/index-DJmxEKh2.js"),__vite__mapDeps([57,4,1,2,5,12]),import.meta.url),requiresAuth:!0}),n.register("vault",{loader:()=>s(()=>import("../chunks/index-Cb2JoKoJ.js"),__vite__mapDeps([58,1,21,59,4,2,5,60,61,12,13]),import.meta.url),requiresAdmin:!0}),n.register("knowledge-bank",{loader:()=>s(()=>import("../chunks/index-FVNNNjOi.js"),__vite__mapDeps([62,1,12,13]),import.meta.url),requiresAuth:!0}),n.register("apex-toolbox",{loader:()=>s(()=>import("../chunks/index-CNqiow0G.js"),__vite__mapDeps([63,4,1,2,5,64,12,13]),import.meta.url),requiresAuth:!0}),n.register("self-diag",{loader:()=>s(()=>import("../chunks/index-B_1_zgSr.js"),__vite__mapDeps([65,4,1,2,5,21,12,13]),import.meta.url),requiresAuth:!0}),n.register("admin-backup",{loader:()=>s(()=>import("../chunks/index-f6wcHfLR.js"),__vite__mapDeps([66,1,21,3,4,2,5,12,13]),import.meta.url),requiresAdmin:!0}),n.init(),f.emit("boot:routerReady",{ctx:m}),(async()=>{try{const t=window.location.pathname.replace(/[^/]+$/,"")+"index.html?_v="+Date.now(),o=await fetch(t,{method:"GET",cache:"no-store",signal:AbortSignal.timeout(5e3)});if(!o.ok)return;const h=(await o.text()).match(/data-app-ver="(v[\d.]+)"/)?.[1];if(!h||h===v)return;const g="apex_v13_force_reload_"+h;if(sessionStorage.getItem(g)){u.warn("boot",`version mismatch ${v} vs ${h} mais reload déjà tenté → abandonne (loop guard)`);return}sessionStorage.setItem(g,String(Date.now())),u.warn("boot",`🔄 Version stale détectée : local ${v}, remote ${h} → force reload`);try{if("serviceWorker"in navigator){const E=await navigator.serviceWorker.getRegistrations();await Promise.all(E.map(b=>b.unregister()))}if("caches"in window){const E=await caches.keys();await Promise.all(E.map(b=>caches.delete(b)))}}catch(E){u.warn("boot","force reload cleanup failed",{err:E})}window.location.href=window.location.pathname+"?_forceupd="+h+"&t="+Date.now()}catch(t){u.debug("boot","force version check skipped",{err:t})}})(),"serviceWorker"in navigator){navigator.serviceWorker.register("./sw.js").then(o=>{u.info("boot","SW registered",{scope:o.scope}),setInterval(()=>{o.update().catch(()=>{})},300*1e3),document.addEventListener("visibilitychange",()=>{document.hidden||o.update().catch(()=>{})})}).catch(o=>{u.warn("boot","SW register failed",{err:o})});let t=!1;navigator.serviceWorker.addEventListener("controllerchange",()=>{t||(t=!0,window.location.reload())}),navigator.serviceWorker.addEventListener("message",o=>{const d=o.data;d?.type&&(d.type==="push_resubscribed"?(u.info("push","SW auto-resubscribed",{endpoint:d.endpoint?.slice(0,60)}),f.emit("push:resubscribed",{endpoint:d.endpoint})):d.type==="notification_clicked"&&f.emit("notification:clicked",{url:d.url}))})}s(async()=>{const{bootstrapServices:t}=await import("../chunks/services-bootstrap-xbodHlKY.js");return{bootstrapServices:t}},__vite__mapDeps([67,4,1,2,5]),import.meta.url).then(({bootstrapServices:t})=>{const o=m.isAdmin?S:_.get("user")?.id??null;return t(o)}).then(t=>{const o=t.filter(d=>d.ok).length;u.info("boot",`services-bootstrap : ${o}/${t.length} OK`)}).catch(t=>{u.warn("boot","services-bootstrap failed (non-blocking)",{err:t})}),s(async()=>{const{pushAutoInit:t}=await import("../chunks/push-auto-init-iT31C8oE.js");return{pushAutoInit:t}},__vite__mapDeps([68,4,1,2,5,69]),import.meta.url).then(({pushAutoInit:t})=>{const o=m.isAdmin?S:_.get("user")?.id??"anon";return t.autoInit(o)}).then(t=>{u.info("push","auto-init complete",{env:t.environment,subscribed:t.subscribed,needs_install:t.needs_install_guide}),f.emit("push:status",t)}).catch(t=>{u.warn("push","auto-init failed (non-blocking)",{err:t})});let l=!1;const a=async()=>{if(!(l||!navigator.onLine)){l=!0;try{if("serviceWorker"in navigator)try{const h=await navigator.serviceWorker.getRegistration();h&&h.update()}catch{}const t=location.pathname.replace(/[^/]*$/,"")+"index.html?__forceupd="+Date.now()+"&_r="+Math.random().toString(36).slice(2),A=(await(await fetch(t,{cache:"reload",headers:{"Cache-Control":"no-cache, no-store, must-revalidate",Pragma:"no-cache"}})).text()).match(/data-app-ver=['"]([^'"]+)['"]/);if(A?.[1]&&A[1]!==v){u.info("boot",`🔄 force-update: local=${v} → remote=${A[1]} — reload imminent`);try{if("serviceWorker"in navigator){const h=await navigator.serviceWorker.getRegistrations();await Promise.all(h.map(g=>g.unregister()))}}catch{}try{if("caches"in window){const h=await caches.keys();await Promise.all(h.map(g=>caches.delete(g)))}}catch{}setTimeout(()=>{location.replace(location.pathname+"?_forceupd="+Date.now()+"&_v="+(A[1]??"new"))},300)}}catch(t){u.warn("boot","force-update check failed",{err:t})}finally{l=!1}}};setTimeout(()=>void a(),500),setTimeout(()=>void a(),3e3),document.addEventListener("visibilitychange",()=>{document.hidden||a()}),window.addEventListener("focus",()=>void a()),setInterval(()=>void a(),300*1e3),window.addEventListener("online",()=>{_.set("online",!0),f.emit("network:online",{}),u.info("network","Online")}),window.addEventListener("offline",()=>{_.set("online",!1),f.emit("network:offline",{}),u.info("network","Offline")}),n.dispatch(),setTimeout(()=>{const t=document.getElementById("apex-splash");t&&(t.hidden=!0,setTimeout(()=>t.remove(),600))},100);const p=Math.round(performance.now()-m.startedAt);u.info("boot",`APEX ${v} ready in ${p}ms`),f.emit("boot:complete",{ctx:m,bootMs:p})}D().catch(m=>{console.error("[APEX boot crash]",m);const e=document.getElementById("apex-root");e&&(e.innerHTML=`
    ...
    `);const r=document.getElementById("apex-rescue-btn");r&&(r.style.display="flex")});window.__APEX__={ver:v,di:P,store:_,logger:u};export{w as e,R as m,n as r,_ as s};
//# sourceMappingURL=main-D3PSGmlM.js.map
