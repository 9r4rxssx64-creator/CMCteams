...
${b.map(o=>`- ${o.name} : ${o.description}`).join(`
`)}`),i.push(`## Règles permanentes prioritaires
${I.map((o,p)=>`${p+1}. ${o}`).join(`
...
`)}`);try{const o=localStorage.getItem("apex_v13_persistent_memory");if(o){const p=JSON.parse(o);if(Array.isArray(p)&&p.length>0){const t=p.sort((n,d)=>(d.importance??50)-(n.importance??50)).slice(0,50);i.push(`## Mémoire persistante cross-session (${p.length} entries totales, top 50)
...
`)}getFacts(){return this.facts}getLessons(){return this.lessons}getProjects(){return b}persist(){try{localStorage.setItem("apex_v13_facts",JSON.stringify(this.facts)),localStorage.setItem("apex_v13_lessons",JSON.stringify(this.lessons))}catch(e){u.warn("memory","persist failed (quota?)",{err:e})}}}const x=new T;class k{state={};listeners=new Map;initialized=!1;init(e){if(this.initialized)return;this.initialized=!0;const r=this.loadPersisted();this.state={user:null,view:"landing",isStreaming:!1,online:navigator.onLine,appVer:"",isAdmin:!1,commerceEnabled:!0,theme:"dark",...e,...r}}get(e){return this.state[e]}set(e,r){const c=this.state[e];c!==r&&(this.state[e]=r,this.persistKey(e,r),_.emit("store:change",{key:e,value:r}),this.notify(e,r,c))}subscribe(e,r){const c=this.listeners.get(e)??new Set;return c.add(r),this.listeners.set(e,c),()=>c.delete(r)}snapshot(){return{...this.state}}notify(e,r,c){this.listeners.get(e)?.forEach(i=>{try{i(r,c)}catch{}})}PERSISTED_KEYS=new Set(["theme","commerceEnabled"]);persistKey(e,r){if(this.PERSISTED_KEYS.has(e))try{localStorage.setItem(`apex_v13_${e}`,JSON.stringify(r))}catch{}}loadPersisted(){const e={};for(const r of this.PERSISTED_KEYS)try{const c=localStorage.getItem(`apex_v13_${r}`);c&&(e[r]=JSON.parse(c))}catch{}return e}}const h=new k;class R{routes=new Map;currentRoute="";rootEl=null;initialized=!1;register(e,r){this.routes.set(e,r)}init(){this.initialized||(this.initialized=!0,this.rootEl=document.getElementById("apex-root"),window.addEventListener("hashchange",()=>void this.dispatch()))}navigate(e){if(location.hash===`#${e}`){this.dispatch();return}location.hash=`#${e}`}async dispatch(){if(!this.rootEl){u.warn("router","dispatch called before init");return}const e=location.hash.replace(/^#\/?/,"")||this.defaultRoute(),r=this.routes.get(e);if(!r){u.warn("router",`Unknown route: ${e} → fallback`),this.renderNotFound(e);return}const c=h.get("user")!==null,i=h.get("isAdmin");if(r.requiresAuth&&!c){this.navigate("login");return}if(r.requiresAdmin&&!i){this.renderForbidden();return}const l=this.currentRoute;this.currentRoute=e,_.emit("route:change",{from:l,to:e}),h.set("view",e);try{await(await r.loader()).render(this.rootEl)}catch(o){E.capture(o,{source:"manual"}),this.renderError(E.toUserMessage(o))}}defaultRoute(){return h.get("user")?"chat":"landing"}renderNotFound(e){this.rootEl&&(this.rootEl.innerHTML=`
      <div class="ax-empty">
    ...
    `)}escape(e){return e.replace(/[&<>"']/g,r=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[r]??r)}}const a=new R,v="v13.3.4",y="kdmc_admin";async function L(){const m={startedAt:performance.now(),online:navigator.onLine,pwaInstalled:window.matchMedia("(display-mode: standalone)").matches,isAdmin:!1};E.installGlobalHandlers(),u.info("boot",`APEX ${v} starting`,{ctx:m});const e=async(t,n)=>{try{await n()}catch(d){u.warn("boot",`Service init failed: ${t} (continuing degraded)`,{err:d})}};await e("sentry",async()=>{const{sentryBridge:t}=await s(async()=>{const{sentryBridge:n}=await import("../chunks/monitoring-B17vNBOa.js").then(d=>d.s);return{sentryBridge:n}},[],import.meta.url);await t.init()}),await e("bodyguard",async()=>{const{bodyguard:t}=await s(async()=>{const{bodyguard:n}=await import("../chunks/bodyguard-Dov7nvCD.js");return{bodyguard:n}},__vite__mapDeps([0,1,2]),import.meta.url);t.install()}),await e("audit-log",async()=>{const{auditLog:t}=await s(async()=>{const{auditLog:n}=await import("../chunks/apex-tools-registry-DloDnFZi.js").then(d=>d.c);return{auditLog:n}},__vite__mapDeps([2,1]),import.meta.url);t.init(),await t.record("boot.start",{details:{ver:v}})}),await e("auto-backup",async()=>{const{autoBackup:t}=await s(async()=>{const{autoBackup:n}=await import("../chunks/auto-backup-BqcuCluZ.js");return{autoBackup:n}},__vite__mapDeps([3,4,1,2,5]),import.meta.url);await t.init()}),await e("observability",async()=>{const{observability:t}=await s(async()=>{const{observability:n}=await import("../chunks/monitoring-B17vNBOa.js").then(d=>d.b);return{observability:n}},[],import.meta.url);t.init()}),await e("firebase-queue",async()=>{const{firebaseQueue:t}=await s(async()=>{const{firebaseQueue:n}=await import("../chunks/firebase-queue-Q3XdyKqJ.js");return{firebaseQueue:n}},__vite__mapDeps([6,4,1,2,5]),import.meta.url);t.init()}),await e("sentinels",async()=>{const{sentinels:t}=await s(async()=>{const{sentinels:d}=await import("../chunks/sentinels-D4Yg3WKo.js");return{sentinels:d}},__vite__mapDeps([7,4,1,2,5]),import.meta.url),{bootstrapSentinelsRegistry:n}=await s(async()=>{const{bootstrapSentinelsRegistry:d}=await import("../chunks/sentinels-registry-DIoclfJN.js");return{bootstrapSentinelsRegistry:d}},__vite__mapDeps([8,4,1,2,5,7]),import.meta.url);n(),t.init()}),"serviceWorker"in navigator||u.warn("boot","Service Worker not supported — degraded mode"),(!("crypto"in window)||!window.crypto.subtle)&&u.error("boot","Web Crypto API not available — vault DISABLED"),h.init({user:null,view:"landing",isStreaming:!1,online:m.online,appVer:v}),await x.init().catch(t=>{u.error("boot","Memory init failed (degraded)",{err:t})});const{firebase:r}=await s(async()=>{const{firebase:t}=await import("../chunks/apex-tools-dispatch-BDZxzUZ0.js").then(n=>n.a);return{firebase:t}},__vite__mapDeps([4,1,2,5]),import.meta.url);if(await r.init().catch(t=>{u.error("boot","Firebase init failed (degraded offline mode)",{err:t})}),!localStorage.getItem("apex_v13_migrated"))try{const{migrate:t}=await s(async()=>{const{migrate:n}=await import("../chunks/migrate-v12-to-v13-BH7vN9c4.js");return{migrate:n}},__vite__mapDeps([9,1]),import.meta.url);await t(),localStorage.setItem("apex_v13_migrated",new Date().toISOString()),u.info("boot","Migration v12→v13 completed")}catch(t){u.error("boot","Migration failed (continuing with empty state)",{err:t})}const{auth:i}=await s(async()=>{const{auth:t}=await import("../chunks/auth-CT9-pFQt.js");return{auth:t}},__vite__mapDeps([10,4,1,2,5]),import.meta.url);if(m.isAdmin=await i.isAdmin().catch(()=>!1),h.set("isAdmin",m.isAdmin),a.register("landing",{loader:()=>s(()=>import("../chunks/index-Busk5cFs.js"),__vite__mapDeps([11,4,1,2,5,10,12,13]),import.meta.url)}),a.register("login",{loader:()=>s(()=>import("../chunks/index-Busk5cFs.js"),__vite__mapDeps([11,4,1,2,5,10,12,13]),import.meta.url)}),a.register("chat",{loader:()=>s(()=>import("../chunks/index-BTuUQgkE.js"),__vite__mapDeps([14,4,1,2,5,15,16,17,18,12,19,13]),import.meta.url),requiresAuth:!0}),a.register("admin",{loader:()=>s(()=>import("../chunks/index-BRH0TGQX.js"),__vite__mapDeps([20,4,1,2,5,21,22,23,10,18,24,12,13]),import.meta.url),requiresAdmin:!0}),a.register("credentials",{loader:()=>s(()=>import("../chunks/index-CPNX6KA6.js"),__vite__mapDeps([25,4,1,2,5,21,26,12,13]),import.meta.url),requiresAdmin:!0}),a.register("studios",{loader:()=>s(()=>import("../chunks/index-Bl6HCO2k.js"),__vite__mapDeps([27,4,1,2,5]),import.meta.url),requiresAuth:!0}),a.register("pro",{loader:()=>s(()=>import("../chunks/index-Du-uNswy.js"),__vite__mapDeps([28,4,1,2,5]),import.meta.url),requiresAuth:!0}),a.register("laurence",{loader:()=>s(()=>import("../chunks/index-TS14TUWe.js"),__vite__mapDeps([29,4,1,2,5,30]),import.meta.url),requiresAuth:!0}),a.register("settings",{loader:()=>s(()=>import("../chunks/index-BGqx2aYt.js"),__vite__mapDeps([31,4,1,2,5]),import.meta.url),requiresAuth:!0}),a.register("sentinels",{loader:()=>s(()=>import("../chunks/index-D_zqe9Ij.js"),__vite__mapDeps([32,4,1,2,5,21]),import.meta.url),requiresAdmin:!0}),a.register("browser",{loader:()=>s(()=>import("../chunks/index-Q5npWPCq.js"),__vite__mapDeps([33,4,1,2,5,21,34]),import.meta.url),requiresAuth:!0}),a.register("crypto",{loader:()=>s(()=>import("../chunks/index-NPYRSQTs.js"),__vite__mapDeps([35,1]),import.meta.url),requiresAuth:!0}),a.register("domotique",{loader:()=>s(()=>import("../chunks/index-BtnZR27Y.js"),__vite__mapDeps([36,1]),import.meta.url),requiresAuth:!0}),a.register("workflow",{loader:()=>s(()=>import("../chunks/index-Dl_IgUgo.js"),__vite__mapDeps([37,1]),import.meta.url),requiresAuth:!0}),a.register("remote",{loader:()=>s(()=>import("../chunks/index-CYigzzRW.js"),__vite__mapDeps([38,4,1,2,5,26]),import.meta.url),requiresAuth:!0}),a.register("notes",{loader:()=>s(()=>import("../chunks/index-CuZ8KudW.js"),__vite__mapDeps([39,1,21,4,2,5]),import.meta.url),requiresAuth:!0}),a.register("calendar",{loader:()=>s(()=>import("../chunks/index-BMKpz_Ov.js"),__vite__mapDeps([40,1,4,2,5]),import.meta.url),requiresAuth:!0}),a.register("billing",{loader:()=>s(()=>import("../chunks/index-gbMyZnO8.js"),__vite__mapDeps([41,1,4,2,5]),import.meta.url),requiresAuth:!0}),a.register("calculators",{loader:()=>s(()=>import("../chunks/index-CUrrHf9M.js"),__vite__mapDeps([42,1]),import.meta.url),requiresAuth:!0}),a.register("archive",{loader:()=>s(()=>import("../chunks/index-BFy6-uAW.js"),__vite__mapDeps([43,1,4,2,5]),import.meta.url),requiresAuth:!0}),a.register("studio-music",{loader:()=>s(()=>import("../chunks/index-Cgtq0GKq.js"),__vite__mapDeps([44,1,34,4,2,5]),import.meta.url),requiresAuth:!0}),a.register("studio-video",{loader:()=>s(()=>import("../chunks/index-Cwja5O8B.js"),__vite__mapDeps([45,1,34,4,2,5]),import.meta.url),requiresAuth:!0}),a.register("studio-cv",{loader:()=>s(()=>import("../chunks/index-BdEkUdbO.js"),__vite__mapDeps([46,4,1,2,5]),import.meta.url),requiresAuth:!0}),a.register("studio-invoice",{loader:()=>s(()=>import("../chunks/index-BiYvQ5tA.js"),__vite__mapDeps([47,1,4,2,5]),import.meta.url),requiresAuth:!0}),a.register("studio-contract",{loader:()=>s(()=>import("../chunks/index-Cb33Tq_m.js"),__vite__mapDeps([48,1,4,2,5]),import.meta.url),requiresAuth:!0}),a.register("studio-logo",{loader:()=>s(()=>import("../chunks/index-DKtjoIBh.js"),__vite__mapDeps([49,1,4,2,5]),import.meta.url),requiresAuth:!0}),a.register("studio-presentation",{loader:()=>s(()=>import("../chunks/index-EtmZhPWQ.js"),__vite__mapDeps([50,1,4,2,5]),import.meta.url),requiresAuth:!0}),a.register("studio-prefecture",{loader:()=>s(()=>import("../chunks/index-q0oJxdK2.js"),__vite__mapDeps([51,1,4,2,5]),import.meta.url),requiresAuth:!0}),a.register("studio-clip",{loader:()=>s(()=>import("../chunks/index-C6vhDCT5.js"),__vite__mapDeps([52,1,4,2,5]),import.meta.url),requiresAuth:!0}),a.register("studio-photo",{loader:()=>s(()=>import("../chunks/index-CFedNBxH.js"),__vite__mapDeps([53,1,4,2,5]),import.meta.url),requiresAuth:!0}),a.register("pro-business",{loader:()=>s(()=>import("../chunks/index-D_cBQ5Vj.js"),__vite__mapDeps([54,1]),import.meta.url),requiresAuth:!0}),a.register("pro-education",{loader:()=>s(()=>import("../chunks/index-Cnmdxqv0.js"),__vite__mapDeps([55,1]),import.meta.url),requiresAuth:!0}),a.register("pro-certifications",{loader:()=>s(()=>import("../chunks/index-D6B42LM7.js"),__vite__mapDeps([56,1]),import.meta.url),requiresAuth:!0}),a.register("dashboard",{loader:()=>s(()=>import("../chunks/index-B_5GfC23.js"),__vite__mapDeps([57,4,1,2,5,12]),import.meta.url),requiresAuth:!0}),a.register("vault",{loader:()=>s(()=>import("../chunks/index-C07PWvD9.js"),__vite__mapDeps([58,1,59,4,2,5,60,61,12,13]),import.meta.url),requiresAdmin:!0}),a.register("knowledge-bank",{loader:()=>s(()=>import("../chunks/index-FVNNNjOi.js"),__vite__mapDeps([62,1,12,13]),import.meta.url),requiresAuth:!0}),a.register("apex-toolbox",{loader:()=>s(()=>import("../chunks/index-Df9M75HL.js"),__vite__mapDeps([63,4,1,2,5,64,12,13]),import.meta.url),requiresAuth:!0}),a.register("self-diag",{loader:()=>s(()=>import("../chunks/index-CPma73Qj.js"),__vite__mapDeps([65,4,1,2,5,21,12,13]),import.meta.url),requiresAuth:!0}),a.register("admin-backup",{loader:()=>s(()=>import("../chunks/index-_cRBVCM9.js"),__vite__mapDeps([66,1,3,4,2,5,12,13]),import.meta.url),requiresAdmin:!0}),a.init(),_.emit("boot:routerReady",{ctx:m}),"serviceWorker"in navigator){navigator.serviceWorker.register("./sw.js").then(n=>{u.info("boot","SW registered",{scope:n.scope}),setInterval(()=>{n.update().catch(()=>{})},300*1e3),document.addEventListener("visibilitychange",()=>{document.hidden||n.update().catch(()=>{})})}).catch(n=>{u.warn("boot","SW register failed",{err:n})});let t=!1;navigator.serviceWorker.addEventListener("controllerchange",()=>{t||(t=!0,window.location.reload())}),navigator.serviceWorker.addEventListener("message",n=>{const d=n.data;d?.type&&(d.type==="push_resubscribed"?(u.info("push","SW auto-resubscribed",{endpoint:d.endpoint?.slice(0,60)}),_.emit("push:resubscribed",{endpoint:d.endpoint})):d.type==="notification_clicked"&&_.emit("notification:clicked",{url:d.url}))})}s(async()=>{const{bootstrapServices:t}=await import("../chunks/services-bootstrap-Ba-LI4Zg.js");return{bootstrapServices:t}},__vite__mapDeps([67,4,1,2,5]),import.meta.url).then(({bootstrapServices:t})=>{const n=m.isAdmin?y:h.get("user")?.id??null;return t(n)}).then(t=>{const n=t.filter(d=>d.ok).length;u.info("boot",`services-bootstrap : ${n}/${t.length} OK`)}).catch(t=>{u.warn("boot","services-bootstrap failed (non-blocking)",{err:t})}),s(async()=>{const{pushAutoInit:t}=await import("../chunks/push-auto-init-CiHlnkQg.js");return{pushAutoInit:t}},__vite__mapDeps([68,4,1,2,5,69]),import.meta.url).then(({pushAutoInit:t})=>{const n=m.isAdmin?y:h.get("user")?.id??"anon";return t.autoInit(n)}).then(t=>{u.info("push","auto-init complete",{env:t.environment,subscribed:t.subscribed,needs_install:t.needs_install_guide}),_.emit("push:status",t)}).catch(t=>{u.warn("push","auto-init failed (non-blocking)",{err:t})});let l=!1;const o=async()=>{if(!(l||!navigator.onLine)){l=!0;try{if("serviceWorker"in navigator)try{const f=await navigator.serviceWorker.getRegistration();f&&f.update()}catch{}const t=location.pathname.replace(/[^/]*$/,"")+"index.html?__forceupd="+Date.now()+"&_r="+Math.random().toString(36).slice(2),g=(await(await fetch(t,{cache:"reload",headers:{"Cache-Control":"no-cache, no-store, must-revalidate",Pragma:"no-cache"}})).text()).match(/data-app-ver=['"]([^'"]+)['"]/);if(g?.[1]&&g[1]!==v){u.info("boot",`🔄 force-update: local=${v} → remote=${g[1]} — reload imminent`);try{if("serviceWorker"in navigator){const f=await navigator.serviceWorker.getRegistrations();await Promise.all(f.map(A=>A.unregister()))}}catch{}try{if("caches"in window){const f=await caches.keys();await Promise.all(f.map(A=>caches.delete(A)))}}catch{}setTimeout(()=>{location.replace(location.pathname+"?_forceupd="+Date.now()+"&_v="+(g[1]??"new"))},300)}}catch(t){u.warn("boot","force-update check failed",{err:t})}finally{l=!1}}};setTimeout(()=>void o(),500),setTimeout(()=>void o(),3e3),document.addEventListener("visibilitychange",()=>{document.hidden||o()}),window.addEventListener("focus",()=>void o()),setInterval(()=>void o(),300*1e3),window.addEventListener("online",()=>{h.set("online",!0),_.emit("network:online",{}),u.info("network","Online")}),window.addEventListener("offline",()=>{h.set("online",!1),_.emit("network:offline",{}),u.info("network","Offline")}),a.dispatch(),setTimeout(()=>{const t=document.getElementById("apex-splash");t&&(t.hidden=!0,setTimeout(()=>t.remove(),600))},100);const p=Math.round(performance.now()-m.startedAt);u.info("boot",`APEX ${v} ready in ${p}ms`),_.emit("boot:complete",{ctx:m,bootMs:p})}L().catch(m=>{console.error("[APEX boot crash]",m);const e=document.getElementById("apex-root");e&&(e.innerHTML=`
    ...
    `);const r=document.getElementById("apex-rescue-btn");r&&(r.style.display="flex")});window.__APEX__={ver:v,di:S,store:h,logger:u};export{E as e,x as m,a as r,h as s};
//# sourceMappingURL=main-CPmwu7dM.js.map
