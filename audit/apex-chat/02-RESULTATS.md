...
| **LIVE domaine** (`audit-live.yml`) | ✅ **EXÉCUTÉE** | Toutes les surfaces répondent **sauf `lingua.kd-mc.com`** (hors périmètre Apex Chat, signalé à part). **11/09** : ce run (`34517173393`, 18 h 54) est **antérieur** au correctif de l'écran blanc de Lingua (`97253ebdb`, 20 h 52, déployé ensuite) ; en local la vérification Lingua donne 26/26 ; le workflow pose désormais son verdict par surface dans un check-run, relancé par ce push |
